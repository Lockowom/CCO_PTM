...
app.post('/api/traspasos-ai', express.json({ limit: '1mb' }), async (req, res) => {
  ...
  if (!rateLimitOk(`ia:${token.slice(-24) || req.ip}`, 20, 5 * 60 * 1000)) {
    return res
      .status(429)
      .json({ error: { message: 'Demasiadas peticiones de IA. Espera unos minutos.' } });
  }
  try {
...
const IA_TOOLS = [
  {
    def: {
      name: 'kpis',
      description:
        'Indicadores/resumen general del sistema (operaciones/N.V. por estado, urgentes e incidencias; tickets de Post-Venta; stock). Úsalo para preguntas de panorama ("cómo vamos", "cuántas NV pendientes", "resumen").',
  ...
  },
  {
    def: {
      ...
      description:
        'Busca notas de venta / operaciones de despacho por texto (NV, cliente, vendedor, factura) y/o estado. Devuelve filas con cliente, estado, fechas, transportista, urgente, valor.',
      input_schema: {
        type: 'object',
        properties: {
          q: {
            type: 'string',
            description: 'Texto a buscar (NV, cliente, vendedor, factura). Opcional.'
          },
          estado: {
            type: 'string',
            description: 'Filtrar por estado exacto (p.ej. "En Proceso", "Entregado"). Opcional.'
          },
          limit: { type: 'integer', description: 'Máx filas (1-50, def 20).' }
        }
      }
    },
    rpc: 'ia_buscar_operaciones',
    map: (i) => ({ p_q: i.q || null, p_estado: i.estado || null, p_limit: i.limit || 20 })
  },
  {
    def: {
      name: 'buscar_stock',
      description:
        'Busca stock/inventario consolidado por código de producto (SKU) o descripción. Devuelve disponible, reserva, stock total y bodega.',
      input_schema: {
        type: 'object',
        properties: {
          q: { type: 'string', description: 'SKU o descripción del producto. Opcional.' },
          limit: { type: 'integer', description: 'Máx filas (1-50, def 20).' }
        }
      }
    },
    rpc: 'ia_buscar_stock',
    map: (i) => ({ p_q: i.q || null, p_limit: i.limit || 20 })
  },
  {
    def: {
      name: 'buscar_tickets',
      description:
        'Busca tickets de Post-Venta / servicio técnico por texto (número, cliente, técnico, equipo, serie) y/o estado. Devuelve estado, prioridad, técnico, equipo, región y fechas.',
      input_schema: {
        type: 'object',
        properties: {
          q: { type: 'string', description: 'Texto a buscar. Opcional.' },
          estado: {
            type: 'string',
            description: 'Filtrar por estado (p.ej. "Abierto"). Opcional.'
          },
          limit: { type: 'integer', description: 'Máx filas (1-50, def 20).' }
        }
      }
...
app.post('/api/asistente', express.json({ limit: '1mb' }), async (req, res) => {
  ...
  if (!rateLimitOk(`asist:${token.slice(-24) || req.ip}`, 30, 5 * 60 * 1000)) {
    return res
      .status(429)
      .json({ error: { message: 'Demasiadas consultas al asistente. Espera unos minutos.' } });
  }
  try {
    const u = await fetch(`${SUPA_URL}/auth/v1/user`, {
      headers: { apikey: SUPA_ANON, Authorization: `Bearer ${token}` }
  ...
  if (!raw || raw.length === 0)
    return res.status(400).json({ error: { message: 'Falta messages.' } });
  let convo = raw
  ...
  try {
        ...
        for (const block of data.content || []) {
          ...
          if (!tool) {
            ...
            try {
              ...
              result = rr.ok
                ? iaSafeParse(txt)
                : { error: 'Sin acceso o error de datos', detalle: txt.slice(0, 200) };
            } catch (e) {
    ...
    return res.json({
      text: 'No pude completar la consulta (demasiados pasos). Reformula, por favor.',
...
app.get('/api/route', async (req, res) => {
  ...
  try {
    ...
    if (data.code !== 'Ok' || !data.routes?.length)
      return res.status(404).json({ error: { message: 'Sin ruta.' } });
...
app.get('/api/version', (req, res) => {
  ...
  res.json({ version: APP_VERSION, buildId: APP_BUILD_ID });
});
...
app.all('/api/*', (req, res) =>
  res.status(404).json({ error: { message: 'Endpoint no existe.' } })
);
...
app.use(
  express.static(join(__dirname, 'dist'), {
    ...
    setHeaders: (res, filePath) => {
      ...
      if (
        /\.(html|webmanifest)$/.test(filePath) ||
        /(?:^|[\\/])(sw\.js|registerSW\.js|workbox-[^\\/]+\.js)$/.test(filePath)
      ) {
