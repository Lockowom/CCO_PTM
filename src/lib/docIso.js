          ...
          },
          {
            ...
            table: {
              ...
              body: [
                ...
                [
                  { text: 'Revisión', bold: true, fontSize: 7 },
                  { text: D.revision, fontSize: 7 }
                ],
                [
                  { text: 'Norma', bold: true, fontSize: 7 },
    ...
    stack: [
      ...
      },
      {
        columns: [
          ...
          {
            text: `Documento controlado · Página ${currentPage} de ${pageCount}`,
            fontSize: 7,
            color: '#94a3b8',
            alignment: 'right'
          }
                  ...
                  ...(D.logo
                    ? [
                        new Paragraph({
                          children: [
                            new ImageRun({
                              data: dataUriToBytes(D.logo),
                              type: 'png',
                              transformation: {
                                width: 120,
                                height: Math.round((120 * D.logo_h) / D.logo_w)
                              }
                            })
                          ]
                        })
                    ...
                    : []),
                  new Paragraph({
                    children: [new TextRun({ text: D.empresa, bold: true, size: 22 })]
                  }),
                  new Paragraph({
                    children: [new TextRun({ text: D.subtitulo, size: 15, color: '64748B' })]
                  })
                ]
              }),
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    ...
                    children: [
                      new TextRun({ text: `Código: ${D.codigo}  ·  Rev. ${D.revision}`, size: 15 })
                    ]
                  }),
                  new Paragraph({
                    alignment: AlignmentType.RIGHT,
                    children: [
                      new TextRun({
                        text: `${D.norma}  ·  Vig. ${D.fecha_revision}`,
                        size: 15,
                        color: '64748B'
                      })
                    ]
  ...
  const footer = new Footer({
    children: [
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `${D.codigo} · Rev. ${D.revision} · ${D.norma} · Documento controlado · Página `,
            size: 14,
            color: '94A3B8'
          }),
