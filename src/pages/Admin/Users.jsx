...
const UsersPage = ({ embedded = false }) => {
  ...
  const bulkMutation = useMutation({
    mutationFn: async ({ ids, accion, valor = null }) => {
      const { data, error } = await supabase.rpc('usuarios_bulk', {
        p_ids: ids,
        p_accion: accion,
        p_valor: valor
      });
  ...
  const doBulk = (accion, valor = null) => {
    ...
    if (
      accion === 'eliminar' &&
      !window.confirm(
        `¿Eliminar ${ids.length} usuario(s) permanentemente? Esta acción no se puede deshacer.`
      )
    )
      return;
  ...
  const StatCard = ({ icon, label, value, glowColor }) => (
    <div className="bg-white backdrop-blur-xl rounded-2xl p-3 sm:p-5 border border-slate-200 relative overflow-hidden group">
      <div
        className={`absolute top-0 right-0 w-24 h-24 bg-${glowColor}-500/10 rounded-full blur-2xl group-hover:bg-${glowColor}-500/20 transition-all`}
      ></div>
      <div className="flex items-center gap-2 sm:gap-4 relative z-10">
        <div
          className={`p-2 sm:p-4 rounded-xl bg-slate-50 border border-slate-200 text-${glowColor}-400 shadow-sm`}
        >
          {icon}
        </div>
        <div className="min-w-0">
          <p className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase tracking-wider mb-1 truncate">
            {label}
          </p>
    ...
    >
        ...
        {!embedded && (
          <div>
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
              <div className="bg-orange-500/10 p-2.5 rounded-xl border border-orange-500/30 shadow-sm">
                <Users className="text-orange-400" size={28} />
              </div>
              Control de <span className="text-orange-400">Accesos</span>
            </h1>
            <p className="text-slate-500 text-sm mt-2 font-medium">
              Administración centralizada de usuarios y roles del sistema
            </p>
          </div>
        )}
      ...
      <div className="bg-white backdrop-blur-xl p-3 rounded-2xl border border-slate-200 shadow-2xl flex flex-col lg:flex-row gap-3 items-stretch lg:items-center sticky top-4 z-30">
        <div className="flex-1 relative w-full group">
          <Search
            className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-orange-400 transition-colors"
            size={20}
          />
          <input
            type="text"
        ...
        <div className="flex gap-2 w-full lg:w-auto overflow-x-auto pb-1 lg:pb-0">
          <select
            className="px-3 py-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl outline-none focus:border-orange-500 font-bold text-slate-700 cursor-pointer text-sm"
            value={roleFilter}
          ...
          <div className="flex bg-slate-50 border border-slate-200 rounded-xl p-1 shrink-0">
            <button
              ...
              title="Tarjetas"
              className={`p-2 rounded-lg transition-colors ${viewMode === 'cards' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-slate-700'}`}
            >
              <LayoutGrid size={18} />
            </button>
            <button
              ...
              title="Tabla"
              className={`p-2 rounded-lg transition-colors ${viewMode === 'table' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-slate-700'}`}
            >
              <List size={18} />
            </button>
          </div>
      ...
      {selected.size > 0 && (
        <div className="sticky top-24 z-30 bg-slate-900 text-white rounded-2xl px-4 py-3 flex flex-wrap items-center gap-3 shadow-xl relative anim-fade-up">
          ...
          <button
            onClick={() => doBulk('activar')}
            disabled={bulkMutation.isPending}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 text-xs font-black hover:bg-emerald-500/30 disabled:opacity-50"
          >
            <Power size={14} /> Activar
          </button>
          <button
            onClick={() => doBulk('desactivar')}
            disabled={bulkMutation.isPending}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-500/20 text-slate-200 border border-slate-400/30 text-xs font-black hover:bg-slate-500/30 disabled:opacity-50"
          >
          ...
          <button
            onClick={() => doBulk('eliminar')}
            disabled={bulkMutation.isPending}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-500/20 text-rose-300 border border-rose-400/30 text-xs font-black hover:bg-rose-500/30 disabled:opacity-50 ml-auto"
          >
      ...
      <div className="flex items-center justify-between text-xs font-bold text-slate-400 relative z-10 px-1">
        ...
        {filteredUsers.length > 0 && (
          ...
          >
            {allVisibleSelected ? <CheckSquare size={15} /> : <Square size={15} />} Seleccionar
            todos
          </button>
        )}
      </div>
                ...
                <tbody>
                  {filteredUsers.map((user) => {
                    ...
                    return (
                      <tr
                        key={user.id}
                        className={`border-b border-slate-100 last:border-0 transition-colors ${isSel ? 'bg-orange-50/60' : 'hover:bg-slate-50'}`}
                      >
                        <td className="p-3">
                          <button onClick={(e) => toggleSelect(user.id, e)}>
                            {isSel ? (
                              <CheckSquare size={16} className="text-orange-600" />
                            ) : (
                              <Square size={16} className="text-slate-300" />
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-2.5 min-w-0">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs shrink-0 ${getRoleBadgeColor(user.rol)}`}
                            >
                        ...
                        <td className="p-3 hidden sm:table-cell">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg border text-xs font-bold ${getRoleBadgeColor(user.rol)}`}
                          >
                        ...
                        <td className="p-3">
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${user.activo ? 'bg-emerald-500' : 'bg-slate-400'}`}
                            />
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
            {filteredUsers.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className={`group bg-white backdrop-blur-md rounded-3xl border shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden relative ${isSel ? 'border-orange-400 ring-2 ring-orange-200' : 'border-slate-200 hover:border-slate-600'}`}
                >
                  <div
                    className={`h-24 w-full absolute top-0 left-0 transition-colors ${user.activo ? 'bg-gradient-to-b from-orange-500/10 to-transparent' : 'bg-slate-100'}`}
                  ></div>

                  {/* checkbox selección */}
                  <button
                    onClick={(e) => toggleSelect(user.id, e)}
                    className="absolute top-3 left-3 z-20 p-1 rounded-lg bg-white/80 backdrop-blur border border-slate-200 shadow-sm"
                  >
                    {isSel ? (
                  ...
                  <div className="p-4 sm:p-6 relative pt-6 sm:pt-8">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {(user.rol === 'ADMIN' || user.es_admin_delegado) && (
                          <span
                            className={`p-1.5 rounded-lg border ${user.es_admin_delegado ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`}
                            title={
                              user.es_admin_delegado ? 'Administrador Delegado' : 'Administrador'
                            }
                          >
                            <Crown size={14} />
                          </span>
                    ...
                    <div className="space-y-1 mb-4">
                      <h3
                        className="font-black text-xl text-slate-900 leading-tight truncate"
                        title={user.nombre}
                      >
                        {user.nombre}
                      </h3>
                      ...
                      <p className="text-[11px] text-slate-400 font-bold flex items-center gap-1">
                        <Clock size={11} /> Último acceso: {fmtUltimo(user.last_seen)}
                      </p>
                    </div>

                    <div className="flex items-center gap-2 mb-6">
                      <div
                        className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-xl border ${getRoleBadgeColor(user.rol)}`}
                      >
                        {getRoleIcon(user.rol)}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-50/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div
            ref={modalRef}
            className="bg-white border border-slate-200 rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden relative max-h-[92vh] overflow-y-auto"
          >
            ...
            <form onSubmit={handleSave} className="p-4 sm:p-8 space-y-4 sm:space-y-6">
              <div className="space-y-5">
                <div className="group">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      required
                      className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/30 transition-all font-bold text-slate-900 placeholder-slate-600"
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-black">
                      @
                    </div>
                    <input
                      type="email"
                ...
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-4 sm:gap-6">
                  <div className="group">
                    ...
                    <div className="relative">
                      <select
                        required
                        className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/30 transition-all font-bold text-slate-900 appearance-none cursor-pointer"
                        value={formData.rol}
                  ...
                  <div>
                    ...
                    >
                      <div
                        className={`w-12 h-6 rounded-full relative transition-colors ${formData.activo ? 'bg-emerald-500' : 'bg-slate-200'}`}
                      >
                        <div
                          className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform shadow-sm ${formData.activo ? 'translate-x-6' : 'translate-x-0'}`}
                        ></div>
                      </div>
                      <span
                        className={`ml-3 font-bold text-sm ${formData.activo ? 'text-emerald-600' : 'text-slate-500'}`}
                      >
                    ...
                    return (
                      <div className="bg-orange-50 rounded-xl p-4 border border-orange-100 text-xs space-y-2">
                        <div className="font-black text-orange-800">
                          Este rol otorga {permisos.length} permiso(s) · verá {modulos.length}{' '}
                          módulo(s)
                          {rolSel.descripcion ? (
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {modulos.map((m) => (
                            <span
                              ...
                              className={`px-2 py-0.5 rounded-md border font-bold ${m.soloAdmin ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-white text-orange-700 border-orange-200'}`}
                            >
                ...
                <div className="bg-amber-500/10 rounded-xl p-4 border border-amber-500/20 mt-4 flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                      <label className="text-sm font-bold text-amber-400 block">
                      ...
                      <div
                        onClick={() =>
                          setFormData((prev) => ({
                            ...prev,
                            es_admin_delegado: !prev.es_admin_delegado
                          }))
                        }
                      ...
                      >
                        <div
                          className={`absolute top-0.5 left-0.5 bg-white w-4 h-4 rounded-full transition-transform shadow-sm ${formData.es_admin_delegado ? 'translate-x-5' : 'translate-x-0'}`}
                        ></div>
                      </div>
                    </div>
              ...
              <div className="pt-6 flex gap-4 border-t border-slate-200">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="flex-1 py-4 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-xl font-bold transition-colors"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="flex-[2] py-4 bg-orange-600 text-white rounded-xl font-black flex items-center justify-center gap-2 shadow-md transition-all hover:scale-[1.02] disabled:opacity-70 disabled:scale-100"
                >
