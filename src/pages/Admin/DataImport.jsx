...
const AccessConfigPanel = ({ onClose }) => {
  ...
  const saveRole = async (role) => {
    ...
    const { error } = await supabase
      .from('tms_roles')
      .update({ import_tabs: role.import_tabs })
      .eq('id', role.id);
    setSaving(null);
    if (!error) {
      setSaved(role.id);
  ...
  const groupedTabs = useMemo(
    () =>
      CATEGORIES.map((cat) => ({
        ...cat,
        tabs: IMPORT_TABS.filter((t) => t.category === cat.id)
      })),
    []
  );
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium">
                Configura qué módulos de carga puede usar cada rol
              </p>
            </div>
          </div>
          <button
                    ...
                    <div className="space-y-2">
                      {groupedTabs.map((cat) => (
                        <div key={cat.id} className="flex items-start gap-3">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider w-24 pt-1 shrink-0">
                            {cat.label}
                          </span>
...
const DataImport = () => {
  ...
  useEffect(() => {
    if (!esNvCat || step !== 'preview' || parsedRows.length === 0) {
      setNvCross(null);
      setNvLenInfo(null);
      return;
    }
    ...
    (async () => {
      ...
      for (const r of parsedRows) {
        ...
        if (nv && setOtro.has(nv)) {
          count++;
          if (ej.length < 6) ej.push(nv);
        }
      }
      setNvCross({
        count,
        canalOtro: otros.join('/').toUpperCase(),
      ...
      if (actualSet.size >= 20) {
        const lenCount = {};
        actualSet.forEach((nv) => {
          const L = normNvVal(nv).length;
          if (L) lenCount[L] = (lenCount[L] || 0) + 1;
        });
        const umbral = Math.max(2, actualSet.size * 0.01);
        const allowed = Object.keys(lenCount)
          .map(Number)
          .filter((L) => lenCount[L] >= umbral)
          .sort((a, b) => a - b);
        if (allowed.length) {
          const maxLen = Math.max(...allowed);
          if (!allowed.includes(maxLen + 1)) allowed.push(maxLen + 1); // permite el crecimiento natural
          const principal = Object.keys(lenCount)
            .map(Number)
            .reduce((a, b) => (lenCount[b] > (lenCount[a] || 0) ? b : a), allowed[0]);
  ...
  const esClienteMonto = (c) => {
    const t = String(c ?? '').trim();
    return t !== '' && /\d/.test(t) && !/[A-Za-zÁÉÍÓÚÑáéíóúñ]/.test(t);
  };
  const nvMalformed = useMemo(() => {
    ...
    const ej = [];
    for (const r of parsedRows) {
      if (esClienteMonto(r.cliente)) {
        count++;
        if (ej.length < 5) ej.push(`N.V ${r.nv}: “${String(r.cliente).slice(0, 20)}”`);
      }
    }
  ...
  const nvLenBad = useMemo(() => {
    ...
    for (const r of parsedRows) {
      ...
      if (L && !allowed.has(L)) {
        count++;
        if (ej.length < 5) ej.push(`N.V ${r.nv} (${L} díg.)`);
      }
    }
    return { count, ejemplos: ej };
      ...
      if (currentTab.smartDedup && currentTab.uniqueKey && rows.length > 0) {
        try {
          ...
          const deletedPromises =
            currentTab.id === 'nv'
              ? keyChunks.map((chunk) =>
                  supabase.from('tms_nv_eliminadas').select(firstKey).in(firstKey, chunk)
                )
              : [];
          ...
          console.error('Row status check error:', _);
  ...
  const getScanField = () => {
    ...
    return (
      SCAN_FIELD_MAP[tabId] ||
      currentTab.columns.find((c) => c.required)?.key ||
      currentTab.columns[0]?.key
    );
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      if (esNvCat && replaceCanal) {
        const { error: purgeErr } = await supabase.rpc('nv_catalogo_purgar_canal', {
          p_canal: canalNv
        });
      ...
      let newRows =
        esNvCat && replaceCanal
          ? parsedRows.slice()
          : parsedRows.filter(
              (_, idx) => rowStatuses[idx] === 'new' || rowStatuses[idx] === 'update'
            );
      ...
      if (currentTab.id === 'nv' && newRows.length > 0) {
        try {
          ...
          if (prepError) {
            errorDetails.push(`Advertencia: ${prepError.message}`);
          } else if (prepData) {
            if (prepData.reseteadas > 0)
              errorDetails.push(`Se reiniciaron ${prepData.reseteadas} N.V. en proceso.`);
            if (prepData.items_cancelados > 0)
              errorDetails.push(`Se cancelaron ${prepData.items_cancelados} ítems faltantes.`);
          }
        } catch (_) {
          console.error('Record upsert error:', _);
        }
      }
      ...
      if (currentTab.uniqueKey) {
        ...
        if (deduplicated > 0) {
          errorDetails.push(
            `${deduplicated} filas duplicadas consolidadas (misma clave: ${currentTab.uniqueKey})`
          );
        }
      }
      ...
      let inserted = 0,
        errors = 0;
      const conflictKeys =
        !currentTab.uniqueKey || currentTab.id === 'control_despacho' ? null : currentTab.uniqueKey;
      ...
      supabase
        ...
        .then(({ error }) => {
          if (error)
            console.error('[DataImport] No se pudo registrar el historial de carga:', error);
        });
  ...
  const handleReset = () => {
    ...
    setSkippedInvalid(0);
    setStep('paste');
    setSkipFirstColumn(false);
    setSyncDeleted(false);
    setNvCross(null);
    setReplaceCanal(false);
    setScannedItems([]);
    setScanError('');
    setManualScanValue('');
    setInputMode('paste');
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5 font-medium">
              {currentTab?.label} · Tabla:{' '}
              <span className="font-mono text-slate-500">{currentTab?.table}</span>
            </p>
          </div>
          <div className="flex items-center gap-2">
            {isFullAccess && (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-violet-600 bg-violet-50 hover:bg-violet-100 rounded-xl transition-colors border border-violet-200"
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-4">
              <div className="flex items-start gap-3 mb-4">
                <div
                  className={`w-8 h-8 rounded-lg ${catColor.active} flex items-center justify-center shrink-0`}
                >
                  <Info size={16} />
                </div>
                <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {currentTab.columns.map((col) => (
                  <span
                    key={col.key}
                    className={`text-[10px] px-2.5 py-1 rounded-md font-mono ${col.required ? 'bg-orange-50 text-orange-700 font-bold border border-orange-200' : 'bg-slate-50 text-slate-500 border border-slate-200'}`}
                  >
                    {col.label}
                    {col.required && <span className="text-orange-400 ml-0.5">*</span>}
                  </span>
                ))}
              </div>
            </div>
            ...
            {activeTab === 'nv' && inputMode === 'paste' && (
              <div className="flex items-center gap-3 mb-4">
                <label className="flex items-center gap-2 text-xs text-slate-600 bg-white px-3.5 py-2.5 rounded-xl border border-slate-200 cursor-pointer select-none font-bold hover:border-slate-300 transition-colors">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (rawText) setTimeout(() => parseData(rawText), 50);
                    }}
                ...
                <label
                  className="flex items-center gap-2 text-xs text-slate-600 bg-white px-3.5 py-2.5 rounded-xl border border-slate-200 cursor-pointer select-none font-bold hover:border-slate-300 transition-colors"
                  ...
                  {!rawText && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none p-6 text-center">
                      ...
                      <p className="text-xs text-slate-400 mb-5">
                        <kbd className="bg-slate-100 px-1.5 py-0.5 rounded text-[10px] border border-slate-200 mx-0.5">
                          Ctrl+V
                        </kbd>{' '}
                  ...
                  <div className="flex flex-col items-center text-center mb-5">
                    ...
                    <p className="text-xs text-slate-400">
                      ...
                      <span className="font-bold text-violet-600">
                        {currentTab.columns.find((c) => c.key === getScanField())?.label ||
                          getScanField()}
                      </span>
                    </p>
                  </div>
                ...
                {scannedItems.length > 0 && (
                  <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                    ...
                    <div className="px-5 py-3 border-t border-slate-100 bg-slate-50">
                      ...
                      >
                        <ArrowRight size={16} />
                        Procesar {scannedItems.length} escaneado
          ...
          <div className="flex flex-col gap-4">
            {skippedInvalid > 0 && (
              <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
                ...
                <span>
                  <b>{skippedInvalid.toLocaleString()}</b> fila(s) se ignoraron por faltar campos
                  obligatorios (
                  {currentTab.columns
                    .filter((c) => c.required)
                    .map((c) => c.label)
                    .join(', ')}
                  ). Se cargarán solo las {parsedRows.length.toLocaleString()} filas válidas.
                  Corrige esas filas en el archivo si necesitas incluirlas.
                </span>
              </div>
            )}
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3">
                  ...
                  <span className="text-sm text-orange-900">
                    ...
                    {nvCross ? (
                      <>
                        {' '}
                        · actualmente hay <b>{nvCross.actual.toLocaleString()}</b> N.V. en{' '}
                        {canalNv.toUpperCase()}.
                      </>
                    ) : (
                      ' · verificando…'
                ...
                {nvCross && nvCross.count > 0 && (
                  <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-800">
                    ...
                    <span>
                      <b>{nvCross.count.toLocaleString()}</b> N.V. de este archivo ya existen en el
                      canal <b>{nvCross.canalOtro}</b>
                      {canalNv !== 'ptm' && (
                        <>
                          {' '}
                          — y PTM no comparte numeración con Orange/Farmapack, así que esto suele
                          indicar que el archivo es de <b>otro canal</b>
                        </>
                      )}
                      . Ej: {nvCross.ejemplos.join(', ')}.{' '}
                      <b>Revisá que el archivo corresponda a {canalNv.toUpperCase()}</b> antes de
                      confirmar.
                    </span>
                  </div>
                )}
                ...
                {nvMalformed.count > 0 && (
                  <div
                    className={`flex items-start gap-2 rounded-xl px-4 py-3 text-sm ${nvBloqueado ? 'bg-red-100 border-2 border-red-400 text-red-900' : 'bg-amber-50 border border-amber-200 text-amber-900'}`}
                  >
                    <AlertCircle
                      size={18}
                      className={`shrink-0 mt-0.5 ${nvBloqueado ? 'text-red-600' : 'text-amber-500'}`}
                    />
                    <span>
                      {nvBloqueado ? (
                        <b>⛔ Archivo descuadrado — carga bloqueada. </b>
                      ) : (
                        <b>Revisa el archivo. </b>
                      )}
                      <b>{nvMalformed.count.toLocaleString()}</b> fila(s) traen el{' '}
                      <b>Nombre Cliente</b> con un monto o número en vez de un nombre (columnas
                      corridas o archivo de otro tipo). Ej: {nvMalformed.ejemplos.join(' · ')}.
                      {nvBloqueado && (
                        <>
                          {' '}
                          Corrige el Excel (que la columna Cliente sea el nombre) y vuelve a
                          pegarlo.
                        </>
                      )}
                    </span>
                ...
                {nvLenBad.count > 0 && (
                  ...
                  >
                    ...
                    <span>
                      {nvBloqueado ? (
                        <b>⛔ Largo de N.V. incorrecto — carga bloqueada. </b>
                      ) : (
                        <b>Revisa el largo de N.V. </b>
                      )}
                      En <b>{canalNv.toUpperCase()}</b> las N.V. suelen tener{' '}
                      <b>{nvLenInfo?.principal}</b> dígitos, pero{' '}
                      <b>{nvLenBad.count.toLocaleString()}</b> fila(s) traen otro largo (posible
                      archivo de otro canal o dato mal pegado). Ej: {nvLenBad.ejemplos.join(' · ')}.
                ...
                <label className="flex items-start gap-2.5 bg-white border border-slate-200 rounded-xl px-4 py-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={replaceCanal}
                    onChange={(e) => setReplaceCanal(e.target.checked)}
                    className="mt-0.5 w-4 h-4 accent-emerald-600"
                  />
                  <span className="text-sm text-slate-700">
                    <b>Reemplazar todo el canal {canalNv.toUpperCase()}</b> con este archivo
                    <span className="block text-xs text-slate-400 mt-0.5">
                      Borra las {nvCross ? nvCross.actual.toLocaleString() : '—'} N.V. actuales de{' '}
                      {canalNv.toUpperCase()} y deja EXACTAMENTE las de este archivo. Úsalo para
                      corregir cargas mezcladas o dejar el canal limpio (sin residuos ni
                      duplicados).
                    </span>
                  </span>
                </label>
            ...
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 bg-white p-2 sm:p-3 rounded-xl border border-slate-200">
              ...
              {stats.update > 0 && (
                <StatPill icon={RefreshCw} label="Actualizar" value={stats.update} color="blue" />
              )}
              {stats.existing > 0 && (
                <StatPill icon={SkipForward} label="Existen" value={stats.existing} color="amber" />
              )}
              {stats.deleted > 0 && (
                <StatPill icon={Trash2} label="Eliminadas" value={stats.deleted} color="rose" />
              )}
              {stats.error > 0 && (
                <StatPill icon={XCircle} label="Errores" value={stats.error} color="red" />
              )}
              ...
              <button
                ...
                disabled={
                  isLoading ||
                  nvBloqueado ||
                  (stats.new === 0 && stats.update === 0 && !(esNvCat && replaceCanal))
                }
                title={
                  nvBloqueado
              ...
              >
                ...
                {isLoading
                  ? uploadProgress.total > 0
                    ...
                    : 'Preparando...'
                  : esNvCat && replaceCanal
                    ? `Reemplazar ${canalNv.toUpperCase()} (${stats.new + stats.update})`
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {parsedRows.slice(0, 500).map((row, idx) => {
                      ...
                      return (
                        <tr
                          key={idx}
                          className={`${bgMap[status]} hover:bg-slate-50/50 transition-colors`}
                        >
                          ...
                          {currentTab.columns.map((col) => (
                            <td
                              key={col.key}
                              className="px-3 py-2.5 text-slate-600 whitespace-nowrap max-w-[180px] truncate text-xs"
                    ...
                    {parsedRows.length > 500 && (
                      <tr>
                        <td
                          colSpan={currentTab.columns.length + 2}
                          className="px-4 py-6 bg-slate-50 text-center text-xs text-slate-400 font-bold"
                        >
                          Vista previa: 500 de {parsedRows.length.toLocaleString()} filas · Se
                          procesarán todas al confirmar
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        {step === 'done' && loadResult && (
          <div className="max-w-lg mx-auto mt-8">
            <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center">
              <div
                className={`w-16 h-16 mx-auto rounded-2xl flex items-center justify-center mb-5 ${loadResult.success ? 'bg-emerald-50 text-emerald-500' : 'bg-red-50 text-red-500'}`}
              >
              ...
              <div
                className={`grid ${loadResult.deduplicated > 0 ? 'grid-cols-4' : 'grid-cols-3'} gap-3 mb-6`}
              >
              ...
              {loadResult.errorDetails?.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-left mb-6 max-h-36 overflow-y-auto">
                  <p className="text-[10px] font-bold text-red-800 uppercase tracking-wider mb-2">
                    Detalle:
                  </p>
                  {loadResult.errorDetails.map((e, i) => (
                    <p key={i} className="text-xs text-red-600 font-mono mb-1">
                      {e}
                    </p>
                  ))}
                </div>
              ...
              <button
                onClick={handleReset}
                className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold text-sm transition-colors"
              >
...
const StatPill = ({ icon: Icon, label, value, color }) => {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold ${colorMap[color]}`}
    >
      <Icon size={13} />
      <span>{label}:</span>
