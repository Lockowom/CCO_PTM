...
export default function ApiKeys() {
  ...
  const cargar = useCallback(async () => {
    ...
    setLog(await listarApiLog());
  }, []);
  useEffect(() => {
    cargar();
  }, [cargar]);

  const copiar = async (txt, tag) => {
    try {
  ...
  return (
    <div className="anim-fade-up space-y-4 max-w-[1100px] mx-auto pb-16">
        ...
        <div className="flex gap-2">
          ...
          {puede && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-orange-500 text-white text-sm font-bold hover:bg-orange-600"
            >
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <p className="text-[11px] font-black text-slate-400 uppercase mb-2 flex items-center gap-1.5">
          <Terminal size={12} /> Endpoint
        </p>
        <div className="flex items-center gap-2 flex-wrap">
          <code className="text-[12px] font-mono bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-slate-700 break-all">
            {API_BASE}
          </code>
          <button
            onClick={() => copiar(API_BASE, 'url')}
...
curl -X POST "${API_BASE}/operaciones/estado" \\
  ...
  -d '{"id":123,"estado":"En Ruta"}'   # cambia estado`}</pre>
        <p className="text-[10px] text-slate-400 mt-1.5">
          Contrato completo: <code>GET {API_BASE}/</code> (sin key). Auth por header{' '}
          <code>x-api-key</code>.
        </p>
      </div>
      ...
      {creada && (
        <div className="rounded-2xl border-2 border-orange-300 bg-orange-50 p-4">
          <p className="text-[12px] font-black text-orange-700 mb-1">
            ⚠ Copia esta clave ahora — no se vuelve a mostrar
          </p>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-[13px] font-mono bg-white border border-orange-200 rounded-lg px-2 py-1.5 text-slate-800 break-all">
              {creada.key}
            </code>
            <button
              onClick={() => copiar(creada.key, 'key')}
              className="px-3 py-1.5 rounded-lg bg-orange-500 text-white text-[12px] font-bold inline-flex items-center gap-1.5"
            >
              {copied === 'key' ? <Check size={13} /> : <Copy size={13} />} Copiar
            </button>
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div className="px-4 py-2.5 border-b border-slate-100 text-[11px] font-black text-slate-400 uppercase">
          Claves ({keys.length})
        </div>
        {keys.length === 0 ? (
          <div className="py-12 text-center text-slate-400 text-sm">
            Sin claves. Crea una para integrar un sistema externo.
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {keys.map((k) => (
              <div
                key={k.id}
                className={`flex items-center gap-3 px-4 py-3 ${!k.activo ? 'opacity-50' : ''}`}
              >
                <span className="font-mono text-[12px] text-slate-500 bg-slate-100 rounded px-1.5 py-0.5 shrink-0">
                  cco_{k.prefijo}…
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-bold text-[13px] text-slate-800">{k.nombre}</span>
                    {!k.activo && (
                  ...
                  <div className="flex items-center gap-1 flex-wrap mt-0.5">
                    {(k.scopes || []).map((s) => (
                      <span
                        key={s}
                        className="text-[9px] font-mono text-slate-500 bg-slate-100 rounded px-1.5 py-0.5"
                      >
                        {s}
                      </span>
                    ))}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div className="px-4 py-2.5 border-b border-slate-100 text-[11px] font-black text-slate-400 uppercase">
          Últimas llamadas ({log.length})
        </div>
        {log.length === 0 ? (
          <div className="py-10 text-center text-slate-400 text-sm">Sin llamadas registradas.</div>
        ) : (
          <div className="divide-y divide-slate-100 max-h-[45vh] overflow-y-auto">
            {log.map((l) => (
              <div key={l.id} className="flex items-center gap-3 px-4 py-2 text-[12px]">
                <span
                  className={`text-[9px] font-black rounded px-1.5 py-0.5 shrink-0 ${l.estado_http < 300 ? 'text-emerald-700 bg-emerald-50' : l.estado_http < 500 ? 'text-amber-700 bg-amber-50' : 'text-red-700 bg-red-50'}`}
                >
                  {l.estado_http}
                </span>
...
function NuevaKeyModal({ onClose, onCreada }) {
  ...
  const crear = async () => {
    ...
    setBusy(true);
    const r = await crearApiKey(nombre.trim(), scopes);
    setBusy(false);
    if (r?.ok) {
    ...
    <div className="fixed inset-0 z-[130] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-[15px] font-black text-slate-800">Nueva API key</h3>
        ...
        <div className="space-y-3">
          <label className="block">
            <span className="text-[11px] font-bold text-slate-500 uppercase">Nombre / sistema</span>
            <input
              autoFocus
              value={nombre}
          ...
          <div>
            ...
            <div className="mt-1 grid grid-cols-2 gap-1.5">
              {SCOPES.map((s) => (
                <label
                  key={s}
                  className={`flex items-center gap-2 text-[12px] font-mono border rounded-lg px-2 py-1.5 cursor-pointer ${scopes.includes(s) ? 'border-orange-400 bg-orange-50 text-orange-700' : 'border-slate-200 text-slate-600'}`}
                >
