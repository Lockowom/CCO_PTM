...
import {
  ...
  agregarMiembroGroup,
  quitarMiembroGroup,
  asignacionesPrincipal,
  asignarRolPrincipal,
  revocarAsignacionPrincipal,
  refrescarGruposDinamicos
        ...
        <div key={row.id} className="flex items-center gap-3 px-4 py-2.5 text-[13px]">
          ...
          {onRemove && (!dynamic || !row.dinamico) && (
            <button
              ...
              className="ml-auto w-8 h-8 rounded-lg hover:bg-red-50 grid place-items-center text-red-400 shrink-0"
              title="Quitar"
            >
              <Trash2 size={15} />
            </button>
        ...
        <div className="p-4 grid sm:grid-cols-3 gap-3 border-b border-slate-100">
          ...
          <label className="block">
            ...
            <select
              ...
              onChange={(e) => setForm((f) => ({ ...f, scopeType: e.target.value, scopeCode: '' }))}
              className={`${inp} mt-1`}
            >
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => setForm((f) => ({ ...f, scopeCode: e.target.value.trim() }))}
              className={`${inp} mt-1 disabled:bg-slate-50`}
              placeholder={form.scopeType === 'global' ? 'No aplica' : 'código'}
            />
          </label>
          <div className="sm:col-span-3 flex justify-end">
            <button
              onClick={onAssign}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-orange-500 text-white text-sm font-bold hover:bg-orange-600"
            >
...
export default function Teams() {
  ...
  const [groupAssignForm, setGroupAssignForm] = useState({
    role: '',
    scopeType: 'global',
    scopeCode: ''
  });
  ...
  const cargarBase = useCallback(async () => {
    ...
    setError(null);
    try {
      const [catalogo, teamRows, groupRows] = await Promise.all([
        catalogoOrg(),
        listarTeams(),
        listarGroups()
      ]);
  ...
  useEffect(() => {
    if (selectedTeam?.id) cargarDetalleTeam(selectedTeam);
  }, [selectedTeam?.id, cargarDetalleTeam]);
  useEffect(() => {
    if (selectedGroup?.id) cargarDetalleGroup(selectedGroup);
  }, [selectedGroup?.id, cargarDetalleGroup]);
  ...
  const assignTeamRole = async () => {
    ...
    const r = await asignarRolPrincipal({
      principalType: 'team',
      principalId: selectedTeam.id,
      role: teamAssignForm.role,
      scopeType: teamAssignForm.scopeType,
      scopeCode: teamAssignForm.scopeCode || null
    });
  ...
  const assignGroupRole = async () => {
    ...
    const r = await asignarRolPrincipal({
      ...
      role: groupAssignForm.role,
      scopeType: groupAssignForm.scopeType,
      scopeCode: groupAssignForm.scopeCode || null
    });
    if (!r?.ok) return toast.error(r?.error || 'No se pudo asignar el rol');
  ...
  const refreshDynamic = async () => {
    const r = await refrescarGruposDinamicos(
      selectedGroup?.tipo === 'dynamic' ? selectedGroup.id : null
    );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-sky-50/60 px-4 py-3 text-[12.5px] text-slate-600">
        `Teams` y `Groups` permiten heredar roles IAM a varios usuarios a la vez. Los equipos
        agrupan personas operativas; los grupos pueden ser `static` o `dynamic`.
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {tab === 'groups' && (
          <button
            onClick={refreshDynamic}
            className="ml-auto inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-600 hover:bg-slate-50"
          >
              ...
              <div className="divide-y divide-slate-100">
                {teams.map((t) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-slate-50 ${selectedTeam?.id === t.id ? 'bg-orange-50/70' : ''}`}
                  >
                    <div className="font-bold text-slate-800">{t.nombre}</div>
                    <div className="text-[11px] text-slate-500">
                      {t.codigo} · {t.departamento || 'Sin departamento'} · {t.miembros} miembro(s)
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div className="grid sm:grid-cols-3 gap-3">
                  <label className="block">
                    ...
                    <input
                      value={teamForm.codigo}
                      onChange={(e) =>
                        setTeamForm((f) => ({ ...f, codigo: e.target.value.toUpperCase() }))
                      }
                      className={`${inp} mt-1`}
                    />
                  </label>
                  ...
                  <label className="block">
                    ...
                    <select
                      value={teamForm.departamento_id}
                      onChange={(e) =>
                        setTeamForm((f) => ({ ...f, departamento_id: e.target.value }))
                      }
                      className={`${inp} mt-1`}
                    >
                      <option value="">—</option>
                ...
                <div className="mt-3 flex items-center justify-between">
                  <label className="text-[12px] font-semibold text-slate-500 inline-flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={teamForm.activo}
                      onChange={(e) => setTeamForm((f) => ({ ...f, activo: e.target.checked }))}
                    />
                    Activo
                  </label>
                  <div className="flex gap-2">
                    {teamForm.id && (
                      <button
                        onClick={removeTeam}
                        className="px-3 py-2 rounded-xl border border-red-200 text-red-500 text-sm font-bold hover:bg-red-50"
                      >
                        Eliminar
                      </button>
                    )}
            ...
            {selectedTeam ? (
              <>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
                  ...
                  <MemberList
                    ...
                    onRemove={
                      puede
                        ? async (userId) => {
                            const r = await quitarMiembroTeam(selectedTeam.id, userId);
                            if (!r?.ok) return toast.error(r?.error || 'No se pudo quitar');
                ...
                <PrincipalAssignments
                  ...
                  roles={cat.roles}
                  form={teamAssignForm}
                  setForm={setTeamAssignForm}
                  onAssign={assignTeamRole}
                  onRemove={
                    puede
                      ? async (id) => {
                          const r = await revocarAsignacionPrincipal(id);
                          if (!r?.ok) return toast.error(r?.error || 'No se pudo revocar');
              ...
              <div className="divide-y divide-slate-100">
                {groups.map((g) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-slate-50 ${selectedGroup?.id === g.id ? 'bg-orange-50/70' : ''}`}
                  >
                    <div className="font-bold text-slate-800">{g.nombre}</div>
                    <div className="text-[11px] text-slate-500">
                      {g.codigo} · {g.tipo} · {g.miembros} miembro(s)
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="space-y-4">
            {puede && (
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-[12px] font-black text-slate-600 uppercase tracking-wide">
                    {groupForm.id ? 'Editar grupo' : 'Nuevo grupo'}
                  </h3>
                  {groupForm.id ? (
                    <button
                      onClick={() => setGroupForm(emptyGroup)}
                      className="text-xs font-bold text-slate-400 hover:text-slate-600"
                    >
                      Limpiar
                    </button>
                ...
                <div className="grid sm:grid-cols-3 gap-3">
                  <label className="block">
                    <span className={lbl}>Código</span>
                    <input
                      value={groupForm.codigo}
                      onChange={(e) =>
                        setGroupForm((f) => ({ ...f, codigo: e.target.value.toUpperCase() }))
                      }
                      className={`${inp} mt-1`}
                    />
                  </label>
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={(e) => setGroupForm((f) => ({ ...f, nombre: e.target.value }))}
                      className={`${inp} mt-1`}
                    />
                  </label>
                  <label className="block">
                    ...
                    <select
                      ...
                      onChange={(e) => setGroupForm((f) => ({ ...f, tipo: e.target.value }))}
                      className={`${inp} mt-1`}
                    >
                ...
                {groupForm.tipo === 'dynamic' && (
                  <div className="grid sm:grid-cols-3 gap-3 mt-3">
                    <label className="block">
                      <span className={lbl}>User IDs</span>
                      <input
                        value={(groupForm.regla?.user_ids || []).join(',')}
                        onChange={(e) =>
                          setGroupForm((f) => ({
                            ...f,
                            regla: {
                    ...
                    <label className="block">
                      <span className={lbl}>Team IDs</span>
                      <input
                        value={(groupForm.regla?.team_ids || []).join(',')}
                        onChange={(e) =>
                          setGroupForm((f) => ({
                            ...f,
                            regla: {
                    ...
                    <label className="block">
                      <span className={lbl}>Department IDs</span>
                      <input
                        value={(groupForm.regla?.department_ids || []).join(',')}
                        onChange={(e) =>
                          setGroupForm((f) => ({
                            ...f,
                            regla: {
                ...
                <div className="mt-3 flex items-center justify-between">
                  <label className="text-[12px] font-semibold text-slate-500 inline-flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => setGroupForm((f) => ({ ...f, activo: e.target.checked }))}
                    />
                    Activo
                  </label>
                  <div className="flex gap-2">
            ...
            {selectedGroup ? (
              <>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
                  ...
                  <MemberList
                    rows={groupMembers}
                    dynamic={selectedGroup.tipo === 'dynamic'}
                    onRemove={
                      puede && selectedGroup.tipo === 'static'
                        ? async (userId) => {
                            const r = await quitarMiembroGroup(selectedGroup.id, userId);
                            if (!r?.ok) return toast.error(r?.error || 'No se pudo quitar');
                            toast.success('Miembro removido');
                            await cargarDetalleGroup(selectedGroup);
                            await cargarBase();
                ...
                <PrincipalAssignments
                  rows={groupAssignments}
                  roles={cat.roles}
                  form={groupAssignForm}
                  setForm={setGroupAssignForm}
                  onAssign={assignGroupRole}
                  onRemove={
                    puede
                      ? async (id) => {
                          const r = await revocarAsignacionPrincipal(id);
