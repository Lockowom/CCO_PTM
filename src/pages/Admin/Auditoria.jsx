...
function diff(antes, despues) {
  ...
  for (const k of keys) {
    ...
    out.push({
      k,
      tipo: sa === undefined ? 'add' : sd === undefined ? 'del' : 'mod',
      antes: sa,
      despues: sd
    });
  }
...
function Fila({ r }) {
  ...
  return (
    <div className="text-[13px]">
      <button
        onClick={() => setOpen((o) => !o)}
        aria-label={open ? 'Contraer detalle' : 'Ver detalle del evento'}
        aria-expanded={open}
      ...
      >
        ...
        <span className={`text-[10px] font-black rounded px-1.5 py-0.5 border shrink-0 ${color}`}>
          {r.accion}
        </span>
        <span className="font-mono text-[11px] text-slate-500 shrink-0">{r.tabla}</span>
        <span className="text-slate-600 truncate">
          reg. <span className="font-mono text-[11px]">{r.registro_id || '—'}</span>
        </span>
        <span className="ml-auto text-slate-500 shrink-0 truncate max-w-[160px]">
          {r.actor}
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50/60 divide-y divide-slate-100 overflow-hidden">
            {cambios.map((c) => (
              <div key={c.k} className="flex items-start gap-2 px-3 py-1.5 text-[12px]">
                {c.tipo === 'add' ? (
                  <Plus size={12} className="text-emerald-500 mt-0.5 shrink-0" />
                ) : c.tipo === 'del' ? (
                  <Minus size={12} className="text-red-500 mt-0.5 shrink-0" />
                ) : (
                  <Pencil size={11} className="text-amber-500 mt-0.5 shrink-0" />
                )}
                <span className="font-mono font-bold text-slate-600 shrink-0">{c.k}</span>
                <span className="min-w-0 break-all">
                  {c.antes !== undefined && (
                    <span className="text-red-500 line-through mr-2">{c.antes}</span>
                  )}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        <select
          value={f.tabla}
          onChange={(e) => setF({ ...f, tabla: e.target.value })}
          className={inp}
        >
        ...
        <select
          value={f.accion}
          onChange={(e) => setF({ ...f, accion: e.target.value })}
          className={inp}
        >
        ...
        <button
          onClick={cargar}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-600 hover:bg-slate-50"
        >
          <RefreshCw size={14} /> Aplicar
        </button>
        <span className="ml-auto text-[11px] text-slate-400 inline-flex items-center gap-1.5">
          <ScrollText size={13} /> {meta.total} eventos registrados
        </span>
      </div>
