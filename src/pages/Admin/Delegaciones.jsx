...
import { Plus, X, Trash2, ArrowRight, CalendarClock, Circle } from 'lucide-react';
import {
  delegaciones as fetchDelegaciones,
  delegar,
  revocarDelegacion,
  catalogoScope
...
function DelegarModal({ usuarios, roles, onClose, onSave }) {
  ...
  const submit = () => {
    if (!f.delegador || !f.delegado || !f.hasta) {
      toast.error('Delegador, cobertura y fecha de término son obligatorios');
      return;
    }
    if (f.delegador === f.delegado) {
      toast.error('El delegador y la cobertura deben ser distintos');
      return;
    }
    onSave({
      delegador: f.delegador,
      delegado: f.delegado,
      role: f.role || null,
    ...
    <div className="fixed inset-0 z-[130] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-[15px] font-black text-slate-800">Nueva delegación</h3>
        ...
        <div className="space-y-3">
          <label className="block">
            <span className={lbl}>Delegador (presta permisos)</span>
            <select
              value={f.delegador}
              onChange={(e) => setF({ ...f, delegador: e.target.value })}
              className={`${inp} mt-1`}
            >
              <option value="">—</option>
              {usuarios.map((u) => (
          ...
          <label className="block">
            <span className={lbl}>Cobertura (recibe permisos)</span>
            <select
              value={f.delegado}
          ...
          <label className="block">
            <span className={lbl}>Rol específico (opcional)</span>
            <select
              value={f.role}
              onChange={(e) => setF({ ...f, role: e.target.value })}
              className={`${inp} mt-1`}
            >
              <option value="">Todos los roles del delegador</option>
              {roles.map((r) => (
...
export default function Delegaciones() {
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      setRows(await fetchDelegaciones(false));
    } catch (e) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-[13px] text-slate-500">
          Sustituciones: un delegador presta sus permisos a otro durante una ventana (vacaciones,
          licencias). Caducan solas.
        </p>
        <button
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        {loading ? (
          ...
          <ListaVacia>Sin delegaciones.</ListaVacia>
        ) : (
          <div className="divide-y divide-slate-100">
            {rows.map((d) => (
              <div key={d.id} className="flex items-center gap-3 px-4 py-2.5 text-[13px]">
                <Circle
                  size={9}
                  className={`shrink-0 rounded-full ${d.vigente ? 'text-emerald-500 fill-emerald-500' : 'text-slate-300 fill-slate-300'}`}
                />
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-slate-400 flex items-center gap-1">
                    <CalendarClock size={11} /> {fmt(d.desde)} → {fmt(d.hasta)}
                    {d.motivo ? ` · ${d.motivo}` : ''}
                  </div>
                </div>
                {d.vigente ? (
                  <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 rounded px-1.5 py-0.5 shrink-0">
                    VIGENTE
                  </span>
                ) : d.activo ? (
                  <span className="text-[10px] font-bold text-slate-400 bg-slate-100 rounded px-1.5 py-0.5 shrink-0">
                    programada/vencida
                  </span>
                ) : (
                  <span className="text-[10px] font-bold text-red-400 bg-red-50 rounded px-1.5 py-0.5 shrink-0">
                    revocada
                  </span>
                )}
                {d.activo && (
