...
export default function Scopes() {
  ...
  const submit = async () => {
    if (!f.userId || !f.role || !f.scopeCode) {
      toast.error('Usuario, rol y código de ámbito son obligatorios');
      return;
    }
    setGuardando(true);
    ...
    setGuardando(false);
    if (r?.ok) {
      toast.success(r.accion === 'actualizado' ? 'Asignación actualizada' : 'Ámbito asignado');
      setF({ ...f, scopeCode: '', expires: '' });
      cargar();
  ...
  const quitar = async (id) => {
    ...
    if (r?.ok) {
      ...
      setAsigs((xs) => xs.filter((x) => x.id !== id));
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-sky-100 bg-sky-50/60 px-4 py-3 flex gap-3">
        ...
        <p className="text-[12.5px] text-slate-600 leading-relaxed">
          Un <b>ámbito</b> otorga un rol <b>solo sobre ciertos datos</b> (p. ej. el centro de costo{' '}
          <span className="font-mono">150</span>). El permiso hace <b>visible</b> el módulo, y el{' '}
          <b>filtrado por dato</b> se aplica donde el módulo adopte
          <span className="font-mono"> can_on_scope</span> /{' '}
          <span className="font-mono">mis_scopes</span>. El rol
          <b> global</b> del usuario se gestiona en la pestaña <b>Usuarios</b>; aquí se agregan
          grants <b>acotados</b>.
        </p>
      </div>
      ...
      {puede && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <label className="block">
              <span className={lbl}>Eje</span>
              <select
                value={f.scopeType}
                onChange={(e) => setF({ ...f, scopeType: e.target.value, scopeCode: '' })}
                className={`${inp} mt-1`}
              >
                {EJES.map((e) => (
            ...
            <label className="block">
              <span className={lbl}>Código de ámbito</span>
              {valoresEje.length > 0 ? (
                <select
                  value={f.scopeCode}
                  onChange={(e) => setF({ ...f, scopeCode: e.target.value })}
                  className={`${inp} mt-1`}
                >
                  <option value="">—</option>
                  {valoresEje.map((v) => (
                ...
                <input
                  value={f.scopeCode}
                  onChange={(e) => setF({ ...f, scopeCode: e.target.value.trim() })}
                  placeholder="código"
                  className={`${inp} mt-1`}
                />
              )}
            ...
            <label className="block">
              <span className={lbl}>Expira (opcional)</span>
              <input
                type="date"
                value={f.expires}
            ...
            {asigs.map((a) => (
              <div key={a.id} className="flex items-center gap-3 px-4 py-2.5 text-[13px]">
                ...
                <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg px-2 py-1 shrink-0">
                  <Target size={11} />
                  {ejeLabel(a.scope_type)}: <span className="font-mono">{a.scope_code}</span>
                </span>
                {fmt(a.expires_at) && (
