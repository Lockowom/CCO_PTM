...
import {
  useBodegasSoftland,
  useGuardarBodegaSoftland,
  useEliminarBodegaSoftland
...
const BodegasSoftland = () => {
  ...
  const guardarFila = async () => {
    ...
    try {
      await guardar.mutateAsync({
        ...
        estado: edit.estado,
        esDestino: edit.es_destino_dictamen,
        activo: edit.activo,
        orden: Number(edit.orden) || 100
      });
  ...
  const borrar = async (b) => {
    ...
    try {
      await eliminar.mutateAsync(b.codigo);
  ...
  if (!isAdmin) {
    return (
      <div className="p-8 text-center text-slate-400 font-bold">
        Solo un administrador puede gestionar las bodegas Softland.
      </div>
    );
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
              Bodegas <span className="text-indigo-600">Softland</span>
            </h1>
                ...
                {bodegas.map((b) => (
                  <tr key={b.codigo} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-[10px] font-black px-2 py-0.5 rounded-md border ${b.estado === 'TRANSITORIO' ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200'}`}
                      >
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <h3 className="font-black text-slate-900">
                {bodegas.some((b) => b.codigo === edit.codigo)
                  ? `Editar bodega ${edit.codigo}`
            ...
            <div className="p-5 grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  value={edit.codigo}
                  disabled={bodegas.some((b) => b.codigo === edit.codigo)}
              ...
              <div>
                ...
                <input
                  value={edit.orden}
                  onChange={(e) =>
                    setEdit({ ...edit, orden: e.target.value.replace(/[^0-9]/g, '') })
                  }
                  inputMode="numeric"
              ...
              <div className="flex items-end gap-4 pb-1">
                <label className="flex items-center gap-2 text-sm font-bold text-slate-600">
                  <input
                    type="checkbox"
                    checked={edit.es_destino_dictamen}
                    onChange={(e) => setEdit({ ...edit, es_destino_dictamen: e.target.checked })}
                  />{' '}
                  Destino dictamen
                </label>
                <label className="flex items-center gap-2 text-sm font-bold text-slate-600">
