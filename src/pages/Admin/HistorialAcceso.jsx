...
import { LogIn, RefreshCw, Search, CalendarClock, Clock } from 'lucide-react';
...
export default function HistorialAcceso() {
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      setRows(await historialAcceso({ desde: f.desde, hasta: f.hasta, q: f.q, limit: 400 }));
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {resumen.ultimo && (
          <span className="ml-auto text-[11px] text-slate-400 inline-flex items-center gap-1.5">
            <Clock size={12} /> último ingreso {hace(resumen.ultimo)}
          </span>
        )}
      </div>
          ...
          <div className="divide-y divide-slate-100">
            {rows.map((r) => (
              <div key={r.id} className="flex items-center gap-3 px-4 py-2.5 text-[13px]">
                <div className="w-9 h-9 rounded-xl bg-slate-900 text-white font-black grid place-items-center shrink-0">
                  {inicial(r.nombre)}
                </div>
                ...
                <div className="text-right shrink-0">
                  <div className="text-[12px] text-slate-600 tabular-nums inline-flex items-center gap-1">
                    <LogIn size={12} className="text-emerald-500" /> {fmt(r.fecha)}
                  </div>
      ...
      <p className="text-[11px] text-slate-400 flex items-center gap-1.5">
        <CalendarClock size={12} /> Registra los <b>ingresos exitosos</b> (Supabase Auth). Se
        muestran los 400 más recientes según el filtro.
      </p>
    </div>
  );
}
