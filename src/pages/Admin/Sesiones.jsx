...
export default function Sesiones() {
  ...
  const cerrar = async (r) => {
    ...
    setBusy(null);
    if (res?.ok) {
      toast.success(`Sesión revocada (${res.revocadas || 0})`);
      cargar();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-[13px] text-slate-500">
          Sesiones activas (Supabase Auth) con presencia y último acceso. {rows.length} en total.
        </p>
        <button
          onClick={cargar}
            ...
            {rows.map((r) => (
              <div key={r.session_id} className="flex items-center gap-3 px-4 py-3 text-[13px]">
                <div className="relative shrink-0">
                  ...
                  <Circle
                    size={9}
                    className={`absolute -bottom-0.5 -right-0.5 rounded-full ${enLinea(r) ? 'text-emerald-500 fill-emerald-500' : 'text-slate-300 fill-slate-300'}`}
                  />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {r.aal === 'aal2' && (
                      <span className="text-[10px] font-bold text-sky-600 bg-sky-50 rounded px-1.5 py-0.5">
                        AAL2
                      </span>
                    )}
                  ...
                  <div className="text-[11px] text-slate-400 truncate">
                    {navUA(r.user_agent)} · {r.ip || 's/IP'} · visto{' '}
                    {fmt(r.refreshed_at || r.created_at)}
                  </div>
                </div>
                <button
                  onClick={() => cerrar(r)}
                  disabled={busy === r.auth_uid}
      ...
      <p className="text-[11px] text-slate-400 flex items-center gap-1.5">
        <UserIcon size={12} /> «Cerrar» revoca las sesiones (refresh tokens) y avisa al equipo por
        realtime para el cierre inmediato.
      </p>
    </div>
  );
}
