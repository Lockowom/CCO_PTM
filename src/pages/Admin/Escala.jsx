...
export default function Escala() {
  ...
  const cargar = useCallback(async () => {
    try {
      setStats(await permisosStats());
    } catch (e) {
      toast.error(e.message || 'No autorizado');
    }
  }, []);
  useEffect(() => {
    cargar();
  ...
  const recalcular = async () => {
    ...
    if (r?.ok) {
      toast.success(`Permisos recalculados (${r.filas} filas)`);
      cargar();
  ...
  const importar = async () => {
    if (filas.length === 0) {
      toast.error('Pega al menos una fila (nombre, email, rol)');
      return;
    }
    if (
      !window.confirm(
        `¿Cargar ${filas.length} usuario(s)? Se crearán identidades reales de acceso.`
      )
    )
      return;
    setImportando(true);
    ...
    if (r?.ok) {
      setResultado(r);
      toast.success(
        `${r.creados} creados · ${r.actualizados} actualizados${r.errores?.length ? ` · ${r.errores.length} con error` : ''}`
      );
      cargar();
  ...
  const copiarPass = () => {
    const lineas = (resultado?.detalle || [])
      .filter((d) => d.password)
      .map((d) => `${d.email}\t${d.password}`)
      .join('\n');
    if (!lineas) {
      toast.info('No hay contraseñas generadas');
      return;
    }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <p className="text-[11px] text-slate-400 mt-2 flex items-center gap-1.5">
          <Database size={12} /> La vista materializada{' '}
          <code className="font-mono">iam.mv_user_permissions</code> se refresca automáticamente
          cada 5 min (pg_cron) y aquí a demanda. El gate de permisos usa la vista viva (sin retraso
          en revocaciones).
        </p>
      </div>
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 space-y-2">
            <div className="flex items-center gap-3 text-[13px] font-bold">
              <span className="inline-flex items-center gap-1 text-emerald-600">
                <Check size={14} /> {resultado.creados} creados
              </span>
              <span className="inline-flex items-center gap-1 text-sky-600">
                <RefreshCw size={13} /> {resultado.actualizados} actualizados
              </span>
              {resultado.errores?.length > 0 && (
                <span className="inline-flex items-center gap-1 text-red-500">
                  <AlertTriangle size={13} /> {resultado.errores.length} error(es)
                </span>
              )}
              {(resultado.detalle || []).some((d) => d.password) && (
                <button
                  onClick={copiarPass}
                  className="ml-auto inline-flex items-center gap-1 text-[12px] font-bold text-orange-600 hover:text-orange-700"
                >
            ...
            {(resultado.detalle || []).filter((d) => d.password).length > 0 && (
              <div className="text-[12px] text-slate-600">
                <p className="font-bold mb-1">
                  Contraseñas generadas (distribúyelas de forma segura):
                </p>
