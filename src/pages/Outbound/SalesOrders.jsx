...
const SalesOrders = () => {
  ...
  // Mutación batch: actualizar múltiples NV de una vez via RPC
  const batchUpdateMutation = useMutation({
    mutationFn: async ({ ids, newStatus, userName }) => {
      const { data, error } = await supabase.rpc('batch_update_nv_estado', {
        p_nv_ids: ids,
    ...
    onSuccess: (data) => {
      ...
      toast.success(`${result?.updated_count || 0} NV actualizadas`);
    },
    onError: (err) => toast.error('Error batch: ' + err.message)
  });

  const handleUpdateStatus = (nv, newStatus) => updateStatusMutation.mutate({ nv, newStatus });

  // Actualizar todas las NV filtradas en un solo click
  const handleBatchUpdate = (newStatus) => {
    const ids = filteredOrders.flatMap(o => o.items?.map(i => i.id) || []);
    if (ids.length === 0) return toast.error('No hay NV para actualizar');
    if (!window.confirm(`¿Actualizar ${filteredOrders.length} NV a "${newStatus}"?`)) return;
    batchUpdateMutation.mutate({ ids, newStatus });
  };
  ...
  return (
    <div ref={containerRef} className="space-y-4 sm:space-y-6 min-h-screen text-slate-800">
      ...
      {/* Acción batch: mostrar solo si hay filtro activo con resultados */}
      {filteredOrders.length > 1 && hasPermission('manage_orders') && (
        <div className="flex items-center gap-2 px-1">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{filteredOrders.length} NV seleccionadas</span>
          {selectedEstado === 'Pendiente' && (
            <button
              onClick={() => handleBatchUpdate('Pendiente Picking')}
              disabled={batchUpdateMutation.isPending}
              className="px-3 py-1.5 bg-cyan-50 text-cyan-700 border border-cyan-200 rounded-lg text-[11px] font-bold hover:bg-cyan-100 transition-all disabled:opacity-50"
            >
              {batchUpdateMutation.isPending ? 'Procesando...' : `Pasar todas a Picking`}
            </button>
          )}
          {selectedEstado === 'LISTO_DESPACHO' && (
            <button
              onClick={() => handleBatchUpdate('Despachado')}
              disabled={batchUpdateMutation.isPending}
              className="px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-[11px] font-bold hover:bg-emerald-100 transition-all disabled:opacity-50"
            >
              {batchUpdateMutation.isPending ? 'Procesando...' : `Despachar todas`}
            </button>
          )}
        </div>
      )}
        ...
        <div className="overflow-x-auto -mx-2 px-2 custom-scrollbar">
          <table className="w-full text-sm text-left min-w-[700px]">
            <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] tracking-widest font-bold border-b border-slate-200">
              <tr>
                <th className="px-2 sm:px-4 md:px-6 py-4">N.V</th>
                <th className="px-2 sm:px-4 md:px-6 py-4">Fecha</th>
                <th className="px-2 sm:px-4 md:px-6 py-4">Cliente</th>
                <th className="px-2 sm:px-4 md:px-6 py-4">Vendedor</th>
                <th className="px-2 sm:px-4 md:px-6 py-4">Producto</th>
                <th className="px-2 sm:px-4 md:px-6 py-4 text-right">Cant.</th>
                <th className="px-2 sm:px-4 md:px-6 py-4 text-center">Estado</th>
                <th className="px-2 sm:px-4 md:px-6 py-4 text-right">Acciones</th>
              </tr>
            </thead>
                ...
                filteredOrders.map((order, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-2 sm:px-4 md:px-6 py-4 text-slate-500 text-xs font-medium">
                      ...
                      <td className="px-2 sm:px-4 md:px-6 py-4 text-slate-500 text-xs font-medium truncate max-w-[100px]">
                        {order.vendedor}
                      </td>
                      <td className="px-2 sm:px-4 md:px-6 py-4">
                        {order.total_items > 1 ? (
        ...
        <div className="fixed inset-0 bg-slate-50/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
            {(() => {
              ...
              return (
                <div className={`bg-slate-50 p-3 sm:p-6 flex justify-between items-start border-b border-slate-200`}>
                  <div className="flex items-center gap-2 sm:gap-4">
                    <div className={`${config.lightBg} ${config.textColor} border ${config.borderColor} p-2 sm:p-3 rounded-xl shadow-sm`}>
                    ...
                    <div>
                      ...
                      <h2 className="text-xl sm:text-3xl font-black text-slate-900">#{selectedOrder.nv}</h2>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {hasPermission('delete_sales_orders') && (
            ...
            <div className="p-3 sm:p-6 max-h-[65vh] overflow-y-auto space-y-4 sm:space-y-6">
              {/* Info Cards */}
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-3 sm:p-5 shadow-sm">
                ...
                <div className="max-h-60 overflow-y-auto space-y-3 custom-scrollbar pr-2">
                  {(selectedOrder.items || [selectedOrder]).map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 sm:gap-4 p-2 sm:p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors border border-slate-200">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white border border-slate-200 rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm">
                        <Box size={16} className="sm:hidden text-slate-500" />
                        <Box size={20} className="hidden sm:block text-slate-500" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-mono text-slate-600 font-bold text-[10px] sm:text-xs">{item.codigo_producto}</p>
                        <p className="text-slate-900 text-xs sm:text-sm font-bold truncate">{item.descripcion_producto}</p>
                      </div>
                      <div className="text-right bg-white border border-slate-200 px-2 sm:px-4 py-1.5 sm:py-2 rounded-xl shadow-sm flex-shrink-0">
                        <p className="text-base sm:text-lg font-black text-orange-600 leading-none mb-1">{item.cantidad}</p>
      ...
      {rlsErrorModal && (
        <div className="fixed inset-0 bg-slate-50/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white max-w-2xl w-full rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
            <div className="bg-rose-50 p-3 sm:p-6 border-b border-rose-100 flex gap-3 sm:gap-4 items-start">
            ...
            <div className="p-3 sm:p-6 space-y-4 bg-slate-50">
