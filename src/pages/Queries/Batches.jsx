    ...
    <span>
      {parts.map((part, i) =>
        part.toLowerCase() === highlight.toLowerCase() ? (
          <mark
            key={i}
            className="bg-yellow-300 text-black font-black px-1 rounded-[3px] ring-1 ring-yellow-500 shadow-sm"
          >
            {part}
          </mark>
        ) : (
...
const Batches = () => {
  ...
  const StatusBadge = ({ status }) => {
    ...
    if (['DISPONIBLE', 'EN_BODEGA', 'ACTIVO'].includes(s))
      color = 'bg-emerald-100 text-emerald-900 border-emerald-200';
    else if (['DESPACHADO', 'ENTREGADO'].includes(s))
      color = 'bg-blue-100 text-blue-900 border-blue-200';
    else if (['RESERVA', 'TRANSITO', 'PENDIENTE'].includes(s))
      color = 'bg-amber-100 text-amber-900 border-amber-200';
    else if (['CUARENTENA', 'BLOQUEADO', 'MERMA'].includes(s))
      color = 'bg-rose-100 text-rose-900 border-rose-200';

    return (
      <span
        className={`px-2.5 py-1 text-[10px] font-black uppercase tracking-wider rounded-md border whitespace-nowrap shadow-sm ${color}`}
      >
        {status}
      </span>
    );
  ...
  const TABLE_CONFIG = {
        ...
        render: (r) => {
          ...
          return (
            <div className="flex flex-col gap-2 min-w-[160px]">
              <div
                className={`flex items-center gap-3 px-4 py-2 rounded-2xl border-2 transition-all ${estilo}`}
              >
                {isAvailable ? (
                ...
                <div className="flex flex-col">
                  <span className="text-[9px] font-black uppercase tracking-widest leading-none mb-1">
                    {isAvailable ? 'Disponible' : enTransito ? 'En Tránsito' : 'Sin Stock'}
                  </span>
                  <span className="text-2xl font-black tracking-tighter leading-none">
                    {isAvailable ? disp : enTransito ? total : 0}
                  </span>
                </div>
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-mono text-[10px] font-black bg-slate-900 text-white px-2 py-1 rounded-lg uppercase tracking-tight inline-block w-fit break-all">
                  {lote || 'Sin lote'}
                </span>
              </div>
              ...
              {hasExpiry && (
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-wider ${isExpired ? 'bg-red-600 text-white border-red-700 animate-bounce' : 'bg-slate-900 text-white border-slate-800'}`}
                >
                  <Clock size={12} />
      ...
      },
      {
        ...
        render: (r) => (
          <div className="flex flex-col min-w-[200px]">
            <span className="font-mono text-[10px] font-black text-orange-500 uppercase mb-1 tracking-widest">
              <HighlightText text={r.codigo_producto} highlight={submittedTerm} />
            </span>
            <span className="font-bold text-slate-900 text-sm leading-tight whitespace-normal break-words">
              <HighlightText text={r.producto || r.descripcion} highlight={submittedTerm} />
            </span>
      ...
      },
      {
        ...
        render: (r) => (
          <div className="flex flex-col gap-1">
            ...
            <span className="font-mono text-xs font-black bg-slate-900 text-white px-3 py-2 rounded-xl border border-slate-700 uppercase tracking-tight shadow-lg inline-block w-fit">
              <HighlightText text={r.partida} highlight={submittedTerm} />
            </span>
          </div>
        )
      },
      {
        ...
        render: (r) => (
          <div className="grid grid-cols-2 gap-2 min-w-[220px]">
            <div
              className={`p-2 rounded-xl border ${r.reserva > 0 ? 'bg-amber-50 border-amber-500' : 'bg-slate-50 border-slate-100 opacity-40'}`}
            >
            ...
            <div
              className={`p-2 rounded-xl border ${r.transitoria > 0 ? 'bg-blue-50 border-blue-500' : 'bg-slate-50 border-slate-100 opacity-40'}`}
            >
            ...
            <div
              className={`p-2 rounded-xl border ${r.consignacion > 0 ? 'bg-purple-50 border-purple-500' : 'bg-slate-50 border-slate-100 opacity-40'}`}
            >
        ...
        render: (r) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase tracking-widest leading-none mb-1">
                  {isAvailable ? 'Disponible' : enTransito ? 'En Tránsito' : 'No Disponible'}
                </span>
                <span className="text-2xl font-black tracking-tighter leading-none">
                  {isAvailable ? disp : enTransito ? total : 0}
                </span>
      ...
      },
      {
        ...
        render: (r) => (
          <div className="flex flex-col min-w-[200px]">
            ...
            <span className="font-bold text-slate-900 text-sm leading-tight whitespace-normal break-words">
              <HighlightText text={r.producto} highlight={submittedTerm} />
            </span>
          </div>
        )
      },
      {
        ...
        render: (r) => (
          <div className="flex flex-col gap-1">
            ...
            <span className="font-mono text-xs font-black bg-slate-900 text-white px-3 py-2 rounded-xl border border-slate-700 uppercase tracking-tight shadow-lg inline-block w-fit">
              <HighlightText text={r.serie} highlight={submittedTerm} />
            </span>
          </div>
        )
      },
    ...
    farmapack: [
      ...
      },
      {
        ...
        accessor: 'producto',
        render: (r) => (
          <span className="font-bold text-slate-900 text-sm whitespace-normal break-words min-w-[200px]">
            <HighlightText text={r.producto || r.descripcion} highlight={submittedTerm} />
          </span>
        )
      },
      {
        ...
        accessor: 'lote',
        render: (r) => (
          <span className="font-mono text-[11px] font-black bg-emerald-100 px-3 py-1.5 rounded-lg text-emerald-900 border border-emerald-300 uppercase tracking-tight whitespace-nowrap shadow-sm">
            <HighlightText text={r.lote} highlight={submittedTerm} />
          </span>
        )
      },
      {
      ...
      },
      {
        ...
        accessor: 'disponible',
        render: (r) => (
          <span className="font-black text-emerald-700 text-lg tracking-tighter">
            {r.disponible}
          </span>
        )
      },
    ...
    pesos: [
      ...
      },
      {
        ...
        render: (r) => (
          <span className="font-bold text-slate-900 text-sm whitespace-normal break-words min-w-[200px]">
            <HighlightText text={r.descripcion} highlight={submittedTerm} />
          </span>
        )
      },
      {
        ...
        render: (r) => (
          <div className="flex items-center gap-2 font-black text-blue-900 bg-blue-100/50 px-3 py-1.5 rounded-lg border border-blue-200 text-sm w-fit whitespace-nowrap">
            <Scale size={14} />
            {r.peso_unitario}
          </div>
        )
      },
      {
        header: 'Dimensiones',
        render: (r) => (
          <span className="text-[11px] font-black text-slate-700 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 whitespace-nowrap">
            {r.largo}x{r.ancho}x{r.alto} cm
          </span>
        )
      },
      {
        ...
        accessor: 'updated_at',
        render: (r) => (
          <div className="flex items-center gap-2 text-[10px] font-black text-slate-500 whitespace-nowrap">
            <Clock size={12} />
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-screen bg-slate-50 relative overflow-hidden font-sans text-slate-900 selection:bg-orange-200 selection:text-orange-900"
    >
      ...
      <div className="relative z-10">
        {/* HEADER & SEARCH */}
        <div
          className={`transition-all duration-700 ease-in-out relative z-20 ${searched || loading ? 'bg-white/80 backdrop-blur-2xl border-b border-slate-200 shadow-sm sticky top-0' : 'flex flex-col items-center justify-center pt-20 sm:pt-32 pb-10 sm:pb-16 px-3 sm:px-4'}`}
        >
          <div
            className={`w-full mx-auto transition-all duration-700 ${searched || loading ? 'max-w-7xl px-3 sm:px-8 py-3 sm:py-4' : 'max-w-4xl'}`}
          >
            {!searched && !loading && (
              <div className="text-center mb-8 sm:mb-12 animate-header">
                ...
                <h1 className="text-3xl sm:text-5xl md:text-7xl font-black text-slate-900 tracking-tighter mb-4 sm:mb-6 leading-tight">
                  Partidas &{' '}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-amber-500 to-orange-600 animate-pulse">
                    Series
                  </span>
                </h1>
                <p className="text-slate-400 font-bold uppercase tracking-[0.2em] sm:tracking-[0.4em] text-[10px] sm:text-xs opacity-60">
            ...
            <div
              className={`flex flex-col animate-search ${searched || loading ? 'md:flex-row gap-6 items-center' : 'gap-8'}`}
            >
              {(searched || loading) && (
                <div
                  className="hidden md:flex items-center gap-4 cursor-pointer group px-4 py-2 rounded-2xl hover:bg-slate-50 transition-all duration-300"
                  onClick={handleClear}
                >
                  ...
                  <div>
                    <h1 className="text-base font-black text-slate-900 leading-none mb-1">
                      Stock Control
                    </h1>
              ...
              <form
                ...
                className={`relative group w-full ${searched || loading ? 'flex-1' : ''}`}
              >
                {/* Search Aura */}
                <div
                  className={`absolute -inset-2 bg-gradient-to-r from-orange-500/20 to-amber-500/20 rounded-[3rem] blur-2xl transition-all duration-700 opacity-0 ${isFocused ? 'opacity-100 scale-105' : 'group-hover:opacity-40'}`}
                />

                <div className="relative flex items-center">
                  <Search
                    className={`absolute left-4 sm:left-6 transition-all duration-500 z-10 ${loading ? 'animate-pulse text-orange-500' : isFocused ? 'text-orange-500 scale-110' : 'text-slate-400'}`}
                    size={searched || loading ? 18 : 22}
                  />
                  <input
                    type="text"
                  ...
                  {searchTerm && (
                    <button
                      type="button"
                      onClick={handleClear}
                      className="absolute right-6 text-slate-300 hover:text-rose-500 p-2 rounded-xl transition-all hover:scale-110 active:scale-95"
                    >
              ...
              <div className="space-y-8">
                  ...
                  {TABS.map((tab) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isActive && (
                          <div
                            className={`absolute -inset-4 opacity-20 blur-2xl ${activeColors.split(' ')[2]}`}
                          />
                        )}

                        <div
                          className={`relative w-10 h-10 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-700 group-hover:rotate-[5deg] flex-shrink-0 ${isActive ? `${activeColors.split(' ')[2]} text-white shadow-xl ${activeColors.split(' ')[1].replace('10', '40')}` : 'bg-slate-100 text-slate-400 group-hover:text-slate-600'}`}
                        >
                          <Icon
                            size={20}
                            className={`sm:hidden ${isActive ? 'animate-[pulse_2s_infinite]' : ''}`}
                          />
                          <Icon
                            size={28}
                            className={`hidden sm:block ${isActive ? 'animate-[pulse_2s_infinite]' : ''}`}
                          />
                        </div>
                        <div className="relative ml-3 sm:ml-6 flex-1 min-w-0">
                          <p
                            className={`text-[8px] sm:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.3em] mb-1 sm:mb-1.5 transition-colors duration-500 truncate ${isActive ? activeColors.split(' ')[3] : 'text-slate-400'}`}
                          >
                            {tab.label}
                          </p>
                          <p className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tighter leading-none group-hover:scale-105 transition-transform duration-500 origin-left">
                            {count}
                          </p>
                        </div>
                        {isActive && (
                          <div className={`relative flex items-center justify-center w-8 h-8`}>
                            <div
                              className={`absolute w-full h-full rounded-full animate-ping opacity-20 ${activeColors.split(' ')[2]}`}
                            />
                ...
                <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 sm:gap-6 bg-white/70 backdrop-blur-2xl p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-white/40 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]">
                  <div className="flex items-center gap-3 sm:gap-5">
                    ...
                    <div>
                      <h2 className="text-base sm:text-xl font-black text-slate-900 leading-none mb-1">
                        Resultados Activos
                      </h2>
                      <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] sm:tracking-[0.3em]">
                        Search Engine v2
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto">
                    <div className="relative flex-1 sm:w-80 group">
                      ...
                      <div className="relative">
                        <Filter
                          className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-orange-500 transition-colors"
                          size={14}
                        />
                        <input
                          type="text"
                  ...
                  <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left border-collapse min-w-[700px] sm:min-w-[1000px]">
                      <thead>
                        <tr className="bg-slate-100 border-b-2 border-slate-200">
                          {TABLE_CONFIG[activeTab].map((col, i) => (
                            <th
                              key={i}
                              className="px-4 py-3 sm:px-8 sm:py-5 text-[10px] sm:text-[11px] font-black text-slate-700 uppercase tracking-widest"
                            >
                              {col.header}
                            </th>
                          ))}
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {(subFilter
                          ? currentData[activeTab].filter((row) =>
                              TABLE_CONFIG[activeTab].some((col) =>
                          ...
                          <tr
                            key={idx}
                            className="result-item hover:bg-slate-50/50 transition-colors group"
                          >
                  ...
                  {(!currentData[activeTab] || currentData[activeTab].length === 0) && (
                    <div className="py-20 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-slate-400 font-bold max-w-sm">
                        Intenta con otro código o verifica la otra pestaña.
                      </p>
                    </div>
                  )}
                </div>
