...
const SalesStatus = () => {
  ...
  const {
      ...
      const [exactRes, fuzzyRes] = await Promise.all([
        withTimeout(
          supabase
            ...
            .select('*')
            .or(
              `nv.ilike.%${debouncedTerm}%,cliente.ilike.%${debouncedTerm}%,codigo_producto.ilike.%${debouncedTerm}%`
            )
    ...
    select: (data) => {
      ...
      if (
        data.length === 1 &&
        data[0]?.nv &&
        data[0].nv.toLowerCase() === debouncedTerm.toLowerCase()
      ) {
        ...
        (payload) => {
          if (selectedNVRef.current && selectedNVRef.current.nv === payload.new?.nv) {
            queryClient.invalidateQueries({
              queryKey: ['sales_details', selectedNVRef.current.nv]
            });
  ...
  const Timeline = ({ data }) => {
    ...
    if (currentStatus === 'Aprobada' || currentStatus === 'Pendiente Picking')
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center group relative w-16 md:w-20">
              <div
                className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center border-[3px] transition-all duration-500 bg-white ${
                  isCompleted
              ...
              <p
                className={`text-[9px] md:text-[11px] font-black uppercase tracking-wider mt-3 text-center transition-colors ${
                  isCompleted
                    ? 'text-emerald-700'
                    : isCurrent
                      ? 'text-indigo-700'
  ...
  return (
    <div
      ref={containerRef}
      className="h-full flex flex-col font-sans p-2 sm:p-4 md:p-6 bg-slate-50 min-h-screen text-slate-700"
    >
        ...
        <div className="w-full sm:max-w-[450px] relative group z-10">
          <div className="absolute inset-y-0 left-3 sm:left-4 flex items-center pointer-events-none">
            {loading ? (
              ...
              <Search
                className="text-slate-500 group-focus-within:text-wms-neon transition-colors"
                size={18}
                strokeWidth={2.5}
              />
            )}
            ...
            {isError ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-500 p-6 text-center">
                ...
                <p className="text-xs font-bold mt-1 opacity-60">
                  Ingresa una Nota de Venta para rastrear
                </p>
              </div>
            ) : (
              results.map((item) => {
                ...
                return (
                  ...
                  >
                    {isSelected && (
                      <div className="absolute left-0 top-0 bottom-0 w-2 bg-wms-neon rounded-l-2xl"></div>
                    )}

                    <div className="flex justify-between items-start mb-2">
                      <span
                        className={`text-xl font-black tracking-tight ${isSelected ? 'text-wms-neon ml-1' : 'text-slate-900'}`}
                      >
                        #{item.nv}
                      </span>
                      <span
                        className={`text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest border ${getStatusColor(item.estado)}`}
                      >
                        {item.estado}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs font-bold mt-3">
                      ...
                      {isSelected && (
                        <ArrowRight size={16} className="text-wms-neon animate-pulse" />
                      )}
                    </div>
                  </div>
                );
          ...
          {loadingDetails ? (
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                  <div>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className="text-wms-neon font-black tracking-widest uppercase text-xs">
                        Información de N.V.
                      </span>
                    </div>
                    <h2 className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tighter text-slate-900 mb-2 ml-[-3px]">
                      #{selectedNV.nv}
                    </h2>
                  ...
                  <div className="flex flex-col items-end gap-3 w-full md:w-auto mt-4 md:mt-0">
                    <div
                      className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest border border-white/20 shadow-lg ${getStatusColor(selectedNV.estado).replace('bg-', 'bg-white text-slate-900 border-')}`}
                    >
                  ...
                  <div className="space-y-4">
                    <div className="bg-white/80 p-4 rounded-xl border border-slate-200 flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <span className="font-mono font-black text-slate-900 text-lg tracking-tight">
                          {selectedNV.codigo_producto}
                        </span>
                      </div>
                    </div>

                    <div>
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 pl-1">
                        Descripción Registrada
                      </p>
                      <p className="font-bold text-slate-700 bg-white/80 border border-slate-200 p-3 rounded-xl leading-snug">
                        {selectedNV.descripcion_producto}
                      </p>
                    </div>

                    <div className="flex items-center justify-between bg-wms-neon/10 p-4 rounded-xl border border-wms-neon/20">
                      <span className="font-black text-wms-neon text-xs uppercase tracking-widest">
                        Cant. Solicitada
                      </span>
                      <div className="flex items-baseline gap-1 bg-slate-50 px-3 py-1 rounded-lg shadow-sm border border-wms-neon/30">
                        <span className="font-black text-2xl text-wms-neon">
                          {selectedNV.cantidad}
                        </span>
                        <span className="font-bold text-wms-neon/70 text-[10px] uppercase tracking-wider">
                          {selectedNV.unidad}
                        </span>
                      </div>
                    </div>
                ...
                <div className="bg-slate-50/50 rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-neon-green transition-shadow relative overflow-hidden">
                  ...
                  {selectedNV.entrega ? (
                    <div className="space-y-4">
                      ...
                      {selectedNV.entrega.tms_rutas && (
                        <div className="bg-wms-neon/10 p-4 rounded-xl border border-wms-neon/20 flex items-center gap-4 group">
                          ...
                          <div>
                            <p className="text-[10px] font-black text-wms-neon/70 uppercase tracking-widest mb-0.5">
                              Ruta Operativa
                            </p>
                            <p className="font-black text-wms-neon text-lg leading-tight">
                              {selectedNV.entrega.tms_rutas.nombre}
                            </p>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="flex items-center gap-4 bg-white/80 p-4 rounded-xl border border-slate-200">
                          ...
                          <div>
                            ...
                            <p className="font-black text-slate-900 text-sm">
                              {selectedNV.entrega.tms_conductores.nombre}{' '}
                              {selectedNV.entrega.tms_conductores.apellido}
                            </p>
                    ...
                    <div className="h-full min-h-[220px] flex flex-col items-center justify-center text-slate-500 bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200 p-6 text-center">
                      ...
                      <p className="font-black text-slate-700 text-base tracking-tight">
                        Sin Datos de Despacho
                      </p>
                      <p className="text-xs font-bold text-slate-500 mt-2 max-w-[200px]">
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-500 bg-slate-50/20 p-10 relative">
              ...
              <div className="relative z-10 text-center flex flex-col items-center">
                ...
                <h3 className="font-black text-3xl tracking-tight text-slate-900 mb-3">
                  Auditoría WMS
                </h3>
                <p className="font-bold text-slate-500 max-w-sm">
                  Selecciona una Nota de Venta de la lista para inspeccionar toda su trazabilidad en
                  tiempo real.
                </p>
              </div>
            </div>
