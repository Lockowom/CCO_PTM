...
const LocationsSheet = () => {
  ...
  return (
    <div ref={containerRef} className="space-y-6 min-h-screen bg-wms-dark text-slate-300 p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-wms-border pb-4 relative">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-sm mt-1 font-medium ml-1">Vista directa de inventario sin agrupamiento por layout</p>
        </div>
        <div className="flex gap-3 relative z-10 w-full md:w-auto">
          <div className="flex-1 md:flex-none flex items-center bg-wms-panel border border-wms-border rounded-xl px-3 focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all">
            <Search size={18} className="text-slate-400 mr-2" />
            <input
              className="h-11 bg-transparent outline-none text-sm text-white w-full placeholder-slate-500"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
        <div className="bg-wms-panel/80 backdrop-blur-xl rounded-2xl border border-wms-border p-5 shadow-2xl">
          <div className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Total Ubicaciones</div>
          <div className="text-3xl font-black text-white">{stats.totalUbicaciones}</div>
        </div>
      ...
      <div className="bg-wms-panel/80 backdrop-blur-xl rounded-2xl border border-wms-border shadow-2xl relative z-10 overflow-hidden">
        <div className="p-5 border-b border-wms-border flex items-center gap-3 bg-wms-dark/50">
          <div className="bg-wms-neon/20 p-1.5 rounded-lg border border-wms-neon/30">
            <Activity size={18} className="text-wms-neon" />
          </div>
          <span className="font-black text-white tracking-tight">Detalle de Inventario</span>
          <span className="ml-auto bg-wms-dark border border-wms-border px-2 py-1 rounded-md text-xs font-bold text-slate-400">
        ...
        <div className="p-6">
          {loading && !rows.length ? (
            <div className="flex flex-col items-center justify-center py-12 text-slate-400 gap-3">
              <div className="w-10 h-10 border-4 border-wms-border border-t-indigo-400 rounded-full animate-spin"></div>
            ...
            <div className="space-y-4">
              {filtered.map(r => (
                <div key={r.ubicacion} className="bg-wms-dark/50 border border-wms-border rounded-xl p-5 hover:border-slate-500 transition-colors">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-sm text-slate-400 bg-wms-panel px-3 py-1 rounded-lg border border-wms-border">
                      Total: <span className="font-black text-white">{r.total}</span>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {r.detalles.slice(0, 12).map(d => (
                      <span key={`${r.ubicacion}-${d.codigo}-${d.serie}-${d.partida}`} className="px-2.5 py-1.5 rounded-lg bg-wms-panel text-slate-300 border border-wms-border text-xs font-medium flex items-center gap-1.5">
                        <Box size={12} className="text-slate-500"/>
                        <span className="font-bold text-white font-mono">{d.codigo}</span>
                        ...
                        {d.talla ? <span className="text-slate-400">T {d.talla}</span> : ''}
                        {d.color ? <span className="text-slate-400">C {d.color}</span> : ''}
                    ...
                    {r.detalles.length > 12 && (
                      <span className="px-2.5 py-1.5 rounded-lg bg-wms-dark text-slate-500 border border-wms-border text-xs font-bold">
