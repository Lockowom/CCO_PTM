...
const TODOS_ESTADOS = [
  { key: 'Pendiente', label: 'Pendiente', icon: Hourglass, bgColor: 'bg-slate-500', lightBg: 'bg-slate-500/20', textColor: 'text-slate-300', borderColor: 'border-slate-500/30' },
  { key: 'PENDIENTE', label: 'Pendiente', icon: Hourglass, bgColor: 'bg-slate-500', lightBg: 'bg-slate-500/20', textColor: 'text-slate-300', borderColor: 'border-slate-500/30' },
...
const HistorialNV = () => {
  ...
  return (
    <div ref={container} className="bg-wms-dark min-h-screen text-slate-300 p-6 space-y-8">
      {/* Header Moderno */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 page-header">
        <div>
          ...
          <p className="text-slate-400 text-lg mt-2 ml-1">Visión global y trazabilidad de todas las Notas de Venta</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={exportToCSV}
            className="bg-emerald-600/20 border border-emerald-500/50 hover:bg-emerald-600 text-emerald-300 hover:text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.2)] transition-all hover:scale-105 active:scale-95"
          >
            <Download size={20} />
            Exportar CSV
          </button>
          <button 
            onClick={() => refetch()}
            disabled={loading}
            className="p-3 text-slate-400 bg-wms-panel border border-wms-border hover:bg-indigo-600/20 hover:text-indigo-400 hover:border-indigo-500/50 rounded-xl transition-all shadow-lg"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-wms-panel/80 backdrop-blur-xl p-5 rounded-3xl border border-wms-border shadow-xl hover:shadow-[0_0_20px_rgba(99,102,241,0.15)] hover:border-indigo-500/30 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
        ...
        <div className="bg-wms-panel/80 backdrop-blur-xl p-5 rounded-3xl border border-wms-border shadow-xl hover:shadow-[0_0_20px_rgba(16,185,129,0.15)] hover:border-emerald-500/30 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
        ...
        <div className="bg-wms-panel/80 backdrop-blur-xl p-5 rounded-3xl border border-wms-border shadow-xl hover:shadow-[0_0_20px_rgba(59,130,246,0.15)] hover:border-blue-500/30 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
        ...
        <div className="bg-wms-panel/80 backdrop-blur-xl p-5 rounded-3xl border border-wms-border shadow-xl hover:shadow-[0_0_20px_rgba(249,115,22,0.15)] hover:border-orange-500/30 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
      ...
      <div className="bg-wms-panel/90 backdrop-blur-xl p-2 rounded-2xl border border-wms-border shadow-xl flex flex-col lg:flex-row gap-2 items-center filters-bar sticky top-4 z-30">
        <div className="flex-1 relative w-full group">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 bg-wms-dark border border-wms-border rounded-xl outline-none focus:ring-2 focus:ring-wms-neon transition-all text-white placeholder-slate-600 font-medium"
            value={searchTerm}
        ...
        <div className="flex gap-2 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0">
          <select
            ...
            className="px-4 py-3 bg-wms-dark hover:bg-slate-800 border border-wms-border rounded-xl outline-none focus:ring-2 focus:ring-wms-neon font-bold text-slate-300 cursor-pointer transition-colors min-w-[140px]"
          >
          ...
          <input
            ...
            className="px-4 py-3 bg-wms-dark hover:bg-slate-800 border border-wms-border rounded-xl outline-none focus:ring-2 focus:ring-wms-neon font-bold text-slate-300 cursor-pointer transition-colors [color-scheme:dark]"
          />
      ...
      {/* Tabla Moderna */}
      <div className="bg-wms-panel/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-wms-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-wms-dark text-slate-400 uppercase text-xs tracking-wider font-bold border-b border-wms-border">
              <tr>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="8" className="px-6 py-24 text-center text-slate-500">
                    <div className="flex flex-col items-center justify-center gap-4">
                      <div className="p-6 bg-wms-dark rounded-full border border-wms-border shadow-inner">
                ...
                filteredOrders.map((order, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-wms-dark/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 font-bold text-white truncate max-w-[200px]" title={order.cliente}>
                        {order.cliente}
                      </td>
                      <td className="px-6 py-4 text-slate-500 text-xs font-medium uppercase">{order.vendedor}</td>
                      <td className="px-6 py-4">
                        <div className="font-mono font-bold text-slate-300">{order.codigo_producto}</div>
                        <div className="text-xs text-slate-500 truncate max-w-[250px] font-medium">{order.descripcion_producto}</div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="font-black text-white text-lg">{order.cantidad}</div>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button 
                          ...
                          className="bg-wms-dark hover:bg-indigo-500/20 text-slate-400 hover:text-indigo-400 p-2 rounded-xl border border-wms-border hover:border-indigo-500/50 transition-all shadow-sm hover:shadow-[0_0_10px_rgba(99,102,241,0.3)]"
                          title="Ver Detalle"
                        >
                          <Eye size={18} />
                        </button>
        ...
        {filteredOrders.length > 0 && (
          <div className="px-6 py-4 bg-wms-dark border-t border-wms-border text-xs font-bold text-slate-500 uppercase tracking-wider flex justify-between items-center">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-wms-panel w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-wms-border transform transition-all scale-100">
            {(() => {
              const config = getEstadoConfig(selectedOrder.estado);
              return (
                <div className={`bg-wms-dark p-8 flex justify-between items-start border-b border-wms-border relative overflow-hidden`}>
                  ...
                  <div className="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                      ...
                      <h2 className="text-4xl font-black text-white">#{selectedOrder.nv}</h2>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-3 relative z-10">
                    <button 
                      onClick={() => setSelectedOrder(null)} 
                      className="p-2 bg-wms-dark border border-wms-border hover:bg-slate-800 rounded-full text-slate-500 hover:text-rose-500 transition-colors"
                    >
                      <X size={24} />
                    </button>
            ...
            <div className="p-8 space-y-8 bg-wms-panel">
              <div className="grid grid-cols-3 gap-6">
                <div className="bg-wms-dark p-4 rounded-2xl border border-wms-border shadow-inner">
                ...
                <div className="bg-wms-dark p-4 rounded-2xl border border-wms-border shadow-inner col-span-2">
                  <div className="flex items-center gap-2 mb-2 text-slate-500">
                    <User size={16} />
                    <p className="text-xs font-bold uppercase">Cliente</p>
                  </div>
                  <p className="font-bold text-white text-lg truncate" title={selectedOrder.cliente}>
              ...
              <div className="bg-indigo-900/20 p-6 rounded-3xl border border-indigo-500/30 relative overflow-hidden shadow-[0_0_20px_rgba(79,70,229,0.1)]">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between gap-6">
                  ...
                  <div className="flex flex-col items-end justify-center min-w-[120px] bg-wms-dark/80 p-4 rounded-2xl backdrop-blur-sm border border-indigo-500/30 shadow-inner">
              ...
              <div className="flex items-center justify-between text-xs font-medium text-slate-500 pt-4 border-t border-wms-border">
