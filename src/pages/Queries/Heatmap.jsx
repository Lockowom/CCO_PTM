...
const Heatmap = () => {
  ...
  const { user } = useAuth();
  const { layout, inventory, stats, loading, fetchWarehouseData, updateLocationState } =
    useWarehouseStore();
  ...
  const heatmapData = useMemo(() => {
    ...
    Object.entries(RACK_CONFIG).forEach(([rackId, config]) => {
      ...
      racks[rackId].positions.forEach((p) => {
        p.maxQty = maxQtyInLevel;
      });
    });
    ...
    return {
      ...
      levelPercent:
        levelTotal > 0 ? Math.round(((levelOccupied + levelBlocked) / levelTotal) * 100) : 0,
  ...
  const filteredRacks = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      entries = entries.filter(
        ([id, data]) => id.includes(q) || data.positions.some((p) => p.locCode.includes(q))
      );
    }
    return entries;
  ...
  return (
    <div ref={containerRef} className="h-full flex flex-col bg-[#F8F9FB] overflow-hidden">
      ...
      <header className="shrink-0 bg-white border-b border-slate-200 px-3 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={exportCSV}
              className="flex items-center gap-2 bg-slate-800 text-white px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-slate-700 transition-colors active:scale-[0.97]"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-slate-100 p-0.5 rounded-lg flex-wrap">
              ...
              {Object.keys(RACK_CONFIG).map((rId) => (
                <button
                  key={rId}
                  onClick={() => setSelectedRack(selectedRack === rId ? null : rId)}
          ...
          <div className="hm-fade grid grid-cols-2 lg:grid-cols-5 gap-3">
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              ...
              <div className="mt-3 h-2 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-1000 ${globalPercent >= 80 ? 'bg-rose-500' : globalPercent >= 50 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                  style={{ width: `${globalPercent}%` }}
                />
              </div>
              <p className="text-[10px] text-slate-400 font-medium mt-1.5">
                {stats.ocupadas} / {TOTAL_POSITIONS}
              </p>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
              ...
              <span className="text-4xl font-extrabold text-slate-900 tracking-tight">
                {heatmapData.levelPercent}%
              </span>
              <div className="mt-3 h-2 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-slate-800 rounded-full transition-all duration-700"
                  style={{ width: `${heatmapData.levelPercent}%` }}
                />
              </div>
              <p className="text-[10px] text-slate-400 font-medium mt-1.5">
                {heatmapData.levelUsed} / {heatmapData.levelTotal}
              </p>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
              ...
              <span className="text-4xl font-extrabold text-blue-600 tracking-tight">
                {heatmapData.levelOccupied}
              </span>
              <p className="text-[10px] text-slate-400 font-medium mt-3">
                con stock en nivel {selectedLevel}
              </p>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
              ...
              <span className="text-4xl font-extrabold text-red-500 tracking-tight">
                {heatmapData.levelBlocked}
              </span>
              <p className="text-[10px] text-slate-400 font-medium mt-3">
                bloqueadas en nivel {selectedLevel}
              </p>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
              ...
              <span className="text-4xl font-extrabold text-emerald-600 tracking-tight">
                {heatmapData.levelTotal - heatmapData.levelUsed}
              </span>
              <p className="text-[10px] text-slate-400 font-medium mt-3">
                disponibles en nivel {selectedLevel}
              </p>
            </div>
          </div>
        </div>
            ...
            {rackSummary.map((rack) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${rack.percent >= 80 ? 'bg-rose-50 text-rose-600' : rack.percent >= 50 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}
                  >
                    {rack.percent}%
                  </span>
                </div>
                <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${rack.percent >= 80 ? 'bg-rose-500' : rack.percent >= 50 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                ...
                <div className="flex justify-between mt-1">
                  ...
                  {rack.blocked > 0 && (
                    <span className="text-[9px] text-red-400 font-bold">{rack.blocked} bloq</span>
                  )}
                </div>
              </button>
            ))}
          ...
          {loading && filteredRacks.length === 0 ? (
            ...
            filteredRacks.map(([rackId, data]) => {
              ...
              return (
                <div
                  key={rackId}
                  className="rack-section bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-slate-300 transition-all"
                >
                  <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-slate-50/50">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${pct >= 80 ? 'bg-rose-50 text-rose-600' : pct >= 50 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}
                      >
                        {pct}%
                      </span>
                    </div>
                  ...
                  <div className="p-4 overflow-x-auto">
                    <div className="flex gap-1 min-w-fit">
                      {data.positions.map((pos) => {
                        ...
                        const isHighlighted =
                          searchQuery &&
                          pos.locCode.toUpperCase().includes(searchQuery.toUpperCase());
                        ...
                        return (
                          ...
                          >
                            {pos.estado === 'NO_DISPONIBLE' ? (
                              ...
                              <>
                                <span className={`text-[8px] font-bold ${style.text} leading-none`}>
                                  {pos.posKey}
                                </span>
                                {pos.qty > 0 && (
                                  <span
                                    className={`text-[7px] font-extrabold ${style.text} leading-none opacity-70`}
                                  >
                                    {pos.qty}
                                  </span>
                                )}
      ...
      {detailModal && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg mx-4 max-h-[80vh] flex flex-col"
          ...
          >
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        detailModal.estado === 'NO_DISPONIBLE'
                    ...
                    <span className="text-[11px] text-slate-400 font-medium">
                      {detailModal.items.reduce((a, i) => a + (Number(i.cantidad) || 0), 0)} uds ·{' '}
                      {detailModal.items.length} SKUs
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-3 border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center gap-2">
                  ...
                  {Object.entries(LOCATION_STATES).map(([key, cfg]) => (
                    <button
                      key={key}
                      disabled={changingState || detailModal.estado === key}
            ...
            <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between rounded-b-2xl">
              <span className="text-[10px] text-slate-400 font-medium">
                {detailModal.locCode.split('-')[0]} · Pos {detailModal.locCode.split('-')[1]} · Niv{' '}
                {detailModal.locCode.split('-')[2]}
              </span>
              <button
                onClick={() => setDetailModal(null)}
                className="px-4 py-2 bg-slate-800 text-white rounded-lg text-xs font-bold hover:bg-slate-700 transition-colors active:scale-95"
              >
                Cerrar
              </button>
            </div>
