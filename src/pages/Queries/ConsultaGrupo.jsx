...
export default function ConsultaGrupo() {
  ...
  const buscar = useCallback(async (texto) => {
    const t = (texto ?? '').trim();
    if (!t) { setRows([]); setBuscado(false); return; }
    setLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 sm:p-6 space-y-4 text-slate-700">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-5 sm:px-7 py-4 sm:py-5 flex items-center gap-4">
        <div className="w-12 h-12 bg-indigo-50 border border-indigo-100 rounded-2xl grid place-items-center text-indigo-600 shrink-0"><Tags size={22} /></div>
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900">Grupo por <span className="text-indigo-600">SKU</span></h1>
          <p className="text-xs sm:text-sm text-slate-500">Escribe el código (o nombre) del producto y ve a qué grupo pertenece</p>
        </div>
      </div>
      ...
      <div className="relative max-w-2xl">
        ...
        <input
          value={q} onChange={onChange} autoFocus
          onKeyDown={(e) => { if (e.key === 'Enter') { clearTimeout(debRef.current); buscar(q); } }}
        ...
        />
        {loading && <Loader2 size={18} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 animate-spin" />}
      </div>

      <div className="max-w-2xl space-y-2">
        {rows.map((r) => (
          <div key={r.codigo} className="bg-white rounded-2xl border border-slate-200 px-4 py-3 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-slate-100 text-slate-500 grid place-items-center shrink-0"><Package size={17} /></div>
        ...
        {!buscado && (
          <p className="text-[12px] text-slate-400 px-1">Los grupos se cargan/actualizan desde <b>Carga Masiva → Grupos de SKU</b> (Excel del ERP).</p>
        )}
      </div>
    </div>
  );
