...
export default function SolicitudPublica() {
  ...
  const enviar = async (e) => {
    ...
    try {
      ...
      const { data, error: fnErr } = await supabase.functions.invoke('postventa-publico', { body: payload });
      ...
      if (fnErr && !body?.ok) throw new Error(body?.error || fnErr.message || 'No se pudo enviar la solicitud.');
        ...
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-8 text-center">
          ...
          {ok.numero && (
            <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 mb-5">
              <p className="text-[11px] font-bold text-orange-600 uppercase tracking-wide">Número de folio</p>
              <p className="text-2xl font-black text-orange-600 tracking-tight">{ok.numero}</p>
              <p className="text-[11px] text-orange-500/80 mt-1">Guarda este número para dar seguimiento.</p>
            </div>
          )}
          <button
            onClick={() => { setOk(null); setForm(FORM_INICIAL); cargadoEn.current = Date.now(); }}
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4">
      <div className="w-full max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6">
          <img src="/logo-ptm.png" alt="PTM Health Care" className="h-14 mx-auto mb-3 object-contain" />
        ...
        <form onSubmit={enviar} className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Campo label="Empresa / Cliente" req>
              <input className={inputCls} value={form.cliente} onChange={set('cliente')} placeholder="Nombre de la empresa o cliente" maxLength={300} />
            </Campo>
            <Campo label="Contacto" req hint="Nombre, teléfono o correo para ubicarte.">
              <input className={inputCls} value={form.contacto} onChange={set('contacto')} placeholder="Nombre y teléfono / correo" maxLength={300} />
            </Campo>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Campo label="Equipo / Modelo" req hint="Si no conoces el modelo exacto, describe el equipo.">
              <input className={inputCls} value={form.equipo_modelo} onChange={set('equipo_modelo')} placeholder="Ej: Monitor multiparámetro, Cardiomax…" maxLength={300} />
            </Campo>
            <Campo label="N° de serie" hint="Opcional.">
              <input className={inputCls} value={form.numero_serie} onChange={set('numero_serie')} placeholder="Opcional" maxLength={120} />
            </Campo>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Campo label="Tipo de solicitud">
              <select className={inputCls} value={form.tipo_solicitud} onChange={set('tipo_solicitud')}>
                <option value="">Selecciona…</option>
                {PV_TIPOS_SOLICITUD.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </Campo>
