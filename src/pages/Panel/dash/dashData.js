...
async function runDashboardRead(
  action,
  fn,
  { screen = 'PanelDashboard', payload = null, slowMs = 1200, message = '' } = {}
) {
...
function calcularTiemposCiclo(rows) {
  const etapasDef = [
    {
      nombre: 'En Proceso → Shipping',
      from: (r) => fechaAprobEfectiva(r),
      to: (r) => r.fecha_shipping
    },
  ...
  const cuelloBotella =
    conDatos.length > 0 ? conDatos.reduce((max, e) => (e.dias > max.dias ? e : max)) : null;
...
function clasificarIncidencia(texto, observaciones = '') {
  const t = `${texto || ''} ${observaciones || ''}`
    ...
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase();
  if (t.includes('direccion') || t.includes('domicilio') || t.includes('contacto'))
    return 'direccion';
  if (
  ...
  for (const r of rows) {
    ...
    if (!key.endsWith(':')) {
      ...
      const ta =
        ...
        0;
      const tb =
        Date.parse(prev.fecha_estado || '') ||
        Date.parse(prev.fecha_aprobacion_real || '') ||
        Date.parse(prev.fecha_aprobacion || '') ||
        0;
...
export async function fetchActivasLista() {
  const cols =
    'nv_ptm,nv_orange,nv_farmapack,varios,cliente,vendedor,estado,transportista,fecha_compromiso,guia,factura,fecha_aprobacion,fecha_aprobacion_real';
  const allRows = [];
  let from = 0;
  const pageSize = 1000;
  while (true) {
  ...
  for (const item of mapped) {
    ...
    if (
      !existing ||
      ESTADOS_FLUJO_ORDEN.indexOf(item.estado) > ESTADOS_FLUJO_ORDEN.indexOf(existing.estado)
    ) {
      dedup.set(key, item);
    }
  }
...
export async function fetchConsolidados() {
  ...
  const byId = {};
  (nvs || []).forEach((n) => {
    (byId[n.consolidado_id] = byId[n.consolidado_id] || []).push({
      id: n.id,
      nv: n.nv,
      canal: n.canal,
      cliente: n.cliente
...
export async function fetchConsolidadoKeys() {
  return runDashboardRead(
    ...
    async () => {
      const now = Date.now();
      if (
        consolidadoKeysCache.data &&
        now - consolidadoKeysCache.ts < CONSOLIDADO_KEYS_CACHE_TTL_MS
      ) {
      ...
      const run = async () => {
        ...
        const enrichRow = (r) => {
          ...
          if (!r.fecha_compromiso) {
            r.fecha_compromiso =
              calcFechaCompromiso(r.fecha_aprobacion, r.fecha_aprobacion_real) || null;
          }
        };
        ...
        const activasFiltradas = activasSnapshot.filter((r) =>
          dentroRangoAprob(r, dateFrom, dateTo)
        );
        ...
        const tasaEntrega =
          despachosReales > 0 ? ((entregadas / despachosReales) * 100).toFixed(1) : '0';
        ...
        const leadTimeTardanza = tardanzaCount > 0 ? (tardanzaSum / tardanzaCount).toFixed(1) : '0';
        const validEntregados = tardanzaCount + atiempoCount;
        const pctAtiempo =
          validEntregados > 0 ? ((atiempoCount / validEntregados) * 100).toFixed(0) : '0';

        const tieneIncidenciaActiva = (r) => {
          const incidencia = String(r?.incidencia || '').trim();
          const estadoIncidencia = String(r?.estado_incidencia || '')
            .trim()
            .toUpperCase();
        ...
        const nvsSemana = rowsM.filter((r) => {
          ...
          const regStr =
            typeof r.fecha_registro_nv === 'string'
        ...
        const acumular = (r) => {
          const e = r.estado || 'null';
          if (!estadoGrouped[e])
            estadoGrouped[e] = { farmapack: 0, orange: 0, ptm: 0, varios: 0, total: 0 };
        ...
        rowsM.forEach((r) => {
          ...
          if (!weekKey) return;
          if (!ltWeeks[weekKey])
            ltWeeks[weekKey] = { tardanzaSum: 0, tardanzaCount: 0, atiempoCount: 0 };
          const promesa = fechaPromesaEfectiva(r);
          if (!promesa) return;
        ...
        rows.forEach((r) => {
          ...
          if (!descartado && !terminado) {
            ...
            const fcEfectiva =
              r.fecha_compromiso ||
              calcFechaCompromiso(r.fecha_aprobacion, r.fecha_aprobacion_real);
        ...
        rowsM.forEach((r) => {
          ...
          if (!transpPerf[t])
            transpPerf[t] = { total: 0, entregadas: 0, aTiempo: 0, tardanzaSum: 0, tardanzaN: 0 };
          transpPerf[t].total++;
              ...
              if (promesa) {
                ...
                if (Math.abs(diff) <= 30) {
                  if (diff <= 0) transpPerf[t].aTiempo++;
                    ...
                    transpPerf[t].tardanzaSum += diff;
                    transpPerf[t].tardanzaN++;
                  }
                }
              }
            }
        ...
        const rankingTransportistas = Object.entries(transpPerf)
          .map(([nombre, d]) => ({
            ...
            pctATiempo:
              d.aTiempo + d.tardanzaN > 0
                ? +((d.aTiempo / (d.aTiempo + d.tardanzaN)) * 100).toFixed(0)
                : null,
        ...
        rowsM.forEach((r) => {
          ...
          if (!v || v === '—') return;
          if (!vendPerf[v])
            vendPerf[v] = { total: 0, entregadas: 0, aTiempo: 0, activas: 0, reabiertas: 0 };
          vendPerf[v].total++;
        ...
        ESTADOS_ACTIVOS.forEach((est) => {
          ...
          if (nvsEstancadas.length > 0) {
            estancadas.push({
              estado: est,
              cantidad: nvsEstancadas.length,
              nvs: nvsEstancadas.slice(0, 5)
            });
          }
        });
        ...
        const funnelEstados = FUNNEL_ETAPAS.map(({ etapa, incluye }) => {
          let cantidad = 0;
          rows.forEach((r) => {
            if (incluye.includes(r.estado || '')) cantidad++;
          });
          activasFiltradas.forEach((r) => {
            // sumar activas que estén en esta etapa (dentro del rango de fechas)
            if (ESTADOS_ACTIVOS.includes(r.estado || '') && incluye.includes(r.estado || ''))
              cantidad++;
          });
    ...
    async () => {
      ...
      const run = async () => {
        ...
        while (true) {
          ...
          if (dateFrom)
            q = q.or(
              `fecha_aprobacion_real.gte.${dateFrom},and(fecha_aprobacion_real.is.null,fecha_aprobacion.gte.${dateFrom})`
            );
          if (dateTo)
            q = q.or(
              `fecha_aprobacion_real.lte.${dateTo},and(fecha_aprobacion_real.is.null,fecha_aprobacion.lte.${dateTo})`
            );
      ...
      return filtered
        .map((r) => {
          ...
          if (r.fecha_despacho && fechaPromesaReal) {
            ...
            diasEntrega = Math.round((desp.getTime() - comp.getTime()) / (1000 * 60 * 60 * 24));
          }
          return {
            nv: (r.nv_ptm && String(r.nv_ptm)) || r.nv_orange || r.nv_farmapack || r.varios || '—',
            cliente: r.cliente || '—',
...
export async function fetchTVEstados() {
  return runDashboardRead(
    ...
    async () => {
      ...
      const cached = getDashMapCache(tvEstadosCache, cacheKey, TV_ESTADOS_CACHE_TTL_MS);
      if (cached) return cached;
      ...
      const run = async () => {
        ...
        const msPerDay = 1000 * 60 * 60 * 24;
        const diasDesde = (f) =>
          f ? Math.floor((hoyMs - new Date(f + 'T12:00:00').getTime()) / msPerDay) : null;
        ...
        const estados = ORDEN.filter((e) => (grupos[e]?.length || 0) > 0).map((e) => ({
          estado: e,
          cantidad: grupos[e].length,
          nvs: grupos[e].sort((a, b) => (b.urgente ? 1 : 0) - (a.urgente ? 1 : 0))
        }));

        return setDashMapCacheValue(tvEstadosCache, cacheKey, { estados, total, urgentes });
      };

      const promise = run().catch((error) => {
        tvEstadosCache.delete(cacheKey);
        throw error;
      });
      return setDashMapCachePromise(tvEstadosCache, cacheKey, promise);
    },
    {
      screen: 'PanelTV',
...
export async function fetchTVData() {
  return runDashboardRead(
    'fetch_tv_data',
    async () => {
      const [tvEstados, dashData] = await Promise.all([fetchTVEstados(), fetchDashboardData()]);

      const hoy = new Date().toISOString().slice(0, 10);
      const entregadasHoy =
        dashData.estadoTable?.filter((r) => r.fecha_entregado?.slice(0, 10) === hoy).length || 0;
