...
function diasBadge(dias) {
  ...
  const color = dias > 5 ? "bg-red-500/20 text-red-400" : dias > 2 ? "bg-amber-500/20 text-amber-400" : "bg-gray-700/40 text-gray-400";
  return (
...
function NvCard({ n, estado }) {
  ...
  const contextInfo = () => {
    if (estado === ESTADOS.EN_RUTA || estado === ESTADOS.CURRIER) {
      return (
        <div className="flex items-center gap-1.5">
          <span className="text-cyan-400 text-xs font-bold">{n.transportista !== "—" ? n.transportista : ""}</span>
    ...
    if (estado === ESTADOS.ENTREGADO) {
      return (
        <div className="flex items-center gap-1.5">
          {n.fecha_entregado && <span className="text-emerald-400 text-[11px] font-medium">{n.fecha_entregado}</span>}
          {n.transportista !== "—" && <span className="text-gray-500 text-[10px]">· {n.transportista}</span>}
        </div>
      );
    }
    if (estado === ESTADOS.SHIPPING || estado === ESTADOS.EN_PROCESO || estado.startsWith("P /")) {
      ...
      return (
        <div className="flex flex-col items-end gap-1">
          <div className="flex items-center gap-1.5">
            <span className="text-gray-500 text-[9px] uppercase tracking-wide">{aprobReal ? "Aprob. real" : "Aprob."}</span>
            {n.fecha_aprob_efectiva
              ? <span className={`text-[11px] font-medium ${aprobReal ? "text-emerald-400" : "text-gray-400"}`}>{n.fecha_aprob_efectiva}</span>
          ...
          <div className="flex items-center gap-1.5">
            {n.diasDesdeAprobacion !== null
              ...
              : diasBadge(n.diasEnEstado)}
            {n.fecha_compromiso && <span className="text-gray-500 text-[10px]">comp: {n.fecha_compromiso}</span>}
          </div>
        </div>
      );
    }
    return n.transportista !== "—" ? <span className="text-gray-500 text-[10px]">{n.transportista}</span> : null;
  };

  return (
    <div className={`flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors ${n.urgente ? "spotlight-card bg-red-500/10" : "bg-white/[0.03] border border-gray-800/40 hover:bg-white/[0.05]"}`}>
...
export default function TVDashboard() {
  ...
  const maxCantidad = useMemo(() => Math.max(...estadosArr.map(e => e.cantidad), 1), [estadosArr]);
  const detalleNvs = useMemo(() =>
    sel === "URGENTES" ? urgentesArr
    : sel ? (estadosArr.find(e => e.estado === sel)?.nvs || [])
    : [],
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center font-black text-xl tracking-tight">PTM</div>
          <div>
            <h1 className="text-2xl font-bold tracking-tight leading-tight">Centro de Control Logístico</h1>
            <p className="text-gray-500 text-xs mt-0.5">PTM Health Care · Seguimiento en tiempo real</p>
          </div>
        </div>
        <div className="flex items-center gap-6">
          ...
          >
            ...
            <div className="text-left">
              <CountUp end={urgentes.length} className={`text-3xl font-black tabular-nums leading-none ${urgentes.length > 0 ? "text-red-400" : "text-gray-500"}`} />
              <p className="text-gray-400 text-[10px] font-medium">Urgentes</p>
            </div>
          </button>
          <div className="text-right">
            <CountUp end={total} className="text-5xl font-black tabular-nums text-orange-400 leading-none" />
          ...
          <div className="flex flex-col items-end gap-1 border-l border-gray-800 pl-5">
            ...
            <span className={`text-[11px] font-medium ${autoRotate ? "text-orange-400" : "text-gray-600"}`}>
      ...
      <div className="flex-1 flex gap-5 min-h-0 overflow-hidden">
        ...
        <div className={`flex flex-col justify-center transition-all ${sel ? "w-[45%]" : "w-full"}`} style={{ gap: `${Math.min(14, Math.max(6, Math.floor(70 / numEstados)))}px` }}>
          {estados.map(e => {
            ...
            return (
              <button key={e.estado} onClick={() => handleManualSel(sel === e.estado ? null : e.estado)}
                ...
                <div className="flex items-center gap-2 w-36 shrink-0 justify-end">
                  ...
                  <span className="text-gray-200 text-sm font-semibold text-right truncate">{e.estado}</span>
                </div>
                <div className="flex-1 bg-gray-800/40 rounded-lg overflow-hidden relative" style={{ height: barH }}>
                  <div
                    className="h-full rounded-lg transition-all duration-1000 ease-out flex items-center justify-between px-4"
                    style={{ width: `${Math.max(pct, 5)}%`, background: `linear-gradient(90deg, ${color}, ${color}cc)` }}
                  >
                    <CountUp end={e.cantidad} className="text-xl font-black text-white drop-shadow-lg tabular-nums" />
                  </div>
                  {nUrg > 0 && (
                    <span className="absolute right-12 top-1/2 -translate-y-1/2 text-[10px] text-red-400 font-bold flex items-center gap-0.5 animate-pulse">🚨 {nUrg}</span>
                  )}
          ...
          <div className="w-[55%] bg-[#111118] rounded-2xl border border-gray-800/60 flex flex-col min-h-0 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-800/60 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold">{sel === "URGENTES" ? "🚨 NVs Urgentes" : `${ICONOS[sel] || ""} ${sel}`}</h2>
                <span className="text-gray-500 text-base font-medium">({detalleNvs.length})</span>
              </div>
              <div className="flex items-center gap-3">
                {sel !== "URGENTES" && (
                  <span className="text-gray-500 text-xs">
                    {sel === ESTADOS.EN_RUTA || sel === ESTADOS.CURRIER ? "Destacado: transportista" :
                     sel === ESTADOS.ENTREGADO ? "Destacado: fecha entrega" :
                     sel === ESTADOS.SHIPPING ? "Días desde aprobación" :
                     sel === ESTADOS.EN_PROCESO || sel?.startsWith("P /") ? "Días desde aprobación" : ""}
                  </span>
                )}
                <button onClick={() => handleManualSel(null)} className="text-gray-500 hover:text-white text-2xl leading-none px-2 transition-colors">×</button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
              {detalleNvs.length === 0 ? (
                ...
                <div key={`${n.canal}-${n.nv}-${i}`} className="anim-list-item" style={{ animationDelay: `${Math.min(i * 40, 400)}ms` }}>
