...
function presetToRange(preset) {
  ...
  switch (preset) {
    ...
    default: return { from: "2020-01-01", to };
  }
...
function fromRecord(r) {
  ...
  const pages = Array.isArray(r.config?.pages) && r.config.pages.length > 0
...
export default function PanelBuilderReal() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (remote.length > 0) {
        ...
        setActiveId(savedActive && mapped.find(l => l.id === savedActive) ? savedActive : mapped[0].id);
        return;
      }
      ...
      const seed = legacy.length > 0
        ? legacy.map((l) => l.pages ? l : { ...l, pages: [{ id: genPageId(), name: "Hoja 1", widgets: l.widgets || [], gridLayout: l.gridLayout || [] }] })
        : [{
            id: "dash_" + Date.now(),
            name: "Mi Dashboard",
            owner: user?.nombre || "",
      ...
      if (legacy.length > 0) { try { localStorage.removeItem("ptm_builder_layouts"); } catch { /* ignore */ } }
    })();
    return () => { cancelled = true; };
  }, []);

  const active = useMemo(() => layouts.find((l) => l.id === activeId) || null, [layouts, activeId]);
  const canEdit = useMemo(() => active ? puedeEditar(rol, active.minRoleEdit || "supervisor") : false, [active, rol]);
  ...
  const updateActivePage = useCallback((patch) => {
    setLayouts((prev) => {
      const pageId = activePage?.id;
      const next = prev.map((l) => l.id !== activeId ? l : {
        ...l,
  ...
  const handleSetRol = useCallback((r) => {
  ...
  const reloadCalcFields = useCallback(async () => {
    try { setCalcFields(await fetchCalculatedFields()); } catch { /* ignore */ }
  }, []);
  ...
  const calcMeta = useMemo(() => calcFields.map((c) => ({ nombre: c.nombre, tipo: c.tipo })), [calcFields]);
  ...
  // Add widget
  const handleAddWidget = useCallback((type, dataSource) => {
    ...
    if (["pie-chart", "donut-chart", "horizontal-bars", "stat-list", "funnel", "timeline"].includes(type) && ds.type === "array") {
      defaultCfg.labelField = ds.fields.find(f => f.type === "string")?.key || ds.fields[0].key;
      defaultCfg.valueField = ds.fields.find(f => f.type === "number")?.key || ds.fields[1]?.key;
    }
    if (type === "gauge") {
      const pf = ds.fields.find(f => f.type === "percent") || ds.fields.find(f => f.type === "number");
  ...
  const handleAddPage = useCallback(() => {
    if (!active) return;
    const newPage = { id: genPageId(), name: `Hoja ${active.pages.length + 1}`, widgets: [], gridLayout: [] };
  ...
  const handlePlantillaCumplimiento = useCallback(() => {
    ...
    const sem = (title, dataSource, valueField, opts) => ({
      id: genWidgetId(), type: "semaforo", title, dataSource,
      config: { valueField, format: "percent", higherIsBetter: true, okThreshold: 85, critThreshold: 60, ...opts },
    });
    const widgets = [
      sem("Estado Cumplimiento", "cumplimientoNV", "pct", { okThreshold: 90, critThreshold: 70, trendField: "pctATiempo" }),
      ...
      sem("Tardanza Prom (días)", "kpis", "leadTimeTardanza", { format: "days", higherIsBetter: false, okThreshold: 1, critThreshold: 5, trendField: "leadTime" }),
      {
        id: genWidgetId(), type: "horizontal-bars", title: "NV en riesgo por estado", dataSource: "alertasOperacionales",
        config: { labelField: "estado", valueField: "cantidad", color: "#dc2626" },
      },
      {
        id: genWidgetId(), type: "line-chart", title: "Tendencia % A Tiempo / OTIF", dataSource: "tendencia",
        config: {
          xField: "label",
          yFields: [
  ...
  const handlePlantillaGraficos = useCallback(() => {
    ...
    const widgets = [
      ...
      {
        id: genWidgetId(), type: "semaforo", title: "Estado Cumplimiento",
        dataSource: "cumplimientoNV",
        config: { valueField: "pct", format: "percent", higherIsBetter: true, okThreshold: 90, critThreshold: 70, trendField: "pctATiempo" },
      },
      {
        id: genWidgetId(), type: "gauge", title: "Fill Rate",
        dataSource: "fillRate",
        config: { valueField: "pct", format: "percent", min: 0, max: 100 },
      },
      {
        id: genWidgetId(), type: "semaforo", title: "Lead Time Promedio",
        dataSource: "kpis",
        config: { valueField: "leadTimeTardanza", format: "days", higherIsBetter: false, okThreshold: 1, critThreshold: 5, trendField: "leadTime", suffix: " días" },
      },
      {
        id: genWidgetId(), type: "scorecard", title: "Tardanza Promedio",
        dataSource: "kpis",
        config: { valueField: "leadTimeTardanza", format: "days", suffix: " días", color: "#dc2626", icon: "⏱" },
      },
      // Fila 2: Fecha Compromiso vs Total NV (donut) + NV en Riesgo (barras)
      {
        id: genWidgetId(), type: "donut-chart", title: "Fecha Compromiso vs Total NV",
        dataSource: "cumplimientoDetalle",
        config: {
          labelField: "label", valueField: "valor",
          yFields: [
            { key: "valor", label: "Cantidad", color: "#16a34a" },
          ],
          colorField: "label",
        },
      },
      {
        id: genWidgetId(), type: "horizontal-bars", title: "NV en Riesgo vs Fecha Compromiso",
      ...
      {
        id: genWidgetId(), type: "bar-chart", title: "Lead Time Semanal",
      ...
      {
        id: genWidgetId(), type: "area-chart", title: "Tardanza Semanal",
        dataSource: "weeklyTrend",
        config: {
          xField: "semana",
          yFields: [
            { key: "tardanza", label: "Tardanza (días)", color: "#dc2626" },
          ],
          fillOpacity: 0.3,
        },
      },
      ...
      {
        id: genWidgetId(), type: "line-chart", title: "Tendencia % A Tiempo / OTIF",
  ...
  const configWidgetObj = configWidget ? activePage?.widgets.find((w) => w.id === configWidget) : null;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-4 py-2.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] w-14 text-right" style={{ color: saving === "error" ? "#c62828" : "#9ca3af" }}>
              {saving === "saving" ? "Guardando…" : saving === "saved" ? "Guardado ✓" : saving === "error" ? "Error" : ""}
            </span>
            ...
            {canEdit ? (
              ...
              <span className="px-3 py-1.5 text-[12px] rounded-lg bg-gray-100 text-gray-400 font-medium" title={`Requiere rol ${active?.minRoleEdit || "supervisor"}+`}>
                Solo lectura
              </span>
            )}
          </div>
      ...
      <div className="bg-gray-50 border-b border-gray-200 sticky top-[53px] z-40">
        <div className="max-w-[1600px] mx-auto px-4 py-2 flex items-center gap-2 flex-wrap">
          <span className="text-[11px] text-gray-500 font-semibold uppercase tracking-wide shrink-0">Período:</span>
          ...
          {datePreset === "custom" && (
            <div className="flex items-center gap-1.5 ml-2">
              <input type="date" className="px-2 py-1 text-[12px] border border-gray-200 rounded-md" value={customFrom} onChange={e => setCustomFrom(e.target.value)} />
              <span className="text-gray-400 text-[11px]">→</span>
              <input type="date" className="px-2 py-1 text-[12px] border border-gray-200 rounded-md" value={customTo} onChange={e => setCustomTo(e.target.value)} />
            </div>
          )}
                ...
                <div key={p.id} className="flex items-center shrink-0">
                  {renamingPage === p.id ? (
                    <input
                      autoFocus
                      defaultValue={p.name}
                      onBlur={(e) => { handleRenamePage(p.id, e.target.value.trim() || p.name); setRenamingPage(null); }}
                      onKeyDown={(e) => { if (e.key === "Enter") { handleRenamePage(p.id, e.target.value.trim() || p.name); setRenamingPage(null); } if (e.key === "Escape") setRenamingPage(null); }}
                    ...
                    >
                      {p.name}
                      {isActive && <span className="ml-1.5 text-[10px] text-gray-400">({p.widgets.length})</span>}
                    </button>
                  )}
                  {editMode && canEdit && isActive && active.pages.length > 1 && (
                    <button onClick={() => handleDeletePage(p.id)} className="text-gray-300 hover:text-red-500 text-[11px] px-1" title="Eliminar hoja">✕</button>
                  )}
                </div>
              );
            })}
      ...
      <main ref={gridRef} className="max-w-[1600px] mx-auto px-4 py-4">
        {activePage && activePage.widgets.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            <div className="w-20 h-20 rounded-2xl bg-gray-100 flex items-center justify-center text-3xl mb-4">+</div>
            ...
            {canEdit ? (
              ...
              <p className="text-sm">No tienes permiso para editar este dashboard (requiere rol {active?.minRoleEdit || "supervisor"}+).</p>
            )}
          </div>
        )}
        ...
        {activePage && activePage.widgets.length > 0 && (
          <ResponsiveGridLayout
            ...
            rowHeight={60}
            dragConfig={{ enabled: editMode && canEdit, handle: ".drag-handle", bounded: false, threshold: 3 }}
      ...
      {calcOpen && (
        <CamposCalculados editable={!!canEdit} onClose={() => { setCalcOpen(false); reloadCalcFields(); }} />
      )}
