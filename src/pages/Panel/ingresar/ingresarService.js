...
async function runPanelRead(
  action,
  fn,
  { screen = 'PanelIngresar', payload = null, slowMs = 900, message = '' } = {}
) {
...
async function readWithPoolRetry(makeQuery, { ms, label, attempts = 3, signal } = {}) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    Logger.warn(result.error, {
      ...
      message: `Reintento de lectura por saturación transitoria del pool: ${label}`,
      attempt: attempt + 1,
      delayMs
    });
...
async function runPanelMutation(
  action,
  fn,
  { screen = 'PanelIngresar', payload = null, message = '' } = {}
) {
  const startedAt = performance.now();
  try {
    const result = await fn();
...
const vendorTokens = (v) =>
  normWords(v)
    .split(' ')
    .filter((token) => token && !STOP_WORDS_VENDEDOR.has(token));
const colDe = (canal) =>
  canal === 'ptm'
    ? 'nv_ptm'
    : canal === 'orange'
      ...
      : canal === 'farmapack'
        ...
        : 'varios';
const canalDe = (r) =>
  r.nv_ptm ? 'ptm' : r.nv_orange ? 'orange' : r.nv_farmapack ? 'farmapack' : 'varios';
...
function mapOperacionRow(r) {
  ...
  return {
    ...
    fechaCompromiso: soloFecha(r.fecha_compromiso),
    guia: r.guia || '',
    factura: r.factura || '',
    fechaAprobacion: soloFecha(r.fecha_aprobacion),
    fechaAprobacionReal: soloFecha(r.fecha_aprobacion_real),
...
function dedupeRankedItems(rows, term, limit = 200) {
  ...
  (rows || []).forEach((item) => {
    ...
    if (
      !prev ||
      score > prev.score ||
      (score === prev.score && recencyScore(item) > recencyScore(prev.item))
    ) {
...
export async function listaActivas({ force = false, full = true, limit = 400 } = {}) {
  return runPanelRead(
    ...
    async () => {
      ...
      targetCache.promise = run().catch((error) => {
  ...
  return runPanelRead(
    ...
    async () => {
      ...
      const run = async () => {
        ...
        if (isNumericLookup) {
          const exactResult = await readWithPoolRetry(
            () =>
              supabase
                .from(OPERACIONES_BASE_TABLE)
                .select(LISTA_COLS)
                .or(
                  `nv_ptm.eq.${Number(safe)},nv_orange.eq.${safe},nv_farmapack.eq.${safe},varios.eq.${safe}`
                )
                .order('fecha_estado', { ascending: false, nullsFirst: false })
                .order('id', { ascending: false })
                .limit(4),
          ...
          const exactMapped = dedupeRankedItems(
            (exactResult.data || []).map(mapOperacionRow),
            t,
            Math.min(limit, 20)
          );
          ...
          const numericResult = await readWithPoolRetry(
            () =>
              supabase
                .from(OPERACIONES_READ_VIEW)
                .select(LISTA_COLS)
                .or(
                  `nv_ptm.eq.${Number(safe)},nv_orange.ilike.${numericPrefix},nv_farmapack.ilike.${numericPrefix},varios.ilike.${numericPrefix},guia.ilike.${numericPrefix},factura.ilike.${numericPrefix}`
                )
                .order('fecha_estado', { ascending: false, nullsFirst: false })
                .order('id', { ascending: false })
                .limit(softLimit),
          ...
          const numericRanked = dedupeRankedItems(
            (numericResult.data || []).map(mapOperacionRow),
            t,
            limit
          );
        ...
        const ors = [];
        ors.push(
          `nv_orange.ilike.${like}`,
          `nv_farmapack.ilike.${like}`,
          `varios.ilike.${like}`,
          `cliente.ilike.${like}`,
          `vendedor.ilike.${like}`,
          `guia.ilike.${like}`,
          `factura.ilike.${like}`,
          `transportista.ilike.${like}`
        );
        let data = null;
        let error = null;
  ...
  return runPanelRead(
    ...
    async () => {
      ...
      const run = async () => {
        ...
        const result = {
          estados: ESTADOS_SELECCIONABLES,
          transportistas,
          tiposDespacho: TIPOS_DESPACHO
        };
...
export async function costoDeVendedor(vendedor) {
  ...
  const scored = data
    .map((row) => {
      ...
      const coversBase =
        baseTokens.length > 0 && baseTokens.every((token) => nameTokens.includes(token));
      const coversCandidate =
        nameTokens.length > 0 && nameTokens.every((token) => baseTokens.includes(token));
      let score = 0;
      if (exact) score += 1000;
    ...
    async () => {
      ...
      const run = async () => {
        ...
        if (r) {
          const result = {
            ...
            data: {
              ...
              ccosto,
              division,
              fecha_compromiso: soloFecha(r.fecha_compromiso),
              fecha_registro_nv: soloFecha(r.fecha_registro_nv)
            }
          };
        ...
        return setMapCacheValue(lookupCache, cacheKey, {
          found: false,
          autoFill: { cliente, vendedor, ccosto, division }
        });
      };
    ...
    async () => {
      ...
      return all.map((r) => {
        ...
        EXPORT_COLS.forEach(([k, h]) => {
          ...
          else if (DATE_COLS.has(k))
            v = fmtFecha(v); // dd/mm/aaaa
...
export async function guardar(payload) {
  const p = {
    ...payload,
    id: payload?.id ?? (payload?.mode === 'update' ? payload?.lookup?.row : null)
  };
...
export async function puedeEditarOperacion(id) {
  ...
  if (error)
    return { permitida: false, message: error.message || 'No se pudo validar el acceso IAM.' };
...
export async function puedeCambiarEstadoOperacion(id, estado = null) {
  if (!id) return { permitida: false, message: 'N.V. no encontrada.' };
  const { data, error } = await supabase.rpc('iam_puede_cambiar_estado_nv', {
    p_id: id,
    p_estado: estado
  });
  if (error)
    return {
      permitida: false,
      message: error.message || 'No se pudo validar la transición de estado.'
    };
...
export async function actualizarCampos(id, dirty) {
  ...
  Object.entries(dirty || {}).forEach(([k, v]) => {
    const fk = COL_A_FORM[k] || k;
    p[fk] = v;
  });
  return runPanelMutation(
    'actualizar_campos_nv',
    async () => {
      const { data, error } = await supabase.rpc('guardar_nv', { p });
      const result = rpcResult(data, error);
      if (result?.ok !== false) resetIngresarCaches();
...
export async function solicitarReapertura(id, motivo) {
  return runPanelMutation(
    ...
    async () => {
      const { data, error } = await supabase.rpc('solicitar_reapertura_nv', {
        p_operacion_id: id,
        p_motivo: motivo
      });
