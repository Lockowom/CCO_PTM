...
export default function FlujoMaestro() {
  ...
  const seedSeq = (mdl) => {
    let mx = 0;
    [...(mdl.nodes || []), ...(mdl.edges || [])].forEach((o) => {
      const k = parseInt(String(o.id).replace(/\D/g, ''), 10) || 0;
      if (k > mx) mx = k;
    });
    seqRef.current = mx + 1;
  };
  ...
  const cargar = useCallback(
    async (cod) => {
      ...
      let mdl = null,
        tit = null;
      try {
        const row = await obtenerFlujo(cod);
        if (row?.modelo?.nodes) {
          mdl = row.modelo;
          tit = row.titulo;
        }
      ...
      if (!mdl) {
        mdl = { nodes: d.seed.nodes, edges: d.seed.edges };
        tit = d.seed._meta?.titulo || d.titulo;
      }
      setModel({
        nodes: mdl.nodes.map((n) => ({ ...n })),
        edges: mdl.edges.map((e) => ({ ...e }))
      });
      setTitulo(tit || d.titulo);
      seedSeq(mdl);
      setDirty(false);
      setSel(null);
      setInfo(null);
  ...
  const onWheel = (e) => {
    ...
    const mx = e.clientX - r.left,
      my = e.clientY - r.top;
    setView((v) => {
      const ns = Math.min(2.5, Math.max(0.1, v.s * (e.deltaY < 0 ? 1.12 : 0.89)));
      const k = ns / v.s;
      return { s: ns, x: mx - (mx - v.x) * k, y: my - (my - v.y) * k };
    });
  };
  const onBgDown = (e) => {
    if (e.target.closest('[data-node]') || e.target.closest('[data-elabel]')) return;
    setSel(null);
    setInfo(null);
  ...
  const onMove = (e) => {
    if (drag.current) {
      const dx = (e.clientX - drag.current.px) / view.s,
        dy = (e.clientY - drag.current.py) / view.s;
      drag.current.px = e.clientX;
      drag.current.py = e.clientY;
      drag.current.moved = true;
      setNodes((ns) =>
        ns.map((n) => (n.id === drag.current.id ? { ...n, x: n.x + dx, y: n.y + dy } : n))
      );
      return;
    }
    if (pan.current) {
  ...
  const onNodeClick = (e, n) => {
    ...
    if (connectFrom !== null) {
      ...
      if (connectFrom !== n.id) {
        setModel((m) => ({
          ...m,
          edges: [...m.edges, { id: uid('e'), from: connectFrom, to: n.id, label: '' }]
        }));
        mark();
      }
      setConnectFrom('');
  ...
  const onNodeDouble = (e, n) => {
    e.stopPropagation();
    if (!edit) return;
    const t = window.prompt('Etiqueta del nodo:', n.label);
    if (t == null) return;
    patchNode(n.id, { label: t });
  };

  const patchNode = (id, patch) => {
    setNodes((ns) => ns.map((x) => (x.id === id ? { ...x, ...patch } : x)));
    mark();
  };
  ...
  const addNode = (type) => {
    const el = wrapRef.current;
    const cx = (el.clientWidth / 2 - view.x) / view.s,
      cy = (el.clientHeight / 2 - view.y) / view.s;
    const d = SHAPE[type].dim;
    const n = {
      id: uid('n'),
      type,
      label: type === 'decision' ? '¿Decisión?' : 'Nuevo',
      x: Math.round(cx - d[0] / 2),
      y: Math.round(cy - d[1] / 2),
      w: d[0],
      h: d[1]
    };
    setModel((m) => ({ ...m, nodes: [...m.nodes, n] }));
    setSel({ kind: 'node', id: n.id });
    mark();
  };
  const delSel = useCallback(() => {
    ...
    if (sel.kind === 'node')
      setModel((m) => ({
        nodes: m.nodes.filter((n) => n.id !== sel.id),
        edges: m.edges.filter((e) => e.from !== sel.id && e.to !== sel.id)
      }));
  ...
  const guardar = async () => {
    const r = await guardarFlujo(codigo, titulo, model);
    if (r?.ok) {
  ...
  const exportar = () => {
    const blob = new Blob(
      [
        JSON.stringify(
          { _meta: { titulo, codigo }, nodes: model.nodes, edges: model.edges },
          null,
          2
        )
      ],
  ...
  const importar = (e) => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!Array.isArray(j.nodes) || !Array.isArray(j.edges)) throw 0;
        setModel({ nodes: j.nodes.map((n) => ({ ...n })), edges: j.edges.map((x) => ({ ...x })) });
        seedSeq(j);
        mark();
        requestAnimationFrame(() => fitNodes(j.nodes));
        toast.success('Importado — recuerda Guardar');
      } catch {
        toast.error('JSON inválido (se espera {nodes, edges})');
      }
    };
    rd.readAsText(f);
    e.target.value = '';
  ...
  const recortarDelMaestro = async () => {
    ...
    const re = DOMAIN[codigo];
    if (!re) {
      toast.error('Este diagrama no tiene filtro de dominio');
      return;
    }
    if (
      dirty &&
      !window.confirm('Se reemplazará el contenido actual con el recorte del maestro. ¿Continuar?')
    )
      return;
    let m = null;
    try {
      const row = await obtenerFlujo('maestro');
      if (row?.modelo?.nodes) m = row.modelo;
    } catch {
      /* seed */
    }
    ...
    if (!nodes.length) {
      toast.error('El maestro no tiene nodos de este dominio');
      return;
    }
    ...
    toast.success(
      `Recortado: ${nodes.length} nodos · ${edges.length} conexiones (revisa y Guarda)`
    );
  };
    ...
    <div className="anim-fade-up mx-[calc(50%-50vw)] px-3 sm:px-6 lg:px-8 pb-6">
      <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 leading-tight">
              Mapa de Procesos{' '}
              {dirty && <span className="text-orange-500 text-sm align-middle">• sin guardar</span>}
            </h1>
            <p className="text-[13px] text-slate-500">
              {model.nodes.length} nodos · {model.edges.length} conexiones ·{' '}
              {edit ? 'edición' : 'vista'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <select
            value={codigo}
            onChange={(e) => cambiarDiagrama(e.target.value)}
            className="py-2 px-3 rounded-xl border border-slate-200 text-sm font-bold text-slate-700 bg-white outline-none focus:border-orange-400"
          >
            {DIAGRAMS.map((d) => (
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-7 py-2 rounded-xl border border-slate-200 text-sm outline-none focus:border-orange-400 w-32"
            />
            {q && (
              <button
                onClick={() => setQ('')}
          ...
          <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl px-1 py-1">
            <button
              onClick={() => setView((v) => ({ ...v, s: Math.max(0.1, v.s * 0.89) }))}
            ...
            <button
              onClick={() => setView((v) => ({ ...v, s: Math.min(2.5, v.s * 1.12) }))}
              className="w-8 h-8 rounded-lg hover:bg-slate-100 grid place-items-center text-slate-500"
            >
              <ZoomIn size={16} />
            </button>
          ...
          {puedeEditar && (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-bold transition-colors ${edit ? 'bg-slate-800 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
            >
              <Pencil size={15} /> {edit ? 'Salir' : 'Editar'}
            </button>
          )}
        </div>
      </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap mb-2 bg-white border border-slate-200 rounded-xl px-2 py-1.5">
          ...
          {TB.map(([t, Icon]) => (
            <button
              key={t}
              onClick={() => addNode(t)}
              className="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[12px] font-bold text-slate-600 hover:bg-slate-100"
            >
              <Icon size={13} /> {t[0].toUpperCase() + t.slice(1)}
            </button>
          ))}
          <span className="w-px h-5 bg-slate-200 mx-1" />
          <button
            onClick={() => {
              setConnectFrom(connectFrom === null ? '' : null);
              setSel(null);
            }}
            className={`inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[12px] font-bold ${connectFrom !== null ? 'bg-orange-500 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
          >
          ...
          <button
            onClick={delSel}
            disabled={!sel}
            className="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[12px] font-bold text-red-600 hover:bg-red-50 disabled:opacity-40"
          >
          ...
          <button
            onClick={exportar}
            className="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[12px] font-bold text-slate-500 hover:bg-slate-100"
          >
          ...
          {codigo !== 'maestro' && (
            <button
              onClick={recortarDelMaestro}
              title="Extrae del Flujo Maestro los nodos de este dominio"
              className="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[12px] font-bold text-orange-600 hover:bg-orange-50"
            >
              <Scissors size={13} /> Recortar del maestro
            </button>
          )}
          <button
            onClick={() => cargar(codigo)}
            className="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[12px] font-bold text-slate-500 hover:bg-slate-100"
          >
          ...
          <button
            onClick={guardar}
            disabled={!dirty}
            className="ml-auto inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-bold bg-orange-500 text-white hover:bg-orange-600 disabled:opacity-40"
          >
      ...
      >
        ...
        >
          ...
          >
            <defs>
              <marker
                ...
                orient="auto"
              >
                <path d="M0,0 L7,3 L0,6 Z" fill={U.arrow} />
              </marker>
            </defs>
            {model.edges.map((e) => {
              const a = byId[e.from],
                b = byId[e.to];
              if (!a || !b) return null;
              const p1 = border(a, b.x + b.w / 2, b.y + b.h / 2),
                p2 = border(b, a.x + a.w / 2, a.y + a.h / 2);
              ...
              return (
                <g key={e.id}>
                  ...
                  <path
                    ...
                    strokeWidth="14"
                    fill="none"
                    style={{ cursor: edit ? 'pointer' : 'default', pointerEvents: 'stroke' }}
                    onPointerDown={(ev) => {
                      ev.stopPropagation();
                      if (edit) setSel({ kind: 'edge', id: e.id });
                    }}
                  />
                  {e.label && (
                    <g
                    ...
                    >
                      <rect
                        x={mx - e.label.length * 3.2 - 4}
                        y={my - 8}
                        width={e.label.length * 6.4 + 8}
                        height={16}
                        rx={4}
                        fill={U.labelBg}
                        stroke={on ? '#f97316' : U.labelBorder}
                      />
                      <text
                        x={mx}
                        y={my + 3}
          ...
          {model.nodes.map((n) => {
            const t = { ...(SHAPE[n.type] || SHAPE.tarea), ...(C[n.type] || C.tarea) };
            const hot = term && n.label.toLowerCase().includes(term);
            const on = sel?.kind === 'node' && sel.id === n.id;
            const src = connectFrom === n.id;
            const bc = n.color || t.border;
            return (
              <div
                key={n.id}
                data-node
                ...
                style={{
                  ...
                  background: t.fill,
                  color: t.text,
                  border: `2px solid ${hot || on || src ? '#f97316' : bc}`,
                  borderRadius: t.pill ? 999 : 12,
                  fontSize: 12,
                  whiteSpace: 'pre-line',
                  cursor: edit
                    ? connectFrom !== null
        ...
        {edit && (selNode || selEdge) && (
          <div
            className={`absolute top-3 right-3 w-60 border rounded-xl shadow-lg p-3 space-y-2.5 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}
          >
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-slate-400 uppercase inline-flex items-center gap-1">
                <SlidersHorizontal size={12} /> Propiedades
              </span>
              <button onClick={() => setSel(null)} className="text-slate-400 hover:text-slate-200">
                <X size={14} />
              </button>
            </div>
            {selNode && (
              <>
                <label className="block">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Etiqueta</span>
                  <textarea
                    rows={2}
                    value={selNode.label}
                    onChange={(e) => patchNode(selNode.id, { label: e.target.value })}
                    className="mt-1 w-full border border-slate-200 rounded-lg px-2 py-1.5 text-[13px] outline-none focus:border-orange-400 resize-none"
                  />
                </label>
                <label className="block">
                  ...
                  <select
                    ...
                    className="mt-1 w-full border border-slate-200 rounded-lg px-2 py-1.5 text-[13px] outline-none focus:border-orange-400"
                  >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    type="color"
                    value={selNode.color || C[selNode.type].border}
                    onChange={(e) => patchNode(selNode.id, { color: e.target.value })}
                    className="w-8 h-8 rounded border border-slate-200 p-0.5"
                  />
                  {selNode.color && (
                    <button
                      onClick={() => patchNode(selNode.id, { color: undefined })}
        ...
        {info && !edit && (
          <div
            className={`absolute bottom-3 right-3 w-64 border rounded-xl shadow-lg p-3 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}
          >
            <div className="flex items-start justify-between gap-2">
              <span className="text-[10px] font-black text-slate-400 uppercase">{info.type}</span>
              <button onClick={() => setInfo(null)} className="text-slate-400 hover:text-slate-200">
                <X size={14} />
              </button>
            </div>
            <p
              className={`text-[14px] font-black mt-0.5 whitespace-pre-line ${isDark ? 'text-slate-100' : 'text-slate-800'}`}
            >
              {info.label}
            </p>
            {jump ? (
              <button
                onClick={() => nav(jump)}
                className="mt-2 w-full inline-flex items-center justify-center gap-1.5 py-2 rounded-lg bg-orange-500 text-white text-[12px] font-bold hover:bg-orange-600"
              >
                <ExternalLink size={13} /> Ir al módulo
              </button>
            ) : (
        ...
        <div
          className={`absolute bottom-3 left-3 backdrop-blur border rounded-xl px-3 py-2 flex flex-wrap items-center gap-3 text-[10px] ${isDark ? 'bg-slate-900/80 border-slate-700 text-slate-300' : 'bg-white/90 border-slate-200 text-slate-500'}`}
        >
          <span className="inline-flex items-center gap-1">
            <span
              className="w-3 h-3 rounded-full border-2"
              style={{ borderColor: C.inicio.border, background: C.inicio.fill }}
            />{' '}
            Inicio
          </span>
          <span className="inline-flex items-center gap-1">
            <span
              ...
              style={{ borderColor: C.tarea.border, background: C.tarea.fill }}
            />{' '}
            Tarea
          </span>
          <span className="inline-flex items-center gap-1">
            <span
              className="w-3 h-3 rounded border-2"
              style={{ borderColor: C.decision.border, background: C.decision.fill }}
            />{' '}
            Decisión
          </span>
          <span className="inline-flex items-center gap-1">
            <span
              className="w-3 h-3 rounded-full border-2"
              style={{ borderColor: C.fin.border, background: C.fin.fill }}
            />{' '}
            Fin
          </span>
          {edit ? (
