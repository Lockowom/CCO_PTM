...
function HeroChip({ icon: Icon, children, tone = 'slate' }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[11px] sm:text-xs font-black ${tones[tone] || tones.slate}`}
    >
      <Icon size={13} />
      <span>{children}</span>
    </div>
...
function HeroMetric({ label, value, helper, tone = 'orange' }) {
  ...
  return (
    <div
      className={`rounded-2xl border bg-gradient-to-br ${tones[tone] || tones.orange} p-4 backdrop-blur-sm`}
    >
...
function CarpetaCalidadTrazabilidad() {
  ...
  const misAreas = useMemo(
    () =>
      new Set(
        areas
          .filter((a) => Array.isArray(a.roles) && a.roles.includes(user?.rol))
          .map((a) => a.codigo)
      ),
    [areas, user?.rol]
  );
  const puedeActuar = (a) =>
    isAdmin ||
    misAreas.has(a.area_responsable) ||
    hasPermission('manage_inventory') ||
    hasPermission('manage_quality');
  ...
  const solicitudes = useMemo(
    () =>
      acciones.filter(
        (a) =>
          TIPOS_BODEGA.has(a.tipo_accion) && (a.estado === 'PENDIENTE' || a.estado === 'EN_PROCESO')
      ),
    [acciones]
  );
  ...
  return (
    <div className="mx-3 sm:mx-6 mt-3 sm:mt-4 shrink-0">
      <div className="relative overflow-hidden rounded-[1.75rem] border border-emerald-200/80 bg-white shadow-[0_18px_50px_-30px_rgba(16,185,129,0.45)]">
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="text-left min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <div className="font-black text-slate-900 text-sm sm:text-base tracking-wide">
              ...
              <div className="text-[11px] sm:text-xs text-slate-500 line-clamp-2">
                Solicitudes derivadas por Calidad a bodega. Genera el traspaso, envía el correo y
                conserva la trazabilidad operativa.
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span className="px-3 py-1 rounded-full text-[11px] font-black bg-emerald-600 text-white shadow-sm">
              {solicitudes.length}
            </span>
            {abierta ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50/80 px-4 py-3">
                ...
                <div className="text-xs text-amber-600">
                  Solicitudes que ya están siendo gestionadas
                </div>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <span className="font-mono font-black text-slate-700 text-xs tracking-wide">
                        {a.folio}
                      </span>
                      <div className="flex items-center gap-1">
                        {a.prioridad === 'URGENTE' && (
                          <span className="text-[9px] font-black uppercase px-1.5 py-0.5 rounded-md border bg-rose-100 text-rose-700 border-rose-200 flex items-center gap-0.5">
                            <AlertTriangle size={10} /> Urgente
                          </span>
                        )}
                        <span
                          className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ${meta.cls}`}
                        >
                          {meta.label || a.estado}
                        </span>
                      </div>
                    ...
                    <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-3">
                      ...
                      <p className="text-xs text-slate-500 mt-1 line-clamp-2">
                        {a.producto || 'Sin nombre descriptivo'}
                      </p>
                    </div>

                    <div className="flex flex-wrap items-center gap-1.5 mt-3">
                      <span className="text-[9px] font-black px-2 py-1 rounded-md border bg-indigo-50 text-indigo-700 border-indigo-200">
                        {TIPO_LABEL[a.tipo_accion] || a.tipo_accion}
                      </span>
                      {a.dictamen && (
                        <span
                          className={`text-[9px] font-black px-2 py-1 rounded-md border ${DICTAMEN_CLS[a.dictamen] || 'bg-slate-100 text-slate-600 border-slate-200'}`}
                        >
                          {a.dictamen}
                        </span>
                      )}
                      {a.bodega_destino && (
                        <span className="text-[9px] font-black px-2 py-1 rounded-md border bg-slate-100 text-slate-600 border-slate-200">
                          Destino {a.bodega_destino}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      ...
                      <div className="rounded-xl bg-slate-50 border border-slate-100 px-2.5 py-2">
                        ...
                        <div className="mt-1 text-[11px] font-bold text-slate-700 truncate">
                          {a.cantidad != null ? `${a.cantidad} u` : 'N/A'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 mt-3.5 pt-3 border-t border-slate-100">
                      {puedeActuar(a) && (
                        <button
                          ...
                          disabled={correoEnviado.isPending}
      ...
      {traza && (
        <TrazabilidadModal
          codigo={traza.codigo_producto}
          partida={traza.partida}
          ubicacion={traza.ubicacion}
          producto={traza.producto}
          onClose={() => setTraza(null)}
        />
      )}
    </div>
  );
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <HeroChip icon={Sparkles} tone="orange">
                    Reformado
                  </HeroChip>
                  <HeroChip icon={Boxes} tone="indigo">
                    Operación embebida
                  </HeroChip>
                ...
                <p className="mt-2 text-sm sm:text-base text-slate-500 max-w-3xl">
                  Unificamos la experiencia del módulo para que el acceso a traspasos, ajustes y
                  correos operativos tenga mejor jerarquía visual, mejor foco de acciones y una
                  lectura más clara para bodega.
                </p>
              </div>
            </div>
            ...
            <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50/80 p-4">
              ...
              <div className="mt-3 space-y-2 text-xs text-slate-500">
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                  1. Revisa solicitudes derivadas por Calidad.
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                  2. Ejecuta el traspaso o ajuste en el módulo embebido.
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                  3. Marca el correo enviado y conserva la trazabilidad.
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3 px-1">
          <div>
            <div className="text-sm sm:text-base font-black text-slate-900">
              Entorno operativo integrado
            </div>
            <div className="text-xs text-slate-500">
              Sin marco extra y con altura automática para evitar doble scroll.
            </div>
          </div>
            ...
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4 bg-[linear-gradient(180deg,rgba(248,250,252,0.96),rgba(241,245,249,0.92))] backdrop-blur-sm rounded-[1.5rem]">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Preparando el entorno de traspasos dentro de CCO.
                </p>
              </div>
            </div>
          )}
