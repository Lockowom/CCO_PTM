...
import { ShieldCheck, ShieldAlert, Smartphone, KeyRound, Trash2, Check, Loader2, QrCode, UserCheck, CalendarClock, ArrowRight } from 'lucide-react';
...
const fmtF = (ts) => { if (!ts) return '—'; const d = new Date(ts); return isNaN(d) ? '—' : d.toLocaleDateString('es-CL', { day: '2-digit', month: '2-digit', year: '2-digit' }); };
...
function DelegacionSelfService() {
  ...
  const cargar = useCallback(async () => {
    try {
      ...
      setCob(c); setUsuarios(u);
    } catch { /* noop */ }
  }, []);
  useEffect(() => { cargar(); }, [cargar]);

  const crear = async () => {
    if (!f.delegado || !f.hasta) { toast.error('Elige a quién cubre y hasta cuándo'); return; }
    setBusy(true);
    const r = await delegar({ delegado: f.delegado, hasta: new Date(f.hasta).toISOString(), motivo: f.motivo || null });
    setBusy(false);
    if (r?.ok) { toast.success('Delegación creada'); setF({ delegado: '', hasta: '', motivo: '' }); cargar(); }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3">
      <div className="flex items-center gap-2"><UserCheck size={17} className="text-orange-500" /><h3 className="font-black text-slate-800 text-[14px]">Delegación de permisos (vacaciones)</h3></div>

      {cob.cubro?.length > 0 && (
        <div className="rounded-xl border border-sky-100 bg-sky-50/60 px-3 py-2 text-[12.5px] text-slate-600">
          Estás <b>cubriendo</b> a: {cob.cubro.map((c) => `${c.delegador} (hasta ${fmtF(c.hasta)})`).join(', ')}
        </div>
      )}

      <p className="text-[12px] text-slate-500">Presta tus permisos a un compañero durante una ausencia. Caduca sola en la fecha indicada.</p>
      <div className="grid sm:grid-cols-3 gap-2">
        <select value={f.delegado} onChange={(e) => setF({ ...f, delegado: e.target.value })} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-orange-400">
          <option value="">¿Quién cubre?</option>
          {usuarios.map((u) => <option key={u.id} value={u.id}>{u.nombre}</option>)}
        </select>
        <input type="date" value={f.hasta} onChange={(e) => setF({ ...f, hasta: e.target.value })} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-orange-400" />
        <input value={f.motivo} onChange={(e) => setF({ ...f, motivo: e.target.value })} placeholder="Motivo (opcional)" className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-orange-400" />
      </div>
      <button onClick={crear} disabled={busy} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-500 text-white text-sm font-bold hover:bg-orange-600 disabled:opacity-50">
        {busy ? <Loader2 size={15} className="animate-spin" /> : <UserCheck size={15} />} Delegar mis permisos
      </button>

      {cob.delego?.length > 0 && (
        <div className="pt-1 space-y-1.5">
          <p className="text-[11px] font-black text-slate-500 uppercase tracking-wide">Delegaciones activas que otorgué</p>
          {cob.delego.map((d) => (
            <div key={d.id} className="flex items-center gap-2 text-[12.5px] px-3 py-2 rounded-xl border border-slate-100 bg-slate-50/60">
              <ArrowRight size={13} className="text-slate-300" />
              <span className="font-bold text-slate-700">{d.delegado}</span>
              <span className="text-[11px] text-slate-400 inline-flex items-center gap-1"><CalendarClock size={11} /> hasta {fmtF(d.hasta)}</span>
              <button onClick={() => revocar(d.id)} className="ml-auto w-7 h-7 rounded-lg hover:bg-red-50 grid place-items-center text-red-400"><Trash2 size={14} /></button>
            </div>
          ))}
        </div>
      )}
