...
const VerificarCertificado = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!folio) {
        if (alive) {
          setRes({ valido: false, motivo: 'Falta el folio a verificar' });
          setLoading(false);
        }
        return;
      }
      try {
        ...
        if (alive) {
          setRes({ valido: false, motivo: e.message || 'Error de verificación' });
          setLoading(false);
        }
      }
    })();
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 text-center border-b border-slate-100">
          <div className="flex items-center justify-center gap-2 text-slate-800">
            ...
            <span className="font-black tracking-tight">
              PTM CHILE LTDA. — Verificación de documento
            </span>
          </div>
        ...
        {loading ? (
          ...
          <div className="p-6">
            <div
              className={`rounded-2xl p-5 flex items-center gap-4 mb-5 ${ok ? 'bg-emerald-50 border border-emerald-200' : 'bg-rose-50 border border-rose-200'}`}
            >
              {ok ? (
                <ShieldCheck size={38} className="text-emerald-600 shrink-0" />
              ) : (
                <ShieldX size={38} className="text-rose-600 shrink-0" />
              )}
              <div>
                <p className={`font-black text-lg ${ok ? 'text-emerald-800' : 'text-rose-800'}`}>
                  {ok ? 'Documento válido' : 'No válido'}
                </p>
                <p className={`text-sm ${ok ? 'text-emerald-700' : 'text-rose-700'}`}>
                  {res?.motivo}
                </p>
              </div>
            </div>
            ...
            {res?.folio && (
              <div className="space-y-0.5">
                ...
                <Row
                  k="Resultado"
                  v={
                    res.resultado === 'CONFORME'
                      ? 'Conforme'
                      : res.resultado === 'NO_CONFORME'
                        ? 'No conforme'
                        : res.resultado
                  }
                />
                ...
                <Row
                  k="Fecha de firma"
                  v={res.firmado_en ? new Date(res.firmado_en).toLocaleString('es-CL') : '—'}
                />
                <Row k="Algoritmo" v={res.algoritmo} />
              </div>
            )}
            <p className="text-[11px] text-slate-400 text-center mt-5">
              Verificación criptográfica (HMAC-SHA256) contra el registro original. Cualquier
              alteración del documento invalida la firma.
            </p>
          </div>
        )}
      </div>
