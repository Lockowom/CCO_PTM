...
const MiBandeja = () => {
  ...
  const pendientesMias = acciones.filter(
    (a) =>
      (a.estado === 'PENDIENTE' || a.estado === 'EN_PROCESO') && misAreas.has(a.area_responsable)
  ).length;

  const nombreAreas =
    areas
      .filter((a) => misAreas.has(a.codigo))
      .map((a) => a.label)
      .join(', ') || (puedeVerTodo ? 'Calidad / Supervisión' : 'Sin área asignada');

  const marcarEnProceso = async (a) => {
    try {
      await resolver.mutateAsync({ accionId: a.id, resolucion: '', estado: 'EN_PROCESO' });
  ...
  const cerrar = async (a) => {
    if (!texto.trim()) {
      toast.error('Escribe qué se hizo para cerrar');
      return;
    }
    try {
  ...
  return (
    <div className="h-full bg-slate-50 p-3 sm:p-6 min-h-screen">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
              Mi <span className="text-sky-600">Bandeja</span>
            </h1>
            <p className="text-slate-500 font-bold text-sm">
              {nombreAreas} · {pendientesMias} pendiente(s)
            </p>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5">
        ...
        <button
          onClick={() => setSoloPend(true)}
          className={`px-3 py-2 rounded-xl text-xs font-black border ${soloPend ? 'bg-sky-600 text-white border-sky-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
        >
          Pendientes
        </button>
        <button
          onClick={() => setSoloPend(false)}
          className={`px-3 py-2 rounded-xl text-xs font-black border ${!soloPend ? 'bg-sky-600 text-white border-sky-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
        >
          Todas
        </button>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-xs text-slate-300">
            No tienes tareas {soloPend ? 'pendientes' : ''} asignadas a tu área.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {lista.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className={`bg-white rounded-2xl border p-5 flex flex-col ${abierta ? 'border-amber-200' : 'border-slate-200'}`}
              >
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-2">
                  ...
                  {verTodas && puedeVerTodo && (
                    <span className="text-[10px] font-black px-2 py-0.5 rounded-md border bg-teal-50 text-teal-700 border-teal-200">
                      {area?.label || a.area_responsable}
                    </span>
                  )}
                  {a.bodega_destino && (
                    <span className="text-[10px] font-black px-2 py-0.5 rounded-md border bg-slate-100 text-slate-600 border-slate-200">
                      BD {a.bodega_destino}
                    </span>
                  )}
                  {a.dictamen && (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 rounded-xl border border-slate-200 text-slate-500 font-black text-xs hover:bg-slate-50"
                        >
                          Cancelar
                        </button>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {abierta && !puedeResolver(a) && (
                        <span className="text-[11px] text-slate-400 flex items-center gap-1.5">
                          <Clock size={13} /> {area?.label || a.area_responsable}
                        </span>
                      )}
                    </div>
                  )}
