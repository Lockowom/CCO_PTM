        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
              Acciones de <span className="text-emerald-600">Calidad</span>
            </h1>
            <p className="text-slate-500 font-bold text-sm">
              Acciones recomendadas por Calidad, asignadas a cada área · {pendientes} pendiente(s)
            </p>
          </div>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-base font-bold text-slate-400">
            Sin acciones {filtro === 'pendientes' ? 'pendientes' : ''}
          </h3>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-2">
                  ...
                  {a.dictamen && (
                    <span
                      className={`text-[10px] font-black px-2 py-0.5 rounded-md border ${DICTAMEN_CLS[a.dictamen] || 'bg-slate-100 text-slate-600 border-slate-200'}`}
                    >
                      {a.dictamen}
                    </span>
                  )}
                ...
                {a.estado === 'RESUELTA' && (
                  <p className="text-[11px] text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg px-2.5 py-1.5 mt-2">
                    ...
                    {a.resuelto_en
                      ? ` · ${new Date(a.resuelto_en).toLocaleDateString('es-CL')}`
                      : ''}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {puedeResolver(a) ? (
                          ...
                          <span className="text-[11px] text-slate-400 flex items-center gap-1.5">
                            <Clock size={13} /> Esperando a {area?.label || a.area_responsable}
                          </span>
                        )}
                        {(isAdmin || canManageQuality) && (
                          <button
                            onClick={() => anularAccion(a)}
                            title="Anular"
                            className="px-3 py-2 rounded-xl border border-slate-200 text-slate-400 font-black text-xs hover:bg-rose-50 hover:text-rose-600"
                          >
                            <Ban size={14} />
                          </button>
                        )}
