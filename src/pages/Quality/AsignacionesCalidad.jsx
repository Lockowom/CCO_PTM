...
import {
  ClipboardList, Search, Loader2, Plus, X, Trash2, AlertTriangle, PackageCheck,
  UserPlus, FileText, Ban, ArrowRight,
} from 'lucide-react';
import {
...
const AsignarModal = ({ onClose }) => {
  ...
  const buscar = useCallback(async () => {
    setBuscando(true);
    try { setCandidatos(await fetchCandidatos(query, false)); }
  ...
  const add = (c) => {
    const k = keyOf(c);
    if (sel.some(s => s._key === k)) { toast.info('Ese SKU ya está en la asignación'); return; }
    setSel(prev => [...prev, {
      _key: k,
  ...
  const enviar = async () => {
    if (sel.length === 0) { toast.error('Elige al menos un SKU'); return; }
    try {
      ...
      onClose();
    } catch (e) { toast.error(`No se pudo asignar: ${e.message}`); }
  };
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-3" onClick={onClose}>
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <h3 className="font-black text-slate-900 flex items-center gap-2"><UserPlus size={18} className="text-emerald-600" /> Asignar SKUs a Calidad</h3>
        ...
        <div className="p-5 overflow-y-auto space-y-4">
          ...
          <div className="flex gap-2">
            <div className="flex-1 flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 focus-within:border-emerald-400">
              ...
              <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && buscar()}
                placeholder="Buscar por SKU, descripción o ubicación…" className="flex-1 text-sm outline-none bg-transparent" />
            </div>
          ...
          {/* Seleccionados */}
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">SKUs a asignar ({sel.length})</p>
            {sel.length === 0 ? (
              <p className="text-xs text-slate-400">Busca y agrega los SKUs que Calidad debe revisar.</p>
            ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Motivo de la revisión</label>
              <input value={motivo} onChange={e => setMotivo(e.target.value)} placeholder="Ej. próximo a vencer / daño detectado / reclamo"
        ...
        <div className="p-4 border-t border-slate-100 flex justify-end gap-2">
          ...
          <button onClick={enviar} disabled={crear.isPending || sel.length === 0}
            ...
            {crear.isPending ? <Loader2 size={16} className="animate-spin" /> : <UserPlus size={16} />} Asignar a Calidad
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="mb-6">
      <div className="flex items-center justify-between gap-3 mb-3">
        <h3 className="text-sm font-black text-slate-700 flex items-center gap-2">
          <ClipboardList size={16} className="text-emerald-500" /> Revisiones asignadas por Inventario
          {pendientes > 0 && <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">{pendientes} pendiente(s)</span>}
        </h3>
        {canAssign && (
      ...
      {isLoading ? (
        <div className="flex justify-center py-8"><Loader2 className="animate-spin text-emerald-500" size={26} /></div>
        ...
        <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-6 text-center">
          ...
          <p className="text-xs text-slate-300">{canAssign ? 'Usa “Asignar SKUs a Calidad” para enviar productos a revisión.' : 'Inventario aún no ha asignado revisiones.'}</p>
        </div>
      ) : (
            ...
            return (
              <div key={a.id} className={`bg-white rounded-2xl border p-4 ${abierta ? 'border-amber-200' : 'border-slate-200'}`}>
                ...
                <p className="text-xs text-slate-500 line-clamp-2 mt-0.5">
                  {skus.slice(0, 3).map(s => s.codigo_producto).join(', ')}{skus.length > 3 ? '…' : ''}
                </p>
                {a.motivo && <p className="text-xs text-slate-400 mt-1 italic">“{a.motivo}”</p>}
                <p className="text-[11px] text-slate-400 mt-2">
                  {a.asignado_nombre ? `Por ${a.asignado_nombre}` : 'Inventario'} · {a.created_at ? new Date(a.created_at).toLocaleDateString('es-CL') : ''}
                </p>

                {abierta && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {canManageQuality && (
                      <button onClick={() => onGenerarInforme(a)}
                    ...
                    {canAssign && (
                      <button onClick={() => anularAsig(a)} title="Anular"
                ...
                {a.estado === 'RESUELTA' && (
                  <p className="text-[11px] text-emerald-600 font-bold mt-3 flex items-center gap-1">
                    <FileText size={12} /> Resuelta{a.resuelto_nombre ? ` por ${a.resuelto_nombre}` : ''}
                  </p>
                )}
              </div>
            );
