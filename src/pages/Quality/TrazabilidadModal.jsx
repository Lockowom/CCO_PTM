...
const ESTADO_EV_CLS = (estado) => {
  ...
  if (['CUARENTENA', 'PENDIENTE', 'EN_PROCESO', 'EN_REVISION', 'REPROCESO'].includes(e)) return 'text-amber-600';
...
const TrazabilidadModal = ({ codigo, partida, ubicacion, producto, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-3" onClick={onClose}>
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div className="min-w-0">
            <h3 className="font-black text-slate-900 flex items-center gap-2"><GitBranch size={18} className="text-emerald-600" /> Trazabilidad</h3>
            <p className="text-xs text-slate-500 truncate mt-0.5">
              <span className="font-bold">{codigo}</span>{producto ? ` · ${producto}` : ''}
              {ubicacion ? <span className="inline-flex items-center gap-0.5 ml-1"><MapPin size={11} /> {ubicacion}</span> : null}
            </p>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 shrink-0"><X size={18} /></button>
        </div>
        ...
        {flag && (
          <div className="px-5 pt-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${flagMeta?.cls || 'bg-slate-100 text-slate-600 border-slate-200'}`}>
        ...
        <div className="p-5 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-xs text-slate-300">Este producto aún no tiene checklist, dictamen, acción ni salida registrada.</p>
            </div>
          ) : (
                ...
                return (
                  <li key={i} className="ml-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md border ${hm.cls || 'bg-slate-100 text-slate-600 border-slate-200'}`}>{hm.label || ev.hito}</span>
                      <span className="text-sm font-black text-slate-800">{ev.titulo}</span>
                      {ev.estado && <span className={`text-xs font-black ${ESTADO_EV_CLS(ev.estado)}`}>{ev.estado.replace('_', ' ')}</span>}
                      {ev.folio && <span className="text-[10px] font-mono font-black text-slate-400">{ev.folio}</span>}
                    </div>
