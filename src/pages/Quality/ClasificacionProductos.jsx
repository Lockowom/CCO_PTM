...
import { Tags, UploadCloud, Loader2, CheckCircle, ListChecks } from 'lucide-react';
...
export default function ClasificacionProductos() {
  ...
  const puede =
    hasPermission('manage_quality') || hasPermission('manage_monitoreo') || user?.rol === 'ADMIN';
  ...
  const ejecutar = async () => {
    ...
    try {
      ...
      setResumen({
        procesados,
        importacion: recl?.importacion || 0,
        nacional: recl?.nacional || 0,
        filas: rows.length
      });
      toast.success(
        `Clasificación cargada: ${procesados.toLocaleString()} productos · recepciones reclasificadas`
      );
      cargarStats();
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 sm:p-6 space-y-4 text-slate-700">
      <div className="relative overflow-hidden bg-white rounded-2xl border border-slate-200 shadow-sm px-5 sm:px-7 py-4 sm:py-5 flex items-center gap-4">
        <div className="w-12 h-12 bg-emerald-50 border border-emerald-100 rounded-2xl grid place-items-center text-emerald-600 shrink-0">
          <Tags size={22} />
        </div>
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900">
            Clasificación de <span className="text-emerald-600">Productos</span>
          </h1>
          <p className="text-xs sm:text-sm text-slate-500">
            Grupos comerciales del ERP para el checklist de Calidad
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-center">
          <div className="text-2xl font-black text-slate-800 tabular-nums">
            {stats.clasificados.toLocaleString()}
          </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4">
        <div className="flex items-start gap-2 text-sm text-slate-600">
          ...
          <p>
            Carga el mapeo <b>producto → grupo comercial</b> (desde el maestro del ERP) y{' '}
            <b>reclasifica</b> las recepciones ya registradas. Los productos que no estén en el
            maestro quedan como <b>Sin clasificar</b> hasta la próxima carga.
          </p>
        </div>
        ...
        {cargando && prog.total > 0 && (
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1 text-center">
              {prog.hechos.toLocaleString()} / {prog.total.toLocaleString()}
            </p>
          </div>
        )}
        ...
        {resumen && (
          <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 p-3 text-sm text-emerald-800 flex items-start gap-2">
            ...
            <span>
              <b>{resumen.procesados.toLocaleString()}</b> productos clasificados. Recepciones
              reclasificadas: {resumen.importacion.toLocaleString()} importación ·{' '}
              {resumen.nacional.toLocaleString()} nacional.
            </span>
          </div>
        )}
        ...
        {!puede && (
          <p className="text-[11px] text-slate-400">
            Necesitas permiso de gestión de Calidad para cargar la clasificación.
          </p>
        )}
      </div>
    </div>
