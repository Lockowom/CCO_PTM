...
export default function Insumos() {
  ...
  const visibles = useMemo(
    () =>
      items.filter((i) => {
        ...
        if (
          term &&
          ![i.nombre, i.codigo_ptm, i.medida]
            .filter(Boolean)
  ...
  const porCategoria = useMemo(() => {
    ...
    visibles.forEach((i) => {
      (map[i.categoria] || (map[i.categoria] = [])).push(i);
    });
    return map;
  }, [visibles]);
  ...
  const seleccionarPorReponer = () =>
    setSel(new Set(items.filter((i) => semaforo(i) !== 'ok').map((i) => i.id)));
    ...
    <div className="anim-fade-up space-y-5 max-w-6xl mx-auto pb-24">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">
            <Package className="text-orange-500" /> Panel de Insumos
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">
            Stock de insumos de embalaje y despacho · semáforo de reposición
          </p>
        </div>
        <button
          onClick={cargar}
          className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50"
        >
        ...
        {[
          ...
          return (
            ...
            >
              ...
              <div>
                <div className="text-2xl font-black" style={{ color: m.text }}>
                  {n}
                </div>
                <div className="text-[12px] font-semibold" style={{ color: m.text }}>
                  {m.label}
                </div>
              </div>
            </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-orange-400 bg-white"
          />
        </div>
        <button
          onClick={() => setSoloReponer((v) => !v)}
          className={`px-3 py-2.5 rounded-xl text-sm font-bold border transition-colors ${soloReponer ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
        >
          Solo por reponer
        </button>
        <button
          onClick={seleccionarPorReponer}
          className="px-3 py-2.5 rounded-xl text-sm font-bold border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
        >
      ...
      {loading ? (
        ...
        CATEGORIAS.filter((c) => (porCategoria[c] || []).length > 0).map((cat) => (
          <section key={cat}>
            <h2 className="text-[11px] font-black text-slate-400 uppercase tracking-wider mb-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {porCategoria[cat].map((i) => (
                <InsumoCard
                  ...
                  onToggle={() => toggleSel(i.id)}
                  puedeGestionar={puedeGestionar}
                  onCantidad={(v) => aplicarCantidad(i, v)}
                  onSaved={cargar}
                />
              ))}
            </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200 shadow-[0_-4px_16px_rgba(15,23,42,0.08)]">
          <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
            <span className="text-sm font-semibold text-slate-700">
              {sel.size} insumo{sel.size !== 1 ? 's' : ''} seleccionado{sel.size !== 1 ? 's' : ''}
            </span>
...
function InsumoCard({ item, selected, onToggle, puedeGestionar, onCantidad, onSaved }) {
  ...
  const commit = (v) => {
    ...
    setEditCant(String(n));
    if (n !== item.cantidad) onCantidad(n);
  };
  ...
  return (
    <div
      className={`rounded-2xl border-2 bg-white p-4 transition-shadow ${selected ? 'ring-2 ring-orange-300' : ''}`}
    ...
    >
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        {puedeGestionar ? (
          <div className="flex items-center gap-1">
            <button
              onClick={() => commit(item.cantidad - 1)}
            ...
            <input
              value={editCant}
              onChange={(e) => setEditCant(e.target.value.replace(/[^0-9.]/g, ''))}
        ...
        <div className="text-right">
          ...
          {puedeGestionar && (
            <button
              onClick={() => setCfg((v) => !v)}
              className="inline-flex items-center gap-1 text-[11px] text-slate-400 hover:text-orange-500 mt-0.5"
            >
      ...
      <div className="mt-2 h-1.5 rounded-full bg-slate-100 overflow-hidden">
        <div
          ...
          style={{
            width: `${Math.min(100, item.umbral_bajo > 0 ? (item.cantidad / (item.umbral_bajo * 2)) * 100 : item.cantidad > 0 ? 100 : 0)}%`,
            background: m.color
          }}
        />
      </div>
      <div className="mt-1 text-[10px] text-slate-400">
        Bajo ≤ {item.umbral_bajo} · Crítico ≤ {item.umbral_critico}
      </div>
      ...
      {cfg && (
        <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 gap-2">
          <label className="text-[11px] font-bold text-slate-500">
            ...
            <input
              value={bajo}
              onChange={(e) => setBajo(e.target.value.replace(/[^0-9.]/g, ''))}
              className="mt-1 w-full border border-slate-200 rounded-lg px-2 py-1 text-sm outline-none focus:border-orange-400"
            />
          </label>
          <label className="text-[11px] font-bold text-slate-500">
            Umbral crítico
            <input
              value={crit}
              onChange={(e) => setCrit(e.target.value.replace(/[^0-9.]/g, ''))}
              className="mt-1 w-full border border-slate-200 rounded-lg px-2 py-1 text-sm outline-none focus:border-orange-400"
            />
          </label>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h3 className="text-[15px] font-black text-slate-800 flex items-center gap-2">
            <Mail size={18} className="text-orange-500" /> Solicitud de insumos
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <label className="block">
            <span className="text-[12px] font-bold text-slate-500">
              Correo del destinatario (compras / bodega)
            </span>
            <input
              value={dest}
              onChange={(e) => setDest(e.target.value)}
              placeholder="ej: compras@ptm.cl"
              className="mt-1 w-full border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-orange-400"
            />
          </label>
          <div>
                ...
                return (
                  ...
                  >
                    ...
                    <input
                      value={pedir[i.id]}
                      onChange={(e) =>
                        setPedir((p) => ({ ...p, [i.id]: e.target.value.replace(/[^0-9.]/g, '') }))
                      }
                      placeholder="pedir"
                      className="w-20 text-center border border-slate-200 rounded-lg px-2 py-1 text-sm outline-none focus:border-orange-400"
                    />
                  </div>
                );
              })}
          ...
          <div>
            <span className="text-[12px] font-bold text-slate-500">
              Vista previa (así se verá al pegar en el correo)
            </span>
            <div
              className="mt-1 text-[12px] bg-white border border-slate-200 rounded-xl p-3 max-h-56 overflow-auto"
