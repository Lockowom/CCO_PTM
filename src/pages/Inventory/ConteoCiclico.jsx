...
import {
  ...
  useCrearBloque,
  useEditarBloque,
  useAgregarBloqueItem,
  useEliminarBloqueItem,
  useRegistrarAuditoria,
  useProyecciones,
  useGuardarProyeccion,
  useEliminarProyeccion,
  useRegistrarConteo,
  useConteos,
  useEliminarConteo,
  fetchLotesSeries,
...
export default function ConteoCiclico() {
  ...
  const activeTab =
    TABS.some((t) => t.id === tab) && puedeTab(tab)
      ? tab
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 sm:p-6 space-y-4 sm:space-y-6 text-slate-700">
        ...
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 tracking-tight">
              Conteo <span className="text-orange-600">Cíclico</span>
            </h1>
      ...
      {activeTab === 'contar' && (
        <TabContar sesionId={sesionId} setSesionId={setSesionId} sesiones={sesiones} />
      )}
      {activeTab === 'sesiones' && (
        <TabSesiones
          sesiones={sesiones}
...
function TabContar({ sesionId, setSesionId, sesiones }) {
  ...
  if (detalle) {
    if (serie) {
      const r = detalle.rows.find((x) => x.tipo === 'S' && x.valor === serie);
      esperado = r ? Number(r.disponible) : null;
    } else if (partida) {
      const r = detalle.rows.find((x) => x.tipo === 'P' && x.valor === partida);
      esperado = r ? Number(r.disponible) : null;
  ...
  const guardar = async () => {
    if (!activa) {
      toast.error('Elegí una sesión abierta');
      return;
    }
    if (!sku) {
  ...
  const scanSku = () =>
    startScan({
      onScan: (v) => {
        const val = v.trim().toUpperCase();
        toast.success(`SKU escaneado: ${val}`);
        buscarSku(val);
      },
      onError: (msg) => toast.error(msg)
    });
  const scanPartida = () =>
    startScan({
      onScan: (v) => {
        setPartida(v.trim());
        setSerie('');
        toast.success(`Partida escaneada: ${v.trim()}`);
      },
      onError: (msg) => toast.error(msg)
    });
  const scanSerie = () =>
    startScan({
      onScan: (v) => {
        setSerie(v.trim());
        setPartida('');
        toast.success(`Serie escaneada: ${v.trim()}`);
      },
      onError: (msg) => toast.error(msg)
    });
  const btnScan =
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 flex items-center gap-2 flex-wrap">
          ...
          <select
            value={sesionId}
            onChange={(e) => setSesionId(e.target.value)}
            className="px-3 py-2 rounded-xl border border-slate-200 text-sm font-bold flex-1 min-w-[12rem]"
          >
          ...
          <button
            onClick={nueva}
            className="px-3 py-2 rounded-xl bg-orange-600 text-white text-xs font-black flex items-center gap-1.5 hover:bg-orange-700"
          >
            <Plus size={14} /> Nueva
          </button>
        </div>
        ...
        {!activa && (
          <div className="rounded-xl bg-amber-50 border border-amber-100 px-4 py-3 text-sm text-amber-700">
            Elegí (o creá) una <b>sesión abierta</b> para agrupar los conteos.
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6 space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                onBlur={() => sku && !detalle && buscarSku(sku)}
          ...
          {detalle && (
            <div className="rounded-xl bg-slate-50 p-3 text-sm">
              {detalle.loading ? (
                <span className="text-slate-400">Buscando stock…</span>
              ) : (
                <>
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-slate-500">
                    <span>
                      Stock sistema: <b className="text-slate-700">{n(stockTotal)}</b>
                    </span>
                  ...
                  {detalle.rows.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {detalle.rows.map((r, i) => {
                        const on =
                          (r.tipo === 'P' && partida === r.valor) ||
                          (r.tipo === 'S' && serie === r.valor);
                        return (
                          <button
                            key={i}
                            type="button"
                          ...
                          >
                            {r.tipo === 'P' ? '' : '🔢 '}
                            {r.valor || '(sin partida)'} · {n(r.disponible)}
                          </button>
                        );
                      })}
                    </div>
          ...
          <div>
            ...
            <div className="flex items-stretch gap-2">
              <button
                type="button"
                onClick={() => step(-1)}
                className="w-14 rounded-xl border border-slate-200 text-2xl font-black text-slate-600 hover:bg-slate-50"
              >
                −
              </button>
              <input
                className={inp + ' flex-1 text-center text-2xl font-black'}
                inputMode="decimal"
                value={cantidad}
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={limpiar}
              className="flex-1 px-4 py-3 rounded-xl border border-slate-200 text-slate-600 font-black hover:bg-slate-50"
            >
              Limpiar
            </button>
            <button
              type="button"
              onClick={guardar}
              disabled={registrar.isPending || !activa}
              className="flex-[2] px-4 py-3 rounded-xl bg-orange-600 text-white font-black flex items-center justify-center gap-2 hover:bg-orange-700 disabled:opacity-50"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 h-fit">
        ...
        {recientes.length === 0 && (
          <div className="text-slate-400 text-sm">Aún no hay conteos en esta sesión.</div>
        )}
              ...
              <div key={r.id} className="flex items-center gap-2 py-2 text-sm">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate font-bold text-slate-700">
                    {r.codigo_producto}
                    {r.partida ? <span className="text-slate-400"> · {r.partida}</span> : ''}
                  </div>
                  <div className="truncate text-xs text-slate-400">
                    {r.ubicacion || 's/ubic'} · contado {n(r.cantidad_contada)} / sist{' '}
                    {n(r.cantidad_sistema)}
                  </div>
                </div>
                <button onClick={() => borrar(r.id)} className="text-slate-400 hover:text-rose-500">
                  <Trash2 size={13} />
                </button>
              </div>
            );
...
function TabSesiones({ sesiones, isAdmin, onOpen }) {
  ...
  const toggle = async (s) => {
    try {
      await cerrar.mutateAsync({ id: s.id, reabrir: s.estado !== 'abierta' });
      toast.success(s.estado === 'abierta' ? 'Sesión cerrada' : 'Sesión reabierta');
    } catch (e) {
      toast.error(e.message);
    }
  };
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-black text-slate-900 flex items-center gap-2">
          <Layers size={18} className="text-orange-500" /> Sesiones de conteo
        </h2>
        <button
          onClick={nueva}
          className="px-4 py-2 rounded-xl bg-orange-600 text-white text-sm font-black flex items-center gap-1.5 hover:bg-orange-700"
        >
          <Plus size={15} /> Nueva
        </button>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {sesiones.length === 0 && (
          <div className="text-slate-400 text-sm col-span-full">No hay sesiones todavía.</div>
        )}
        {sesiones.map((s) => (
          <div
            key={s.id}
            className={`rounded-2xl border p-4 flex flex-col gap-2 ${s.estado === 'abierta' ? 'border-orange-200 bg-orange-50/40' : 'border-slate-200 bg-slate-50'}`}
          >
            <div className="flex items-center justify-between gap-2">
              <span className="font-black text-slate-900 truncate">{s.nombre}</span>
              <span
                className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full border ${s.estado === 'abierta' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-slate-200 text-slate-500 border-slate-300'}`}
              >
                {s.estado}
              </span>
            </div>
            ...
            <div className="flex gap-2 mt-1">
              <button
                onClick={() => onOpen(s.id)}
                className="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-xs font-black hover:bg-white flex items-center justify-center gap-1"
              >
                <ClipboardCheck size={13} /> Reportes
              </button>
              {isAdmin && (
                <button
                  onClick={() => toggle(s)}
                  className="px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-xs font-black hover:bg-white flex items-center gap-1"
                >
                  {s.estado === 'abierta' ? (
                    <>
                      <Lock size={13} /> Cerrar
  ...
  return (
    <>
      <button
        ...
        disabled={!difs.length}
        title={
          difs.length
            ? `Generar correo con ${difs.length} diferencia(s)`
      ...
      {abierto && (
        <CorreoAjusteModal difs={difs} conPartida={conPartida} onClose={() => setAbierto(false)} />
      )}
    </>
  );
}
...
function CorreoAjusteModal({ difs, conPartida, onClose }) {
  ...
  const [obs, setObs] = useState('');
  useEffect(() => {
    localStorage.setItem('conteo_correo_dest', dest);
  }, [dest]);
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h3 className="font-black text-slate-800 flex items-center gap-2">
            <Mail size={17} className="text-orange-500" /> Correo de ajuste de inventario
          </h3>
        ...
        <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
          ...
          <div className="rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border-b border-slate-200">
              ...
              <span
                className={`text-xs font-black ${impacto < 0 ? 'text-rose-600' : 'text-emerald-600'}`}
              >
                Impacto: {money(impacto)}
              </span>
            </div>
              ...
              {difs.map((x) => {
                ...
                return (
                  ...
                  >
                    ...
                    {conPartida && x.partida && (
                      <span className="font-mono text-slate-400">{x.partida}</span>
                    )}
                    <span className="flex-1 text-slate-500 truncate">{x.descripcion}</span>
                    <span
                      className={`font-black ${x.diferencia < 0 ? 'text-rose-600' : 'text-amber-600'}`}
                    >
                      {x.diferencia > 0 ? '+' : ''}
                      {n(x.diferencia)}
                    </span>
                    <span
                      className={`px-1.5 py-0.5 rounded-md border text-[10px] font-bold ${chip(x.estado)}`}
                    >
                      {x.estado}
                    </span>
                  </label>
      ...
      sheets: [
        {
          name: 'Conciliación',
          rows: rows.map((x) => ({
            SKU: x.codigo_producto,
            Descripción: x.descripcion,
            UM: x.unidad_medida,
            Contado: x.contado,
            Sistema: x.sistema,
            Diferencia: x.diferencia,
            'Costo unit.': x.costo_unitario,
            'Impacto ($)': x.impacto,
            Estado: x.estado
          }))
        }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between p-4">
          <h2 className="font-black text-slate-900 flex items-center gap-2">
            <ClipboardCheck size={18} className="text-orange-500" /> Conciliación
          </h2>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl border border-slate-200 text-slate-600 text-xs font-black flex items-center gap-1.5 hover:bg-slate-50 disabled:opacity-40"
            >
...
function TabAjuste({ sesionId }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between p-4">
        <h2 className="font-black text-slate-900 flex items-center gap-2">
          <Download size={18} className="text-orange-500" /> Ajuste para el ERP (por SKU + partida)
        </h2>
        <div className="flex items-center gap-2">
...
function TabBloques() {
  ...
  const nuevo = async () => {
    ...
    try {
      const b = await crear.mutateAsync({ bodega, nombre });
      toast.success(`Bloque ${b.codigo} creado`);
      setAbierto(b.codigo);
    } catch (e) {
      toast.error(e.message);
    }
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6">
      <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
        <h2 className="font-black text-slate-900 flex items-center gap-2">
          <Boxes size={18} className="text-orange-500" /> Bloques / pallets
        </h2>
        <div className="flex items-center gap-2">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {bloques.map((b) => (
          <button
            key={b.id}
            onClick={() => setAbierto(b.codigo)}
            className="text-left rounded-2xl border border-slate-200 p-4 hover:border-orange-300 hover:bg-orange-50/30 transition-colors"
          >
        ...
        {!isLoading && bloques.length === 0 && (
          <div className="text-slate-400 text-sm col-span-full">Sin bloques.</div>
        )}
      </div>
...
function BloqueModal({ codigo, onClose }) {
  ...
  useEffect(() => {
    ...
    const origin = /localhost|capacitor/i.test(window.location.origin)
      ? 'https://cco-ptm.onrender.com'
  ...
  const add = async () => {
    ...
    try {
      ...
      setForm({ codigo: '', cantidad: '', partida: '', serie: '' });
  ...
  const auditarBloque = async () => {
    ...
    for (const it of b.items || []) {
      const c = window.prompt(
        `Contado para ${it.codigo_producto}${it.partida ? ' / ' + it.partida : ''} (esperado ${it.cantidad}):`,
        String(it.cantidad)
      );
      if (c === null) return;
      items.push({
        ...
        partida: it.partida,
        serie: it.serie,
        esperada: Number(it.cantidad) || 0,
        contada: Number(c) || 0
      });
    }
    try {
      const a = await auditar.mutateAsync({ bloqueId: b.id, codigoBloque: codigo, items });
      toast[a.estado === 'cuadrado' ? 'success' : 'info'](
        `Auditoría ${a.estado} (${a.items_dif} con diferencia)`
      );
    } catch (e) {
      toast.error(e.message);
    }
  ...
  const cerrarBloque = async () => {
    try {
      await editar.mutateAsync({ id: b.id, estado: b.estado === 'activo' ? 'cerrado' : 'activo' });
    } catch (e) {
      toast.error(e.message);
    }
  };
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        ...
        {isLoading ? (
          ...
          <div className="p-4 space-y-4">
            <div className="flex flex-wrap gap-4 items-center">
              {qr && (
                <img src={qr} alt="QR" className="w-32 h-32 rounded-lg border border-slate-200" />
              )}
              <div className="text-sm space-y-1">
                ...
                <a
                  href={`/inventory/bloque/${codigo}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-orange-600 font-bold flex items-center gap-1 text-xs"
                >
                  <ExternalLink size={12} /> Abrir página del bloque
                </a>
                <div className="flex gap-2 pt-1">
                  <button
                    onClick={cerrarBloque}
                    className="px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-black hover:bg-slate-50"
                  >
                    {b.estado === 'activo' ? 'Cerrar bloque' : 'Reabrir'}
                  </button>
                  <button
                    onClick={auditarBloque}
                    disabled={!b.items?.length}
                    className="px-3 py-1.5 rounded-lg bg-orange-600 text-white text-xs font-black disabled:opacity-40 flex items-center gap-1"
                  >
                    <ClipboardCheck size={13} /> Auditar
                  </button>
                </div>
              </div>
            </div>
            ...
            {b.estado === 'activo' && (
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 items-end bg-slate-50 rounded-xl p-3">
                <input
                  ...
                  placeholder="SKU"
                  className="px-2 py-1.5 rounded-lg border border-slate-200 text-sm col-span-2"
                />
                <input
                  value={form.partida}
                ...
                <button
                  onClick={add}
                  className="px-3 py-1.5 rounded-lg bg-slate-900 text-white text-sm font-black"
                >
                  + Ítem
                </button>
              </div>
            ...
            <div className="border border-slate-100 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(b.items || []).map((it) => (
                    <tr key={it.id} className="border-t border-slate-50">
                      ...
                      <td className="px-3 py-2 text-right">
                        {b.estado === 'activo' && (
                          <button
                            onClick={() => elimItem.mutate({ id: it.id, codigoBloque: codigo })}
                  ...
                  {(b.items || []).length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-3 py-4 text-slate-400 text-center text-xs">
                        Sin ítems.
                      </td>
                    </tr>
                  )}
              ...
              <div className="text-xs text-slate-500">
                ...
                {b.auditorias.map((a) => (
                  <div key={a.id} className="flex items-center gap-2 py-0.5">
                    <span
                      className={`px-2 py-0.5 rounded-full border text-[10px] font-black ${a.estado === 'cuadrado' ? chip('CUADRADO') : chip('FALTA')}`}
                    >
                      {a.estado}
                    </span>
                    <span>
                      {a.auditor_nombre} · {a.items_ok}/{a.items_total} OK · esperado{' '}
                      {n(a.esperado_total)} / contado {n(a.contado_total)}
                    </span>
                  </div>
                ))}
              </div>
...
function calcProy(p) {
  const oc = Number(p.cantOc) || 0,
    manual = Number(p.cantBx) || 0,
    cxb = Number(p.cantXBx) || 0;
  const pie = Number(p.pie) || 0,
    alt = Number(p.altura) || 0;
...
const toNums = (p) => ({
  prod: p.prod,
  cantOc: Number(p.cantOc) || 0,
  cantBx: Number(p.cantBx) || 0,
  cantXBx: Number(p.cantXBx) || 0,
  pie: Number(p.pie) || 0,
...
function TabProyeccion() {
  ...
  const set = (id, campo, valor) => {
    ...
    timers.current[id] = setTimeout(async () => {
      ...
      try {
        if (row) await guardar.mutateAsync({ id, ...toNums(row) });
      } catch (e) {
        if (vivo.current) toast.error(e.message || 'No se pudo guardar');
      } finally {
  ...
  const agregar = async () => {
    try {
      const nueva = await guardar.mutateAsync({
        id: null,
        prod: '',
        cantOc: 0,
        cantBx: 0,
  ...
  const duplicar = async (p) => {
    try {
      const nueva = await guardar.mutateAsync({ id: null, ...toNums(p) });
      setRows((rs) => [...rs, fromRow(nueva)]);
    } catch (e) {
      toast.error(e.message);
    }
  ...
  const totales = rows.reduce(
    (t, p) => {
      const c = calcProy(p);
      return {
        pallets: t.pallets + c.pallets,
        enteros: t.enteros + c.palletsEnteros,
        cajas: t.cajas + c.cajas
      };
    },
      ...
      sheets: [
        {
          ...
          rows: rows.map((p, i) => {
            const c = calcProy(p);
            return {
              Proyección: `Proyección ${i + 1}`,
              Producto: p.prod,
              'Cant. OC': Number(p.cantOc) || 0,
              'Cant. x caja': Number(p.cantXBx) || 0,
              Cajas: c.cajas,
              'Origen cajas': c.esManual ? 'manual' : 'auto',
              'Pie del pallet': Number(p.pie) || 0,
              Altura: Number(p.altura) || 0,
              'Cajas x pallet': c.bxPallet,
              'Pallets usados': Math.round(c.pallets * 100) / 100,
              'Pallets completos': c.palletsEnteros,
              'Distribución pareja': c.balanceado
                ? textoBalanceado(c.balanceado)
                : c.palletsEnteros
                  ? `${c.palletsEnteros} de ${c.bxPallet}`
                  : '',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-black text-slate-900 flex items-center gap-2">
            <Calculator size={18} className="text-orange-500" /> Proyección de palletizado
            {guardando && (
              <span className="text-[11px] font-bold text-orange-500">· guardando…</span>
            )}
          </h2>
          <button
            onClick={exportar}
        ...
        {!isLoading && rows.length === 0 && (
          <div className="text-slate-400 text-sm">
            Sin proyecciones. Agregá una para calcular pallets.
          </div>
        )}
          ...
          return (
            <div key={p.id} className="rounded-2xl border border-slate-200 p-4 space-y-3">
              ...
              <input
                ...
                value={p.prod}
                onChange={(e) => set(p.id, 'prod', e.target.value)}
              />
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
                <div>
                  ...
                  <input
                    ...
                    placeholder="20000"
                    value={p.cantOc}
                    onChange={(e) => set(p.id, 'cantOc', e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400">
                  ...
                  <input
                    className={inp}
                    inputMode="numeric"
                    placeholder={c.cajasAuto > 0 ? `auto: ${c.cajasAuto}` : 'auto'}
                    value={p.cantBx}
                    onChange={(e) => set(p.id, 'cantBx', e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400">
                  ...
                  <input
                    ...
                    placeholder="50"
                    value={p.cantXBx}
                    onChange={(e) => set(p.id, 'cantXBx', e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400">
                  ...
                  <input
                    ...
                    placeholder="6"
                    value={p.pie}
                    onChange={(e) => set(p.id, 'pie', e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400">
                  ...
                  <input
                    ...
                    placeholder="6"
                    value={p.altura}
                    onChange={(e) => set(p.id, 'altura', e.target.value)}
                  />
                </div>
              </div>
              ...
              <div
                className={
                  'rounded-xl p-3 text-sm ' + (ok ? 'bg-orange-50' : 'bg-slate-50 text-slate-400')
                }
              >
                {ok ? (
                  <div className="space-y-1">
                    <div className="flex flex-wrap gap-x-5 gap-y-1 text-slate-600">
                      <span>
                        📦 Cajas: <b className="text-slate-800">{n(c.cajas)}</b>
                        {c.esManual && (
                      ...
                      <span>
                        🧱 Cajas x pallet: <b className="text-slate-800">{n(c.bxPallet)}</b>
                      </span>
                      <span>
                        Unid. reales: <b className="text-slate-800">{n(c.unidadesReales)}</b>
                      </span>
                    </div>
                    <div className="text-base font-black text-orange-600">
                      🚛 {n(Math.round(c.pallets * 100) / 100)} pallets
                      <span className="ml-2 text-sm font-semibold text-slate-500">
                        → {n(c.palletsEnteros)} pallet{c.palletsEnteros === 1 ? '' : 's'}
                      </span>
                    </div>
                    {c.balanceado ? (
                      <div className="space-y-0.5 text-sm text-slate-600">
                        <div>
                          🤝 <b>Parejo:</b> {textoBalanceado(c.balanceado)}
                        </div>
                        <div className="text-slate-400">
                          📚 Llenado máximo: {n(c.palletsEnteros - 1)} de {n(c.bxPallet)} y el
                          último con {n(c.cajasUltimo)}
                        </div>
                      </div>
                    ) : (
                      c.palletsEnteros >= 1 && (
                        <div className="text-sm text-slate-500">
                          {c.palletsEnteros === 1
                            ? `Un solo pallet con ${n(c.cajas)} caja${c.cajas === 1 ? '' : 's'}`
                            : `Todos llenos: ${n(c.palletsEnteros)} pallets de ${n(c.bxPallet)} cajas justas`}
                        </div>
                      )
                    )}
                  </div>
        ...
        <button
          onClick={agregar}
          className="w-full px-3 py-2.5 rounded-xl bg-orange-600 text-white text-sm font-black flex items-center justify-center gap-1.5 hover:bg-orange-700"
        >
          <Plus size={15} /> Agregar proyección
        </button>

        <div className="rounded-xl bg-slate-50 border border-slate-100 p-3 text-sm text-slate-500">
          💡 <b>Cajas</b> = Cant. OC ÷ unid. por caja (redondeado hacia arriba), salvo que llenes{' '}
          <b>Cajas (manual)</b>, que manda sobre el cálculo — igual que CANT BX en tu planilla.{' '}
          <b>Cajas x pallet</b> = pie × altura. <b>Pallets</b> = cajas ÷ cajas por pallet. Las
          proyecciones se guardan en la base: todos ven las mismas.
        </div>
      </div>
    </div>
  );
...
function TablaReporte({ isLoading, rows, cols }) {
  ...
  if (!rows.length)
    return <div className="p-6 text-slate-400 text-sm">No hay conteos para mostrar.</div>;
  return (
        ...
        <thead className="bg-slate-50 text-[10px] uppercase text-slate-400">
          <tr>
            {cols.map(([k, label]) => (
              <th
                key={k}
                className={`px-3 py-2 ${['num', 'dif', 'money'].includes(cols.find((c) => c[0] === k)[2]) ? 'text-right' : 'text-left'}`}
              >
                {label}
              </th>
            ))}
              ...
              {cols.map(([k, , type]) => {
                ...
                if (type === 'estado')
                  return (
                    <td key={k} className="px-3 py-2">
                      <span
                        className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full border ${chip(v)}`}
                      >
                        {v}
                      </span>
                    </td>
                ...
                if (type === 'money')
                  return (
                    <td
                      key={k}
                      className={`px-3 py-2 text-right font-bold ${Number(v) < 0 ? 'text-rose-600' : 'text-slate-700'}`}
                    >
                      {money(v)}
                    </td>
                  );
                if (type === 'dif')
                  return (
                    <td
                      key={k}
                      className={`px-3 py-2 text-right font-black ${Number(v) < 0 ? 'text-rose-600' : Number(v) > 0 ? 'text-amber-600' : 'text-emerald-600'}`}
                    >
                      {Number(v) > 0 ? '+' : ''}
                      {n(v)}
                    </td>
                ...
                return (
                  <td key={k} className="px-3 py-2 text-slate-600 max-w-xs truncate">
                    {v}
                  </td>
                );
              })}
