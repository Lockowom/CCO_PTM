...
import {
  ...
  useCargarStock, parseStockFile, useEnviarEmil,
...
function TablaAnalisis({ filtro, titulo, alerta = false, conDiagnostico = false }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 space-y-3">
      ...
      <Tabla rows={rows} isLoading={isLoading} conDiagnostico={conDiagnostico} sel={sel} setSel={setSel} onProcesadas={setVis} />
    </div>
  );
}
...
function BarraSeleccion({ sel, setSel, rows }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap bg-orange-50 border border-orange-200 rounded-xl px-3 py-2">
      ...
      <button onClick={() => setModal('traspasos')}
...
function EnviarEmilModal({ tipo, rows, onClose }) {
  ...
  const submit = async () => {
    try {
      const res = await enviar.mutateAsync({ tipo, rows, accion, obs: esAjuste ? obs : '', cantidadDe });
      toast.success(`${n(res.agregados)} registro(s) enviados a ${esAjuste ? 'Ajustes' : 'Traspasos'}${res.omitidos ? ` · ${n(res.omitidos)} ya estaban en la lista` : ''}. Ábrelos en Inventario → Traspasos y Ajustes.`);
      onClose(true);
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4" onClick={() => onClose(false)}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="font-black text-slate-900 flex items-center gap-2">
            <Send size={16} className="text-orange-500" /> Generar {esAjuste ? 'Ajuste' : 'Traspaso'} ({n(rows.length)})
          </h3>
        ...
        <div className="max-h-36 overflow-y-auto rounded-xl border border-slate-100 divide-y divide-slate-50 text-xs">
          {rows.slice(0, 12).map((r) => (
            <div key={r.codigo} className="px-3 py-1.5 flex items-center justify-between gap-2">
              ...
              {r.ps_equivalente && <span className="text-indigo-600 font-bold whitespace-nowrap">→ {r.ps_equivalente}</span>}
            </div>
          ))}
          {rows.length > 12 && <div className="px-3 py-1.5 text-slate-400">… y {n(rows.length - 12)} más</div>}
        </div>
        ...
        <div className="space-y-2 text-xs">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <label className="inline-flex items-center gap-1.5 font-bold text-slate-600">
              <input type="radio" checked={cantidadDe === 'disponible'} onChange={() => setCantidadDe('disponible')} /> Disponible
            </label>
            <label className="inline-flex items-center gap-1.5 font-bold text-slate-600">
              <input type="radio" checked={cantidadDe === 'total'} onChange={() => setCantidadDe('total')} /> Stock Total
            </label>
          </div>
          {esAjuste && (
            <>
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <select value={accion} onChange={(e) => setAccion(e.target.value)} className="px-2 py-1.5 rounded-lg border border-slate-200 font-bold">
              ...
              {conEquiv > 0 && (
                <p className="text-indigo-600 font-bold">
                  {n(conEquiv)} tienen código P/S equivalente: van como recodificación (destino = código nuevo).
                </p>
              )}
            </>
          )}
          <p className="text-slate-400">Quedan como <b>PENDIENTE</b> en el módulo; ahí completas {esAjuste ? 'partidas y observaciones' : 'origen, destino y partidas'} antes de enviar el correo.</p>
        </div>

        <div className="flex items-center justify-between gap-2">
          <Link to="/inventory/traspasos" className="text-xs font-black text-slate-500 hover:text-orange-600 inline-flex items-center gap-1">
            <ExternalLink size={13} /> Abrir Traspasos y Ajustes
          </Link>
          <div className="flex gap-2">
            <button onClick={() => onClose(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-xs font-black text-slate-600 hover:bg-slate-50">Cancelar</button>
            <button onClick={submit} disabled={enviar.isPending}
              ...
              {enviar.isPending ? <RefreshCw size={13} className="animate-spin" /> : <Send size={13} />} Enviar
            </button>
          </div>
        </div>
      </div>
...
function Tabla({ rows, isLoading, conDiagnostico, sel, setSel, onProcesadas }) {
  ...
  const vis = useMemo(() => {
    ...
    else if (f.alerta === 'sin') out = out.filter((r) => !r.antiguo_disponible && !r.no_activo_stock && !r.anomalia);
    ...
    const { col, dir } = orden;
    return [...out].sort((a, b) => (COLS_NUM.has(col)
      ? (Number(a[col] || 0) - Number(b[col] || 0)) * dir
      : String(a[col] ?? '').localeCompare(String(b[col] ?? ''), 'es') * dir));
  }, [rows, f, orden]);

  useEffect(() => { onProcesadas?.(vis); }, [vis, onProcesadas]);

  const totales = useMemo(() => vis.reduce((t, r) => ({
    disponible: t.disponible + Number(r.disponible || 0), reserva: t.reserva + Number(r.reserva || 0),
    transitoria: t.transitoria + Number(r.transitoria || 0), consignacion: t.consignacion + Number(r.consignacion || 0),
    stock_total: t.stock_total + Number(r.stock_total || 0),
  }), { disponible: 0, reserva: 0, transitoria: 0, consignacion: 0, stock_total: 0 }), [vis]);
  ...
  const Sel = ({ valor, onChange, opts, label }) => (
    <select value={valor} onChange={(e) => { onChange(e.target.value); setLimite(200); }}
  ...
  const Th = ({ col, children, right }) => (
    <th className={`py-2 pr-2 cursor-pointer select-none whitespace-nowrap hover:text-slate-600 ${right ? 'text-right' : ''}`}
      onClick={() => setOrden((o) => ({ col, dir: o.col === col ? -o.dir : 1 }))}
      title="Ordenar">
      {children}{orden.col === col ? (orden.dir === 1 ? ' ▲' : ' ▼') : ''}
    </th>
  );
    ...
    <div className="space-y-2">
      {rows.length > 0 && (
        <div className="flex items-center gap-1.5 flex-wrap">
          <Sel valor={f.estado} onChange={(v) => setF((x) => ({ ...x, estado: v }))} label="Estado: todos" opts={opciones.estado.map((v) => [v, v])} />
          <Sel valor={f.activo} onChange={(v) => setF((x) => ({ ...x, activo: v }))} label="Activo: todos" opts={opciones.activo.map((v) => [v, v])} />
          <Sel valor={f.um} onChange={(v) => setF((x) => ({ ...x, um: v }))} label="UM: todas" opts={opciones.um.map((v) => [v, v])} />
          <Sel valor={f.dup} onChange={(v) => setF((x) => ({ ...x, dup: v }))} label="Duplicado: todos" opts={[['si', 'Con P/S equivalente'], ['no', 'Sin equivalente']]} />
          <Sel valor={f.alerta} onChange={(v) => setF((x) => ({ ...x, alerta: v }))} label="Alertas: todas"
            opts={[['antiguo_disp', '⚠ Antiguo c/Disponible'], ['no_activo_stock', '⚠ No activo c/Stock'], ['anomalia', '⚠ Anomalía'], ['sin', 'Sin alertas']]} />
          <Sel valor={f.stock} onChange={(v) => setF((x) => ({ ...x, stock: v }))} label="Stock: todo"
            opts={[['disp', 'Con Disponible > 0'], ['con', 'Con Stock Total > 0'], ['sin', 'Sin stock']]} />
          {hayFiltro && (
            <button onClick={() => { setF(FILTROS_INICIALES); setLimite(200); }}
      ...
      <div className="overflow-x-auto">
        ...
        {!isLoading && rows.length > 0 && !vis.length && <div className="text-slate-400 text-center py-10">Ningún registro pasa los filtros aplicados.</div>}
        {!isLoading && vis.length > 0 && (
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-slate-400 uppercase tracking-wide border-b border-slate-100">
                <th className="py-2 pr-2 w-6"><input type="checkbox" checked={todasSel} onChange={toggleTodas} title="Seleccionar todo (lo filtrado)" /></th>
                <Th col="codigo">Código</Th>
                <Th col="producto">Producto</Th>
                <Th col="unidad_medida">UM</Th>
                <Th col="disponible" right>Disp.</Th>
                <Th col="reserva" right>Res.</Th>
                <Th col="transitoria" right>Trans.</Th>
                <Th col="consignacion" right>Consig.</Th>
                <Th col="stock_total" right>Total</Th>
            ...
            <tbody>
              {visibles.map((r) => (
                <tr key={r.codigo} className={`border-b border-slate-50 hover:bg-slate-50/60 ${sel.has(r.codigo) ? 'bg-orange-50/60' : ''}`}>
                  <td className="py-1.5 pr-2"><input type="checkbox" checked={sel.has(r.codigo)} onChange={() => toggle(r.codigo)} /></td>
...
function TabDetalle() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 space-y-3">
      ...
      <BarraSeleccion sel={sel} setSel={setSel} rows={rows} />
      <Tabla rows={rows} isLoading={isLoading} conDiagnostico={false} sel={sel} setSel={setSel} onProcesadas={setVis} />
    </div>
  );
}
