  ...
  const buscar = async (termRaw) => {
    ...
    try {
      ...
      (data || []).forEach((r) => {
        ...
        if (r.ubicacion)
          g.ubicaciones.push({ ubicacion: r.ubicacion, cantidad: Number(r.cantidad) || 0 });
  ...
  return (
    <div className="anim-slide-in min-h-dvh bg-black text-white flex flex-col font-mono">
        ...
        <div className="flex gap-2">
          ...
          {isSupportedDevice && (
            <button
              ...
              onClick={escanear}
              disabled={isScanning}
              className="px-4 min-w-[44px] bg-blue-600 active:bg-blue-700 text-white rounded-lg flex items-center justify-center disabled:opacity-50"
              aria-label="Escanear"
            >
              <Camera size={20} />
            </button>
      ...
      >
        ...
        {grupos?.map((g) => (
          <div
            key={g.codigo || g.descripcion}
            className="anim-fade-up bg-slate-900 border border-slate-800 rounded-xl overflow-hidden"
          >
