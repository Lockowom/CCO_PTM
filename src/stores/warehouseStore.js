...
export const useWarehouseStore = create((set, get) => ({
  ...
  toggleHeatmap: () =>
    set((state) => ({ ui: { ...state.ui, heatmapMode: !state.ui.heatmapMode } })),
  setCamera: (camera) =>
    set((state) => ({ ui: { ...state.ui, camera: { ...state.ui.camera, ...camera } } })),
  ...
  fetchWarehouseData: async (force = false) => {
    ...
    if (
      !force &&
      get().loaded &&
      Object.keys(get().inventory).length > 0 &&
      Date.now() - get().lastLoadedAt < CACHE_TTL
    )
      return;
      ...
      try {
        ...
        (layoutRows || []).forEach((row) => {
          ...
          layoutMap[normUbic] = {
            ...row,
            ubicacion: normUbic,
            cantidad: (inventoryMap[normUbic] || []).reduce(
  ...
  subscribeToRealtime: () => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'wms_ubicaciones' },
        debouncedFetch
      )
