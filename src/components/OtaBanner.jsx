...
export default function OtaBanner() {
  ...
  if (!req) return null;
  const actualizar = async () => { setBusy(true); const r = await buscarActualizacion(); setBusy(false); if (r.estado === 'error') toast.error('No se pudo actualizar'); };
  ...
  return (
    <div className="fixed top-0 inset-x-0 z-[200] bg-amber-500 text-white px-4 py-2 flex items-center justify-center gap-3 text-[13px] font-bold shadow-lg">
      ...
      <span className="truncate">{req.msg || `Actualización requerida (mín. ${req.min}). Tu versión: ${req.cur}.`}</span>
      <button onClick={actualizar} disabled={busy} className="ml-1 inline-flex items-center gap-1.5 bg-white/20 hover:bg-white/30 rounded-lg px-2.5 py-1 shrink-0">
        {busy ? <RefreshCw size={13} className="animate-spin" /> : <DownloadCloud size={13} />} Actualizar
      </button>
    </div>
  );
}
