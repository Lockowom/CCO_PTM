  ...
  <span className="ti-signature font-black tracking-tight text-2xl sm:text-3xl">
    {text.split('').map((ch, i) =>
      ch === ' ' ? (
        ...
        <span key={i} className="ti-letter" style={{ animationDelay: `${i * 0.06}s` }}>
          {ch}
        </span>
      )
    )}
...
const StockFreshness = ({ table, label }) => {
  ...
  const canImport =
    user?.rol === 'ADMIN' || user?.es_admin_delegado || hasPermission('manage_data_import');
  ...
  const info = useMemo(() => {
    ...
    return { rec, lastUpdate, isStale, aplica24h };
  }, [latest, table]);

  const { rec, lastUpdate, isStale, aplica24h } = info;
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-8 mt-4 sm:mt-6 space-y-4">
      ...
      >
        <div
          className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-3xl opacity-40 ${isStale ? 'bg-red-300' : 'bg-emerald-300'}`}
        />
            ...
            {lastUpdate ? (
              <div className="flex flex-col sm:flex-row sm:items-center gap-x-6 gap-y-1">
                <div className="flex items-center gap-2 text-slate-800">
                  ...
                  <span className="font-bold capitalize text-sm sm:text-base truncate">
                    {fmtDate(lastUpdate)}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-slate-700">
                  <Clock size={15} className="text-slate-400 shrink-0" />
                  <span className="font-extrabold tabular-nums text-sm sm:text-base">
                    {fmtTime(lastUpdate)}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-slate-700">
                  ...
                  <span className="font-bold text-sm sm:text-base truncate">
                    {rec.usuario_nombre || 'Desconocido'}
                  </span>
                </div>
              </div>
            ) : (
              <p className="text-slate-500 font-semibold text-sm">
                Aún no hay registros de carga para esta tabla.
              </p>
            )}

            {aplica24h && (
              <p className="mt-1.5 text-[11px] sm:text-xs font-semibold text-slate-400 flex items-center gap-1.5">
                <Clock size={12} /> Vigencia: ciclo diario de{' '}
                <span className="font-black text-slate-500">
                  {RESET_LABEL} a {RESET_LABEL} hrs
                </span>
              </p>
            )}
          </div>
        ...
        <div className="ti-banner-glow relative overflow-hidden rounded-2xl sm:rounded-3xl border-2 border-red-300 bg-white">
          ...
          <div className="p-5 sm:p-8 text-center">
            ...
            <p className="text-slate-600 font-semibold max-w-xl mx-auto leading-relaxed">
              ...
              <span className="font-black text-slate-800">24 horas</span>, con corte diario a las{' '}
              <span className="font-black text-slate-800">{RESET_LABEL} hrs</span>.
            </p>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col items-center">
              <span className="text-[11px] font-bold uppercase tracking-[0.3em] text-slate-400 mb-0.5">
                Atte
              </span>
