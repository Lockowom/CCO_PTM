      ...
      >
        ...
        <div className="relative bg-gradient-to-br from-orange-500 to-amber-500 text-white px-6 pt-6 pb-5">
          ...
          <p className="text-white/80 text-sm mt-0.5">
            Versión {notas[0]?.version} · esto es lo nuevo
          </p>
        </div>
                ...
                {n.cambios.map((c, i) => {
                  ...
                  return (
                    <li key={i} className="flex items-start gap-2">
                      {meta ? (
                        <span
                          className={`shrink-0 inline-flex items-center gap-1 text-[9px] font-black uppercase px-1.5 py-0.5 rounded-md border ${meta.cls} mt-0.5`}
                        >
        ...
        <div className="px-5 py-4 border-t border-slate-100 space-y-3">
          {(() => {
            const presentes = [
              ...new Set(
                notas
                  .flatMap((n) => n.cambios.map((c) => (typeof c === 'string' ? null : c.tipo)))
                  .filter(Boolean)
              )
            ];
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 justify-center">
                {presentes.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`inline-flex items-center justify-center w-4 h-4 rounded border ${m.cls}`}
                      >
                        <Icon size={9} />
                      </span>
                      {m.label}
          ...
          <button
            onClick={cerrar}
            className="w-full py-3 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-black text-sm transition-colors"
          >
            ¡Entendido!
          </button>
        </div>
