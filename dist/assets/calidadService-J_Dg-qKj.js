import { d as f, u, c as m } from './query-vendor-BNjBrM5A.js';
import { s as r, u as v, L as w, w as A } from './index-Pwuftu2X.js';
const C = () => {
    var e;
...
function R(e = {}) {
  return {
    ...e,
    fecha: h(e.fecha),
    periodo_desde: h(e.periodo_desde),
    periodo_hasta: h(e.periodo_hasta)
  };
}
function D(e) {
...
async function $(e, o = !1) {
  ...
  try {
    ...
    throw (i == null ? void 0 : i.name) === 'AbortError' || a.signal.aborted
      ? new Error(
  ...
  try {
    const { data: i, error: n } = await r
      .rpc('calidad_salida_candidatos', {
        p_query: e || '',
        p_limit: Math.min(Math.max(Number(o) || 100, 1), 200)
      })
      .abortSignal(a.signal);
    if (n) throw n;
    return i || [];
...
async function Y(e, o = '') {
  const { data: a, error: t } = await r.rpc('calidad_lotes_series', {
    p_codigo: e,
    p_query: o || '',
    p_limit: 300
  });
...
async function T({ title: e, body: o, payload: a }) {
  try {
...
function X({ codigo: e, ubicacion: o, estadoLabel: a, tipo: t }) {
  return T({
    title: `⚠️ Calidad: ${a}`,
    body: `${e}${o ? ` en ${o}` : ''} dictaminado como ${a}. Requiere movimiento/gestión por Inventario.`,
    payload: { codigo_producto: e, ubicacion: o || '', tipo: t }
  });
}
async function Z(e) {
...
function ee() {
  const e = u();
  return m({
    mutationFn: async ({ cabecera: o, items: a, asignacionId: t = null }) => {
      const i = t ? 'crear_informe_monitoreo_asignacion' : 'crear_informe_monitoreo',
        n = { ...(t ? { p_asignacion_id: t } : {}), p_cabecera: R(o), p_items: a },
        { data: c, error: d } = await r.rpc(i, n);
      if (d) throw N(d);
      return c;
    },
...
function ae() {
  ...
  return m({
    mutationFn: async ({ informeId: o, estado: a }) => {
      const { error: t } = await r.from('tms_monitoreo_informes').update({ estado: a }).eq('id', o);
      if (t) throw t;
    },
    onSuccess: () => e.invalidateQueries({ queryKey: ['monitoreo_informes'] })
  });
...
function oe() {
  const e = u();
  return m({
    mutationFn: async ({ informeId: o, cabecera: a, items: t }) => {
      const { data: i, error: n } = await r.rpc('actualizar_informe_monitoreo', {
        p_informe_id: o,
        p_cabecera: R(a),
        p_items: t
      });
...
function re() {
  const e = u();
  return m({
    mutationFn: async ({ informeId: o, cabecera: a, reporte: t, hallazgos: i }) => {
      let n = o;
      if (n) {
        const { error: s } = await r
          .from('tms_monitoreo_informes')
          .update({ ...a, reporte: t, total_items: i.length })
          .eq('id', n);
        if (s) throw s;
      } else {
        ...
        const { data: E, error: b } = await r
          .from('tms_monitoreo_informes')
          .insert({ ...a, numero: s, tipo_informe: 'DANOS', reporte: t, total_items: i.length })
          .select()
          .single();
        if (b) throw b;
        n = E.id;
      ...
      for (const s of i) {
        ...
        if (s.id && d.has(s.id)) {
          ...
          const { data: E, error: b } = await r
            ...
            .single();
          if (b) throw b;
          (l.add(E.id), _.push({ ...s, id: E.id }));
        }
      }
      const y = [...d].filter((s) => !l.has(s));
      if (y.length) {
...
async function se({ informeId: e, itemId: o, blob: a, descripcion: t, user: i }) {
  ...
  const c = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    ...
    l = `${e}/${o || 'general'}/${C()}.${d}`,
    { error: _ } = await r.storage.from(g).upload(l, a, { contentType: c, upsert: !1 });
  if (_) throw _;
  const { data: y } = r.storage.from(g).getPublicUrl(l),
    { data: s, error: p } = await r
      ...
      .insert({
        ...
        creado_nombre: (i == null ? void 0 : i.nombre) || null
      })
      .select()
      .single();
  if (p) throw (await r.storage.from(g).remove([l]), p);
  return s;
}
async function ce(e) {
...
function de() {
  ...
  return m({
    mutationFn: async ({
      ...
      const { data: d, error: l } = await r.rpc('monitoreo_dictaminar', {
        p_item_id: o,
        p_dictamen: a,
        p_bodega_destino: t || null,
        p_accion: i || null,
  ...
  return f({
    ...
    queryFn: async () => {
      try {
        ...
        if (a) throw a;
        const t = { PENDIENTE: 0, EN_PROCESO: 1, NO_CONFORME: 2, CONFORME: 3 };
        return (o || []).sort((i, n) => (t[i.estado] ?? 9) - (t[n.estado] ?? 9));
      } catch (o) {
        if (D(o))
          return (
            w.warn(o, {
...
function Ee() {
  ...
  return m({
    mutationFn: async (o) => {
      const { data: a, error: t } = await r.rpc('firmar_certificado', { p_tarea_id: o });
      if (t) throw t;
      return a;
    },
...
function L() {
  ...
  return f({
    ...
    queryFn: async () => {
      ...
      if (a) throw a;
      const t = { PENDIENTE: 0, EN_PROCESO: 1, RESUELTA: 2, ANULADA: 3 };
      return (o || []).sort((i, n) => (t[i.estado] ?? 9) - (t[n.estado] ?? 9));
    }
  });
}
...
function ye() {
  const e = u();
  return m({
    mutationFn: async ({ skus: o, motivo: a, prioridad: t }) => {
      const { data: i, error: n } = await r.rpc('crear_asignacion_calidad', {
        p_skus: o || [],
        p_motivo: a ?? null,
        p_prioridad: t || 'NORMAL'
      });
      if (n) throw n;
      return i;
    },
    ...
    if (typeof a == 'string')
      try {
        ...
        if (t != null && t.message) {
          ...
          return (
            (i.code = t.code || (e == null ? void 0 : e.code)),
            (i.status = Number((e == null ? void 0 : e.status) || 409)),
            (i.lockedByName = t.locked_by_name || null),
            (i.lockedAt = t.locked_at || null),
            i
          );
        }
      } catch {}
  return e instanceof Error ? e : new Error('No se pudo procesar la tarea de Calidad');
}
async function Oe(e) {
  const { data: o, error: a } = await r.rpc('tomar_asignacion_calidad', { p_asignacion_id: e });
  if (a) throw N(a);
  return o;
}
async function Ie(e, o) {
  const { data: a, error: t } = await r.rpc('guardar_progreso_asignacion_calidad', {
    p_asignacion_id: e,
    p_progress_data: o || {}
  });
  if (t) throw N(t);
...
async function Se(e) {
  if (!e) return null;
  const { data: o, error: a } = await r.rpc('liberar_asignacion_calidad', { p_asignacion_id: e });
  if (a) throw N(a);
  return o;
}
function Ne() {
  ...
  return m({
    mutationFn: async (o) => {
      const { data: a, error: t } = await r.rpc('anular_asignacion_calidad', {
        p_asignacion_id: o
      });
      if (t) throw t;
      return a;
...
const M = [
  ...
  ve = ['Documento', 'Conteo', 'Inspección visual', 'Medición', 'Registro fotográfico', 'Sistema'],
  F = {
    BAJO: {
      emoji: '🟢',
      label: 'RIESGO BAJO',
...
function qe(e) {
  ...
  let t = 0;
  (a.pallet === 'Malo' ? (t += 2) : a.pallet === 'Regular' && (t += 1),
    a.film === 'Incorrecto' && (t += 1),
    ['golpes', 'deformada', 'humedad'].forEach((n) => {
      a[n] === 'Sí' && (t += 2);
    }),
    Object.entries(e || {}).forEach(([n, c]) => {
      n !== '_extras' && (c == null ? void 0 : c.estado) === 'NO' && (t += 2);
    }));
  const i = t >= 4 ? 'ALTO' : t >= 1 ? 'MEDIO' : 'BAJO';
  return { key: i, score: t, ...F[i] };
}
function Pe(e) {
  ...
  let a = 0,
    t = 0,
    i = 0;
  Object.entries(o).forEach(([l, _]) => {
    l !== '_extras' &&
      ((_ == null ? void 0 : _.estado) === 'OK'
        ? (a += 1)
        : (_ == null ? void 0 : _.estado) === 'NO'
          ? (t += 1)
          : (_ == null ? void 0 : _.estado) === 'NA' && (i += 1));
  });
  const n = a + t,
    c = e != null && e.created_at ? new Date(e.created_at) : null,
    d = e != null && e.completado_en ? new Date(e.completado_en) : null;
  return {
    items: a + t + i,
    ok: a,
    no: t,
    ...
    minutos: c && d ? Math.max(0, Math.round((d - c) / 6e4)) : null,
...
const Le = [
  ...
  Fe = (e, o) => {
    const a = Number(String(e ?? '').replace(',', '.')),
      t = Number(String(o ?? '').replace(',', '.'));
    return !Number.isFinite(a) || !Number.isFinite(t) || a <= 0
      ? null
      : Math.abs(t - a) / a <= x
        ? 'CONFORME'
        : 'REVISAR';
...
async function Be({ tareaId: e, tipo: o, blob: a }) {
  if ((a == null ? void 0 : a.size) > 7864320)
    throw new Error('La foto pesa demasiado y no se pudo comprimir. Prueba con otra (JPG/PNG).');
  const i = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    n =
      {
        'image/jpeg': 'jpg',
        'image/png': 'png',
    ...
    { error: d } = await r.storage.from(g).upload(c, a, { contentType: i, upsert: !1 });
  if (d) throw d;
  return c;
}
async function Ke(e) {
...
async function ke({ tareaId: e, tipo: o, blob: a }) {
  if ((a == null ? void 0 : a.size) > 7864320)
    throw new Error('La foto pesa demasiado y no se pudo comprimir. Prueba con otra (JPG/PNG).');
  const i = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    n =
...
function je() {
  ...
  return m({
    mutationFn: async ({
      ...
      const { data: l, error: _ } = await r.rpc('crear_tarea_salida_manual', {
        p_nv: o,
        p_skus: a || [],
        p_cliente: t ?? null,
        p_guia: i ?? null,
...
function Qe() {
  ...
  return m({
    mutationFn: async (o) => {
      const { data: a, error: t } = await r.rpc('eliminar_tarea_calidad', { p_tarea_id: o });
      if (t) throw t;
      return a;
    },
    onSuccess: () => {
...
function Ve() {
  ...
  return m({
    mutationFn: async (o) => {
      const { data: a, error: t } = await r.rpc('eliminar_asignacion_calidad', {
        p_asignacion_id: o
      });
      if (t) throw t;
      return a;
...
function He() {
  const e = u();
  return m({
    mutationFn: async ({ codigo: o, nombre: a, estado: t, esDestino: i, activo: n, orden: c }) => {
      const { data: d, error: l } = await r.rpc('guardar_bodega_softland', {
        p_codigo: o,
        p_nombre: a,
        p_estado: t || 'DISPONIBLE',
        p_es_destino: !!i,
        p_activo: n !== !1,
        p_orden: c ?? 100
      });
...
function Ye() {
  ...
  return m({
    mutationFn: async (o) => {
      const { data: a, error: t } = await r.rpc('eliminar_bodega_softland', { p_codigo: o });
      if (t) throw t;
      return a;
    },
    onSuccess: () => e.invalidateQueries({ queryKey: ['bodegas_softland'] })
...
function Xe(e, o, a) {
  return f({
    ...
    staleTime: 3e4,
    queryFn: async () => {
      const { data: t, error: i } = await r.rpc('trazabilidad_producto', {
        p_codigo: e,
        p_partida: o ?? null,
        p_ubicacion: a ?? null
      });
      if (i) throw i;
      return t || { codigo: e, estado_actual: null, eventos: [] };
    }
  });
}
function Ze() {
...
function ea() {
  return f({
    ...
    queryFn: async () => {
      ...
      return (e || []).sort((t, i) => (a[t.estado] ?? 9) - (a[i.estado] ?? 9));
    }
  });
}
function aa() {
  ...
  return m({
    mutationFn: async ({
      ...
      const { data: d, error: l } = await r.rpc('crear_accion_calidad', {
        p_item_id: o,
        p_tipo_accion: a,
        p_area: t,
        p_descripcion: i ?? null,
...
function oa() {
  const e = u();
  return m({
    mutationFn: async ({ accionId: o, resolucion: a, estado: t }) => {
      const { data: i, error: n } = await r.rpc('resolver_accion_calidad', {
        p_accion_id: o,
        p_resolucion: a ?? '',
        p_estado: t || 'RESUELTA'
      });
...
function ta() {
  ...
  return m({
    mutationFn: async (o) => {
      const { data: a, error: t } = await r.rpc('anular_accion_calidad', { p_accion_id: o });
      if (t) throw t;
      return a;
    },
    onSuccess: () => e.invalidateQueries({ queryKey: ['calidad_acciones'] })
...
function ia() {
  ...
  return m({
    mutationFn: async (o) => {
      const { data: a, error: t } = await r.rpc('accion_a_ticket_pv', { p_accion_id: o });
      if (t) throw t;
      return a;
    },
    onSuccess: () => {
...
function ra() {
  ...
  return m({
    mutationFn: async ({ accionId: o, referencia: a }) => {
      const { data: t, error: i } = await r.rpc('accion_correo_enviado', {
        p_accion_id: o,
        p_referencia: a || null
      });
      if (i) throw i;
...
function na() {
  ...
  return m({
    mutationFn: async ({ accionId: o, referencia: a }) => {
      const { data: t, error: i } = await r.rpc('accion_registrar_referencia', {
        p_accion_id: o,
        p_referencia: a
      });
      if (i) throw i;
...
function sa() {
  ...
  return m({
    mutationFn: async ({
      ...
      const { data: d, error: l } = await r.rpc('guardar_checklist_ingreso', {
        p_tarea_id: o,
        p_checklist: a || {},
        p_observaciones: t ?? null,
        p_finalizar: !!i,
        p_resultado: n ?? null,
