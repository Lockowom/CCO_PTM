...
import { r as n, u as Z, b as C } from './react-vendor-CByR7_Pi.js';
import { a as J, b as i } from './dashData-ragXIUCX.js';
import { L as Q, s as G } from './index-BGSkqVb2.js';
      ...
      const b = performance.now(),
        u = (N) => {
          const j = N - b,
            c = Math.min(j / r, 1),
            p = f + o * X(c);
          (y(p.toFixed(x)), c < 1 ? (a.current = requestAnimationFrame(u)) : (g.current = s));
        };
      return ((a.current = requestAnimationFrame(u)), () => cancelAnimationFrame(a.current));
    }, [s, r, x]),
    e.jsxs('span', { className: E, children: [h, w, m] })
  );
}
const L = 3e4,
  Y = 8e3,
  ...
  V = { [i.EN_PROCESO]: '⚙', [i.SHIPPING]: '📋', [i.EN_RUTA]: '🛣', [i.ENTREGADO]: '✓' };
function I(s) {
...
function te({ n: s, estado: r }) {
  const x = M[r] || '#6b7280',
    h = () => {
      ...
      if (r === i.SHIPPING || r === i.EN_PROCESO) {
        ...
        return e.jsxs('div', {
          ...
          children: [
            r === i.SHIPPING &&
              ...
              e.jsxs('span', {
                className:
                  'rounded bg-violet-500/20 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-violet-300',
                children: [
                  '⏸',
                  ' ',
                  s.shippingSubestado === 'REZAGADA_COMERCIAL'
                    ? 'Rezagada comercial'
                    : 'Retiro cliente'
                ]
              }),
            ...
            e.jsxs('div', {
              className: 'flex items-center gap-1.5',
              children: [
                s.diasDesdeAprobacion !== null ? I(s.diasDesdeAprobacion) : I(s.diasEnEstado),
                s.fecha_compromiso &&
                  e.jsxs('span', {
                    className: 'text-gray-500 text-[10px]',
...
function oe() {
  const s = Z(),
    ...
    N = n.useRef(0),
    j = n.useCallback((t) => {
      f((l) => (l === t ? (b(!0), null) : (b(!1), (u.current = Date.now() + ee), t)));
    }, []),
  ...
  (n.useEffect(() => {
    c();
    const t = setInterval(c, L);
    return () => clearInterval(t);
  }, [c]),
    n.useEffect(() => {
      let t;
      const l = G.channel('panel_tv_rt')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'tms_operaciones' }, () => {
          (clearTimeout(t), (t = setTimeout(c, 1200)));
        })
        .on(
          'postgres_changes',
        ...
        .subscribe();
      return () => {
        (clearTimeout(t), G.removeChannel(l));
      };
    }, [c]),
    n.useEffect(() => {
      ...
      const t = setInterval(() => {
        ...
        const l = r.estados.map((d) => d.estado);
        l.length !== 0 && ((N.current = (N.current + 1) % l.length), f(l[N.current]));
      }, Y);
      return () => clearInterval(t);
    }, [r, o]));
  const p = (r == null ? void 0 : r.estados) ?? [],
    O = (r == null ? void 0 : r.urgentes) ?? [],
    P = n.useMemo(() => Math.max(...p.map((t) => t.cantidad), 1), [p]),
    S = n.useMemo(() => {
      var t;
      return a === 'URGENTES'
        ? O
        : a
          ? ((t = p.find((l) => l.estado === a)) == null ? void 0 : t.nvs) || []
          : [];
    }, [a, p, O]),
    [R, F] = n.useState({ s: 1, left: 0, top: 0 });
  n.useEffect(() => {
    const t = () => {
      const l = window.innerWidth,
        d = window.innerHeight,
        v = Math.min(l / 1920, d / 1080);
      F({ s: v, left: (l - 1920 * v) / 2, top: (d - 1080 * v) / 2 });
    };
    return (
      t(),
          ...
          e.jsxs('div', {
            ...
            children: [
              ...
              e.jsxs('p', {
                ...
                children: ['Reintentando cada ', L / 1e3, 's…']
              })
            ]
          })
        ]
  ...
  const { estados: _, total: k, urgentes: A } = r,
    U = _.length || 1,
    H = a === 'URGENTES' ? '#ef4444' : (a && M[a]) || '#6b7280',
                ...
                e.jsxs('div', {
                  ...
                  children: [
                    e.jsxs('button', {
                      ...
                      className: `flex items-center gap-3 rounded-xl px-4 py-2.5 transition-all ${A.length > 0 ? 'bg-red-500/15 border-glow-red' : 'bg-gray-800/40 border-2 border-gray-700/50'} ${a === 'URGENTES' ? 'ring-2 ring-red-400' : ''}`,
                      children: [
                        ...
                        e.jsxs('div', {
                          ...
                          children: [
                            e.jsx(D, {
                              ...
                              className: `text-3xl font-black tabular-nums leading-none ${A.length > 0 ? 'text-red-400' : 'text-gray-500'}`
                            }),
                            e.jsx('p', {
            ...
            e.jsxs('div', {
              ...
              children: [
                e.jsxs('div', {
                  ...
                  style: { gap: `${Math.min(14, Math.max(6, Math.floor(70 / U)))}px` },
                  children: [
                    _.map((t) => {
                      const l = P > 0 ? (t.cantidad / P) * 100 : 0,
                        d = M[t.estado] || '#6b7280',
                        v = V[t.estado] || '●',
                        q = k > 0 ? ((t.cantidad / k) * 100).toFixed(0) : '0',
                        $ = t.nvs.filter((W) => W.urgente).length,
                        B = a === t.estado,
                        K = Math.min(60, Math.max(36, Math.floor(520 / U)));
                      return e.jsxs(
                        'button',
                        {
                          onClick: () => j(a === t.estado ? null : t.estado),
                          className: `flex items-center gap-4 text-left rounded-xl transition-all shrink-0 px-3 py-1 ${B ? 'bg-white/[0.06] ring-2 ring-white/20 scale-[1.01]' : 'hover:bg-white/[0.03]'}`,
                          children: [
                            e.jsxs('div', {
                            ...
                            : S.map((t, l) =>
                                e.jsx(
                                  'div',
                                  {
                                    ...
                                    style: { animationDelay: `${Math.min(l * 40, 400)}ms` },
