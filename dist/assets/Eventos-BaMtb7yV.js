...
function Te() {
  const { hasPermission: a, user: i } = ce(),
    ...
    p = r.useCallback(async () => {
      const [s, l, o, J] = await Promise.all([xe({ agregado: k }), me(), pe({}), he()]);
      (n(s), R(l), O(o), y(J));
    }, [k]);
  r.useEffect(() => {
    p();
  ...
  const _ = async (s, l) => {
    ...
    G = [
      ['mis', `Mis notificaciones${h.length ? ` (${h.length})` : ''}`, W],
      ['stream', 'Stream de eventos', q],
      ['metricas', 'Métricas / SLA', Q],
      ['reglas', `Reglas (${w.length})`, U],
      ['bandeja', 'Bandeja de salida', Y]
    ],
    [f, K] = r.useState('OT'),
    [d, H] = r.useState(null),
    [X, P] = r.useState(!1);
  ...
  return e.jsxs('div', {
    ...
    children: [
                                      ...
                                      children: [
                                        ...
                                        e.jsx('div', {
                                          ...
                                          children: e.jsx('div', {
                                            ...
                                            style: {
                                              width: `${Math.round(((Number(s.dwell_prom_horas) || 0) / Z) * 100)}%`,
                ...
                m &&
                  e.jsxs('button', {
                    ...
                    children: [
                      e.jsx(ie, { size: 13 }),
                      ' ',
                      $ ? 'Enviando…' : `Enviar push pendientes${C ? ` (${C})` : ''}`
                    ]
                  })
              ]
            }),
      ...
      T && e.jsx(we, { data: T, onClose: () => N(null), onSave: I })
    ]
  });
}
