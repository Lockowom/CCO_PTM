import { u as Le, d as ze, c as Fe, j as e } from './query-vendor-CzTZLhyg.js';
import { r as p } from './react-vendor-CByR7_Pi.js';
import { u as Me, s as k } from './index-DpKQy1E-.js';
import { u as $e } from './useRealtimeTable-7JQgQ4NY.js';
import { u as Ue } from './useBarcodeScanner-BNkS6wI8.js';
import { u as Be, g as Ge } from './animation-vendor-DqxLxWcj.js';
import { l as Ve } from './logUpload-DyKNEMYe.js';
import {
  h as ae,
  aq as qe,
  P as ue,
...
import { utils as D, writeFile as we } from './xlsx-B2eTCt_Q.js';
import {
  R as ve,
  B as st,
  C as at,
...
const it = ['3-4', '1HCX20', '1HCX40', '2HCX40', 'LCL', 'AEREO'],
  Ce = {
    EN_REVISION: {
      label: 'En Revisión',
      color: 'bg-amber-500',
...
async function xt(o, R) {
  for (let u = 0; u < o.length; u += oe) {
    const w = o
        .slice(u, u + oe)
        .map((i) => ({
          recepcion_id: R,
          reff: i.reff.toUpperCase(),
      ...
      { error: g } = await k.from('tms_recepcion_items_nacionales').insert(w);
    if (g) throw new Error(`Falló el bloque ${Math.floor(u / oe) + 1}: ${g.message}`);
  }
}
async function Se(o) {
  const { count: R, error: u } = await k
    .from('tms_recepcion_items_nacionales')
    .select('*', { count: 'exact', head: !0 })
    .eq('recepcion_id', o);
  if (u) throw u;
  const w = [];
  for (let g = 0; g < (R || 0); g += Ee) {
    const i = g + Ee - 1,
      { data: v, error: _ } = await k
        .from('tms_recepcion_items_nacionales')
...
const Ct = () => {
    ...
    const { user: o } = Me(),
      R = Le(),
      { startScan: u, isScanning: w } = Ue(),
      g = p.useRef(null),
      i =
        (o == null ? void 0 : o.rol) === 'ADMIN' ||
        (o == null ? void 0 : o.es_admin_delegado) === !0 ||
        (o == null ? void 0 : o.rol) === 'CONTROL_CALIDAD',
      [v, _] = p.useState('dashboard'),
      [j, q] = p.useState(null),
      [m, P] = p.useState({ search: '', estado: '', desde: '', hasta: '' }),
      [pt, mt] = p.useState(!1),
      [h, B] = p.useState(null),
      [c, y] = p.useState({
    ...
    (p.useEffect(() => {
      ...
      try {
        ...
        if (s) {
          ...
          a && (((t = a.items) != null && t.length) || a.header) && H(a);
        }
      } catch {}
    }, []),
      p.useEffect(() => {
        if (v !== 'form' || (j !== null && !ne)) return;
        const t = c || {},
          s =
        ...
        try {
          if (s) {
            const a = { header: c, items: d, editingId: j, ts: Date.now() };
    ...
    const ce = () => {
      ...
      Y = () =>
        i ? !0 : (b.error('Solo Control Calidad o Administrador pueden modificar recepciones'), !1),
      Ae = () => {
        Y() &&
          E &&
          (y((t) => ({ ...t, ...E.header })),
          L(Array.isArray(E.items) ? E.items : []),
          q(E.editingId ?? null),
          z(!0),
          _('form'));
      },
      G = (t) => (t || '').trim().toUpperCase(),
      Z = p.useMemo(() => {
        const t = {};
        for (const s of d) {
          const a = G(s.serie);
    ...
    (Be(
      ...
      p.useEffect(() => {
        v === 'form' && !i && (_('dashboard'), q(null));
      }, [v, i]),
      $e('tms_recepciones_nacionales', [['recepciones_nac']]));
    const { data: F = [], isLoading: ke } = ze({
      ...
      S = p.useMemo(
        () =>
          F.filter((t) => {
            ...
            return !(
              (m.estado && t.estado !== m.estado) ||
              (m.desde && t.fecha_recepcion < m.desde) ||
              (m.hasta && t.fecha_recepcion > m.hasta)
            );
          }),
        [F, m]
      ),
      M = !!(m.search || m.estado || m.desde || m.hasta),
      I = p.useMemo(() => {
        const t = S,
          s = t.length,
          a = t.filter((A) => A.estado === 'EN_REVISION').length,
          l = t.filter((A) => A.estado === 'COMPLETADO').length,
          x = t.reduce((A, V) => A + (V.cant_bultos || 0), 0),
          N = t.reduce((A, V) => A + (V.pallets_usados || 0), 0),
          T = s > 0 ? (N / s).toFixed(1) : 0,
          $ = s > 0 ? Math.round(x / s) : 0,
          r = F.length,
          n = F.reduce((A, V) => A + (V.pallets_usados || 0), 0),
          se = r > 0 ? (n / r).toFixed(1) : 0;
        return {
          total: s,
          enRevision: a,
          completados: l,
      ...
      O = p.useMemo(() => {
        const t = S,
          ...
          a = {};
        (t.forEach((r) => {
          const n = r.proveedor || 'N/A';
          (a[n] || (a[n] = { proveedor: n, bultos: 0, pallets: 0, recepciones: 0, promPallets: 0 }),
            (a[n].bultos += r.cant_bultos || 0),
            (a[n].pallets += r.pallets_usados || 0),
            (a[n].recepciones += 1));
        }),
        ...
        const l = Object.values(a)
            .sort((r, n) => n.bultos - r.bultos)
            .slice(0, 10),
          x = {};
        t.forEach((r) => {
          if (!r.fecha_recepcion) return;
          const n = r.fecha_recepcion.slice(0, 7);
          (x[n] || (x[n] = { mes: n, bultos: 0, pallets: 0, recepciones: 0 }),
            (x[n].bultos += r.cant_bultos || 0),
            (x[n].pallets += r.pallets_usados || 0),
            (x[n].recepciones += 1));
        });
        const N = Object.values(x).sort((r, n) => r.mes.localeCompare(n.mes));
        N.forEach((r) => {
          const [n, se] = r.mes.split('-');
          r.label = `${s[parseInt(se) - 1]} ${n.slice(2)}`;
        });
        const T = {};
        t.forEach((r) => {
          const n = r.tipo_contenedor || 'N/A';
          T[n] = (T[n] || 0) + 1;
        });
        const $ = Object.entries(T).map(([r, n]) => ({ name: r, value: n }));
        return { porProveedor: l, porMes: N, porTipo: $ };
      }, [S]),
      ee = Fe({
        mutationFn: async () => {
          if (!i)
            throw new Error('Solo Control Calidad o Administrador pueden guardar recepciones');
          if (!c.proveedor) throw new Error('Proveedor es obligatorio');
          ...
          if (j) {
            const { error: s } = await k
              ...
              .update({
                ...
                proveedor: c.proveedor.toUpperCase(),
                oc: c.oc || null,
                cant_bultos: parseInt(c.cant_bultos) || 0,
                pallets_usados: parseInt(c.pallets_usados) || 0,
                tipo_contenedor: c.tipo_contenedor,
                notas: c.notas || null,
                items_count: d.length,
                estado: c.oc ? 'COMPLETADO' : 'EN_REVISION',
                updated_at: new Date().toISOString()
              })
              .eq('id', j);
            if (s) throw s;
            await k.from('tms_recepcion_items_nacionales').delete().eq('recepcion_id', j);
          } else {
            const { data: s, error: a } = await k
              .from('tms_recepciones_nacionales')
              .insert({
                fecha_recepcion: c.fecha_recepcion,
                proveedor: c.proveedor.toUpperCase(),
                ...
                tipo_contenedor: c.tipo_contenedor,
                notas: c.notas || null,
                productos: d.map((l) => l.reff).join(', '),
                cantidades: d.map((l) => l.cantidad).join(', '),
                items_count: d.length,
                estado: c.oc ? 'COMPLETADO' : 'EN_REVISION',
                usuario_nombre:
                  (o == null ? void 0 : o.nombre) || (o == null ? void 0 : o.email) || 'Usuario'
              })
              .select('id')
              .single();
            if (a) throw a;
        ...
        onSuccess: (t, s) => {
          (b.success(j ? 'Recepción actualizada' : 'Recepción guardada correctamente'),
            ...
            Ve({
              ...
              usuarioNombre: (o == null ? void 0 : o.nombre) || (o == null ? void 0 : o.email)
            }),
            ce(),
            te(),
            _('dashboard'));
      ...
      de = async (t, s) => {
        if (
          ...
          try {
            const { data: a, error: l } = await k.rpc('eliminar_recepcion_completa', {
              p_id: t,
              p_origen: 'NACIONAL'
            });
            if (l) throw l;
            ...
            (b.success(`Recepción de ${s} eliminada`),
              R.invalidateQueries({ queryKey: ['recepciones_nac'] }),
              (h == null ? void 0 : h.id) === t && B(null));
          } catch (a) {
            b.error('Error al eliminar: ' + a.message);
          }
      },
      Re = () => {
        const t = (f.reff || '').trim().toUpperCase();
        if (!t) {
        ...
        const s = G(f.serie);
        if (s) {
          const l = d.findIndex((x) => G(x.serie) === s);
          if (
            l !== -1 &&
...
¿Agregarla de todos modos? Quedará marcada como duplicada.`)
        ...
        (L((l) => [
          ...l,
          { ...f, reff: t, cantidad: parseInt(f.cantidad) || 1, descripcion: '', um: 'UNI', _id: a }
        ]),
          C({ reff: '', cantidad: 1, serie: '', lote: '', box: '', fecha_vencimiento: '' }),
          z(!0),
          b.success('Ítem agregado y guardado ✓', { duration: 1500 }),
          Oe(t)
            .then((l) => {
              l && L((x) => x.map((N) => (N._id === a ? { ...N, descripcion: l } : N)));
            })
            .catch((l) => console.error('[Reception] Falló lookup de descripción:', l)));
      },
      Ie = (t) => {
        (L((s) => s.filter((a, l) => l !== t)), z(!0));
      },
      Oe = async (t) => {
        try {
          const { data: s } = await k
      ...
      ie = (t) => {
        u({
          onScan: (s) => {
            const a = s.trim();
            (C((l) => ({ ...l, [t]: a })), b.success(`${t.toUpperCase()} escaneado: ${a}`));
          },
          onError: (s) => b.error(s)
        });
      },
      ...
      De = (t, s) => {
        ...
        ((x['!cols'] = [
          ...
          D.book_append_sheet(l, x, 'Detalle Items'));
        const N = [
            {
              ...
              'TOTAL CANTIDAD': s.reduce((r, n) => r + (n.cantidad || 0), 0)
            }
          ],
          T = D.json_to_sheet(N);
        ((T['!cols'] = [
          ...
          D.book_append_sheet(l, T, 'Resumen'));
        const $ = `RecepcionNacional_${t.proveedor}_${t.fecha_recepcion || 'sin-fecha'}.xlsx`;
        (we(l, $), b.success(`Archivo descargado: ${$}`));
      },
      Pe = () => {
        if (S.length === 0) {
        ...
        ((a['!cols'] = [
          ...
          D.book_append_sheet(s, a, 'Recepciones'),
          we(s, `Recepciones_Nacionales_${new Date().toLocaleDateString('en-CA')}.xlsx`),
                  ...
                  e.jsxs(e.Fragment, {
                    children: [
                      e.jsxs('button', {
                        ...
                        children: [e.jsx(qe, { size: 14 }), ' Exportar']
                      }),
                      i
                        ? e.jsxs('button', {
                            onClick: () => {
                  ...
                  e.jsxs(e.Fragment, {
                    children: [
                      ...
                      e.jsxs('button', {
                        ...
                        className:
                          'px-3 py-2 bg-white border border-slate-300 hover:bg-slate-50 text-slate-600 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors',
                        children: [e.jsx(Ke, { size: 14 }), ' Volver']
                      })
                    ]
                  })
              ]
                    ...
                    e.jsxs('div', {
                      ...
                      children: [
                        ...
                        e.jsxs('p', {
                          ...
                          children: [
                            ((me = E.items) == null ? void 0 : me.length) || 0,
                            ' ítem(s)',
                            (he = E.header) != null && he.proveedor
                              ? ` · ${E.header.proveedor}`
                              : '',
              ...
              e.jsxs('div', {
                ...
                children: [
                  e.jsx(U, {
                    label: 'Recepciones',
                    value: I.total,
                    sub: M ? `de ${I.globalTotal}` : null
                  }),
                  e.jsx(U, { label: 'En Revisión', value: I.enRevision, accent: 'text-amber-600' }),
                  e.jsx(U, { label: 'Completados', value: I.completados, accent: 'text-teal-600' }),
                  e.jsx(U, { label: 'Total Bultos', value: I.totalBultos.toLocaleString() }),
                  e.jsx(U, {
                    label: 'Prom. Pallets',
                    value: I.promPallets,
                    accent: 'text-slate-800',
                    sub: M ? `global: ${I.globalPromPallets}` : null
                  }),
                  e.jsx(U, { label: 'Prom. Bultos', value: I.promBultos })
                ]
              }),
              e.jsxs('div', {
                className:
                ...
                children: [
                  e.jsxs('div', {
                    ...
                    children: [
                      ...
                      e.jsx('input', {
                        ...
                        value: m.search,
                        onChange: (t) => P((s) => ({ ...s, search: t.target.value })),
                        className:
                  ...
                  e.jsxs('select', {
                    value: m.estado,
                    onChange: (t) => P((s) => ({ ...s, estado: t.target.value })),
                    className:
                  ...
                  e.jsx('input', {
                    type: 'date',
                    value: m.desde,
                    onChange: (t) => P((s) => ({ ...s, desde: t.target.value })),
                    className:
                      'px-2 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm outline-none focus:border-slate-400'
                  }),
                  e.jsx('span', {
                  ...
                  e.jsx('input', {
                    type: 'date',
                    value: m.hasta,
                    onChange: (t) => P((s) => ({ ...s, hasta: t.target.value })),
                    className:
                      'px-2 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm outline-none focus:border-slate-400'
                  }),
                  M &&
                    e.jsx('button', {
                      onClick: () => P({ search: '', estado: '', desde: '', hasta: '' }),
                      className:
                ...
                e.jsxs('div', {
                  ...
                  children: [
                    ...
                    e.jsxs('span', {
                      children: [
                        'Mostrando ',
                        e.jsx('b', { className: 'text-slate-700', children: S.length }),
                        ' de',
                        ' ',
                        F.length,
                        m.search &&
                          e.jsxs('span', {
                            children: [
                              ' ',
                              '· Proveedor: ',
                              e.jsx('b', { className: 'text-slate-700', children: m.search })
                            ]
                          })
                      ]
                    })
                    ...
                    ? e.jsx('div', {
                        ...
                        children: e.jsxs('table', {
                          ...
                          children: [
                            ...
                            e.jsx('tbody', {
                              children: O.porProveedor.map((t, s) => {
                                const a = Math.max(...O.porProveedor.map((x) => x.bultos)),
                                  l = a > 0 ? (t.bultos / a) * 100 : 0;
                                return e.jsxs(
                                  'tr',
                                  {
                                    className: `border-b border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors ${s % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`,
                                    onClick: () => P((x) => ({ ...x, search: t.proveedor })),
                                    children: [
                                      e.jsx('td', {
                        ...
                        e.jsx('tbody', {
                          children: ke
                            ...
                            : S.length === 0
                              ...
                              : S.map((t, s) => {
                                  const a = Ce[t.estado] || Ce.PENDIENTE;
                                  return e.jsxs(
                                    'tr',
                                    {
                            ...
                            e.jsxs('div', {
                              ...
                              children: [
                                ...
                                e.jsx('input', {
                                  ...
                                  onChange: (t) =>
                                    y((s) => ({ ...s, fecha_recepcion: t.target.value })),
                                  className:
                            ...
                            e.jsxs('div', {
                              ...
                              children: [
                                ...
                                e.jsx('input', {
                                  ...
                                  value: c.proveedor,
                                  onChange: (t) =>
                                    y((s) => ({ ...s, proveedor: t.target.value.toUpperCase() })),
                                  className:
                            ...
                            e.jsxs('div', {
                              ...
                              children: [
                                ...
                                e.jsx('input', {
                                  type: 'text',
                                  value: c.oc,
                                  onChange: (t) => y((s) => ({ ...s, oc: t.target.value })),
                                  className:
                                    'w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-emerald-400',
                                  placeholder: '21073...'
                                })
                          ...
                          children: [
                            e.jsxs('div', {
                              children: [
                                ...
                                e.jsx('input', {
                                  ...
                                  value: c.cant_bultos,
                                  onChange: (t) =>
                                    y((s) => ({ ...s, cant_bultos: t.target.value })),
                                  className:
                            ...
                            e.jsxs('div', {
                              children: [
                                ...
                                e.jsx('input', {
                                  type: 'number',
                                  value: c.pallets_usados,
                                  onChange: (t) =>
                                    y((s) => ({ ...s, pallets_usados: t.target.value })),
                                  className:
                                    'w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-emerald-400',
                                  placeholder: '0',
                                  min: '0'
                        ...
                        e.jsxs('div', {
                          children: [
                            ...
                            e.jsx('select', {
                              ...
                              onChange: (t) =>
                                y((s) => ({ ...s, tipo_contenedor: t.target.value })),
                              className:
                                'w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-emerald-400',
                              children: it.map((t) => e.jsx('option', { value: t, children: t }, t))
                            })
                          ]
                        }),
                        e.jsxs('div', {
                          children: [
                            ...
                            e.jsx('textarea', {
                              rows: 2,
                              value: c.notas,
                              onChange: (t) => y((s) => ({ ...s, notas: t.target.value })),
                              className:
                      ...
                      e.jsxs('div', {
                        ...
                        children: [
                          ...
                          e.jsx('input', {
                            ...
                            value: f.reff,
                            onChange: (t) =>
                              C((s) => ({ ...s, reff: t.target.value.toUpperCase() })),
                            className:
                          ...
                          e.jsxs('div', {
                            children: [
                              ...
                              e.jsx('input', {
                                type: 'number',
                                value: f.cantidad,
                                onChange: (t) => C((s) => ({ ...s, cantidad: t.target.value })),
                                className:
                          ...
                          e.jsxs('div', {
                            children: [
                              ...
                              e.jsx('input', {
                                type: 'text',
                                value: f.box,
                                onChange: (t) => C((s) => ({ ...s, box: t.target.value })),
                                className:
                          ...
                          e.jsxs('div', {
                            ...
                            children: [
                              e.jsx('input', {
                                type: 'text',
                                value: f.serie,
                                onChange: (t) => C((s) => ({ ...s, serie: t.target.value })),
                                className:
                          ...
                          e.jsxs('div', {
                            ...
                            children: [
                              e.jsx('input', {
                                type: 'text',
                                value: f.lote,
                                onChange: (t) => C((s) => ({ ...s, lote: t.target.value })),
                                className:
                                  'flex-1 p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-base font-mono outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all',
                      ...
                      e.jsxs('div', {
                        ...
                        children: [
                          ...
                          e.jsx('input', {
                            ...
                            onChange: (t) =>
                              C((s) => ({ ...s, fecha_vencimiento: t.target.value })),
                            className:
                              'w-full p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-base font-bold outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all'
                          })
                        ]
                        ...
                        children: [
                          ...
                          d.length > 0 &&
                            e.jsxs('span', {
                              ...
                              children: [
                                'Total: ',
                                d.reduce((t, s) => t + (parseInt(s.cantidad) || 0), 0),
                                ' unidades'
                              ]
                            })
                        ]
                        ...
                        children:
                          d.length === 0
                            ? e.jsxs('div', {
                                ...
                                children: [
                                  e.jsx(Ze, { size: 40, className: 'mx-auto mb-2 opacity-30' }),
                            ...
                            : e.jsxs('table', {
                                ...
                                children: [
                                  ...
                                  e.jsx('tbody', {
                                    children: d.map((t, s) => {
                                      const a = !!G(t.serie) && Z.has(G(t.serie));
                                      return e.jsxs(
                                        'tr',
                                        {
                    ...
                    e.jsxs('div', {
                      ...
                      children: [
                        e.jsxs('h3', {
                          ...
                          children: ['Recepción — ', h.proveedor]
                        }),
                        e.jsxs('div', {
                          className:
                            'flex flex-wrap gap-2 sm:gap-4 mt-2 text-xs sm:text-sm text-slate-300',
                          children: [
                            e.jsxs('span', { children: ['📅 ', pe(h.fecha_recepcion)] }),
                            h.oc && e.jsxs('span', { children: ['📋 OC: ', h.oc] }),
                            e.jsxs('span', { children: ['📦 ', h.cant_bultos || 0, ' bultos'] }),
                            e.jsxs('span', {
                              children: ['🏗️ ', h.pallets_usados || 0, ' pallets']
                            }),
                            e.jsxs('span', { children: ['🚛 ', h.tipo_contenedor] })
                          ]
                        })
                      ]
                    }),
                ...
                e.jsxs('div', {
                  ...
                  children: [
                    ...
                    e.jsxs('button', {
                      ...
                      children: [e.jsx(tt, { size: 16 }), ' DESCARGAR EXCEL']
                    }),
                    i &&
                      e.jsxs('button', {
                        onClick: () => de(h.id, h.proveedor),
                        className:
                ...
                e.jsxs('div', {
                  ...
                  children: [
                    e.jsxs('span', {
                      ...
                      children: [
                        'Total ítems: ',
                        ((be = h.items) == null ? void 0 : be.length) || 0
                      ]
                    }),
                    e.jsxs('span', {
                      ...
                      children: [
                        'Total cantidad:',
                        ' ',
                        (h.items || []).reduce((t, s) => t + (s.cantidad || 0), 0)
                      ]
                    })
                  ]
                })
  ...
  U = ({ label: o, value: R, accent: u, sub: w }) =>
    e.jsxs('div', {
      ...
      children: [
        ...
        e.jsx('p', {
          className: `text-lg sm:text-2xl font-bold tabular-nums ${u || 'text-slate-800'}`,
          children: R
        }),
        w &&
          e.jsx('p', { className: 'text-[9px] sm:text-[10px] text-slate-400 mt-0.5', children: w })
      ]
    });
