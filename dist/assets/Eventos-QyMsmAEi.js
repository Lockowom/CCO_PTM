...
function Se() {
  const { hasPermission: a, user: i } = ce(),
    ...
    [h, y] = r.useState([]),
    [$, N] = r.useState(null),
    [k, O] = r.useState('todos'),
    [T, M] = r.useState(!1),
    p = r.useCallback(async () => {
      const [s, l, o, H] = await Promise.all([xe({ agregado: k }), me(), pe({}), he()]);
      (n(s), R(l), z(o), y(H));
    }, [k]);
  r.useEffect(() => {
    p();
  ...
  const _ = async (s, l) => {
    ...
    G = [
      ['mis', `Mis notificaciones${h.length ? ` (${h.length})` : ''}`, U],
      ['stream', 'Stream de eventos', W],
      ['metricas', 'Métricas / SLA', q],
      ['reglas', `Reglas (${w.length})`, J],
      ['bandeja', 'Bandeja de salida', Q]
    ],
    [f, K] = r.useState('OT'),
    [d, X] = r.useState(null),
    [Z, P] = r.useState(!1);
                                      ...
                                      children: [
                                        ...
                                        e.jsx('div', {
                                          ...
                                          children: e.jsx('div', {
                                            ...
                                            style: {
                                              width: `${Math.round(((Number(s.dwell_prom_horas) || 0) / Y) * 100)}%`,
                ...
                m &&
                  e.jsxs('button', {
                    ...
                    children: [
                      e.jsx(ie, { size: 13 }),
                      ' ',
                      T ? 'Enviando…' : `Enviar push pendientes${C ? ` (${C})` : ''}`
                    ]
                  })
              ]
            }),
