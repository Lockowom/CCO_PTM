...
const re = Object.fromEntries(ee.map((t) => [t.id, t.label])),
  ...
  ge = () => {
    const { user: t, hasPermission: C } = K(),
      h = (t == null ? void 0 : t.rol) === 'ADMIN' || (t == null ? void 0 : t.es_admin_delegado),
      v = C('manage_quality'),
      { data: n = [], isLoading: _, refetch: k, isFetching: f } = V(),
      { data: d = [] } = W(),
      x = Z(),
      w = X();
    ...
    const [c, R] = r.useState('pendientes'),
      ...
      S = n.filter((e) => e.estado === 'PENDIENTE' || e.estado === 'EN_PROCESO').length,
      O = async (e) => {
        try {
                                          ...
                                          children: [
                                            e.estado === 'PENDIENTE' &&
                                              s.jsxs('button', {
                                                ...
                                                children: [s.jsx(Q, { size: 14 }), ' En proceso']
                                              }),
                                            s.jsxs('button', {
                                              onClick: () => {
                                                (b(e.id), m(''));
