...
import { r as n } from './react-vendor-CA7EHQ1X.js';
import { u as Vt, s as M } from './index-CXl7fTg4.js';
import {
  o as ut,
  r as Ue,
  af as Ft,
...
import { u as Jt } from './useBarcodeScanner-CdHcw5UK.js';
...
const be = [
    ...
    ...['BD 21', 'BD 5', 'BD 24', 'BD 3', 'BD 7', 'BD 99', 'BD 22'].map((d) => ({
      id: `inv_${d.replace(' ', '').toLowerCase()}`,
      label: `${d}`,
      category: 'inventario',
      icon: pt,
      table: 'tms_inventario_general',
      uniqueKey: 'bodega,codigo_producto',
      defaultValues: { bodega: d },
      columns: [
        { key: 'codigo_producto', label: 'Cod. Producto', required: !0, type: 'text' },
        { key: 'producto', label: 'Producto', required: !1, type: 'text' },
        { key: 'unidad_medida', label: 'Cod. U. Medida', required: !1, type: 'text' },
      ...
      helpText: `Inventario de ${d}. La bodega se asigna automáticamente. Si el producto ya existe, se actualizará.`,
      smartDedup: !1
    })),
    {
      id: 'nv',
    ...
    ...['ptm', 'orange', 'farmapack'].map((d) => ({
      id: `nvcat_${d}`,
      label: d === 'ptm' ? 'N.V PTM' : d === 'orange' ? 'N.V ORANGE' : 'N.V FARMAPACK',
      category: 'ventas',
      icon: Ue,
      table: 'tms_nv_catalogo',
      uniqueKey: 'nv,canal',
      defaultValues: { canal: d },
      columns: [
      ...
      helpText: `Catálogo NV → Cliente/Vendedor del canal ${d.toUpperCase()}. Columnas: Fecha, N.Venta, Nombre Cliente, Nombre Vendedor, Monto Neto. Solo entran las N.V. NUEVAS; las que ya existen se detectan y se OMITEN (no se duplican al re-subir). Para corregir/actualizar datos existentes, marcá "Reemplazar canal".`,
      smartDedup: !0,
      allowUpdate: !1
    })),
    {
  ...
  Fe = [
    { id: 'inventario', label: 'Inventario', icon: Ot, color: 'blue' },
    { id: 'ventas', label: 'Ventas / Despacho', icon: Kt, color: 'emerald' },
    { id: 'trazabilidad', label: 'Trazabilidad', icon: Ve, color: 'violet' },
    { id: 'wms', label: 'WMS', icon: Oe, color: 'orange' },
    { id: 'maestros', label: 'Maestros', icon: jt, color: 'slate' }
  ],
  vt = {
  ...
  Le = be.map((d) => d.id),
  ts = ({ onClose: d }) => {
    const [L, F] = n.useState([]),
      [oe, Q] = n.useState(!0),
      [O, D] = n.useState(null),
      [Y, ge] = n.useState(null);
    n.useEffect(() => {
      (async () => {
        const { data: u } = await M.from('tms_roles')
          .select('id, nombre, import_tabs')
          .order('nombre');
        (F((u || []).filter((f) => !Ct.includes(f.id))), Q(!1));
      })();
    }, []);
    const _e = (u, f) => {
        F((E) =>
          E.map((o) => {
            if (o.id !== u) return o;
            const P = Array.isArray(o.import_tabs) ? o.import_tabs : [],
              C = P.includes(f) ? P.filter((B) => B !== f) : [...P, f];
            return { ...o, import_tabs: C };
          })
        );
      },
      K = (u) => {
        F((f) =>
          f.map((E) => {
            if (E.id !== u) return E;
            const P =
              (Array.isArray(E.import_tabs) ? E.import_tabs : []).length === Le.length
                ? []
                : [...Le];
            return { ...E, import_tabs: P };
          })
        );
      },
      fe = async (u) => {
        D(u.id);
        const { error: f } = await M.from('tms_roles')
          .update({ import_tabs: u.import_tabs })
          .eq('id', u.id);
        (D(null), f || (ge(u.id), setTimeout(() => ge(null), 2e3)));
      },
      X = n.useMemo(
        () => Fe.map((u) => ({ ...u, tabs: be.filter((f) => f.category === u.id) })),
        []
      );
    return e.jsx('div', {
          ...
          e.jsx('div', {
            ...
            children: oe
              ? e.jsx('div', {
                  ...
                  children: e.jsx(le, { size: 24, className: 'animate-spin text-slate-400' })
                })
              : e.jsx('div', {
                  className: 'space-y-4',
                  children: L.map((u) => {
                    const f = Array.isArray(u.import_tabs) ? u.import_tabs : [],
                      E = f.length === Le.length;
                    return e.jsxs(
                      'div',
                      {
                              ...
                              e.jsxs('div', {
                                ...
                                children: [
                                  e.jsx('button', {
                                    ...
                                    children: E ? 'Quitar todo' : 'Seleccionar todo'
                                  }),
                                  e.jsxs('button', {
                                    onClick: () => fe(u),
                                    disabled: O === u.id,
                                    className: `flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${Y === u.id ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-800 text-white hover:bg-slate-700'} disabled:opacity-50`,
                                    children: [
                                      O === u.id
                                        ? e.jsx(le, { size: 12, className: 'animate-spin' })
                                        : Y === u.id
                                          ? e.jsx(kt, { size: 12 })
                                          : e.jsx(Qt, { size: 12 }),
                                      Y === u.id ? 'Guardado' : 'Guardar'
                                    ]
                                  })
                                ]
                              })
                                ...
                                'div',
                                {
                                  ...
                                  children: [
                                    ...
                                    e.jsx('div', {
                                      ...
                                      children: o.tabs.map((P) => {
                                        const C = f.includes(P.id);
                                        return e.jsx(
                                          'button',
                                          {
                                            onClick: () => _e(u.id, P.id),
                                            className: `px-2.5 py-1 rounded-md text-[11px] font-bold transition-all border ${C ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-slate-200 text-slate-400 hover:border-slate-300'}`,
                                            children: P.label
                                          },
                                          P.id
                                        );
  ...
  is = () => {
    var rt, lt, ot, nt, it, dt;
    const { user: d } = Vt(),
      [L, F] = n.useState(null),
      [oe, Q] = n.useState(!1),
      O = Ct.includes(d == null ? void 0 : d.rol);
    n.useEffect(() => {
      if (d != null && d.rol) {
        if (O) {
          F('*');
        ...
        (async () => {
          const { data: t } = await M.from('tms_roles')
            .select('import_tabs')
            .eq('id', d.rol)
            .single();
    ...
    const D = n.useMemo(
        () =>
          L === null
            ...
            : L === '*' || O
              ? be
              : Array.isArray(L)
                ? be.filter((t) => L.includes(t.id))
                : [],
        [L, O]
      ),
      Y = n.useMemo(() => {
        const t = new Set(D.map((s) => s.category));
        return Fe.filter((s) => t.has(s.id));
      }, [D]),
      ge = ((rt = Y[0]) == null ? void 0 : rt.id) || 'ventas',
      _e = ((lt = D[0]) == null ? void 0 : lt.id) || 'nv',
      [K, fe] = n.useState(_e),
      [X, u] = n.useState(ge),
      [f, E] = n.useState(''),
      [o, P] = n.useState([]),
      ...
      [Ge, Se] = n.useState(!1),
      [ne, qe] = n.useState(!1),
      [T, ie] = n.useState(null),
      [J, de] = n.useState('paste'),
      [Ee, He] = n.useState(!1),
      [Ye, We] = n.useState(!1),
      [w, he] = n.useState(null),
      [ee, ce] = n.useState(null),
      [te, Ze] = n.useState(!1),
      Qe = n.useRef(null),
      ze = n.useRef(null),
      [ue, Ae] = n.useState('paste'),
      [G, ye] = n.useState([]),
      [Xe, ve] = n.useState(''),
      [pe, De] = n.useState(''),
      { startScan: Je, isScanning: et, isSupportedDevice: wt } = Jt();
    n.useEffect(() => {
      var t, s;
      if (D.length > 0 && !D.find((a) => a.id === K)) {
        const a = (t = Y[0]) == null ? void 0 : t.id,
          c = (s = D[0]) == null ? void 0 : s.id;
        (a && u(a), c && fe(c));
      }
    }, [D]);
    const l = be.find((t) => t.id === K),
      R = !!((ot = l == null ? void 0 : l.id) != null && ot.startsWith('nvcat_')),
      z = ((nt = l == null ? void 0 : l.defaultValues) == null ? void 0 : nt.canal) || '',
      je = (t) => {
        const s = String(t ?? '').trim();
        return /^\d+\.0+$/.test(s) ? s.split('.')[0] : s;
      },
      tt = n.useCallback(async (t) => {
        const s = new Set();
        for (const a of t) {
          let c = 0;
          const g = 1e3;
          for (;;) {
            const { data: x, error: b } = await M.from('tms_nv_catalogo')
              .select('nv')
              .eq('canal', a)
              .range(c, c + g - 1);
            if (b || !x || x.length === 0 || (x.forEach((_) => s.add(je(_.nv))), x.length < g))
              break;
            c += g;
          }
        }
    ...
    n.useEffect(() => {
      ...
      return (
        (async () => {
          const s = z === 'ptm' ? ['orange', 'farmapack'] : ['ptm'],
            [a, c] = await Promise.all([tt(s), tt([z])]);
          if (!t) return;
          const g = [];
          let x = 0;
          for (const b of o) {
            const _ = je(b.nv);
            _ && a.has(_) && (x++, g.length < 6 && g.push(_));
          }
          if (
            (he({ count: x, canalOtro: s.join('/').toUpperCase(), actual: c.size, ejemplos: g }),
            c.size >= 20)
          ) {
            const b = {};
            c.forEach((A) => {
            ...
            const _ = Math.max(2, c.size * 0.01),
              $ = Object.keys(b)
                .map(Number)
                .filter((A) => b[A] >= _)
                .sort((A, v) => A - v);
            if ($.length) {
              const A = Math.max(...$);
              $.includes(A + 1) || $.push(A + 1);
              const v = Object.keys(b)
                .map(Number)
                .reduce((p, j) => (b[j] > (b[p] || 0) ? j : p), $[0]);
              ce({ allowed: $, principal: v, muestra: c.size });
            } else ce(null);
          } else ce(null);
        })().catch(() => {
          t && (he(null), ce(null));
    ...
    const _t = (t) => {
      ...
      xe = n.useMemo(() => {
        if (!R || o.length === 0) return { count: 0, ejemplos: [] };
        let t = 0;
        const s = [];
        for (const a of o)
          _t(a.cliente) &&
      ...
      me = n.useMemo(() => {
        if (!R || !ee || o.length === 0) return { count: 0, ejemplos: [] };
        const t = new Set(ee.allowed);
        let s = 0;
        const a = [];
        for (const c of o) {
          const x = je(c.nv).length;
          x && !t.has(x) && (s++, a.length < 5 && a.push(`N.V ${c.nv} (${x} díg.)`));
        }
        return { count: s, ejemplos: a };
      }, [R, ee, o]),
      St = o.length ? xe.count / o.length : 0,
      qt = w && o.length ? w.count / o.length : 0,
      Et = o.length ? me.count / o.length : 0,
      I =
        R &&
        ((xe.count >= 15 && St >= 0.15) ||
          (qt >= 0.5 && ((w == null ? void 0 : w.count) || 0) >= 15) ||
          (me.count >= 15 && Et >= 0.15)),
      zt = n.useMemo(() => D.filter((t) => t.category === X), [D, X]),
      Ne = n.useCallback(
        async (t) => {
          if (!t.trim()) return;
          Se(!0);
...
`
          ...
          const a = s[0],
            c = a.includes('	') ? '	' : a.includes(';') ? ';' : ',',
            g = a.split(c).map((p) => p.trim()),
            _ = (
              l.columns.some((p) =>
                g.some(
                  (j) =>
                    ...
                    j.toLowerCase().replace(/[^a-z0-9]/g, '') ===
                      p.key.toLowerCase().replace(/[^a-z0-9]/g, '')
                )
              )
                ? s.slice(1)
                : s
            ).map((p) => {
              const j = p.split(c).map((N) => N.trim()),
                U = {};
              let W = 0;
              return (
                l.columns.forEach((N) => {
                    ...
                    if (r && r.trim() !== '')
                      if (r.length < 6 || !/\d/.test(r)) r = null;
                        ...
                        if (m) {
                          const [S, y, i, k] = m,
                            V = k.length === 2 ? `20${k}` : k,
                            Ie = `${V}-${i.padStart(2, '0')}-${y.padStart(2, '0')}`,
                            h = new Date(V, parseInt(i) - 1, y);
                          h.getFullYear() == V &&
                          h.getMonth() == parseInt(i) - 1 &&
                          h.getDate() == y
                            ? (r = Ie)
                            : (r = null);
                        } else {
                          ...
                          if (isNaN(S)) r = null;
                            ...
                            const y = new Date(S),
                              i = y.getFullYear();
                            i > 1900 && i < 2100 ? (r = y.toISOString().split('T')[0]) : (r = null);
                          }
                        }
                      }
                    else r = null;
                ...
                l.defaultValues &&
                  Object.entries(l.defaultValues).forEach(([N, r]) => {
                    U[N] || (U[N] = r);
                  }),
                U
              );
            ...
            A = l.columns.filter((p) => p.required),
            v = _.filter((p) => A.every((j) => $(p[j.key])));
          if ((Be(_.length - v.length), P(v), l.smartDedup && l.uniqueKey && v.length > 0))
            try {
              const p = l.uniqueKey.split(',').map((h) => h.trim()),
                j = p[0],
                U = [...new Set(v.map((h) => h[j]).filter(Boolean))],
                W = p.join(','),
                N = 500,
                r = [];
              for (let h = 0; h < U.length; h += N) r.push(U.slice(h, h + N));
              const m = r.map((h) => M.from(l.table).select(W).in(j, h).limit(h.length)),
                S =
                  l.id === 'nv' ? r.map((h) => M.from('tms_nv_eliminadas').select(j).in(j, h)) : [],
                [y, i] = await Promise.all([
                  Promise.all(m),
                  S.length > 0 ? Promise.all(S) : Promise.resolve([])
                ]),
                k = new Set();
              y.forEach((h) => {
                (h.data || []).forEach((se) => {
                  k.add(
                    p
                      .map((ae) => {
                        var H;
                        return (H = se[ae]) == null ? void 0 : H.toString().trim().toUpperCase();
                      })
                      .join('|')
                  );
                });
              ...
              i.forEach((h) => {
                (h.data || []).forEach((se) => {
                  var H;
                  const ae = (H = se[j]) == null ? void 0 : H.toString().trim().toUpperCase();
                  ae && V.add(ae);
                });
              });
              const Ie = v.map((h) => {
                var H;
                const se = (H = h[j]) == null ? void 0 : H.toString().trim().toUpperCase();
                if (!se) return 'error';
                if (V.has(se)) return 'deleted';
                const ae = p
                  .map((Ut) => {
                    var ct;
                    return (ct = h[Ut]) == null ? void 0 : ct.toString().trim().toUpperCase();
                  })
                  .join('|');
                return k.has(ae) ? (l.allowUpdate ? 'update' : 'existing') : 'new';
              });
              B(Ie);
            } catch (p) {
              (console.error('Row status check error:', p), B(v.map(() => 'new')));
            }
          else B(v.map(() => 'new'));
          (de('preview'), Se(!1));
        },
      ...
      Pe = () => {
        var s, a;
        if (!l) return 'codigo_producto';
        const t = l.id.startsWith('inv_') ? 'inventario_general' : l.id;
        return (
          es[t] ||
          ((s = l.columns.find((c) => c.required)) == null ? void 0 : s.key) ||
          ((a = l.columns[0]) == null ? void 0 : a.key)
        );
      },
      Pt = async (t) => {
        try {
          const { data: s } = await M.from('tms_matriz_codigos')
      ...
      ke = n.useCallback(
        async (t) => {
          ...
          if (
            (l.defaultValues &&
              Object.entries(l.defaultValues).forEach(([x, b]) => {
                a[x] = b;
              }),
            t.includes('	') || t.includes('|') || t.includes(';'))
          ) {
            const x = t.includes('	') ? '	' : t.includes('|') ? '|' : ';',
              b = t.split(x).map((_) => _.trim());
            l.columns.forEach((_, $) => {
              b[$] && (a[_.key] = b[$]);
            });
          } else a[s] = t.trim();
          ...
          if (g && !a.producto) {
            const x = await Pt(g);
            x &&
              (a.producto || (a.producto = x.producto),
              a.unidad_medida || (a.unidad_medida = x.unidad_medida));
          }
          ye((x) => [...x, { ...a, _scannedAt: Date.now(), _rawValue: t }]);
        },
        [l]
      ),
      Tt = n.useCallback(async () => {
        ve('');
        const t = await Je({ onError: (s) => ve(s) });
        t && (await ke(t));
      }, [Je, ke]),
      st = n.useCallback(async () => {
        pe.trim() && (await ke(pe.trim()), De(''));
      }, [pe, ke]),
      Rt = (t) => {
        ye((s) => s.filter((a, c) => c !== t));
      },
      $t = n.useCallback(() => {
        ...
        (P(t), B(t.map(() => 'new')), de('preview'));
      }, [G]),
      [Te, Ce] = n.useState({ current: 0, total: 0 }),
      It = async () => {
        if (o.length !== 0) {
          if (I) {
            Xt.error(
          ...
          if (!(
            R &&
            te &&
            !window.confirm(`REEMPLAZAR canal ${z.toUpperCase()}:

Se borrarán las ${(w == null ? void 0 : w.actual) ?? '—'} N.V. actuales de ${z.toUpperCase()} y quedarán solo las ${o.length} de este archivo.

¿Continuar?`)
          )) {
            (qe(!0), ie(null), Ce({ current: 0, total: 0 }));
            try {
              if (R && te) {
                const { error: r } = await M.rpc('nv_catalogo_purgar_canal', { p_canal: z });
                if (r) throw new Error(`No se pudo reemplazar el canal: ${r.message}`);
              }
              let t = R && te ? o.slice() : o.filter((r, m) => C[m] === 'new' || C[m] === 'update');
              if (t.length === 0) {
                (ie({
                  success: !0,
                  total: o.length,
              ...
              const s = [];
              if (l.id === 'nv' && t.length > 0)
                try {
                  const r = t.map((y) => {
                      var i, k;
                      return {
                        nv: (i = y.nv) == null ? void 0 : i.toString(),
                        codigo_producto: (k = y.codigo_producto) == null ? void 0 : k.toString(),
                        cantidad: y.cantidad,
                        cliente: y.cliente
                      };
                    }),
                    { data: m, error: S } = await M.rpc('prepare_nv_import', {
                      payload: r,
                      sync_deleted: Ye
                    });
                  S
                    ? s.push(`Advertencia: ${S.message}`)
                    : m &&
                      (m.reseteadas > 0 &&
                        s.push(`Se reiniciaron ${m.reseteadas} N.V. en proceso.`),
                      m.items_cancelados > 0 &&
                        s.push(`Se cancelaron ${m.items_cancelados} ítems faltantes.`));
                } catch (r) {
                  console.error('Record upsert error:', r);
                }
              let a = 0;
              if (l.uniqueKey) {
                const r = l.uniqueKey.split(',').map((i) => i.trim()),
                  m = new Map(),
                  S = [];
                t.forEach((i) => {
                  const k = r.map((V) => (i[V] || '').toString().trim());
                  k.some((V) => V !== '') ? m.set(k.join('|'), i) : S.push(i);
                });
                const y = t.length;
                ((t = [...m.values(), ...S]),
                  (a = y - t.length),
                  a > 0 &&
                    s.push(`${a} filas duplicadas consolidadas (misma clave: ${l.uniqueKey})`));
              }
              o.length > 5e3 && E('');
              let c = 0,
                g = 0;
              const x = !l.uniqueKey || l.id === 'control_despacho' ? null : l.uniqueKey,
                b = 1e3,
                _ = 2,
                $ = 6e4,
                A = [];
              for (let r = 0; r < t.length; r += b) A.push(t.slice(r, r + b));
              const v = A.length;
              let p = 0;
              Ce({ current: 0, total: v });
              const j = async (r, m) => {
                const S = new AbortController(),
                  y = setTimeout(() => S.abort(), $);
                try {
                  const { data: i, error: k } = await M.rpc('bulk_upsert', {
                    p_table: l.table,
                    p_data: r,
                    p_conflict_keys: x
                  }).abortSignal(S.signal);
                  if (k) throw k;
                  ((c += (i == null ? void 0 : i.inserted) || 0),
                    (g += (i == null ? void 0 : i.errors) || 0),
                    i != null && i.error
                      ? s.push(`Lote ${m}: ${i.error}`)
                      : (i == null ? void 0 : i.errors) > 0 &&
                        s.push(`Lote ${m}: ${i.errors} errores server-side`));
                } catch (i) {
                  g += r.length;
                  const k =
                    (i == null ? void 0 : i.name) === 'AbortError'
                      ? 'tiempo de espera agotado (60s)'
                      : (i == null ? void 0 : i.message) || 'error desconocido';
                  (s.push(`Lote ${m}: ${k}`), console.error(`Chunk ${m} error:`, k));
                } finally {
                  (clearTimeout(y), p++, Ce({ current: p, total: v }));
                }
              };
              for (let r = 0; r < A.length; r += _) {
                const m = A.slice(r, r + _);
                await Promise.all(m.map((S, y) => j(S, r + y + 1)));
              }
              const U = [...C];
              let W = 0;
              (o.forEach((r, m) => {
                (C[m] === 'new' || C[m] === 'update') && ((U[m] = W < c ? 'loaded' : 'error'), W++);
              }),
                B(U));
              const N = o.length - t.length - a;
              (M.from('tms_historial_cargas')
                .insert([
                  {
                    usuario_id: d == null ? void 0 : d.id,
                    usuario_nombre:
                      (d == null ? void 0 : d.nombre) ||
                      (d == null ? void 0 : d.email) ||
                      'Desconocido',
                    modulo: l.label,
                    tabla_destino: l.table,
                    registros_totales: o.length,
                    registros_nuevos: c,
                    registros_actualizados: C.filter((r) => r === 'update').length,
                    registros_error: g
                  }
                ])
                .then(({ error: r }) => {
                ...
                ie({
                  ...
                  message:
                    g === 0
                      ? `${c} registros cargados${a > 0 ? ` · ${a} duplicados consolidados` : ''}${N > 0 ? ` · ${N} ignorados` : ''}`
                      : `${c} cargados · ${g} con error${a > 0 ? ` · ${a} duplicados` : ''}${N > 0 ? ` · ${N} ignorados` : ''}`
                }),
                de('done'));
            } catch (t) {
              ie({
      ...
      Mt = (t) => {
        u(t);
        const s = D.find((a) => a.category === t);
        s && at(s.id);
      },
      q = {
        total: o.length,
        new: C.filter((t) => t === 'new').length,
        update: C.filter((t) => t === 'update').length,
        existing: C.filter((t) => t === 'existing').length,
        deleted: C.filter((t) => t === 'deleted').length,
        loaded: C.filter((t) => t === 'loaded').length,
        error: C.filter((t) => t === 'error').length
      },
      $e = Fe.find((t) => t.id === X),
      Lt = vt[($e == null ? void 0 : $e.color) || 'slate'];
    return L === null
      ? e.jsx('div', {
                          ...
                          e.jsxs('p', {
                            ...
                            children: [
                              ...
                              e.jsx('span', {
                                ...
                                children: l == null ? void 0 : l.table
                              })
                            ]
                          })
                        ]
                        ...
                        children: [
                          O &&
                            e.jsxs('button', {
                              ...
                              children: [e.jsx(Nt, { size: 14 }), ' Accesos']
                            }),
                          J !== 'paste' &&
                            e.jsxs('button', {
                              onClick: Re,
                              ...
                              children: [e.jsx(we, { size: 14 }), ' Nueva carga']
                            })
                        ]
                      })
                    ]
                    ...
                    children: Y.map((t) => {
                      ...
                      return e.jsxs(
                        'button',
                        {
                          onClick: () => Mt(t.id),
                          disabled: ne,
                          className: `flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-2 rounded-xl text-xs font-bold transition-all border whitespace-nowrap flex-shrink-0 ${a ? c.active : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50'} disabled:opacity-50`,
                          ...
                          children: e.jsxs('div', {
                            ...
                            children: [
                              e.jsxs('button', {
                                ...
                                children: [e.jsx(xt, { size: 14 }), ' Pegar datos']
                              }),
                              e.jsxs('button', {
                                onClick: () => Ae('scan'),
                                ...
                                children: [e.jsx(mt, { size: 14 }), ' Escanear QR / Código']
                              })
                            ]
                          })
                        }),
                              ...
                              e.jsxs('label', {
                                ...
                                children: [
                                  e.jsx('input', {
                                    ...
                                    checked: Ee,
                                    onChange: (t) => {
                                      (He(t.target.checked), f && setTimeout(() => Ne(f), 50));
                                    },
                                    className:
                              ...
                              e.jsxs('label', {
                                ...
                                children: [
                                  e.jsx('input', {
                                    type: 'checkbox',
                                    checked: Ye,
                                    onChange: (t) => We(t.target.checked),
                                    className:
                          ...
                          e.jsxs(e.Fragment, {
                            children: [
                              e.jsxs('div', {
                                ...
                                onClick: () => {
                                  var t;
                                  return (t = Qe.current) == null ? void 0 : t.focus();
                                },
                                children: [
                                  !f &&
                                    e.jsxs('div', {
                              ...
                              f &&
                                e.jsx('div', {
                                  ...
                                  children: e.jsxs('button', {
                                    ...
                                    children: [
                                      Ge
                                        ? e.jsx(le, { size: 16, className: 'animate-spin' })
                                        : e.jsx(gt, { size: 16 }),
                                      'Analizar datos'
                                    ]
                                  })
                                      ...
                                      e.jsxs('p', {
                                        ...
                                        children: [
                                          ...
                                          e.jsx('span', {
                                            ...
                                            children:
                                              ((it = l.columns.find((t) => t.key === Pe())) == null
                                                ? void 0
                                                : it.label) || Pe()
                                          })
                                        ]
                                  ...
                                  wt &&
                                    e.jsx('button', {
                                      ...
                                      children: et
                                        ? e.jsxs(e.Fragment, {
                                            children: [
                                              e.jsx(le, { size: 20, className: 'animate-spin' }),
                                              ' Escaneando...'
                                            ]
                                          })
                                        : e.jsxs(e.Fragment, {
                                            children: [e.jsx(Ht, { size: 20 }), ' Abrir Cámara']
                                          })
                                    }),
                                  e.jsxs('div', {
                                    className: 'flex items-center gap-2',
                                    ...
                                    e.jsxs('div', {
                                      ...
                                      children: [
                                        e.jsxs('div', {
                                          className: 'flex items-center gap-2',
                                          children: [
                                            e.jsx(mt, { size: 14, className: 'text-violet-500' }),
                                            e.jsx('span', {
                      ...
                      children: [
                        Ke > 0 &&
                          e.jsxs('div', {
                            ...
                            children: [
                              e.jsx(Z, { size: 18, className: 'shrink-0 mt-0.5' }),
                              e.jsxs('span', {
                                children: [
                                  e.jsx('b', { children: Ke.toLocaleString() }),
                          ...
                          e.jsxs('div', {
                            ...
                            children: [
                              e.jsxs('div', {
                                ...
                                children: [
                                  e.jsx(Ue, { size: 18, className: 'text-orange-600 shrink-0' }),
                                  e.jsxs('span', {
                                    className: 'text-sm text-orange-900',
                                    children: [
                                      'Destino: catálogo del canal ',
                                      e.jsx('b', { className: 'uppercase', children: z }),
                                      w
                                        ? e.jsxs(e.Fragment, {
                                            children: [
                                              ' · actualmente hay ',
                                              e.jsx('b', { children: w.actual.toLocaleString() }),
                                              ' N.V. en ',
                                              z.toUpperCase(),
                                              '.'
                                            ]
                              ...
                              w &&
                                ...
                                e.jsxs('div', {
                                  ...
                                  children: [
                                    ...
                                    e.jsxs('span', {
                                      children: [
                                        e.jsx('b', { children: w.count.toLocaleString() }),
                                        ' N.V. de este archivo ya existen en el canal ',
                                        e.jsx('b', { children: w.canalOtro }),
                                        z !== 'ptm' &&
                                          e.jsxs(e.Fragment, {
                                            children: [
                              ...
                              xe.count > 0 &&
                                e.jsxs('div', {
                                  className: `flex items-start gap-2 rounded-xl px-4 py-3 text-sm ${I ? 'bg-red-100 border-2 border-red-400 text-red-900' : 'bg-amber-50 border border-amber-200 text-amber-900'}`,
                                  children: [
                                    e.jsx(Z, {
                                      size: 18,
                                      className: `shrink-0 mt-0.5 ${I ? 'text-red-600' : 'text-amber-500'}`
                                    }),
                                    e.jsxs('span', {
                                      children: [
                                        I
                                        ...
                                        ' con un monto o número en vez de un nombre (columnas corridas o archivo de otro tipo).',
                                        ' ',
                                        'Ej: ',
                                        xe.ejemplos.join(' · '),
                                        '.',
                                  ...
                                  children: [
                                    ...
                                    e.jsxs('span', {
                                      children: [
                                        ...
                                        'En ',
                                        e.jsx('b', { children: z.toUpperCase() }),
                                        ' las N.V. suelen tener ',
                                        e.jsx('b', {
                                          children: ee == null ? void 0 : ee.principal
                                        }),
                                        ' dígitos, pero ',
                                        e.jsx('b', { children: me.count.toLocaleString() }),
                                        ' fila(s) traen otro largo (posible archivo de otro canal o dato mal pegado).',
                                        ' ',
                                        'Ej: ',
                                        me.ejemplos.join(' · '),
                                        '.',
                              ...
                              e.jsxs('label', {
                                ...
                                children: [
                                  ...
                                  e.jsxs('span', {
                                    ...
                                    children: [
                                      e.jsxs('b', {
                                        children: ['Reemplazar todo el canal ', z.toUpperCase()]
                                      }),
                                      ' con este archivo',
                                      e.jsxs('span', {
                        ...
                        e.jsxs('div', {
                          ...
                          children: [
                            e.jsx(re, { icon: Oe, label: 'Total', value: q.total, color: 'slate' }),
                            e.jsx(re, {
                              icon: Me,
                              label: 'Nuevas',
                              value: q.new,
                            ...
                            e.jsxs('button', {
                              onClick: It,
                              disabled: ne || I || (q.new === 0 && q.update === 0 && !(R && te)),
                              title: I
                              ...
                              children: [
                                ...
                                ne
                                  ? Te.total > 0
                                    ? `Lote ${Te.current}/${Te.total}...`
                                    : 'Preparando...'
                                  : R && te
                                    ? `Reemplazar ${z.toUpperCase()} (${q.new + q.update})`
                                    : `Confirmar (${q.new + q.update})`
                              ]
                            })
                          ]
                        }),
                                    ...
                                    o.slice(0, 500).map((t, s) => {
                                      const a = C[s] || 'new',
                                        ...
                                        g = {
                                          ...
                                          error: e.jsx(yt, { size: 13, className: 'text-red-500' })
                                        },
                                        x = {
                                          new: 'Nueva',
                                          update: 'Actualiza',
                                      ...
                                      return e.jsxs(
                                        'tr',
                                        {
                                          className: `${c[a]} hover:bg-slate-50/50 transition-colors`,
                                          children: [
                                            e.jsx('td', {
                                              className:
                  ...
                  J === 'done' &&
                    ...
                    e.jsx('div', {
                      ...
                      children: e.jsxs('div', {
                        ...
                        children: [
                          e.jsx('div', {
                            className: `w-16 h-16 mx-auto rounded-2xl flex items-center justify-center mb-5 ${T.success ? 'bg-emerald-50 text-emerald-500' : 'bg-red-50 text-red-500'}`,
                            children: T.success ? e.jsx(Me, { size: 32 }) : e.jsx(Z, { size: 32 })
                          }),
                          e.jsx('h2', {
                            className: 'text-lg font-extrabold text-slate-900 mb-2',
                            children: T.success ? 'Carga completada' : 'Error en la carga'
                          }),
                          e.jsx('p', {
                          ...
                          e.jsxs('div', {
                            className: `grid ${T.deduplicated > 0 ? 'grid-cols-4' : 'grid-cols-3'} gap-3 mb-6`,
                            children: [
                              e.jsxs('div', {
                          ...
                          ((dt = T.errorDetails) == null ? void 0 : dt.length) > 0 &&
                            e.jsxs('div', {
                              className:
                          ...
                          e.jsxs('button', {
                            ...
                            children: [
                              e.jsx(we, { size: 14, className: 'inline mr-2' }),
                              'Nueva carga'
                            ]
                          })
                        ]
  ...
  re = ({ icon: d, label: L, value: F, color: oe }) => {
    const Q = {
    ...
    return e.jsxs('div', {
      className: `flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold ${Q[oe]}`,
      children: [
        e.jsx(d, { size: 13 }),
