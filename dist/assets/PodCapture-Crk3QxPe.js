...
import { r as f } from './react-vendor-6aw4XXjH.js';
import { m as L, at as q, aD as U, X as F, c as B, t as p } from './ui-vendor-CTbhg6u_.js';
...
function le({ ordenId: r, onDone: e, onCancel: o }) {
  ...
  f.useEffect(() => {
    ...
    const C = (u) => {
        ...
        return { x: S.clientX - m.left, y: S.clientY - m.top };
      },
      E = (u) => {
        (u.preventDefault(), (h.current = !0));
        const m = C(u);
        (s.beginPath(), s.moveTo(m.x, m.y));
      },
      N = (u) => {
        if (!h.current) return;
        u.preventDefault();
        const m = C(u);
        (s.lineTo(m.x, m.y), s.stroke(), (_.current = !0));
      },
      k = () => {
        h.current = !1;
      };
  ...
  const O = () => {
    ...
    D = () => {
      if (!navigator.geolocation) return p.info('GPS no disponible');
      navigator.geolocation.getCurrentPosition(
        (t) => g(`${t.coords.latitude.toFixed(5)},${t.coords.longitude.toFixed(5)}`),
        () => p.error('No se pudo obtener ubicación')
      );
    },
    T = () =>
      new Promise((t) => {
    ...
    A = async () => {
      if (!a.trim()) return p.info('Indica quién recibió');
      y(!0);
      try {
        ...
        const d = await V(r, t);
        d.ok
          ? (p.success('Entrega registrada'), e == null || e())
          : p.error(d.error || 'No se pudo registrar');
      } catch (t) {
        p.error('Error al subir evidencia: ' + ((t == null ? void 0 : t.message) || ''));
      } finally {
        y(!1);
      }
    };
