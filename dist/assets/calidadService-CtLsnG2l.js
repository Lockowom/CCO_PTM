...
import { s as r, u as v, L as C, w as A } from './index-BSnpYyAh.js';
const w = () => {
    var e;
...
function W({ codigo: e, ubicacion: o, estadoLabel: a, tipo: i }) {
  return T({
    title: `⚠️ Calidad: ${a}`,
...
function Z() {
  const e = u();
  return _({
    mutationFn: async ({ cabecera: o, items: a, asignacionId: i = null }) => {
      const t = i ? 'crear_informe_monitoreo_asignacion' : 'crear_informe_monitoreo',
        n = { ...(i ? { p_asignacion_id: i } : {}), p_cabecera: R(o), p_items: a },
        { data: c, error: d } = await r.rpc(t, n);
      if (d) throw S(d);
      return c;
    },
...
async function ne({ informeId: e, itemId: o, blob: a, descripcion: i, user: t }) {
  ...
  const c = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    ...
    l = `${e}/${o || 'general'}/${w()}.${d}`,
    { error: m } = await r.storage.from(g).upload(l, a, { contentType: c, upsert: !1 });
  if (m) throw m;
    ...
    if (typeof a == 'string')
      try {
        ...
        if (i != null && i.message) {
          ...
          return (
            (t.code = i.code || (e == null ? void 0 : e.code)),
            (t.status = Number((e == null ? void 0 : e.status) || 409)),
            (t.lockedByName = i.locked_by_name || null),
            (t.lockedAt = i.locked_at || null),
            t
          );
        }
      } catch {}
...
async function Oe(e, o) {
  const { data: a, error: i } = await r.rpc('guardar_progreso_asignacion_calidad', {
    p_asignacion_id: e,
    p_progress_data: o || {}
  });
  if (i) throw S(i);
...
const M = [
  ...
  Te = ['Documento', 'Conteo', 'Inspección visual', 'Medición', 'Registro fotográfico', 'Sistema'],
  F = {
    BAJO: {
      emoji: '🟢',
      label: 'RIESGO BAJO',
...
async function Be({ tareaId: e, tipo: o, blob: a }) {
  ...
  const t = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    ...
    c = `salida/${e}/${o.toLowerCase()}-${w()}.${n}`,
    { error: d } = await r.storage.from(g).upload(c, a, { contentType: t, upsert: !1 });
  if (d) throw d;
  return c;
}
async function xe(e) {
...
async function Ue({ tareaId: e, tipo: o, blob: a }) {
  if ((a == null ? void 0 : a.size) > 7864320)
    throw new Error('La foto pesa demasiado y no se pudo comprimir. Prueba con otra (JPG/PNG).');
  const t = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    n =
    ...
    c = `ingreso/${e}/${String(o).toLowerCase()}-${w()}.${n}`,
    { error: d } = await r.storage.from(g).upload(c, a, { contentType: t, upsert: !1 });
  if (d) throw d;
  return c;
}
...
function Ge() {
  ...
  return _({
    mutationFn: async ({
      ...
      const { data: l, error: m } = await r.rpc('crear_tarea_salida_manual', {
        p_nv: o,
        p_skus: a || [],
        p_cliente: i ?? null,
        p_guia: t ?? null,
