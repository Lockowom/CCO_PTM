...
import { o as L, ao as q, aM as U, X as F, d as B, t as m } from './ui-vendor-DggzEJgL.js';
...
async function ae(r) {
  ...
  const { data: c, error: d } = await i.rpc('tms_orden_crear_desde_nv', { p_oper_id: a[0].id });
  return d ? { ok: !1, error: d.message } : c || { ok: !0 };
}
...
async function R(r, e, o, a) {
  const c = new Date().toISOString().replace(/[:.]/g, '-'),
    d = `ordenes/${r}/${e}-${c}.${a || 'jpg'}`,
    { error: g } = await i.storage
      .from(V)
      .upload(d, o, { upsert: !0, contentType: o.type || 'image/jpeg' });
  if (g) throw g;
  return d;
}
async function ce() {
  ...
  const { data: r } = await i.auth.getUser(),
    e = (d = r == null ? void 0 : r.user) == null ? void 0 : d.id;
  if (!e) return null;
...
function de({ ordenId: r, onDone: e, onCancel: o }) {
  ...
  const O = () => {
    ...
    D = async () => {
      ...
      y(!0);
      try {
        const t = { recibido_por: a.trim(), gps: d };
        ...
        l.ok
          ...
          : m.error(l.error || 'No se pudo registrar');
      } catch (t) {
