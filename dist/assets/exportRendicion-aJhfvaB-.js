...
import { s as b, _ as h, c as L, f as C } from './index-DpKQy1E-.js';
async function g(e) {
...
const A = {
  ...
  async upload(e, o, n, i, t) {
    ...
    return (
      ...
      e && a.append('token', e),
      a.append('view_token', o),
      a.append('report_id', n),
      a.append('item_id', i),
      a.append('file', t, t.name || 'evidencia.jpg'),
      g(a)
    );
  }
};
...
const T = {
  dashboard: (e = 200) => u('rendicion_admin_dashboard', { p_limit: e }),
  colaboradoresDetalle: () => u('rendicion_admin_colaboradores_detalle'),
  detalle: (e) => u('rendicion_admin_detalle', { p_id: e }),
  completarCabecera: (e, o) => u('rendicion_admin_completar_cabecera', { p_id: e, p_payload: o }),
  async eliminar(e) {
    const o = await u('rendicion_admin_eliminar', { p_id: e }),
  ...
  crearLink: (e, o, n) =>
    u('rendicion_admin_crear_link', {
      p_nombre: e,
      p_expires_at: o || null,
      p_max_submissions: n ? Number(n) : null
    }),
  toggleLink: (e, o) => u('rendicion_admin_toggle_link', { p_id: e, p_activo: o }),
  guardarCatalogo: (e, o) =>
    u('rendicion_admin_guardar_catalogo', {
      p_tipo: e,
      p_id: o.id || null,
  ...
  guardarColaboradorDetalle: (e) =>
    u('rendicion_admin_guardar_colaborador_detalle', {
      p_id: e.id || null,
      p_nombre: e.nombre,
      p_rut: e.rut || null,
...
async function x(e) {
  var l, p;
  if (!((l = e == null ? void 0 : e.type) != null && l.startsWith('image/')))
  ...
  ((i.width = Math.max(1, Math.round(o.width * n))),
    ...
    (p = o.close) == null || p.call(o));
  let t = 0.8,
    a;
  do ((a = await new Promise((c) => i.toBlob(c, 'image/jpeg', t))), (t -= 0.12));
  while ((a == null ? void 0 : a.size) > 1572864 && t >= 0.4);
  if (!a || a.size > 1572864) throw new Error('No fue posible reducir la foto a 1,5 MB.');
  return new File([a], `${e.name.replace(/\.[^.]+$/, '') || 'evidencia'}.jpg`, {
    type: 'image/jpeg'
  });
}
const N = /[\u200B-\u200D\uFEFF]/g,
  R = new RegExp('\\p{Cc}', 'gu'),
  F = ['Fondo por rendir', 'Rendición de gastos', 'Fondo fijo', 'Anticipo', 'Reembolso'],
  I = [
    'Factura',
    'Boleta',
    ...
    !Array.isArray(e.items) || e.items.length < 1 || e.items.length > 15
      ...
      : (e.items.forEach((n, i) => {
          ...
          (n.fecha || (o[`${t}.fecha`] = 'Selecciona la fecha.'),
            ...
            (!v(n.descripcion) || _(n.descripcion).length < 3) &&
            ...
            I.includes(n.tipo_documento) || (o[`${t}.tipo_documento`] = 'Selecciona el documento.'),
            n.numero_documento &&
...
function y(e, o = {}) {
  const n = o.logo || '',
    ...
    t = e.rendicion,
    a = [...(e.items || [])];
  for (; a.length < 15;) a.push(null);
  const l = [
      ['Nº', 'FECHA', 'Nº BOL/FAC', 'Detalle Descripción de gasto', 'CC', 'Categoría', 'Total'].map(
        (c) => ({ text: c, bold: !0, alignment: 'center', fillColor: '#f3f4f6' })
      ),
      ...a.map((c, r) =>
        c
          ? [
              String(r + 1),
              f(c.fecha),
              s(c.numero_documento || ''),
              s(c.descripcion),
              s(t.centro_costo_codigo),
              s(c.categoria_nombre),
              { text: m(c.monto), alignment: 'right' }
            ]
          : [String(r + 1), '', '', '', '', '', '']
      )
    ],
  ...
  for (let c = 0; c < i.length; c += 4)
    p.push({
      columns: i.slice(c, c + 4).map((r) => {
        var d;
        return {
          width: 120,
          stack: [
            { image: r.image, fit: [110, 145] },
            {
              text: `Gasto ${((d = e.items.find((D) => D.id === r.item_id)) == null ? void 0 : d.orden) || ''}`,
              alignment: 'center',
              margin: [0, 4]
            }
          ]
  ...
  return {
    ...
    content: [
      ...
      {
        table: { headerRows: 1, widths: [18, 52, 62, '*', 34, 65, 58], body: l },
        layout: {
          hLineWidth: () => 0.7,
          vLineWidth: () => 0.7,
          hLineColor: () => '#111827',
    ...
    footer: (c, r) => ({
      text: `${t.folio_texto || ''} · Página ${c} de ${r}`,
      alignment: 'center',
      fontSize: 7,
      color: '#64748b'
    })
...
async function B(e) {
  var r;
  const o = await h(
      () => import('./pdfmake-CkMY3Ap1.js').then((d) => d.p),
      __vite__mapDeps([0, 1])
    ),
    n = await h(() => import('./vfs_fonts-8ICcZKi6.js').then((d) => d.v), __vite__mapDeps([2, 1])),
    i = o.default || o,
    t = n.default || n,
    a = e.rendicion;
  i.vfs = ((r = t.pdfMake) == null ? void 0 : r.vfs) || t.vfs || i.vfs;
  const l = await w('/logo-ptm.png'),
    p = await Promise.all(
      (e.fotos || []).slice(0, 10).map(async (d) => ({ ...d, image: await w(d.url) }))
    ),
    c = y(e, { logo: l, evidenceImages: p.filter((d) => d.image) });
  await L(i.createPdf(c), `${a.folio_texto || 'Rendicion'}.pdf`, { label: 'Rendición de gastos' });
}
async function k(e) {
  const o = await h(() => import('./xlsx-B2eTCt_Q.js'), []),
    ...
    t = [
      ...
      ['UNIDAD', s(i.unidad || '')],
      ['CENTRO DE COSTO', s(i.centro_costo_nombre || '')],
      ['TÉCNICO', s(i.solicitante_tecnico_nombre || i.tecnico || '')],
      ['DETALLE', s(i.detalle || '')],
      [],
  ...
  (e.items || []).forEach((r, d) =>
    t.push([
      d + 1,
      new Date(`${r.fecha}T12:00:00`),
      s(r.numero_documento || ''),
      s(r.descripcion),
      s(i.centro_costo_codigo),
      s(r.categoria_nombre),
      Number(r.monto)
    ])
  );
  const a = 12;
  for (; t.length < a - 1 + 15;) t.push([t.length - (a - 2), '', '', '', '', '', '']);
  t.push(['', '', '', '', '', 'Total General', { f: `SUM(G${a}:G${a + 14})` }]);
  const l = n.utils.aoa_to_sheet(t);
  ((l['!cols'] = [
    { wch: 7 },
    { wch: 14 },
    { wch: 18 },
    ...
    (l['!merges'] = [n.utils.decode_range('C1:G1'), n.utils.decode_range('B9:G9')]));
  for (let r = 0; r < 7; r += 1) {
    const d = l[n.utils.encode_cell({ r: a - 2, c: r })];
    d &&
      (d.s = {
        font: { bold: !0 },
  ...
  for (let r = a - 1; r < t.length; r += 1) {
    const d = l[n.utils.encode_cell({ r, c: 6 })];
    d && (d.z = '$ #,##0');
  }
  const p = n.utils.book_new();
  n.utils.book_append_sheet(p, l, 'Rendición');
  const c = n.write(p, { type: 'array', bookType: 'xlsx' });
  await C(
    new Blob([c], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
    `${i.folio_texto || 'Rendicion'}.xlsx`,
    { label: 'Rendición Excel' }
  );
}
export { I as T, k as a, T as b, _ as c, B as d, $ as e, F as f, v as h, x as o, A as r, P as v };
