...
import { s as t, h as b } from './index-BU_ov3s4.js';
const N = () => {
    var e;
...
async function G(e, o = !1) {
  const a = new AbortController(),
    r = setTimeout(() => a.abort(), 15e3);
  try {
    const { data: i, error: n } = await t
...
async function j(e, o = '') {
  const { data: a, error: r } = await t.rpc('calidad_lotes_series', {
    p_codigo: e,
    p_query: o || '',
    p_limit: 300
  });
...
function k({ codigo: e, ubicacion: o, estadoLabel: a, tipo: r }) {
  return S({
    title: `⚠️ Calidad: ${a}`,
    body: `${e}${o ? ` en ${o}` : ''} dictaminado como ${a}. Requiere movimiento/gestión por Inventario.`,
    payload: { codigo_producto: e, ubicacion: o || '', tipo: r }
  });
}
async function V(e) {
...
function z() {
  ...
  return _({
    mutationFn: async ({ cabecera: o, items: a }) => {
      const { data: r, error: i } = await t.rpc('crear_informe_monitoreo', {
        p_cabecera: o,
        p_items: a
      });
      if (i) throw i;
...
function $() {
  ...
  return _({
    mutationFn: async ({ informeId: o, estado: a }) => {
      const { error: r } = await t.from('tms_monitoreo_informes').update({ estado: a }).eq('id', o);
      if (r) throw r;
    },
    onSuccess: () => e.invalidateQueries({ queryKey: ['monitoreo_informes'] })
  });
...
function J() {
  const e = u();
  return _({
    mutationFn: async ({ informeId: o, cabecera: a, items: r }) => {
      const { data: i, error: n } = await t.rpc('actualizar_informe_monitoreo', {
        p_informe_id: o,
        p_cabecera: a,
        p_items: r
      });
...
function W() {
  const e = u();
  return _({
    mutationFn: async ({ informeId: o, cabecera: a, reporte: r, hallazgos: i }) => {
      let n = o;
      if (n) {
        const { error: s } = await t
          .from('tms_monitoreo_informes')
          .update({ ...a, reporte: r, total_items: i.length })
          .eq('id', n);
        if (s) throw s;
      } else {
        ...
        const { data: E, error: A } = await t
          .from('tms_monitoreo_informes')
          .insert({ ...a, numero: s, tipo_informe: 'DANOS', reporte: r, total_items: i.length })
          .select()
          .single();
        if (A) throw A;
        n = E.id;
      ...
      if (O.length) {
        ...
        const { error: E } = await t.from('tms_monitoreo_items').delete().in('id', O);
        if (E) throw E;
      }
...
async function Z({ informeId: e, itemId: o, blob: a, descripcion: r, user: i }) {
  ...
  if (m) throw m;
  const { data: O } = t.storage.from(g).getPublicUrl(d),
    { data: s, error: p } = await t
      .from('tms_monitoreo_evidencias')
      .insert({
        informe_id: e,
        item_id: o || null,
        imagen_url: O.publicUrl,
        storage_path: d,
        descripcion: r || null,
...
function C() {
  return f({
    ...
    queryFn: async () => {
      ...
      if (o) throw o;
      const a = { PENDIENTE: 0, EN_PROCESO: 1, NO_CONFORME: 2, CONFORME: 3 };
      return (e || []).sort((r, i) => (a[r.estado] ?? 9) - (a[i.estado] ?? 9));
    }
  });
}
function se() {
...
function ce() {
  ...
  return _({
    mutationFn: async (o) => {
      const { data: a, error: r } = await t.rpc('firmar_certificado', { p_tarea_id: o });
      if (r) throw r;
      return a;
    },
...
function _e() {
  const e = u();
  return _({
    mutationFn: async ({ skus: o, motivo: a, prioridad: r }) => {
      const { data: i, error: n } = await t.rpc('crear_asignacion_calidad', {
        p_skus: o || [],
        p_motivo: a ?? null,
        p_prioridad: r || 'NORMAL'
      });
      if (n) throw n;
      return i;
    },
...
function me() {
  const e = u();
  return _({
    mutationFn: async ({ asignacionId: o, informeId: a, estado: r }) => {
      const { data: i, error: n } = await t.rpc('resolver_asignacion_calidad', {
        p_asignacion_id: o,
        p_informe_id: a ?? null,
        p_estado: r || 'RESUELTA'
      });
...
function pe() {
  ...
  return _({
    mutationFn: async (o) => {
      const { data: a, error: r } = await t.rpc('anular_asignacion_calidad', {
        p_asignacion_id: o
      });
      if (r) throw r;
      return a;
...
const R = [
  ...
  ye = ['Documento', 'Conteo', 'Inspección visual', 'Medición', 'Registro fotográfico', 'Sistema'],
  v = {
    BAJO: {
      emoji: '🟢',
      label: 'RIESGO BAJO',
...
function Ie(e) {
  ...
  let r = 0;
  (a.pallet === 'Malo' ? (r += 2) : a.pallet === 'Regular' && (r += 1),
    a.film === 'Incorrecto' && (r += 1),
    ['golpes', 'deformada', 'humedad'].forEach((n) => {
      a[n] === 'Sí' && (r += 2);
    }),
    Object.entries(e || {}).forEach(([n, c]) => {
      n !== '_extras' && (c == null ? void 0 : c.estado) === 'NO' && (r += 2);
    }));
  const i = r >= 4 ? 'ALTO' : r >= 1 ? 'MEDIO' : 'BAJO';
  return { key: i, score: r, ...v[i] };
}
function Ne(e) {
...
const Se = [
  ...
  Ce = (e, o) => {
    const a = Number(String(e ?? '').replace(',', '.')),
      r = Number(String(o ?? '').replace(',', '.'));
    return !Number.isFinite(a) || !Number.isFinite(r) || a <= 0
      ? null
      : Math.abs(r - a) / a <= D
        ? 'CONFORME'
        : 'REVISAR';
  ...
  return (e == null ? void 0 : e.resultado) === 'CONFORME'
    ...
    : (e == null ? void 0 : e.resultado) === 'NO_CONFORME'
      ? (e.disposicion || '') === 'Despachar con salvedades (autorizado)'
        ? { key: 'NARANJA', ...y.NARANJA }
        : { key: 'ROJO', ...y.ROJO }
      : { key: 'PENDIENTE', ...y.PENDIENTE };
}
async function Re({ tareaId: e, tipo: o, blob: a }) {
  if ((a == null ? void 0 : a.size) > 7864320)
  ...
  const i = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    ...
    c = `salida/${e}/${o.toLowerCase()}-${N()}.${n}`,
    { error: l } = await t.storage.from(g).upload(c, a, { contentType: i, upsert: !1 });
  if (l) throw l;
  return c;
}
async function ve(e) {
...
async function Te({ tareaId: e, tipo: o, blob: a }) {
  if ((a == null ? void 0 : a.size) > 7864320)
    throw new Error('La foto pesa demasiado y no se pudo comprimir. Prueba con otra (JPG/PNG).');
  const i = a != null && a.type && a.type.startsWith('image/') ? a.type : 'image/jpeg',
    n =
    ...
    c = `ingreso/${e}/${String(o).toLowerCase()}-${N()}.${n}`,
    { error: l } = await t.storage.from(g).upload(c, a, { contentType: i, upsert: !1 });
  if (l) throw l;
  return c;
}
...
function Le() {
  ...
  return _({
    mutationFn: async (o) => {
      const { data: a, error: r } = await t.rpc('eliminar_tarea_calidad', { p_tarea_id: o });
      if (r) throw r;
      return a;
    },
    onSuccess: () => {
...
function Me() {
  ...
  return _({
    mutationFn: async (o) => {
      const { data: a, error: r } = await t.rpc('eliminar_asignacion_calidad', {
        p_asignacion_id: o
      });
      if (r) throw r;
      return a;
...
function xe() {
  const e = u();
  return _({
    mutationFn: async ({ codigo: o, nombre: a, estado: r, esDestino: i, activo: n, orden: c }) => {
      const { data: l, error: d } = await t.rpc('guardar_bodega_softland', {
        p_codigo: o,
        p_nombre: a,
        p_estado: r || 'DISPONIBLE',
        p_es_destino: !!i,
        p_activo: n !== !1,
        p_orden: c ?? 100
      });
...
function Ue() {
  ...
  return _({
    mutationFn: async (o) => {
      const { data: a, error: r } = await t.rpc('eliminar_bodega_softland', { p_codigo: o });
      if (r) throw r;
      return a;
    },
    onSuccess: () => e.invalidateQueries({ queryKey: ['bodegas_softland'] })
...
function je(e, o, a) {
  return f({
    ...
    staleTime: 3e4,
    queryFn: async () => {
      const { data: r, error: i } = await t.rpc('trazabilidad_producto', {
        p_codigo: e,
        p_partida: o ?? null,
        p_ubicacion: a ?? null
      });
      if (i) throw i;
      return r || { codigo: e, estado_actual: null, eventos: [] };
    }
  });
}
function Qe() {
...
function ze() {
  const e = u();
  return _({
    mutationFn: async ({ accionId: o, resolucion: a, estado: r }) => {
      const { data: i, error: n } = await t.rpc('resolver_accion_calidad', {
        p_accion_id: o,
        p_resolucion: a ?? '',
        p_estado: r || 'RESUELTA'
      });
...
function $e() {
  ...
  return _({
    mutationFn: async (o) => {
      const { data: a, error: r } = await t.rpc('anular_accion_calidad', { p_accion_id: o });
      if (r) throw r;
      return a;
    },
    onSuccess: () => e.invalidateQueries({ queryKey: ['calidad_acciones'] })
...
function Je() {
  ...
  return _({
    mutationFn: async (o) => {
      const { data: a, error: r } = await t.rpc('accion_a_ticket_pv', { p_accion_id: o });
      if (r) throw r;
      return a;
    },
    onSuccess: () => {
...
function He() {
  ...
  return _({
    mutationFn: async ({ accionId: o, referencia: a }) => {
      const { data: r, error: i } = await t.rpc('accion_correo_enviado', {
        p_accion_id: o,
        p_referencia: a || null
      });
      if (i) throw i;
...
function Ye() {
  ...
  return _({
    mutationFn: async ({ accionId: o, referencia: a }) => {
      const { data: r, error: i } = await t.rpc('accion_registrar_referencia', {
        p_accion_id: o,
        p_referencia: a
      });
      if (i) throw i;
