import { u as bt, d as ft, c as gt, j as e } from './query-vendor-CojWQiBV.js';
import { r as x } from './react-vendor-CA7EHQ1X.js';
import { u as jt, f as S, w as fe, s as M, g as Nt } from './index-CXl7fTg4.js';
import { u as vt } from './useRealtimeTable-DyyDuzgP.js';
import { u as wt } from './useBarcodeScanner-CdHcw5UK.js';
import {
  u as yt,
  f as ge,
  ag as Ct,
...
import { l as Ht } from './logUpload-8hNdGiD4.js';
...
const Kt = ['3-4', '1HCX20', '1HCX40', '2HCX40', 'LCL', 'AEREO'],
  ...
  Ce = (i) => {
    ...
    const j = y.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{2,4})$/);
    if (j) {
      const [, I, u, C] = j,
        _ = `${C.length === 2 ? `20${C}` : C}-${String(u).padStart(2, '0')}-${String(I).padStart(2, '0')}`,
...
async function Wt(i, y) {
  for (let j = 0; j < i.length; j += ve) {
    const R = i
      ...
      { error: I } = await M.from('tms_recepcion_items').insert(R);
    if (I) throw new Error(`Falló el bloque ${Math.floor(j / ve) + 1}: ${I.message}`);
  }
}
async function Ze(i) {
  const y = [];
  ...
  for (;;) {
    const R = j + we - 1,
      { data: I, error: u } = await M.from('tms_recepcion_items')
...
const cs = () => {
    ...
    const { user: i } = jt(),
      ...
      [_e, Ee] = x.useState(!1),
      [le, J] = x.useState(''),
      [Q, oe] = x.useState(null),
      [Se, Z] = x.useState(!1),
      ne = x.useRef(null),
      et = x.useRef(null),
      [Re, Y] = x.useState(H),
      ee = 'cco_recepcion_import_draft',
      [D, te] = x.useState(null),
      [ce, ie] = x.useState(!1),
      [se, de] = x.useState(''),
      [ke, z] = x.useState(!1);
    (x.useEffect(() => {
    ...
    const Ie = () => {
      ...
      tt = () => {
        xe() &&
          D &&
          (A((t) => ({ ...t, ...D.header })),
          P(Array.isArray(D.items) ? D.items : []),
          U(D.editingId ?? null),
          z(!0),
          B('form'));
      },
      ...
      ae = x.useMemo(() => c.slice(0, Math.min(c.length, Re)), [c, Re]);
    (x.useEffect(() => {
      if (c.length === 0) {
        Y(H);
        return;
      }
      Y((t) => (c.length <= H ? c.length : Math.min(c.length, Math.max(t, H))));
    }, [c.length]),
      yt(
        () => {
          Gt.from(I.current, {
      ...
      x.useEffect(() => {
        C === 'form' && !u && (B('dashboard'), U(null));
      }, [C, u]),
      vt('tms_recepciones', [['recepciones']]));
    const { data: q = [], isLoading: at } = ft({
        queryKey: ['recepciones'],
        queryFn: async () => {
          const { data: t, error: s } = await M.from('tms_recepciones')
            .select(
      ...
      G = !!(f.search || f.estado || f.desde || f.hasta),
      L = x.useMemo(() => {
        const t = T,
          s = t.length,
          a = t.filter((E) => E.estado === 'EN_REVISION').length,
          r = t.filter((E) => E.estado === 'COMPLETADO').length,
          l = t.reduce((E, w) => E + (w.cant_bultos || 0), 0),
          d = t.reduce((E, w) => E + (w.pallets_usados || 0), 0),
          b = s > 0 ? (d / s).toFixed(1) : 0,
          N = s > 0 ? Math.round(l / s) : 0,
          o = q.length,
          n = q.reduce((E, w) => E + (w.pallets_usados || 0), 0),
          v = o > 0 ? (n / o).toFixed(1) : 0;
        return {
          total: s,
          enRevision: a,
          completados: r,
      ...
      F = x.useMemo(() => {
        ...
        d.forEach((o) => {
          const [n, v] = o.mes.split('-');
          o.label = `${s[parseInt(v) - 1]} ${n.slice(2)}`;
        });
        const b = {};
        t.forEach((o) => {
        ...
        const N = Object.entries(b).map(([o, n]) => ({ name: o, value: n }));
        return { porProveedor: r, porMes: d, porTipo: N };
      }, [T]),
      me = gt({
        mutationFn: async () => {
          if (!u)
          ...
          const s = [
            ...
            a = String(c.length);
          if (_) {
            const { error: r } = await M.from('tms_recepciones')
              .update({
              ...
              .eq('id', _);
            if (r) throw r;
            const { error: l } = await M.from('tms_recepcion_items').delete().eq('recepcion_id', _);
            if (l) throw l;
          } else {
            const { data: r, error: l } = await M.from('tms_recepciones')
              .insert({
                fecha_recepcion: p.fecha_recepcion,
                proveedor: p.proveedor.toUpperCase(),
                oc: p.oc || null,
      ...
      Ae = async (t, s) => {
        if (
          ...
          try {
            const { data: a, error: r } = await M.rpc('eliminar_recepcion_completa', {
              p_id: t,
...
¿Agregarla de todos modos? Quedará marcada como duplicada.`)
        ...
        (P((r) => [
          ...
          O({ reff: '', cantidad: 1, serie: '', lote: '', box: '', fecha_vencimiento: '' }),
          z(!0),
          h.success('Ítem agregado y guardado ✓', { duration: 1500 }),
          Te(t)
            .then((r) => {
              r && P((l) => l.map((d) => (d._id === a ? { ...d, descripcion: r } : d)));
            })
      ...
      lt = async (t) => {
        ...
        const a = await Promise.all(s.map(async (l) => [l, await Te(l)])),
...
¿Quieres agregarlas de todos modos? Quedarán marcadas como duplicadas.`)
      ...
      ot = (t) => {
        ...
        s.scrollTop + s.clientHeight >= s.scrollHeight - 180 &&
          Y((r) => Math.min(c.length, r + ye));
      },
      De = (t) => {
        const s = (t || [])
          .map((w) =>
            Array.isArray(w) ? w.map((k) => String(k ?? '').trim()) : [String(w ?? '').trim()]
          )
          .filter((w) => w.some((k) => k !== ''));
        ...
        a.forEach((w, k) => {
          ...
          Object.entries(Xt).forEach(([W, he]) => {
            !r[W] && he.includes(re) && ((r[W] = k), (l = !0));
          });
        });
        const d = l ? s.slice(1) : s,
          b = (m.reff || '').trim().toUpperCase(),
          N = String(m.lote || '').trim(),
          o = String(m.box || '').trim(),
          n = Ce(m.fecha_vencimiento);
        let v = 0;
        return {
          rows: d
            .map((w) => {
              const k = (be) => {
                  if (l && r[be] !== void 0) return w[r[be]] || '';
                  const Ve = Qt.indexOf(be);
                  return (Ve >= 0 && w[Ve]) || '';
                },
                re = String(k('serie') || '').trim(),
                W = String(k('reff') || b)
                  .trim()
                  .toUpperCase(),
                he = String(k('lote') || N).trim(),
                ut = String(k('box') || o).trim(),
                ht = Ce(k('fecha_vencimiento') || n),
                $e = String(k('cantidad') || '').trim(),
                Ue = ($e && parseInt($e, 10)) || 1;
              return !re || !W
                ? ((v += 1), null)
                : {
                    reff: W,
              ...
              (a = d[0]) != null && a.includes('	')
                ...
                : (r = d[0]) != null && r.includes(';')
                  ...
                  : (l = d[0]) != null && l.includes(',')
                    ? ','
                    : null,
            N = d.map((v) => (b ? v.split(b).map((E) => E.trim()) : [v])),
            { rows: o, skipped: n } = De(N);
          if (!o.length) {
                  ...
                  (s = o[0]) != null && s.includes('	')
                    ...
                    : (a = o[0]) != null && a.includes(';')
                      ...
                      : (r = o[0]) != null && r.includes(',')
                        ? ','
                        : null;
              l = o.map((v) => (n ? v.split(n).map((E) => E.trim()) : [v]));
            }
            const { rows: d, skipped: b } = De(l);
            if (!d.length) {
      ...
      it = () => {
        const t = (m.reff || '').trim().toUpperCase(),
          ...
          b = S.book_new(),
          N = S.json_to_sheet(l);
        N['!cols'] = [{ wch: 18 }, { wch: 24 }, { wch: 18 }, { wch: 14 }, { wch: 12 }, { wch: 10 }];
        const o = S.json_to_sheet(d);
        ((o['!cols'] = [{ wch: 18 }, { wch: 110 }]),
          S.book_append_sheet(b, N, 'MODELO SERIES'),
          S.book_append_sheet(b, o, 'GUIA'));
        const n = `Modelo_Carga_Series_${t || 'REFF_BASE'}.xlsx`;
        (fe(b, n), h.success(`Modelo descargado: ${n}`));
      },
      dt = (t) => {
        (P((s) => s.filter((a, r) => r !== t)), z(!0));
      },
      Te = async (t) => {
        try {
      ...
      Pe = (t) => {
        j({
          onScan: (s) => {
            const a = s.trim();
            (O((r) => ({ ...r, [t]: a })), h.success(`${t.toUpperCase()} escaneado: ${a}`));
          },
          onError: (s) => h.error(s)
        });
      },
        ...
        if (xe())
          try {
            ...
            (A({
              ...
              P((s || []).map((a) => ({ ...a, _id: a.id }))),
              Y(Math.min((s == null ? void 0 : s.length) || 0, H) || H),
              U(t.id),
              z(!1),
              X(null),
              B('form'));
      ...
      pt = (t, s) => {
        ...
        const N = `Recepcion_${t.proveedor}_${t.fecha_recepcion || 'sin-fecha'}.xlsx`;
        (fe(r, N), h.success(`Archivo descargado: ${N}`));
      },
      mt = () => {
        if (T.length === 0) {
        ...
        ((a['!cols'] = [
          ...
          fe(s, `Recepciones_${new Date().toLocaleDateString('en-CA')}.xlsx`),
                  ...
                  e.jsxs(e.Fragment, {
                    children: [
                      e.jsxs('button', {
                        ...
                        children: [e.jsx(Ct, { size: 14 }), ' Exportar']
                      }),
                      u
                        ? e.jsxs('button', {
                            onClick: () => {
                            ...
                            children: [e.jsx(qe, { size: 14 }), ' Nueva Recepción']
                          })
                        : e.jsx('span', {
                            className:
                  ...
                  e.jsxs(e.Fragment, {
                    children: [
                      ...
                      e.jsxs('button', {
                        ...
                        className:
                          'px-3 py-2 bg-white border border-slate-300 hover:bg-slate-50 text-slate-600 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors',
                        children: [e.jsx(_t, { size: 14 }), ' Volver']
                      })
                    ]
                  })
              ]
                    ...
                    e.jsxs('div', {
                      ...
                      children: [
                        e.jsxs('p', {
                          ...
                          children: [e.jsx(He, { size: 15 }), ' Tienes una recepción sin terminar']
                        }),
                        e.jsxs('p', {
                          className: 'text-xs text-amber-700 mt-0.5',
                          children: [
                            ((Me = D.items) == null ? void 0 : Me.length) || 0,
                            ' ítem(s)',
                            (Be = D.header) != null && Be.proveedor
                              ? ` · ${D.header.proveedor}`
                              : '',
              ...
              e.jsxs('div', {
                ...
                children: [
                  e.jsx(K, {
                    label: 'Recepciones',
                    value: L.total,
                    sub: G ? `de ${L.globalTotal}` : null
                  }),
                  e.jsx(K, { label: 'En Revisión', value: L.enRevision, accent: 'text-amber-600' }),
                  e.jsx(K, { label: 'Completados', value: L.completados, accent: 'text-teal-600' }),
                  e.jsx(K, { label: 'Total Bultos', value: L.totalBultos.toLocaleString() }),
                  e.jsx(K, {
                    label: 'Prom. Pallets',
                    value: L.promPallets,
                    accent: 'text-slate-800',
                    sub: G ? `global: ${L.globalPromPallets}` : null
                  }),
                  e.jsx(K, { label: 'Prom. Bultos', value: L.promBultos })
                ]
              }),
              e.jsxs('div', {
                className:
                ...
                e.jsxs('div', {
                  ...
                  children: [
                    ...
                    e.jsxs('span', {
                      children: [
                        'Mostrando ',
                        e.jsx('b', { className: 'text-slate-700', children: T.length }),
                        ' de ',
                        q.length,
                        f.search &&
                          e.jsxs('span', {
                    ...
                    ? e.jsx('div', {
                        ...
                        children: e.jsxs('table', {
                          ...
                          children: [
                            ...
                            e.jsx('tbody', {
                              children: F.porProveedor.map((t, s) => {
                                const a = Math.max(...F.porProveedor.map((l) => l.bultos)),
                          ...
                          children: at
                            ? e.jsx('tr', {
                                children: e.jsxs('td', {
                                  ...
                                  children: [
                                    e.jsx(Ye, { size: 20, className: 'animate-spin mx-auto mb-2' }),
                                    'Cargando...'
                                  ]
                                })
                              })
                            : T.length === 0
                              ...
                              : T.map((t, s) => {
                                  const a = Je[t.estado] || Je.PENDIENTE;
                                  return e.jsxs(
                                    'tr',
                                    {
                  ...
                  e.jsxs('div', {
                    ...
                    children: [
                      ...
                      e.jsxs('div', {
                        ...
                        children: [
                          ...
                          e.jsx('input', {
                            ...
                            value: m.reff,
                            onChange: (t) =>
                              O((s) => ({ ...s, reff: t.target.value.toUpperCase() })),
                            className:
                          ...
                          e.jsxs('div', {
                            children: [
                              ...
                              e.jsx('input', {
                                type: 'number',
                                value: m.cantidad,
                                onChange: (t) => O((s) => ({ ...s, cantidad: t.target.value })),
                                className:
                          ...
                          e.jsxs('div', {
                            children: [
                              ...
                              e.jsx('input', {
                                type: 'text',
                                value: m.box,
                                onChange: (t) => O((s) => ({ ...s, box: t.target.value })),
                                className:
                      ...
                      e.jsxs('p', {
                        ...
                        children: [e.jsx(Ne, { size: 14 }), ' CAMPOS CON ESCÁNER DE CÁMARA']
                      }),
                      e.jsxs('div', {
                        className: 'mb-5',
                        children: [
                          ...
                          e.jsxs('div', {
                            ...
                            children: [
                              e.jsx('input', {
                                type: 'text',
                                value: m.serie,
                                onChange: (t) => O((s) => ({ ...s, serie: t.target.value })),
                                className:
                          ...
                          e.jsxs('div', {
                            ...
                            children: [
                              e.jsx('input', {
                                type: 'text',
                                value: m.lote,
                                onChange: (t) => O((s) => ({ ...s, lote: t.target.value })),
                                className:
                                  'flex-1 p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-base font-mono outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all',
                      ...
                      e.jsxs('div', {
                        ...
                        children: [
                          ...
                          e.jsx('input', {
                            ...
                            onChange: (t) =>
                              O((s) => ({ ...s, fecha_vencimiento: t.target.value })),
                            className:
                              'w-full p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-base font-bold outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all'
                          })
                        ]
                          ...
                          e.jsxs('button', {
                            ...
                            children: [
                              ...
                              e.jsx('div', {
                                ...
                                children: _e ? e.jsx(zt, { size: 18 }) : e.jsx($t, { size: 18 })
                              })
                            ]
                          }),
                          _e &&
...
26010500018	LOTE-1	2026-12-31	B1	1`
                      ...
                      e.jsxs('button', {
                        ...
                        children: [e.jsx(qe, { size: 22 }), ' AGREGAR ÍTEM']
                      })
                    ]
                  }),
                  e.jsxs('div', {
                              ...
                              ce &&
                                e.jsxs(
                                  'span',
                                  {
                                    ...
                                    children: [e.jsx(Ge, { size: 12 }), ' Guardado ', se]
                                  },
                                  se
                                )
                            ]
                        ...
                        children:
                          c.length === 0
                            ? e.jsxs('div', {
                                ...
                                children: [
                                  e.jsx(Ut, { size: 40, className: 'mx-auto mb-2 opacity-30' }),
                                              ...
                                              children: [
                                                ...
                                                a &&
                                                  e.jsxs('span', {
                                                    ...
                                                    children: [e.jsx(We, { size: 9 }), ' DUPLICADA']
                                                  })
                                              ]
                                            }),
                                            e.jsx('td', {
                      ...
                      c.length > ae.length &&
                        e.jsxs('div', {
                          ...
                          children: [
                            ...
                            e.jsxs('button', {
                              type: 'button',
                              onClick: () => Y((t) => Math.min(c.length, t + ye)),
                              className:
                                'text-emerald-700 hover:text-emerald-800 transition-colors',
                              children: ['Cargar ', Math.min(ye, c.length - ae.length), ' más']
                            })
                          ]
                        }),
                      c.length > 0 &&
                        ...
                        e.jsx('div', {
                          ...
                          children: e.jsx('button', {
                            ...
                            children: me.isPending
                              ? e.jsxs(e.Fragment, {
                                  children: [
                                    e.jsx(Ye, { size: 22, className: 'animate-spin' }),
                                    ' GUARDANDO...'
                                  ]
                                })
                              : e.jsxs(e.Fragment, {
                    ...
                    e.jsxs('div', {
                      ...
                      children: [
                        ...
                        e.jsxs('div', {
                          ...
                          children: [
                            e.jsxs('span', { children: ['📅 ', Fe(g.fecha_recepcion)] }),
                  ...
                  children: [
                    u &&
                      e.jsxs('button', {
                        ...
                        children: [e.jsx(ge, { size: 16 }), ' EDITAR']
                      }),
                    e.jsxs('button', {
                      ...
                      children: [e.jsx(qt, { size: 16 }), ' DESCARGAR EXCEL']
                    }),
                    u &&
                      e.jsxs('button', {
                        onClick: () => Ae(g.id, g.proveedor),
                        className:
                        ...
                        children: [e.jsx(je, { size: 16 }), ' ELIMINAR']
                      })
                  ]
                }),
                e.jsx('div', {
                ...
                e.jsxs('div', {
                  ...
                  children: [
                    e.jsxs('span', {
                      ...
                      children: [
                        'Total ítems: ',
                        ((ze = g.items) == null ? void 0 : ze.length) || 0
                      ]
                    }),
                    e.jsxs('span', {
  ...
  K = ({ label: i, value: y, accent: j, sub: R }) =>
    e.jsxs('div', {
      ...
      children: [
        ...
        e.jsx('p', {
          className: `text-lg sm:text-2xl font-bold tabular-nums ${j || 'text-slate-800'}`,
          children: y
        }),
        R &&
