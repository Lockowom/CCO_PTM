...
import { d as K, u as X, c as M, j as e } from './query-vendor-CojWQiBV.js';
import { j as le, r as N, L as W } from './react-vendor-CA7EHQ1X.js';
import {
  ba as re,
  aC as ce,
  R as O,
...
import { s as P, _ as J, u as Y, x as ge, i as ee } from './index-BU_ov3s4.js';
  ...
  return K({
    ...
    queryFn: async () => {
      ...
      for (let i = 0; ; i += 1e3) {
        ...
        if (g) throw g;
        if ((d.push(...(o || [])), !o || o.length < 1e3)) break;
      }
      return d;
    },
    staleTime: 6e4
...
async function he({
  ...
  return { agregados: y.length, omitidos: u };
}
function fe() {
...
function ve() {
  ...
  return M({
    mutationFn: async (s) => {
      let m = 0;
      for (let d = 0; d < s.length; d += 800) {
        const i = s.slice(d, d + 800),
  ...
  return M({
    mutationFn: async (s) => {
      ...
      for (let i = 0; i < s.length; i += 800) {
        ...
        d += (o == null ? void 0 : o.inserted) || 0;
      }
      return { insertados: d, total: s.length };
    },
    onSuccess: () => {
      (a.invalidateQueries({ queryKey: ['analisis_resumen'] }),
        a.invalidateQueries({ queryKey: ['analisis_codigos'] }));
...
async function Ne(a) {
  const s = await J(
      () => import('./index-BU_ov3s4.js').then((d) => d.a6),
      __vite__mapDeps([0, 1, 2, 3, 4, 5])
    ),
    m = s.read(await a.arrayBuffer());
  for (const d of m.SheetNames) {
    const i = s.utils.sheet_to_json(m.Sheets[d], { header: 1, defval: '' });
    for (let r = 0; r < Math.min(i.length, 10); r++) {
      ...
      if (g === -1 || c === -1) continue;
      const p = (b) => o.findIndex((_) => b.test(_)),
        f = p(/^producto$|descrip/),
        y = p(/medida|u\.?m\b/),
        u = p(/reserva/),
        l = p(/transitoria/),
        v = p(/consign/),
        C = p(/stock.*total|total.*stock/),
        k = new Map();
...
async function ye(a) {
  ...
  for (const d of m.SheetNames) {
    const i = s.utils.sheet_to_json(m.Sheets[d], { header: 1, defval: '' });
    for (let r = 0; r < Math.min(i.length, 10); r++) {
      const o = i[r].map((u) => String(u || '').toLowerCase()),
        g = o.findIndex((u) => /c[oó]d/.test(u) && /prod/.test(u)),
        c = o.findIndex((u) => /activo/.test(u));
      if (g === -1 || c === -1) continue;
      const p = o.findIndex((u) => /descrip|^producto$/.test(u)),
        f = o.findIndex((u) => /medida|u\.?m\b/.test(u)),
        y = new Map();
      for (let u = r + 1; u < i.length; u++) {
        const l = String(i[u][g] ?? '').trim();
        l &&
          y.set(l, {
            codigo_producto: l,
            producto: p >= 0 ? String(i[u][p] ?? '').trim() : '',
            unidad_medida: f >= 0 ? String(i[u][f] ?? '').trim() : '',
            activo: /^s[ií]/i.test(String(i[u][c] ?? '').trim())
          });
      }
...
function Ie() {
  const { user: a, hasPermission: s } = Y(),
    d =
  ...
  N.useEffect(() => {
    const p = i.get('tab');
    p && p !== r ? o(p) : !p && r !== 'resumen' && o('resumen');
  }, [i]);
  const g = (p) => ge(s, '/inventory/analisis', p),
    c = B.includes(r) && g(r) ? r : B.find((p) => g(p)) || 'resumen';
  return e.jsxs('div', {
    ...
    children: [
      ...
      c === 'resumen' && e.jsx(ke, { canCargar: d }),
      c === 'antiguos' &&
...
function ke({ canCargar: a }) {
  const { data: s = {}, isLoading: m, refetch: d, isFetching: i } = te(),
    r = ve(),
    o = je(),
    g = N.useRef(null),
    c = N.useRef(null),
...
const q = [];
function I({ filtro: a, titulo: s, alerta: m = !1, conDiagnostico: d = !1 }) {
  ...
  const { data: c = q, isLoading: p } = se(a, i),
    [f, y] = N.useState(q),
    [u, l] = N.useState(() => new Set()),
    v = () =>
      ee({
          ...
          e.jsxs('div', {
            ...
            children: [
              ...
              e.jsxs('button', {
                ...
                children: [e.jsx(Z, { size: 13 }), ' Excel']
              })
            ]
          })
        ]
      }),
      e.jsx(oe, { sel: u, setSel: l, rows: c }),
      e.jsx(ne, { rows: c, isLoading: p, conDiagnostico: d, sel: u, setSel: l, onProcesadas: y })
    ]
  });
}
function oe({ sel: a, setSel: s, rows: m }) {
  const [d, i] = N.useState(null);
  ...
  return e.jsxs('div', {
    ...
    children: [
      ...
      d === 'correo' && e.jsx(Se, { rows: r, onClose: () => i(null) })
    ]
  });
}
function Se({ rows: a, onClose: s }) {
  ...
  N.useEffect(() => {
    try {
      localStorage.setItem('analisis_correo_dest', d);
    } catch {}
  }, [d]);
  const [r, o] = N.useState(''),
    ...
    p = (n, t, x) => c((w) => ({ ...w, [n]: { ...w[n], [t]: x } })),
    f = a.filter((n) => {
      var t, x;
      return (
        ((t = g[n.codigo]) == null ? void 0 : t.tipo) === 'P' &&
        !((x = g[n.codigo]) != null && x.venc)
      );
    }),
    y = new Date().toLocaleDateString('es-CL'),
    u = (n) => (n ? n.split('-').reverse().join('-') : 'POR DEFINIR'),
    ...
    v = [
      ...
      ...a.flatMap((n) => {
        ...
        return [
          ...
          `   Crear con: ${t.tipo}${t.tipo === 'P' ? ` · Vencimiento: ${u(t.venc)}` : ''}${t.nuevo ? ` · Código nuevo: ${t.nuevo}` : ' · Código nuevo: POR CREAR'}`
        ];
      }),
...
`),
    ...
    k = (n) => {
      const t = g[n.codigo] || {};
      return `<tr><td style="${C};font-family:monospace">${n.codigo}</td><td style="${C}">${(n.producto || '').replace(/</g, '&lt;')}</td><td style="${C}">${n.unidad_medida || ''}</td><td style="${C};text-align:right">${h(n.disponible)}</td><td style="${C};text-align:right">${h(n.stock_total)}</td><td style="${C};text-align:center;font-weight:bold;color:${t.tipo === 'P' ? '#c2410c' : '#0369a1'}">${t.tipo}</td><td style="${C};text-align:center">${t.tipo === 'P' ? u(t.venc) : '—'}</td><td style="${C};font-family:monospace;font-weight:bold">${t.nuevo || 'POR CREAR'}</td></tr>`;
    },
...
<table style="border-collapse:collapse;font-size:13px"><tr>${['Código antiguo', 'Descripción', 'UM', 'Disponible', 'Stock Total', 'Crear con', 'Vencimiento', 'Código nuevo'].map((n) => `<th style="padding:4px 8px;border:1px solid #94a3b8;background:#f1f5f9;text-align:left">${n}</th>`).join('')}</tr>${a.map(k).join('')}</table>${r.trim() ? `<p><b>Observaciones:</b> ${r.trim().replace(/</g, '&lt;')}</p>` : ''}<p>Atentamente,<br/>${(m == null ? void 0 : m.nombre) || ''}<br/><span style="color:#94a3b8;font-size:11px">Generado desde CCO · Análisis de Códigos</span></p>`,
    ...
    S = () => {
      if (!_()) return;
      const n = `mailto:${encodeURIComponent(d)}?subject=${encodeURIComponent(l)}&body=${encodeURIComponent(v)}`;
      if (n.length > 7500) {
                          ...
                          children: [
                            ...
                            e.jsx('td', {
                              ...
                              children: e.jsxs('select', {
                                value: t.tipo,
                                onChange: (x) => p(n.codigo, 'tipo', x.target.value),
                              ...
                              children:
                                t.tipo === 'P'
                                  ? e.jsx('input', {
                                      type: 'date',
                                      value: t.venc,
                                      onChange: (x) => p(n.codigo, 'venc', x.target.value),
                            ...
                            e.jsx('td', {
                              ...
                              children: e.jsx('input', {
                                value: t.nuevo,
                                onChange: (x) => p(n.codigo, 'nuevo', x.target.value),
...
function we({ tipo: a, rows: s, onClose: m }) {
  const d = fe(),
    ...
    [p, f] = N.useState('Actualización de código a nomenclatura P/S'),
    y = s.filter((l) => l.ps_equivalente).length,
    u = async () => {
      try {
        const l = await d.mutateAsync({
          tipo: a,
          rows: s,
          accion: r,
          obs: i ? p : '',
...
function ne({ rows: a, isLoading: s, conDiagnostico: m, sel: d, setSel: i, onProcesadas: r }) {
  const [o, g] = N.useState(200),
    ...
    l = N.useMemo(() => {
      ...
      return [...t].sort((j, E) =>
        Ae.has(x)
          ? (Number(j[x] || 0) - Number(E[x] || 0)) * w
          : String(j[x] ?? '').localeCompare(String(E[x] ?? ''), 'es') * w
      );
    }, [a, c, f]);
  N.useEffect(() => {
    r == null || r(l);
  ...
  const v = N.useMemo(
      () =>
        l.reduce(
          (t, x) => ({
            disponible: t.disponible + Number(x.disponible || 0),
            reserva: t.reserva + Number(x.reserva || 0),
            transitoria: t.transitoria + Number(x.transitoria || 0),
            consignacion: t.consignacion + Number(x.consignacion || 0),
            stock_total: t.stock_total + Number(x.stock_total || 0)
          }),
    ...
    b = l.length > 0 && l.every((t) => d.has(t.codigo)),
    _ = () =>
      i((t) => {
        const x = new Set(t);
        return (b ? l.forEach((w) => x.delete(w.codigo)) : l.forEach((w) => x.add(w.codigo)), x);
      }),
    T = (t) =>
      i((x) => {
        const w = new Set(x);
    ...
    S = ({ valor: t, onChange: x, opts: w, label: j }) =>
      e.jsxs('select', {
        value: t,
        onChange: (E) => {
          (x(E.target.value), g(200));
        },
      ...
      a.length > 0 &&
        e.jsxs('div', {
          ...
          children: [
            e.jsx(S, {
              valor: c.estado,
              onChange: (t) => p((x) => ({ ...x, estado: t })),
              label: 'Estado: todos',
              opts: u.estado.map((t) => [t, t])
            }),
            e.jsx(S, {
              valor: c.activo,
              onChange: (t) => p((x) => ({ ...x, activo: t })),
              label: 'Activo: todos',
              opts: u.activo.map((t) => [t, t])
            }),
            e.jsx(S, {
              valor: c.um,
              onChange: (t) => p((x) => ({ ...x, um: t })),
              label: 'UM: todas',
              opts: u.um.map((t) => [t, t])
            }),
            e.jsx(S, {
              valor: c.dup,
              onChange: (t) => p((x) => ({ ...x, dup: t })),
                ...
                e.jsx('tbody', {
                  children: C.map((t) =>
                    e.jsxs(
                      'tr',
                      {
                        className: `border-b border-slate-50 hover:bg-slate-50/60 ${d.has(t.codigo) ? 'bg-orange-50/60' : ''}`,
                        children: [
                          e.jsx('td', {
                            className: 'py-1.5 pr-2',
                            children: e.jsx('input', {
                              type: 'checkbox',
                              checked: d.has(t.codigo),
                              onChange: () => T(t.codigo)
                            })
                          }),
                          e.jsx('td', {
...
function Te() {
  const [a, s] = N.useState(''),
    [m, d] = N.useState('');
  N.useEffect(() => {
    const u = setTimeout(() => s(m.trim()), 400);
          ...
          e.jsxs('div', {
            ...
            children: [
              ...
              e.jsxs('button', {
                ...
                children: [e.jsx(Z, { size: 13 }), ' Exportar libro completo (6 hojas)']
              })
            ]
          })
        ]
      }),
      e.jsx(oe, { sel: p, setSel: f, rows: i }),
      e.jsx(ne, { rows: i, isLoading: r, conDiagnostico: !1, sel: p, setSel: f, onProcesadas: c })
    ]
  });
}
