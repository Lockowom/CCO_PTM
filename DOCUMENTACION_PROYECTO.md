...
> **Versión:** 1.4.26 | **Última actualización:** 2026-06-12
...
|---|---|---|
| 1.4.26 | 2026-06-12 | **CIBERSEGURIDAD: blindaje de formularios + auditoría de acciones + hardening** (cuatro frentes, a pedido del usuario; auditoría previa en BD live + frontend). **(A) Límites de entrada en formularios (anti-XSS/payloads gigantes/datos malformados)**: nueva librería de validación con **zod** — `src/lib/validation/limits.js` (constantes de longitud/rango reutilizables), `schemas.js` (esquemas por formulario) y `validateForm.js` (helper con toast). Se aplicó doble capa (HTML `maxLength`/`type=number min/max` + validación zod en submit) y guarda **anti doble-submit** en: `Users.jsx`, `Roles.jsx`, `Drivers.jsx`, `Reception.jsx`, `CubingRegistry.jsx` (peso/dimensiones ahora con rango), `Entry.jsx`, `RoutePlanning.jsx`, `Tickets.jsx`, `Login.jsx`; y límite por celda en la importación masiva (`DataImport.jsx`). Nueva dep: `zod`. **(B) Control de acceso (accesos indebidos)**: `Cleanup.jsx` (borrado masivo) ahora **verifica permisos en el componente** (gate `ADMIN`/`es_admin_delegado`/`manage_cleanup`) además del guard de ruta y el `is_admin()` del RPC (defensa en profundidad). **(C) Auditoría "quién hizo qué"** (migración `016_audit_and_hardening.sql`, aplicada y verificada en BD live): nueva tabla **`tms_auditoria`** (RLS: `SELECT` solo `is_admin()`; sin escritura directa), trigger genérico `tms_audit_row()` en `AFTER INSERT/UPDATE/DELETE` de `tms_usuarios` y `tms_roles`, y traza de la limpieza masiva dentro de `clean_operational_data`. **(D) Hardening**: `public.update_updated_at_column()` con `search_path` fijo (cierra lint 0011); `REVOKE EXECUTE` a `authenticated` en las funciones de trigger `tms_audit_row`/`tms_usuarios_freeze_privileged` (no deben ser invocables vía `/rpc`); cabeceras de seguridad (`X-Frame-Options`/`CSP`/`nosniff`/`Referrer-Policy`/`Permissions-Policy`) en `netlify.toml` (replicar en Render si aplica); eliminado el código legacy `localStorage.getItem('currentUser')` de `AuthContext.jsx` (reduce superficie post-XSS). **Sin hallazgos de XSS/SQLi** (no hay `dangerouslySetInnerHTML` ni SQL crudo; el query builder de Supabase parametriza). Verificado: tests 45/45, build OK, trigger de auditoría registra cambios (prueba en transacción revertida), advisor de `update_updated_at_column` desaparecido. **Pendiente (decisión)**: el RLS permisivo de 25 tablas operativas es por diseño (empleados de confianza); evaluar convertir RPC `SECURITY DEFINER` de negocio a `INVOKER` y activar "Leaked Password Protection" en Supabase Auth. |
| 1.4.25 | 2026-06-12 | **SEGURIDAD CRÍTICA: cierre de escalada de privilegios en RBAC**. Auditoría de roles/permisos detectó (verificado en BD live) dos vectores explotables por cualquier usuario autenticado. **Modelo real**: el permiso efectivo sale de `tms_roles.permisos_json` (array jsonb por rol; lo lee `AuthContext.loadRoleConfig`), el rol del usuario de `tms_usuarios.rol`, y admin = `rol='ADMIN'` o `es_admin_delegado` (`private.is_admin()`). La tabla `tms_roles_permisos`+`tms_permisos` está **abandonada** (el editor solo escribe `permisos_json`). **Vector 1 (drift de RLS)**: las políticas `*_select_auth`/`auth_all_*` de `tms_usuarios`, `tms_roles`, `tms_permisos`, `tms_roles_permisos`, `tms_modules_config` estaban en producción como `FOR ALL USING(true)` (debían ser `FOR SELECT` según `003_rls_policies.sql`); al combinarse por OR anulaban las políticas `is_admin()` → cualquiera podía `UPDATE tms_usuarios SET rol='ADMIN'`. **Vector 2 (auto-ascenso)**: `usuarios_update_self_or_admin` (necesaria para presence tracking) no restringía columnas → un usuario podía cambiar su propia `rol`/`es_admin_delegado`. **Fix (migración `014_fix_rbac_privilege_escalation.sql`, aplicada a BD live)**: (a) restauradas las 5 políticas a `FOR SELECT` + escritura solo `is_admin()`; (b) trigger `BEFORE UPDATE` `tms_usuarios_freeze_privileged()` que congela `rol`/`es_admin_delegado`/`activo`/`email`/`auth_uid`/`id_usuario` para no-admins (deja libres los campos de presencia/perfil). Verificado con prueba de impersonación: no-admin no puede ascender ni editar permisos de su rol, y el presence sigue funcionando. **Frontend**: `Roles.jsx` ahora genera el ID de rol con `slugifyRoleId` (trim + sin acentos + sin `_` sobrantes + chequeo de unicidad) — corrige la causa de los roles basura `OPERARIO_3_`/`SUPERVISOR_`; `hasPermission` (`AuthContext`) ahora honra `es_admin_delegado` (alineado con el backend). **Saneo de datos** (migración `015`, ver fila siguiente): consolidados los roles basura `OPERARIO_3_`/`SUPERVISOR_`/`TRANSPORTE`. **Pendiente (requiere decisión)**: depurar la tabla normalizada abandonada (`tms_roles_permisos`/`tms_permisos`). |
| roles-consolidacion | 2026-06-12 | **SANEO DE DATOS: consolidación de roles basura** (migración `015_consolidar_roles_basura.sql`, aplicada y verificada en BD live; decisiones tomadas con el usuario). Origen: IDs derivados de nombres con espacios sobrantes antes del fix de `slugifyRoleId` (1.4.25). **(1) `OPERARIO_3_` → rol legítimo**: se renombró a `OPERARIO_3` (nombre `"OPERARIO 3"`) y se propagó el nuevo ID a todas las columnas de rol del esquema — `tms_usuarios` (2 usuarios), `tms_usuarios_activos`, `tms_accesos` (71 filas de log) y `tms_roles_permisos` — porque `tms_usuarios.rol` **no es FK formal**. **(2) `SUPERVISOR_` → se mantiene como rol independiente**: solo se limpió el nombre (`"Supervisor "` → `"Supervisor"`); ID conservado. **(3) `TRANSPORTE` (0 usuarios) → borrado**; `CONDUCTOR` queda como rol único de conductores (el FK `tms_roles_permisos_rol_id_fkey` `ON DELETE CASCADE` limpió sus permisos). Verificado: 0 usuarios huérfanos (ningún `tms_usuarios.rol` apunta a un rol inexistente). Sin cambios de frontend (ningún ID de rol estaba hardcodeado en el código). |
