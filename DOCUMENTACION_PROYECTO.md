...
|---|---|---|
| 1.55.40 | 2026-07-18 | **TMS · Torre de Control — detalle en panel lateral (no overlay) en escritorio**. El detalle de la orden dejaba a oscuras toda la pantalla y tapaba los botones *Nueva orden*/*Actualizar*. Ahora en escritorio (`lg+`) se abre como **columna lateral en línea** junto a la lista (sticky, sin fondo oscuro, aprovechando el ancho): la fila seleccionada se resalta y el panel hace scroll interno con `max-h`. En móvil se mantiene como cajón (overlay) pero con velo más tenue (`bg-slate-900/30`). Hook `matchMedia('(min-width:1024px)')` para alternar. Sin cambios de lógica ni de BD. Build + 77/77 tests OK. |
| 1.55.39 | 2026-07-18 | **TMS · Torre de Control — rediseño visual**. Se rehízo la presentación de `src/pages/TMS/Transporte.jsx` (sin cambios de lógica ni de BD): encabezado con badge naranja, tarjeta **Todos** + KPIs de estado con mejor contraste (número grande, punto de color, gris tenue cuando el conteo es 0 y borde/anillo del color del estado al estar activo), la lista de órdenes vive dentro de un **panel blanco** con buscador integrado, filas con barra de color por estado, chip de patente, chevron al pasar el cursor y contador al pie; estado vacío ilustrado. Detalle del drawer con valores en negrita. Build + 77/77 tests OK. |
| 1.55.38 | 2026-07-17 | **TMS — fix de tipos chofer/ruta + puente usuario↔conductor**. **Migración `107`**: `tms_conductores.id` y `tms_rutas.id` son **uuid**, pero `tms_transporte_ordenes.conductor_id/ruta_id` se habían creado como `bigint` (Fase 1); con la tabla vacía se cambian a `uuid` y se ajusta el cast de `tms_orden_asignar` (sin esto, asignar chofer/ruta fallaba y el filtro "Mis órdenes" nunca calzaba). **Frontend**: `miConductorId()` resuelve el puente correcto `auth.uid()` → `tms_usuarios.id` → `tms_conductores.user_id` (el FK apunta a `tms_usuarios.id`, no al `auth_uid`). Build + 77/77 tests OK. |
| 1.55.37 | 2026-07-17 | **TMS (Transporte) — Fase 3: app del chofer con foto y firma (POD)**. **Migración `106`**: bucket **privado** `tms-pod` (evidencia servida con URL firmada). Componente reutilizable `src/pages/TMS/PodCapture.jsx`: captura **foto** (cámara), **firma** en canvas, **GPS** (geolocalización) y "recibido por"; sube la evidencia a Storage y llama `tms_orden_pod` (que marca la N.V. Entregado). Se usa tanto en el drawer de la Torre de Control como en la nueva pantalla **Mi Ruta** (`/tms/pda`, `src/pages/TMS/MiRuta.jsx`): hoja de ruta móvil del chofer con sus entregas activas (filtro "Mis órdenes" vía `tms_conductores.user_id = auth.uid()`), tarjetas grandes para avanzar de estado y botón **Entregar (POD)**. `tmsService`: `subirEvidencia`/`urlEvidencia`/`miConductorId`. Ruta + permisos + menú de Mi Ruta wireados. Build + 77/77 tests OK. |
| 1.55.36 | 2026-07-17 | **TMS (Transporte) — Fase 2: módulo funcional (Torre de Control)**. Frontend nativo del transporte propio sobre el cimiento de la Fase 1. Ruta `/tms/control`, sección **TMS** en el menú (Operaciones WMS). Página `src/pages/TMS/Transporte.jsx` + `src/services/tmsService.js`: **Torre de Control** con KPIs por estado (Pendiente/Programado/En Carga/Despachado/En Ruta/Entregado) que filtran, buscador y lista de órdenes; **Nueva orden** desde una N.V. (`tms_orden_crear_desde_nv`); **drawer** por orden que, según el estado, ofrece **Asignar** vehículo+chofer+fecha, **avanzar** de estado (transiciones validadas en BD), **POD** (recibido por + GPS con geolocalización → marca la N.V. Entregado), **incidencias** (crear/resolver; Reprogramar vuelve a *programado*) y **cancelar**. **Migración `105`**: permisos `view_tms`/`manage_tms`/`supervise_tms` (casillas en Roles) + fila en `tms_modules_config` (tarjeta en Vistas). Checklist de módulo completo (App/permissions/Navbar/modules.js/migración). Build + 77/77 tests OK. |
| 1.55.35 | 2026-07-17 | **TMS (Transporte) — Fase 1: cimiento de datos (reconstrucción desde 0)**. A partir del flujo modelado por el usuario, **migración `104`** crea el esquema limpio del transporte propio (sin tocar `tms_control_despacho`, que tiene 4.066 filas de Consultas): tablas nuevas `tms_vehiculos`, `tms_transporte_ordenes` (máquina de estados **pendiente_asignacion → programado → en_carga → despachado → en_ruta → entregado → cerrado**, + `cancelado`, con sellos de fecha y campos POD foto/firma/GPS/hora) y `tms_transporte_incidencias` (Retraso/Accidente/Cliente Ausente/Dirección Incorrecta/Producto Dañado). RLS de solo lectura para `authenticated`; escrituras por **RPC gateada** (`_tms_puede_gestionar` = admin o `manage_tms`/`manage_panel`; `anon` sin EXECUTE): `tms_orden_crear_desde_nv` (idempotente, folio `OT-AAAA-####`), `tms_orden_asignar` (vehículo+chofer+ruta → programado), `tms_orden_transicion` (valida transiciones permitidas), `tms_orden_pod` (registra prueba de entrega y **marca la N.V. como Entregado**), `tms_incidencia_crear`/`tms_incidencia_resolver` (Reprogramar vuelve a *programado*). Reusa `tms_conductores`/`tms_rutas` por referencia. Semilla de 3 vehículos demo. Sin frontend aún (Fase 2). Build + 77/77 tests OK. |
| 1.55.34 | 2026-07-17 | **Se retira el módulo Dashboard General** (`/dashboard`). Se elimina la página `src/pages/Dashboard.jsx`, su ruta, permiso (`ROUTE_PERMISSIONS`), grupo de permisos (`view_dashboard`/`view_kpis`) y entradas de `APP_MODULES`/`APP_ROUTES`, la sección *Core* del `Navbar` y su etiqueta en `usePresence`. **Inicio repunteado**: como `/dashboard` era el landing por defecto, ahora lo resuelve **`SmartRedirect`** en `/` (primera ruta accesible por permisos); el **admin** aterriza en **`/panel`** (Panel PTM). Se cambiaron todos los defaults de `/dashboard` a `/` (AuthContext, logo del Navbar) y `/panel` (SmartRedirect, Views). **Migración `103`**: borra la tarjeta `dashboard` de `tms_modules_config`, sus permisos, y limpia (`NULL`) el `landing_page` de roles que apuntaran a `/dashboard`. Build + 77/77 tests OK. |
| 1.55.33 | 2026-07-17 | **Se retiran los módulos TMS (Transporte) y Outbound** (se reconstruirán desde 0). Frontend: se eliminan las 7 páginas TMS (`src/pages/TMS/*`) y las 5 de Outbound (`src/pages/Outbound/*`), sus rutas en `App.jsx`, el `ROUTE_PRIORITY`, `ROUTE_PERMISSIONS`, los grupos de permisos y las entradas de `APP_MODULES`/`APP_ROUTES` en `modules.js`, las categorías del `Navbar`, y las referencias en `CommandPalette` y `usePresence`. **Migración `102`**: borra las tarjetas `tms`/`outbound` de `tms_modules_config` (desaparecen de Admin → Vistas) y sus permisos huérfanos de `tms_permisos`. La **PDA Operativa de Bodega** (`/mobile/pda`, `WarehousePDA`) NO era transporte → se **reubicó bajo Inventario** (permisos `view_stock`/`manage_inventory` conservados y movidos a ese grupo). **No se tocó ninguna tabla de datos** (son compartidas; el nuevo esquema se definirá al construir lo nuevo). Build + 77/77 tests OK. |
| 1.55.32 | 2026-07-17 | **Menú de Inventario usable sin reducir el zoom**. El desplegable de Inventario tenía ~17 accesos (Traspasos, Mapa de Calor, Ubicaciones, 6 de Conteo, 6 de Análisis, Carteles, Insumos) que se salían de la pantalla y obligaban a hacer *zoom out*. Ahora el menú tiene **alto máximo con scroll** (`max-h-[78vh] overflow-y-auto`) y, cuando una categoría tiene **más de 8 accesos**, se muestra en **2 columnas** (panel más ancho). Además las filas se **compactaron** (menos padding, se quitó el subtítulo "Acceso Directo" e ícono más chico) para que quepan muchos más de un vistazo. Aplica a todas las categorías del navbar de escritorio. Sin migración. Build + 77/77 tests OK. |
| 1.55.31 | 2026-07-17 | **Cuadro de Novedades ahora AUTOMÁTICO (adiós al mantenimiento manual)**. Para que nunca vuelva a quedarse atrás, `src/constants/releaseNotes.js` deja de escribirse a mano: un **prebuild** (`scripts/gen_release_notes.js`, enganchado en `package.json` → corre antes de `vite build`) parsea el Changelog (§15) de este documento y genera `releaseNotes.generated.js` con las **15 versiones más recientes** (título = primer texto en negrita de la fila; cuerpo = resto, sin markdown, recortado). `releaseNotes.js` solo re-exporta ese archivo. Resultado: mantener el changelog al día (ya obligatorio) mantiene el cuadro de Novedades al día, sin pasos extra. Documentado en `CLAUDE.md`. Sin migración. Build + 77/77 tests OK. |
| 1.55.30 | 2026-07-17 | **Cuadro de Novedades al día**. El modal de Novedades (`NovedadesModal` ← `src/constants/releaseNotes.js`) muestra las notas cuya versión sea mayor a la última vista; como la última nota era **1.36.0** y la app iba en **1.55.x**, había "dejado de actualizarse" (no es bug: el archivo se llena a mano y quedó rezagado). Se agregaron entradas en lenguaje de usuario para **1.55.0 / 1.55.25 / 1.55.30** (Panel PTM con datos reales + Carga Masiva blindada; Ingresar más potente: buscar cualquier estado, PillNav, transportistas, Excel, fechas bloqueadas; Panel de Insumos + consulta pública + tráfico + auditoría con nombre). Se documentó la regla de mantener `releaseNotes.js` al día en `CLAUDE.md`. Sin migración. Build + 77/77 tests OK. |
| 1.55.29 | 2026-07-17 | **Tráfico de la Consulta pública (Admin → Monitor), sin datos personales**. Antes no había forma de ver cuánta gente usa `/consulta`. **Migración `101`**: tabla `tms_consulta_metricas` (contador **diario**: `total`, `con_resultado`, `sin_resultado`; **no** guarda IP ni el número consultado) y `buscar_nv_publico` la incrementa por día (zona Chile) tras pasar el rate-limit → solo cuentan consultas válidas. RLS: lectura para `authenticated` (agregado no sensible), `anon` sin acceso directo (lo escribe la RPC `SECURITY DEFINER`). **Frontend**: componente `src/components/TraficoConsulta.jsx` montado en **Admin → Monitor** — KPIs (total 7/30/90 días, hoy, % con resultado, sin resultado) + gráfico de barras por día. Sin permiso nuevo (la pantalla Monitor ya es admin). Build + 77/77 tests OK. |
| 1.55.28 | 2026-07-17 | **Panel de Insumos: correo de solicitud más profesional (con tabla)**. `armarCorreoSolicitud` genera ahora (1) **texto plano** con una **tabla ASCII alineada** (columnas #, Insumo, Medida/Código, Quedan, Estado, Solicitar) + saludo/cierre formales, para el `mailto:`; y (2) **cuerpo HTML** con una **tabla real** (encabezados naranja, filas alternadas, badge de estado con color) que se copia al portapapeles. El botón **Copiar tabla** usa `ClipboardItem` con `text/html` (al pegar en Outlook/Gmail se ve la tabla con formato) y respaldo a texto plano; la **vista previa** del modal renderiza ese HTML. Sin migración. Build + 77/77 tests OK. |
| 1.55.27 | 2026-07-17 | **Nuevo módulo: Panel de Insumos (Inventario)**. Panel didáctico (`/inventory/insumos`) para ver el stock de insumos de embalaje/despacho con **semáforo** (🟢 OK · 🟡 por acabarse · 🔴 crítico) según dos umbrales por ítem. **Migración `100`**: tabla `tms_insumos` (categoría CAJAS/PALLETS/OTROS, nombre, medida, código PTM, unidad, cantidad, `umbral_bajo`/`umbral_critico`, orden), RLS de solo lectura para `authenticated` y escrituras por **RPC gateada** (`_insumos_puede_gestionar` = admin o `manage_insumos`/`manage_inventory`): `insumos_guardar`, `insumos_set_cantidad`, `insumos_eliminar` (`anon` sin EXECUTE). Sembrada con la lista PTM (6 cajas C1–C6 con medida+código, 8 de pallets/embalaje, 5 varios). **Frontend** `src/pages/Inventory/Insumos.jsx` + `src/services/insumosService.js`: tarjetas por categoría con edición inline de cantidad (+/−) y umbrales (para `manage_insumos`), resumen por estado, filtro "solo por reponer" y búsqueda. **Solicitud por correo**: se seleccionan los insumos (o "seleccionar por reponer") y se genera un correo (`mailto:` con asunto y cuerpo prellenados + destinatario recordado en localStorage + copiar al portapapeles) pidiendo la reposición con cantidades. Permisos `view_insumos`/`manage_insumos` (módulo Inventario, casillas en Roles). Checklist de módulo completo (App/permissions/Navbar/modules.js/migración). Build + 77/77 tests OK. |
| 1.55.26 | 2026-07-17 | **Ingresar: faltaban transportistas en el selector (p.ej. Transfarma)**. `opciones()` armaba la lista de transportistas con `select ... .limit(5000)` sobre `tms_operaciones`, pero **PostgREST corta en 1.000 filas**: tomaba solo la primera página (sin orden) y los transportistas que solo aparecían más allá quedaban fuera (Transfarma tiene 28 N.V. pero no salía). Ahora la lista se arma con la **unión** del **catálogo maestro** `tms_panel_transportistas` (activos, mantenido en Configuración → Transportistas) **+** todos los transportistas realmente usados en `tms_operaciones` **paginando** con `.order('id').range()` (sin el tope de 1.000), ordenada con `localeCompare('es')`. Además, en el drawer y en el alta, el `<select>` de transportista **siempre incluye el valor ya asignado** a la N.V. aunque no estuviera en el catálogo (no se pierde la asignación). Sin migración. Build + 77/77 tests OK. |
| 1.55.25 | 2026-07-17 | **Ingresar/Buscar: ahora se pueden encontrar N.V. de CUALQUIER estado (no solo activas)**. La lista de *Buscar* solo carga las ~50 N.V. **activas** (En Proceso/Shipping/Currier/En Ruta), así que las **Entregado/NULA/Rechazado** (1.905) no aparecían **ni buscándolas por número** — el buscador solo filtraba en memoria la lista activa. Ahora, al escribir ≥2 caracteres, se dispara una **búsqueda contra toda la tabla** `tms_operaciones` (nueva `buscarOperaciones` en `ingresarService`: por nº N.V., cliente, vendedor, guía, factura o transportista, cualquier estado, dedup por canal:nv, `.limit(300)`, orden por `fecha_estado`), con debounce de 350 ms y spinner. Sin término, se mantiene la vista de activas con su filtro de estados (PillNav). Guardar/eliminar desde un resultado de búsqueda actualiza también esa lista. De paso, `listaActivas` gana `.order('id')` en su paginación (evita filas perdidas/duplicadas si algún día hay >1.000 activas). Sin migración. Build + 77/77 tests OK. |
| 1.55.24 | 2026-07-17 | **Detalle N.V. (Ingresar): F. Aprobación Real y F. Compromiso no editables**. En el drawer de detalle, la **F. Aprobación Real** pasa a **solo lectura** (gris, `readOnly`, etiqueta "(no editable)"), igual que la **F. Compromiso** (que ya era automática). Ambas fechas quedan protegidas contra edición manual desde el detalle; siguen siendo editables solo las fechas operativas (Facturación / Despacho). Sin migración. Build + 77/77 tests OK. |
| 1.55.23 | 2026-07-17 | **Eliminar N.V. (Ingresar) restringido a admin + Angélica**. El borrado destructivo de N.V. ya no lo puede hacer cualquier usuario con `manage_panel`: queda limitado a **administradores** (o delegados) y a **María Angélica** (`angelica@ptm.cl`). **Migración `099`**: gate `_panel_puede_eliminar_nv()` (SECURITY DEFINER, `anon` sin EXECUTE) = `is_admin` **o** usuario activo con email en la allowlist; `eliminar_nv` pasa a usar este gate (antes usaba `_panel_puede_escribir`). Como los permisos son **por rol** y Angélica es `SUPERVISOR_` (compartido con otros), la allowlist va **por usuario** (array de emails, ampliable con un `CREATE OR REPLACE`), no por rol. **Frontend**: el botón *Eliminar NV* del drawer solo se muestra a esos perfiles (`puedeEliminarNV`, mismo criterio que el gate). Crear/editar/cambiar estado sigue con `manage_panel`. **Bug corregido de la 098**: `_panel_actor()` cruzaba `tms_usuarios` por `id` en vez de `auth_uid` → el nombre del operador en la Auditoría nunca resolvía y caía al UUID; ahora usa `auth_uid`. Build + 77/77 tests OK. |
| 1.55.22 | 2026-07-17 | **PillNav también en el filtro de estados de la lista (Buscar)**. Los chips de filtro por estado de la pestaña *Buscar* (con su contador, ej. "En Proceso 8 · Shipping 23 · En Ruta 20") pasan al mismo **PillNav animado** de estados. `PillNavEstado` gana: (1) **badge de contador** (`item.count`) junto al label, con color claro en el pill activo/hover; (2) variante **`inline`** (pills al tamaño de su contenido, no estirados a lo ancho como en el alta) para que el filtro se vea compacto como antes. Sigue siendo un toggle (clic en el estado activo vuelve a "Todos"). Sin migración. Build + 77/77 tests OK. |
| 1.55.21 | 2026-07-17 | **Auditoría del Panel completada e integrada (registro real por operador)**. La bitácora ya estaba conectada a `tms_operaciones_log`, pero (1) la columna `actor` guardaba el **UUID crudo** de `auth.uid()` → la pantalla habría mostrado un identificador ilegible en "Operador". **Migración `098`**: helper `_panel_actor()` (SECURITY DEFINER, `anon` sin EXECUTE) que resuelve **nombre → email → uid** desde `tms_usuarios`, y las 3 RPCs de escritura (`guardar_nv`, `cambiar_estado_nv`, `eliminar_nv`) ahora guardan ese **nombre** en `actor` al registrar create/update/estado/delete. (2) **Frontend** (`PanelConfigReal.jsx`, tab Auditoría): filtros por **acción** (Creación/Edición/Estado/Eliminación) + **búsqueda** por operador/N.V. + botón Actualizar, **badges de color** por acción, contador y un **empty-state explicativo** (la bitácora está vacía porque aún no hay escrituras `cco`: las 1.970 filas son del import `sheet`; se llenará al operar desde Ingresar). Se eliminó el archivo muerto `screens/PanelConfig.jsx` (versión mock con datos de ejemplo, sin usar; la ruta usa `config/PanelConfigReal.jsx`). La Auditoría sigue **fusionada en Configuración** (solo admin, permiso `manage_panel`), como el original. Build + 77/77 tests OK. |
| 1.55.20 | 2026-07-17 | **PillNav para el selector de Estado en Ingresar (alta + detalle)**. El dropdown de Estado se reemplaza por un **PillNav animado** (estilo React Bits, la misma animación "pill" del selector de Canal): componente `PillNavEstado.jsx` (GSAP) con track claro y donde **cada pill se rellena con su propio color de estado** (círculo que sube al hover + fondo activo con el color) y un punto de color a la izquierda. Se usa en el **formulario de creación** (`FormNV.jsx`) y en el **drawer de detalle** (`PanelIngresar.jsx`), ambos alimentados por `ESTADOS_SELECCIONABLES` (En Proceso · Shipping · Currier · En Ruta · Entregado) → todos los estados visibles de un vistazo, sin abrir menús. Se retiró el estado/efectos del dropdown viejo (búsqueda, click-afuera). Sin migración. Build + 77/77 tests OK. |
| 1.55.19 | 2026-07-17 | **Detalle N.V. (Ingresar) — rediseño limpio "como el original"**. El drawer de detalle vuelve al diseño 1:1 del port original: secciones planas sin tarjetas anidadas ni bordes naranja de acento (`Información`/`Progreso`/`Estado y Logística`/`Fechas`/`Documentos` con títulos discretos `uppercase` gris), chips de solo lectura `bg-gray-50` filtrando los campos vacíos, fondo del panel blanco. Se conservan **solo** los arreglos funcionales que CCO sí necesitaba: el drawer se renderiza por **portal a `document.body`** con `z-[120]` (para que el navbar `z-[100]` no tape la parte superior) y la clase `panel-portal` + su scope en `panel.css` (para que los `field-input`/`field-label` mantengan estilo fuera de `.panel-root`). Fechas en grilla 2×2 compacta sin scroll innecesario. Sin migración. Build + 77/77 tests OK. |
| 1.55.10 | 2026-07-16 | **Carga Masiva N.V. blindada contra mezcla de canales**. En las tarjetas N.V PTM/ORANGE/FARMAPACK, al previsualizar: (1) **destino visible** (canal + cuántas N.V. hay hoy en ese canal); (2) **aviso de cruce** — cuenta las N.V. del archivo que ya existen en OTRO canal (PTM no comparte numeración con Orange/Farmapack → un cruce con PTM casi siempre = archivo equivocado), con ejemplos, para revisar antes de confirmar; (3) modo **"Reemplazar canal"** (RPC `nv_catalogo_purgar_canal`, migración `097`, admin/`manage_data_import`) que borra el canal y deja EXACTAMENTE las N.V. del archivo → corrige cargas mezcladas y elimina residuos, sin duplicar. El upsert por (canal,nv) ya evitaba duplicados; esto agrega la detección y el reemplazo controlado. Build + 77/77 tests OK. |
| 1.55.9 | 2026-07-16 | **Export de Ingresar con formato de fecha chileno y orden estable**. La descarga a Excel (`exportarOperaciones`) ahora saca **todas las fechas en dd/mm/aaaa** y los timestamps en dd/mm/aaaa hh:mm (consistente, sin depender del locale de Excel); los valores `$`/bultos/días quedan numéricos. Orden estable por `id` (orden de ingreso). La descarga es **solo lectura** → no bloquea ni altera la tabla: el llenado desde Ingresar sigue normal y siempre se puede descargar el estado actual. Sin migración. Build + 77/77 tests OK. |
| 1.55.8 | 2026-07-16 | **Exportar a Excel desde Ingresar (maestro de N.V. completo)**. Botón *Exportar Excel* en la pestaña **Buscar** de Ingresar que descarga **TODA** la tabla `tms_operaciones` (todas las 41 columnas de negocio con encabezados legibles, todos los registros) a un `.xlsx` — paridad con la "hoja principal". Servicio `exportarOperaciones()` (paginado, orden estable por `id`) + helper `exportToExcel`. La tabla es la fuente de verdad: el llenado desde Ingresar se guarda ahí y de ahí se descarga. Sin migración. Build + 77/77 tests OK. |
| 1.55.7 | 2026-07-16 | **Se retira el iframe del Panel de Vercel (ya migrado a nativo)**. El Panel PTM vive nativo en CCO (`/panel/*` leyendo `tms_operaciones`), así que el dashboard externo embebido vía iframe (`/tools/panel` → `src/pages/Tools/PanelPTM.jsx`, dominio `panel-dashboard-ptm.vercel.app`) quedó redundante y se eliminó: ruta en `App.jsx`, ítem "Panel (Vercel)" del menú (`Navbar.jsx`), entrada en `APP_ROUTES` (`modules.js`), `ROUTE_PERMISSIONS['/tools/panel']`, el `frame-src` de Vercel en el **CSP** (`server.js`) y el archivo. Sin migración (el permiso `view_panel` y el módulo `panel` siguen, ahora para las pantallas nativas). Build + 77/77 tests OK. |
| 1.55.6 | 2026-07-16 | **Panel PTM controlado módulo x módulo (permisos por pantalla)**. El Panel tenía solo `view_panel`/`manage_panel`; ahora, igual que Conteo/Análisis/Post-Venta, cada pantalla tiene su casilla propia en **Admin → Roles**: `panel_ingresar`, `panel_info`, `panel_tv`, `panel_builder` (migración `096`, tabla `tms_permisos`, módulo *Panel PTM*). `ROUTE_PERMISSIONS` de `/panel/ingresar|info|tv|builder` acepta el permiso de la pantalla **o** `view_panel` (umbral que da acceso a todo) → es **aditivo, sin romper roles existentes**. La visibilidad en el menú se deriva sola (Navbar por ROUTE_PERMISSIONS). Verificado: 90 permisos, catálogo BD `tms_permisos` = catálogo Roles del frontend **sin diferencias**. Build + 77/77 tests OK. |
| 1.55.5 | 2026-07-16 | **Pasada de MOTOR + SEGURIDAD (guiada por get_advisors + auditoría paralela)**. **BD (migración `095`)**: (1) **RLS `initplan`** — se envolvieron `auth.uid()/auth.role()/auth.jwt()/is_admin()` en `(select …)` en las **44 policies** que los reevaluaban por fila → se evalúan una sola vez por consulta (gran mejora en tablas grandes); (2) **7 índices** de claves foráneas sin cubrir (joins/borrados más rápidos); (3) **seguridad**: se cerró el `EXECUTE` por `PUBLIC` en `guardar_usuario`/`usuarios_bulk` (+ helpers `_panel_puede_escribir`, `pv_log_historial(_alta)`, `sync_calidad_a_recepcion`) → `anon` ya no las alcanza; (4) `search_path` fijado en 4 funciones. **Frontend**: `dashData.js` — `.order('id')` en los 4 loops paginados con `.range()` (evita filas duplicadas/perdidas entre páginas); **`panelPtm.js`** ahora lee la operación **EN VIVO** de `tms_operaciones` del propio proyecto CCO (antes leía un proyecto Supabase EXTERNO con su anon key embebida → se eliminó la llave cross-project + PII y se corrige la lectura de datos obsoletos post-cutover); `ControlTower`/`Shipping` — `staleTime` + cota `.limit(2000)` en `tms_entregas`. **Pendientes anotados** (no tocados por riesgo/alcance): 23 policies `USING(true)` "todo authenticated" en tablas operativas, `send-push` con API FCM legacy, virtualización de tablas de 1–2k filas (HistorialNV/Batches/DispatchControl). Build + 77/77 tests OK. |
| 1.55.4 | 2026-07-16 | **Usuarios y Roles — upgrade de eficiencia**. `Users.jsx` reescrito: (1) **acciones masivas** con multi-selección (activar/desactivar/cambiar rol/eliminar en lote) vía nueva RPC `usuarios_bulk`; (2) **toggle activo/inactivo en 1 clic** desde la tarjeta/tabla; (3) **vista tabla compacta** + **orden** (nombre/rol/estado/último acceso) además de las tarjetas; (4) **último acceso** por usuario (de `tms_usuarios.last_seen`). Alta/edición ahora pasan por la RPC **transaccional y gateada** `guardar_usuario` (crea cuenta auth + fila en una sola transacción → sin cuentas huérfanas). **Bug corregido**: `Users` y `Roles` compartían el queryKey `['admin_roles']` con formas distintas → contadores de permisos en 0 y se podía borrar un rol en uso; `Users` ahora usa `['roles_catalogo']`. Migración `094` (`guardar_usuario` + `usuarios_bulk`, ambas admin-only, `anon` sin EXECUTE). Revisión de seguridad: la escalada de privilegios ya estaba bloqueada por el trigger `tms_usuarios_freeze_privileged` (verificado), no requería cambio. Build + 77/77 tests OK. |
| 1.55.3 | 2026-07-16 | **Consulta pública de N.V. blindada (uso público seguro)**. La página `/consulta` (sin login, seguimiento tipo courier) se endurece para que sea de uso público sin abrir el sistema. Migración `093`: (1) `buscar_nv_publico` ahora exige **coincidencia EXACTA** del número de documento (NV/factura/guía/N° envío) — se **elimina la búsqueda por substring y por nombre de cliente** (cerraba el vector de enumeración/cosecha de la cartera de clientes), devuelve un **conjunto mínimo no sensible** (sin montos, vendedor, centro de costo, incidencias, fillrate ni empresa de transporte) y aplica **rate-limit por IP en la propia BD** (30/min, tabla `tms_consulta_rate`; frena scraping incluso si llaman la RPC directo con la llave anónima); (2) se **revoca `EXECUTE` a `anon`** en todas las RPC de escritura/borrado del Panel (`guardar_*`/`eliminar_*`/`cambiar_estado_nv`/`toggle_panel_catalogo`/`panel_sync_operaciones`/`batch_update_nv_estado`) — quedan solo para `authenticated`; las únicas 3 superficies anónimas son `buscar_nv_publico`, `verificar_certificado` y `crear_pv_ticket` (intake Post-Venta con anti-bot). Base ya verificada: RLS deja las tablas en `{authenticated}` (anon NO lee nada directo) y el guard `_panel_puede_escribir()` da FALSE para anon. Frontend `ConsultaNV`: UX de número exacto + mensaje de rate-limit. Build + 77/77 tests OK. |
| 1.55.2 | 2026-07-16 | **Ingresar: aviso claro si la N.V. no está en el catálogo + lista de vendedores en Varios**. En canales reales (PTM/Orange/Farmapack), si al buscar una N.V. no hay cliente en el catálogo, se muestra un **cuadro emergente** ("Cliente no encontrado — Realiza la actualización de la carga de N.V. para poder continuar", con botón directo a *Carga Masiva*) y **no** se crea a ciegas (el formulario queda en espera). En **Varios** esto no aplica: el selector de **Vendedor** ahora se alimenta del catálogo CENTRO COSTOS (`tms_panel_vendedores`) como **lista emergente**, y al elegir uno se auto-rellenan **división** y **centro de costo**; el **cliente** se escribe manual. Sin migración. Build + 77/77 tests OK. |
| 1.55.1 | 2026-07-16 | **Autocompletado de N.V. blindado (adiós al bug del BUSCARV del Sheet)**. El cruce N.V.→cliente/vendedor es por **coincidencia EXACTA** (nunca aproximada), así que jamás trae datos de otra N.V. Además se normaliza la N.V. en ambos lados para que siempre calce: migración `092` (`normalizar_nv()` + trigger en `tms_nv_catalogo` que al subir/editar deja `canal` en minúscula y `nv` sin espacios ni sufijo `.0`) y `ingresarService` (`normNV()` aplicado en `lookup`/`buscarNvCatalogo`; el cruce vendedor→c.costo usa `ilike`, tolerante a mayúsculas/espacios). Verificado: subir `PTM/97200.0` o ` 97201 ` queda como `ptm/97200` y `97201`. Build + 77/77 tests OK. |
| 1.55.0 | 2026-07-16 | **Centro de Costos (vendedor → c.costo/división) + cascada en Ingresar**. Migración `091`: carga la hoja **CENTRO COSTOS** en `tms_panel_vendedores` (37 vendedores con su `centro_costo` y `division`; los "NO ACTIVO" quedan inactivos). Se **administran en Configuración → Vendedores** (agregar/editar/activar-desactivar/eliminar, con columnas C. Costo y División). `ingresarService`: nueva `costoDeVendedor()` y el **lookup completa la cascada** NV → vendedor (catálogo CARGA) → **centro de costo/​división** (CENTRO COSTOS) cuando falta, igual que el Sheet. Build + 77/77 tests OK. |
| 1.54.0 | 2026-07-16 | **Carga Masiva — Catálogo N.V. por canal (CARGA PTM/ORANGE/FARMAPACK)**. 3 tarjetas nuevas en **Admin → Carga Masiva** (`N.V PTM`, `N.V ORANGE`, `N.V FARMAPACK`) que cargan el catálogo maestro **NV → Cliente/Vendedor** (equivalente a las hojas CARGA del Sheet). Migración `090`: tabla `tms_nv_catalogo` (`canal, nv, cliente, vendedor, fecha_aprobacion, monto_neto`, PK `(canal,nv)`), agregada al allowlist de `bulk_upsert` (**upsert por canal+nv**, no borra lo previo). Columnas del archivo: Fecha, N.Venta, Nombre Cliente, Nombre Vendedor, Monto Neto (cada tarjeta fija su `canal` con `defaultValues`). **Ingresar**: el lookup ahora trae el **cliente/vendedor preciso desde el catálogo** (para N.V. nuevas y para completar operaciones sin dato) y `guardar_nv` los **persiste al crear** (antes solo para "Varios"). La tabla es legible por otros módulos (RLS authenticated). Build + 77/77 tests OK. |
| 1.53.1 | 2026-07-16 | **Ingresar — estados en desuso removidos**. Se quitan de Ingresar los estados que PTM ya no usa (siempre en 0): `P / VENDEDOR`, `P / STOCK`, `P / RETIRO`, `Currier`, `REFACTURADO`. `ingresarService`: `ESTADOS_SELECCIONABLES` = En Proceso/Shipping/En Ruta/Entregado/NULA/RECHAZADO (selectores del form y del detalle); `ESTADOS_ACTIVOS` = En Proceso/Shipping/En Ruta (lista Buscar + consulta). Las píldoras de filtro además solo muestran estados con **al menos 1** N.V. (se ocultan los 0). Build + 77/77 tests OK. |
| 1.53.0 | 2026-07-16 | **Re-login automático al actualizar (version guard)**. Como el Panel: al desplegar una versión nueva, la app cierra la sesión y obliga a re-loguear. `server.js` expone `GET /api/version` (versión desplegada, `no-store`). Cliente `src/lib/versionGuard.jsx` (`<VersionGuard/>` en `App.jsx`): compara `/api/version` con su `__APP_VERSION__` al montar, cada 60 s y al volver el foco; si difiere → `logout()` (si hay sesión) + `serviceWorker.update()` + `location.reload()` (deja al usuario en el login con el bundle nuevo). Empieza a operar desde este build en adelante. Build + 77/77 tests OK. |
| 1.52.0 | 2026-07-16 | **Panel PTM — Configuración portada 1:1 → PORT COMPLETO del Panel**. Se reemplaza Configuración por el port fiel de `app/configuracion/page.tsx` + `CatalogoMaestro.tsx` (`config/PanelConfigReal.jsx`, `config/CatalogoMaestro.jsx`): pestañas **Transportistas** y **Vendedores** (CRUD real: alta/edición/activar/eliminar) con `extraFields` (C. Costo/División) para vendedores, chip **Estados** deshabilitado, y pestaña **Auditoría** (bitácora `tms_operaciones_log` + resumen por operador). Gate admin/`manage_panel`. Migración `088`: `tms_panel_transportistas`/`tms_panel_vendedores` (sembradas desde operaciones) + RPCs genéricas; servicio `config/configService.js`. **Con esto las 6 pantallas del Panel (Dashboard, Ingresar, Info, TV, Builder, Configuración) están copiadas 1:1** desde `lockowom/panel-` — diseño, animaciones y datos reales de CCO. Build + 77/77 tests OK. |
| 1.51.0 | 2026-07-16 | **Panel PTM — Builder portado 1:1 (con persistencia real)**. Se reemplaza el Builder por el port fiel de `app/builder/page.tsx` + componentes (`PanelBuilderReal.jsx`, `WidgetRenderer`, `ConfigPanel`, `WidgetCatalog`, `LayoutManager`, `CamposCalculados`, + `widget-registry.js`/`format.js` portados): **drag & drop** de widgets con **react-grid-layout@^2.2.3** (misma versión del original), gráficos recharts, catálogo de widgets, panel de configuración y **campos calculados** con el motor de fórmulas real. Migración `087`: tablas `tms_dashboard_layouts`/`tms_builder_calculated_fields` + RPCs (gate `manage_panel`); servicio `builder/builderService.js` (reads a tablas `tms_`, writes por RPC). El rol de edición se deriva del permiso `manage_panel` (auth CCO, sin store propio). Datos reales de `tms_operaciones`. Build + 77/77 tests OK. |
| 1.50.2 | 2026-07-16 | **Panel PTM — Info N.V. portado 1:1**. La pantalla Info se reemplaza por el port fiel de `app/info/page.tsx` (`src/pages/Panel/info/PanelInfoReal.jsx` + `NvDetalle.jsx`): mismo hero, KPIs, timeline, filtros de fecha, paginación (Ver más), debounce, historial en localStorage y animaciones, leyendo real de `tms_operaciones` (`.from('tms_operaciones')`) con `normEstado`/`ESTADO_COLOR`/`soloFecha` de `dash/dashHelpers`. La sesión propia se removió (auth de CCO); el feed de actividad usa el stub de auditoría (muestra "Sin actividad"). `PanelScreens` apunta la ruta Info al nuevo componente. Build + 77/77 tests OK. |
| 1.50.1 | 2026-07-16 | **Panel PTM — Modo TV portado 1:1**. La pantalla TV se reemplaza por el port fiel de `app/tv/page.tsx` (`src/pages/Panel/tv/PanelTVReal.jsx` + `tv/ui/CountUp.jsx`): mismo tablero kiosco 1920×1080, contadores animados `CountUp`, barras por estado, auto-rotación y timings idénticos, leyendo datos reales vía `fetchTVEstados` de `dash/dashData`. `PanelScreens` apunta la ruta TV al nuevo componente. Build OK. |
| 1.50.0 | 2026-07-16 | **Panel PTM — Ingresar: FormNV + Consolidados portados 1:1**. Se integran los componentes REALES del `/ingresar` original: `FormNV` (con su store zustand `useFormNVStore`), `Consolidados`, `PillNavCanal` (+ su CSS/animación gsap), `Toast`, y helpers/`estados` portados byte-a-byte (`src/pages/Panel/ingresar/`). La orquestación (lookup/submit) se cablea a `ingresarService` (RPCs de CCO): `handleLookup` → `applyFound`/`applyNew` del store; `handleSubmit` → `guardar`. Consolidados usa las RPCs `guardar_consolidado`/`eliminar_consolidado` reconstruyendo cada mutación como reemplazo del set. Diseño y animaciones idénticos al original; datos reales de `tms_operaciones`. Build + 77/77 tests OK. |
| 1.49.1 | 2026-07-16 | **Panel PTM — Ingresar: Buscar y Detalle idénticos al original**. La pestaña Buscar y el modal de detalle se reescriben como copia fiel del `/ingresar` original: filas estilo `NvRow` (NV grande + canal + cliente·vendedor + Guía/Fact + Aprob/Real + pill de estado con punto y chevron), píldoras de estado con conteo, contador "N resultados de N activas" + Recargar, y **panel lateral** (drawer) con secciones **Información**, **Progreso** (Stepper timeline Registrada→Aprobada→En Proceso→Shipping→Despachada→En Ruta→Entregada con checks y días entre hitos), **Estado y Logística** (dropdown de estado buscable, tipo despacho, transportista, urgente), **Fechas** (F. Aprob. Real recalcula F. Compromiso +2 días hábiles) y **Documentos**; guardado por diferencias ("Guardar N cambios") y eliminar con confirmación. Build + 77/77 tests OK. |
| 1.49.0 | 2026-07-16 | **Panel PTM — módulo Ingresar COMPLETO (paridad con /ingresar)**. La pantalla Ingresar deja de ser solo un formulario y pasa a tener las 3 pestañas del original: **Buscar** (lista de N.V. activas con filtro por estado en píldoras + búsqueda + fila clicable → **modal de detalle** con edición de todos los campos, cambio de estado y **eliminar**), **Ingresar** (alta/edición: canal → lookup → logística → datos adicionales) y **Consolidados** (crear/listar/eliminar tickets `CON-###` que agrupan N.V., solo `manage_panel`). Servicio `src/pages/Panel/ingresar/ingresarService.js` (lista/lookup/guardar/eliminar/cambiarEstado/consolidados sobre las RPCs). Migración `086`: RPC `eliminar_nv`, tablas `tms_consolidados`/`tms_consolidado_nvs` + RPCs `guardar_consolidado`/`eliminar_consolidado` (gate `manage_panel`). Escritura optimista; permiso `manage_panel` controla los botones de edición/borrado/consolidados. Build + 77/77 tests OK. |
| 1.48.0 | 2026-07-16 | **Panel PTM — CUTOVER: CCO fuente de verdad + escrituras nativas**. Migración `085`: se **apaga el auto-sync** (`cron.unschedule('panel-sync-operaciones')` + `enabled=false`) → el Google Sheet queda congelado y **CCO manda**. Escrituras nativas gateadas por permiso: RPCs `guardar_nv(p jsonb)` (crear/editar) y `cambiar_estado_nv(id, estado, urgente)` (cambio rápido), `SECURITY DEFINER`, gate `_panel_puede_escribir()` (admin o **`manage_panel`**). El **trigger** de `tms_operaciones` ahora estampa `fecha_estado` al cambiar de estado y las fechas de etapa (en proceso/shipping/en ruta/entregado) la 1ª vez —como el onEdit del Sheet—. Bitácora `tms_operaciones_log`. Frontend: `panelService` con `lookupNV`/`guardarNV`/`cambiarEstado` reales (invalidan el micro-caché tras escribir → lecturas frescas al instante); **Ingresar N.V.** con estados canónicos y guardado real; **Info N.V.** con **cambio de estado inline** (dropdown + actualización **optimista**). Nuevo permiso `manage_panel` en `tms_permisos` + `config/modules.js`. Build + 77/77 tests OK. |
| 1.47.0 | 2026-07-16 | **Consulta pública de N.V. (Info N.V. sin login)**. Único acceso público del Panel en CCO: página abierta `/consulta` (`src/pages/Public/ConsultaNV.jsx`, sin login ni navbar, cabecera propia PTM) para que un cliente/courier siga el **estado y la logística** de una nota de venta. Datos vía RPC `buscar_nv_publico(p_q)` (migración `084`, SECURITY DEFINER, ejecutable por `anon`): expone estado, fechas del flujo, transportista/empresa, guía, N° de envío, cliente, canal, N.V., tipo de despacho, bultos, fill rate e incidencia. **OCULTA** montos (valor_factura, valor_nv, costo_flete) y datos internos (vendedor, centro_costo, división, aprob. real, observaciones). Anti-abuso: mín. 3 caracteres y tope 25 resultados. Ruta pública en `App.jsx` (fuera de `ProtectedRoute`, junto a `/soporte`). El Info N.V. interno (`/panel/info`, con montos) sigue bajo login. Build + 77/77 tests OK. |
| 1.46.0 | 2026-07-16 | **Dashboard Panel — copia fiel 1:1 del original (Vercel)**. El dashboard nativo se reemplaza por un **port exacto** del `app/page.tsx` del repo `lockowom/panel-`: misma cabecera ("PANEL DASHBOARD / Resumen Operacional", filtro de fecha, auto-refresh 120 s), mismas secciones en el mismo orden (Notas de Venta, KPIs, banner Calidad, tabla Estado + resumen, Tendencia semanal y Lead time, **Tiempos de Ciclo**, **Alertas Operacionales**, **Rankings** transportistas/vendedores, **Operadores**, **Divisiones**, **Tendencia histórica**) y los **4 modales** (Detalle de estado, Incidencias, Alertas de riesgo, Calidad de datos). Sistema de diseño idéntico (`dash.css` ← `globals.css`: naranja `#f57c00`, tablas con header naranja, badges, animaciones) **scoped a `.dash-root`** para no afectar el resto del Panel. Nueva carpeta `src/pages/Panel/dash/` con `DashboardReal.jsx`, `dashData.js` (port 1:1 de `queries.ts` con las formas exactas, sobre `tms_operaciones`), `dashHelpers.js` (format/estados/businessDays) y 18 componentes + 3 dependencias UI (CountUp, MagicBento, ElasticSlider). Ruteo: `/panel` renderiza el dashboard a pantalla completa (PanelLayout omite su cabecera CCO en esa ruta). Realtime sobre `tms_operaciones_sync`. Build + 77/77 tests OK. |
| 1.45.1 | 2026-07-16 | **Migración Panel PTM → CCO — auto-refresco (doble corrida)**. Migración `083`: mientras el Google Sheet siga siendo la fuente de verdad (antes del cutover), CCO se mantiene fresca reejecutando el import cada **10 min** vía **pg_cron** (`panel-sync-operaciones`). Función `public.panel_sync_operaciones()` (SECURITY DEFINER, EXECUTE revocado a public) lee la PostgREST del Panel (extensión `http`, config en `private.panel_sync_config` — la anon key es **pública**, no secreta) y **reemplaza el set `origen='sheet'` en una sola transacción** (delete+insert atómico → los lectores nunca ven vacío), preservando filas nativas de CCO (`origen='cco'`). Tabla `public.tms_operaciones_sync` (last_sync_at/row_count/ok) para observabilidad. **Al hacer el cutover se apaga con** `select cron.unschedule('panel-sync-operaciones')`. Reemplaza la función temporal `_migrar_operaciones`. |
| 1.45.0 | 2026-07-16 | **Migración Panel PTM → CCO (Fase 3: lecturas reales)**. Las pantallas del Panel dejan los mocks y leen **datos reales** de `tms_operaciones`. Nuevo `src/pages/Panel/panelQueries.js`: port de la lógica de cálculo de `lib/queries.ts` (KPIs, tiempos de ciclo, tendencias, rankings, alertas, calidad de datos) adaptado a CCO (cliente `supabase` de CCO, tabla `tms_operaciones`, estado ya normalizado en BD), con micro-caché de 30 s y paginación `.range()`. `panelService.js` ahora **delega en la capa real** para `getDashboard`/`getDetalle`/`buscarNV`/`getTV`/`getCatalogo` (mismas firmas → las pantallas no cambian); Dashboard, Info N.V., Modo TV y Detalle muestran cifras reales (activas 52, entregadas 1.903, 46 transportistas, 40 vendedores, 11 divisiones). Escrituras (`lookupNV`/`guardarNV`) siguen simuladas hasta la fase de escrituras nativas. Etiquetas "datos de ejemplo" → "datos reales". Build + 77/77 tests OK. |
| 1.44.0 | 2026-07-16 | **Migración Panel PTM → CCO (Fase 0-2: BD y datos)**. Se trae la tabla `operaciones` del Panel (Google Sheet → Supabase `yynlmcxmkrlmhqqoxskb`) a la BD de CCO como **futura fuente de verdad**. Migración `082`: nueva `tms_operaciones` (espejo de las 39 columnas + `id` surrogate **estable**, `origen`, `row_hash`), catálogo `tms_operaciones_estado_cat` (11 estados canónicos con orden/activo/terminal/descarte/color) + FK, función `tms_operaciones_norm_estado` y **trigger** que **normaliza el estado** (`ENTREGADO`→`Entregado`, fusiones Recibido*→Entregado) y refresca `updated_at` en cada escritura, índices de lectura y **RLS** (SELECT a `authenticated`). Decisiones validadas contra los datos reales (1.970 filas): **sin clave única de negocio** (existen parcializaciones legítimas: 1 NV en hasta 65 guías; 5 filas idénticas), `id` que **nunca se regenera** (arregla el `DELETE+INSERT` del Panel que lo regeneraba). **Import histórico** server-side con la extensión `http` (CCO leyó la PostgREST del Panel e insertó vía `jsonb_populate_recordset`, DB→DB, sin pasar por el cliente). **Reconciliación 100%**: filas 1.970=1.970, por estado, por canal, y sumas de control (valor_nv 62.645.519, bultos 3.695) idénticas. Próximo: conectar las pantallas del Panel a `tms_operaciones` real (`panelService`) y habilitar escrituras nativas desde CCO. |
| 1.43.0 | 2026-07-15 | **Panel PTM nativo — Capa de datos unificada (todo conectado)**. Nuevo `src/pages/Panel/panelService.js`: **punto ÚNICO de conexión a datos** (equivalente a `lib/queries.ts`). Todas las pantallas con datos consumen desde ahí (async): Dashboard (`getDashboard`/`getDetalle`), Info (`buscarNV`), Ingresar (`lookupNV`/`guardarNV`), Modo TV (`getTV`) y Configuración (`getCatalogo`) — ya no importan los mocks directamente. Cada pantalla maneja su **loading**. Hoy el servicio devuelve datos de ejemplo; **para datos reales solo se cambia el cuerpo de cada función** (a Supabase/proxy), sin tocar las pantallas. Tests 77/77. |
| 1.42.0 | 2026-07-15 | **Panel PTM nativo — Fase 6: Configuración (solo admin) con Auditoría dentro**. Port nativo de `/configuracion` + `/auditoria` **fusionados** (`screens/PanelConfig.jsx`): la pantalla es **solo para administradores** (gate por `user.rol==='ADMIN' || es_admin_delegado`; muestra "Acceso solo para administradores" si no). Pestañas: **Transportistas** y **Vendedores** (catálogos maestros con alta, activar/desactivar y eliminar) + **Auditoría** (sub-tabs Operaciones/Accesos con filtro y tablas de bitácora). Se **elimina la pantalla/ruta Auditoría** independiente; `ROUTE_PERMISSIONS['/panel/configuracion']` pasa a exigir `manage_roles` (nivel admin), así que en el menú solo la ven los admins. Datos de ejemplo. Tests 77/77. Con esto, **las 7 pantallas del Panel están portadas nativas**. |
| 1.41.0 | 2026-07-15 | **Panel PTM nativo — Builder con el MOTOR real de fórmulas**. Se porta 1:1 el motor `lib/formulaEngine.ts` (+`businessDays`) del repo panel- a `src/pages/Panel/formulaEngine.js` (tokenizer + parser de descenso recursivo + evaluador, **sin `eval()`**): operadores, funciones matemáticas/fecha/lógicas/texto y operacionales PTM (`DATEDIFF`, `HOURS_DIFF`, `BUSINESS_DAYS`, `OTIF_STATUS`, `PRIORIDAD_OPERACIONAL`…), fechas ancladas a Chile. En el Builder, la pestaña **Campos calculados** ahora **evalúa de verdad** cada fórmula contra una N.V. de ejemplo (resultado en vivo, validación de sintaxis, campos referenciados y chips de funciones disponibles que se insertan al hacer clic). Verificado. Tests 77/77. |
| 1.40.0 | 2026-07-15 | **Panel PTM nativo — Fase 5: Builder de widgets**. Port nativo de `/builder` (`screens/PanelBuilder.jsx`): **catálogo** de 9 tipos de widget en 2 pasos (tipo → fuente, vía `PanelModal`), **lienzo** con los widgets renderizados de verdad (KPI, barras, línea, dona, tabla, lista/ranking, **medidor** SVG, texto, separador) usando recharts + mock, **panel de configuración** por widget (título, tipo, fuente, ancho 1/3·2/3·completo, color, valor/contenido) y pestaña **Campos calculados** (crear/eliminar métricas por fórmula). Layout **persistido en localStorage** + botón Guardar. Datos de ejemplo. Tests 77/77. |
| 1.39.0 | 2026-07-15 | **Panel PTM nativo — Fase 4: Modo TV**. Port nativo de `/tv` (`screens/PanelTV.jsx`): tablero **kiosco oscuro** con header (logo, título, contador de **Urgentes**, **NV activas**, "EN VIVO" + auto-rotación + reloj), **barras horizontales por estado** (ícono, nombre, ancho animado, cantidad con CountUp, % y badge de urgentes), clic para fijar un estado, **panel de detalle** con las N.V. de ese estado, **auto-rotación** cada 8 s (se pausa 15 s al hacer clic) y **botón de pantalla completa** (Fullscreen API) para TVs. Datos de ejemplo. Tests 77/77. |
| 1.38.0 | 2026-07-15 | **Panel PTM nativo — Fase 3: Info N.V. (buscador universal)**. Port nativo de `/info` (`screens/PanelInfo.jsx`, estilo CCO, 100% pulido): buscador instantáneo (filtra `MOCK_NVS` por NV/factura/guía/cliente/vendedor/estado), **historial de búsquedas** (localStorage), **chips de resumen** por estado (con color), **estados vacíos** (inicial / sin resultados), y **tarjetas de resultado expandibles** con borde por color de estado, badge de urgente, y detalle completo en 6 secciones (Identificación, Comercial, Valores, Estado, Fechas Clave, Fechas Logística) — filas vacías se ocultan. `ESTADO_COLOR` + `MOCK_NVS` en `mock.js`. Sin datos reales aún. Tests 77/77. |
| 1.37.1 | 2026-07-15 | **Panel PTM nativo — Ingresar N.V. más pulido**. Encabezados de sección con **chip de ícono numerado** (01/02/03), **estado inicial guía** (tarjeta con degradado naranja que explica el flujo antes de buscar), header de "Datos adicionales · opcional" con ícono, y micro-detalles de jerarquía/espaciado. `screens/PanelIngresar.jsx`. Tests 77/77. |
| 1.37.0 | 2026-07-15 | **Panel PTM nativo — Fase 2: Ingresar N.V.** Port nativo de `/ingresar` (`src/pages/Panel/screens/PanelIngresar.jsx`, estilo CCO): Canal (PTM/Orange/Farmapack/Varios) → **N° NV con lookup** (simulado: par=actualizar/impar=crear, muestra cliente/vendedor/ccosto/división) → sección **Varios** (datos manuales) → **Logística** (selector de Estado buscable con puntos de color, toggle **Urgente**, tipo despacho, transportista, fechas) → **Datos adicionales** (colapsable) → **barra inferior fija** con Crear/Actualizar (guardado simulado con toast). Clases `.field-input`/`.field-label` en `panel.css`. Sin datos reales aún. Tests 77/77. |
| 1.36.5 | 2026-07-15 | **Panel PTM nativo — Modal con animación + detalle preciso**. El `PanelModal` entra con animación (sube + aparece, backdrop con fade) igual que los demás modales de CCO. El **detalle es contextual**: al hacer clic en un canal/KPI/estado/alerta, el modal muestra la **cantidad real** de ese ítem y filas coherentes (mismo estado, monto sumado, "… y N más"), generadas de forma determinista con `buildDetalle(titulo, count)`. Tests 77/77. |
| 1.36.4 | 2026-07-15 | **Panel PTM nativo — Se quita el nav interno (redundante)**. `PanelLayout` deja de mostrar su pill-nav propio: la navegación entre las 7 pantallas ya vive en el menú de CCO (Inteligencia → Panel PTM). Queda solo la cabecera de módulo. Tests 77/77. |
| 1.36.3 | 2026-07-15 | **Panel PTM nativo — Integración sin "caja" + modales a pantalla completa**. (1) `PanelLayout` ya no agrega fondo ni padding propios (CCO ya envuelve el contenido): se elimina el efecto de caja anidada y el Panel fluye como un módulo nativo. (2) Nuevo `PanelModal.jsx` con **`createPortal` a `document.body`**: los modales oscurecen TODA la pantalla (incluido el navbar), bloquean el scroll de fondo y cierran con Escape/clic afuera — antes quedaban contenidos dentro del contenedor animado del módulo y no oscurecían todo. Tests 77/77. |
| 1.36.2 | 2026-07-15 | **Panel PTM nativo — Adaptado a la identidad CCO**. El Panel deja el look propio y adopta el de CCO: naranja **`#f97316`** (antes `#f57c00`), superficies blancas con **borde slate y esquinas `rounded-2xl`**, **cabecera de módulo con franja degradada** (`from-orange-500 via-amber-400 to-orange-500`) + ícono naranja + título con acento `text-orange-600`, pill-nav recoloreado y **animación de entrada** (`anim-fade-up`) como los demás ports. Paleta de gráficos/KPIs alineada (verde `#10b981`, azul `#2563eb`, rojo `#e11d48`). `panel.css`, `PanelLayout.jsx`, `PanelHome.jsx`. Las 7 pantallas están en el nav (Inteligencia → Panel PTM). Tests 77/77. |
| 1.36.1 | 2026-07-15 | **Panel PTM nativo — Dashboard pulido (Fase 2a)**. `PanelHome.jsx` se acerca al original: hero **Notas de Venta** por canal (PTM/Orange/Farmapack/Varios), 4 **KPIs operacionales** con ícono y color (NVs Activas, Tardanza, A Tiempo, Fill Rate), **banner de calidad** clicable, **filtro de fecha** con presets + contador de auto-refresh, sección **Tiempos de ciclo** (lead time total, etapas, cuello de botella y barras), y **modal de detalle** al hacer clic en KPIs/estados/alertas. Todo con datos de ejemplo (`mock.js` ampliado). Tests 77/77. |
| 1.36.0 | 2026-07-15 | **Panel PTM nativo — Fase 1 (estructura, datos de ejemplo)**. Inicio del port nativo del repo `lockowom/panel-` (Next.js) a CCO (React/Vite). Nueva área `src/pages/Panel/`: `PanelLayout.jsx` (shell + pill-nav a las 7 pantallas), `PanelHome.jsx` (**Dashboard** portado en detalle con KPIs, tabla de estados, gráficos recharts —tendencia semanal y lead time—, rankings, divisiones y tendencia histórica), `PanelScreens.jsx` (scaffolds de Ingresar/Info/TV/Builder/Auditoría/Configuración), `mock.js` (datos de ejemplo, **sin datos reales aún**) y `panel.css` (estilos propios). Rutas anidadas `/panel/*` en `App.jsx`, `ROUTE_PERMISSIONS` y `APP_ROUTES` con `view_panel`, submenú en Navbar (Inteligencia → Panel PTM, + item "Panel (Vercel)" al iframe). Próximas fases: portar cada pantalla y conectar datos reales. Tests 77/77. |
| 1.35.0 | 2026-07-15 | **Panel PTM embebido (nuevo módulo)**. Se integra el dashboard **Panel PTM** (repo `lockowom/panel-`, app Next.js desplegada en Vercel) dentro de CCO vía **iframe** en `/tools/panel` (`src/pages/Tools/PanelPTM.jsx`). No se vendoriza como estático (es SSR con API routes); se embebe su despliegue `https://panel-dashboard-ptm.vercel.app` (mantiene su propia sesión). Checklist de módulo: ruta en `App.jsx`, `ROUTE_PERMISSIONS['/tools/panel']=['view_panel']`, item en Navbar (Inteligencia), `APP_MODULES`/`APP_ROUTES`/`APP_PERMISSIONS` en `config/modules.js`, y migración `081` (permiso `view_panel` en `tms_permisos` + fila `panel` en `tms_modules_config`). CSP de `server.js`: `frame-src` con el dominio de Vercel. Tests 77/77. |
| 1.34.1 | 2026-07-15 | **Novedades / Notas del parche (estilo juego)**. Nuevo modal `src/components/NovedadesModal.jsx` que muestra los cambios de la(s) versión(es) nuevas la **primera vez que se abre la app tras actualizar** (compara `__APP_VERSION__` con la última vista en `localStorage`; a los usuarios existentes no los molesta hasta la próxima subida). Contenido en `src/constants/releaseNotes.js` (lenguaje de usuario, chips Nuevo/Mejora/Arreglo). Se reabre a mano tocando **"Novedades"** al pie del menú (`mostrarNovedades()` vía evento window). Montado global en `App.jsx`. Sin cambios de BD. Tests 77/77. |
| 1.34.0 | 2026-07-15 | **Carteles de Bodega · Fix Doble/Cuádruple (logo recortado)**. En Doble/Cuádruple el contenido centrado con `overflow-hidden` recortaba el **logo** (arriba) cuando rozaba el alto de la celda, y el código de barras salía altísimo al llenar el 100% del ancho de celdas anchas. Ahora: (1) el barcode se dimensiona por **alto** (`cssHeight`/`barMaxH` por formato) y queda centrado sin superar el ancho, en vez de llenar todo el ancho; (2) en Doble/Cuádruple el contenido se **ancla arriba** (`justify-start`) para que el logo nunca se corte (Único sigue centrado). `src/lib/code128.js` (opción `cssHeight`) y `Carteles.jsx`. Tests 77/77. |
| 1.33.9 | 2026-07-15 | **Carteles de Bodega · Revertir layout (logo completo)**. El reparto vertical de la 1.33.8 agrandaba los elementos y **recortaba el logo** arriba. Se vuelve al layout original (centrado, logo completo, tamaños del Excel) **conservando las dos correcciones**: código en 1 sola línea (auto-fit SVG) y código de barras legible (`width:100%` + `crispEdges` + input recortado). `src/pages/Inventory/Carteles.jsx`. Tests 77/77. |
| 1.33.8 | 2026-07-15 | **Carteles de Bodega · Llenar la hoja (menos espacio en blanco)**. El cartel ahora reparte el contenido verticalmente: bloque superior (logo + código + descripción) y bloque inferior (código de barras anclado abajo) con `justify-between` + `h-full`, así ocupa toda la altura de la celda/hoja en vez de quedar agrupado arriba. Tamaños agrandados por formato (Único: código hasta 62mm, barras `barH` 200, descripción 42pt). `src/pages/Inventory/Carteles.jsx`. Tests 77/77. |
| 1.33.7 | 2026-07-15 | **Carteles de Bodega · Código en 1 línea + código de barras que lee bien**. (1) El código ya no se parte en dos líneas: se dibuja como texto SVG que se **auto-escala al ancho** (`textLength`), siempre en una sola línea, con tope de alto por formato (`codeMax`). (2) Código de barras CODE128: la codificación ya era correcta (tabla + checksum verificados), el problema era de **render** — el `<svg>` no llevaba `width`, caía a ~300px y los módulos quedaban finos/borrosos. Ahora lleva `width:100%` + `height:auto` + `shape-rendering="crispEdges"`, ocupa todo el ancho de la celda (antes 80%) y el texto se **recorta** antes de codificar (un espacio al inicio/fin cambiaba el valor leído). `src/lib/code128.js` y `src/pages/Inventory/Carteles.jsx`. Tests 77/77. |
| 1.33.6 | 2026-07-15 | **Recepción ↔ Calidad · Fix recepciones "Pendiente" sin tarea + higiene de migraciones**. Las recepciones creadas ANTES del trigger `crear_tarea_checklist_ingreso` (mig 075) nunca generaron su tarea de CheckList, así que figuraban "Pendiente de Calidad" en Recepción sin una tarea que completar en Calidad (comunicación rota). Migración `080`: **backfill idempotente** que crea la tarea PENDIENTE faltante (nacionales aplicado: 3 recepciones; importaciones queda como bloque comentado a confirmar, ~64). Además se **reconstruyeron en el repo** las migraciones `076` (flujo inverso Calidad→Recepción: columnas `calidad_*` + trigger `sync_calidad_a_recepcion` + backfill) y `077` (`fecha_vencimiento` por ítem), que se habían aplicado vía MCP sin dejar el archivo. Sin cambio de versión de app. |
| 1.33.6 | 2026-07-14 | **Carga Masiva · Fix "null value in column producto" (Matriz Códigos)**. El filtro de filas en `src/pages/Admin/DataImport.jsx` usaba `.some()` sobre los campos obligatorios: bastaba que UNO no estuviera vacío, así que una fila con `codigo_producto` pero **sin `producto`** pasaba y luego el `bulk_upsert` (set-based, chunks de 1.000) reventaba el **lote completo** por el not-null → 1 fila mala tumbaba 1.000 (de ahí los 2.000 = 2 lotes). Ahora se exige `.every()` (todas las columnas obligatorias presentes; el 0 numérico cuenta como válido). Las filas incompletas se **descartan al parsear** y se informa el conteo en un aviso ámbar en la vista previa. Aplica a **todas** las cargas (Inventario, N.V., Trazabilidad, WMS, Maestros). Sin cambios de BD. Tests 77/77. |
| 1.33.5 | 2026-07-14 | **Lotes y Series · Fin de la "discrepancia" visual**. En `src/pages/Queries/Batches.jsx` un mismo SKU aparece en varias filas porque tiene **varios lotes/partidas** (dato correcto), pero en móvil solo se veía SKU+Disponible y las filas parecían duplicadas/erróneas. Ahora: (1) el badge de estado tiene **3 estados** — Disponible (verde), **En Tránsito** (ámbar, cuando `disponible=0` pero `stock_total>0`, muestra el total y "↳ N en tránsito") y Sin Stock (rojo, solo si el total es 0); antes un lote con stock en tránsito salía como "SIN STOCK 0". (2) Cada fila muestra su **Lote** inline (chip), visible en móvil aunque la columna Partida/Lote se corte, para diferenciar los lotes del mismo SKU. Mismo criterio en la pestaña Series. Sin cambios de BD. Tests 77/77. |
| 1.33.4 | 2026-07-14 | **Post-Venta · Dominio público real fijado como fallback**. `src/lib/publicUrl.js` usa `https://cco-ptm-b05m.onrender.com` como dominio por defecto (sobrescribible por `VITE_PUBLIC_URL`), de modo que el enlace `/soporte` sale correcto **también desde la app nativa/dev** (donde el origin es localhost). El banner ya siempre muestra un enlace absoluto y clicable. Tests 77/77. |
| 1.33.3 | 2026-07-14 | **Post-Venta · Enlace del formulario público con dominio real (no localhost)**. Nuevo helper `src/lib/publicUrl.js` (`publicBaseUrl`/`publicUrl`): el enlace `/soporte` del banner "Formulario público" en TabNuevo ya **no usa `window.location.origin`** (que daba `localhost` en la app nativa/dev). Prioridad: `VITE_PUBLIC_URL` (dominio propio, configurable en Render/CI) → origin solo si es web real (no localhost/IP/capacitor) → ruta relativa. Si no hay dominio configurado, el banner lo avisa. Documentado en `.env.example`. Tests 77/77. |
| 1.33.2 | 2026-07-14 | **Post-Venta · Formulario público sin Fecha/Hora de visita**. Se quitan del formulario `/soporte` los campos **Fecha y Hora de visita deseada**: la agenda de la visita la define el **técnico** al gestionar el ticket, no el solicitante. La RPC y la Edge Function mantienen los params (compatibilidad), solo se retiran de la UI pública. Tests 77/77. |
| 1.33.1 | 2026-07-14 | **Post-Venta · Formulario público con los mismos parámetros que el ticket interno**. El formulario `/soporte` gana **Comuna** (cascada región→comuna con lista de `comunasChile`), **¿Cotizar?**, **Fecha/Hora de visita deseada** y **Observaciones** (se antepone a la nota de origen). El **estado inicial es SIEMPRE `Abierto`** (flujo de entrada; no seleccionable). Se excluyen los campos internos (técnico, N.V., selector de estado). Migración `079`: `crear_pv_ticket_publico` recreada con 4 params opcionales al final (`p_observaciones`, `p_cotizar`, `p_fecha_programada`, `p_hora_programada`); Edge Function `postventa-publico` v2 los pasa. Verificado end-to-end. Tests 77/77. |
| 1.33.0 | 2026-07-14 | **Post-Venta · Formulario PÚBLICO de solicitud de servicio (sin login)**. Nueva ruta abierta **`/soporte`** (`src/pages/Postventa/SolicitudPublica.jsx`, fuera de `ProtectedRoute`) donde cualquier cliente o vendedor crea un ticket sin cuenta y recibe su número de folio. Entra como **borrador** (`origen='Web'`, "Sin Asignar", región/tipo por defecto) para que un técnico lo valide, igual que los de correo. Migración `078`: RPC dedicada `crear_pv_ticket_publico` (SECURITY DEFINER, **NO** usa `_pv_assert`; solo `service_role`) con validación estricta y **rate-limit** por IP (5/10 min) y global (40/min) sobre la nueva bitácora `tms_postventa_publico_log`. Edge Function **`postventa-publico`** (`verify_jwt` off) con anti-spam en capas: honeypot, tiempo mínimo de llenado (`t_ms`) y **Turnstile opcional** (secret `PV_TURNSTILE_SECRET`); llama la RPC con el `service_role` pasando la IP (`x-forwarded-for`). El **Equipo/Modelo es texto libre**. Dentro del módulo, TabNuevo muestra un panel para **copiar/abrir** el enlace público. Tests 77/77. |
| 1.32.2 | 2026-07-14 | **Post-Venta · Modelo/Equipo escribible a mano** en el ticket. Nuevo componente `SelLibre` (input + `datalist`) en `src/pages/Postventa/Postventa.jsx`: reemplaza el `<select>` "Equipo / Modelo" en **Nuevo ticket** y en **Editar** de modo que, si el cliente no conoce la familia, se puede **escribir el modelo manualmente** (a la vez que se pueden elegir las familias reales del stock vía `pv_familias_stock`). Sin cambios de BD (el valor libre viaja en el mismo campo `equipo`). Tests 77/77. |
| 1.32.1 | 2026-07-12 | **Recepción: campo Fecha de Vencimiento por ítem** (Importaciones y Nacionales). Migración `077`: columna `fecha_vencimiento` (date) en `tms_recepcion_items` y `tms_recepcion_items_nacionales`. Frontend: nuevo campo de fecha al agregar el ítem (junto a Lote/Partida), columna **Vence** en la lista de ítems y en el detalle, y **VENCIMIENTO** en el export Excel. Se guarda y se recupera al editar. Tests 77/77. |
