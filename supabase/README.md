...
- `migrations/` — migraciones **numeradas y ordenadas** (ejecutar en orden):
  ...
  - `090_nv_catalogo.sql` — **catálogo N.V. por canal** (hojas CARGA): tabla `tms_nv_catalogo`
    (`canal,nv,cliente,vendedor,fecha_aprobacion,monto_neto`, PK `(canal,nv)`) agregada al allowlist de
    `bulk_upsert` (upsert por canal+nv); `guardar_nv` ampliada para persistir cliente/vendedor al crear
    (no solo en "Varios"). Alimenta el lookup de Ingresar.
  - `091_centro_costos_vendedores.sql` — **CENTRO COSTOS**: carga 37 vendedores en `tms_panel_vendedores`
    con su `centro_costo` y `division` (los "NO ACTIVO" quedan inactivos). Se administran en
    Configuración → Vendedores. Completa la cascada NV → vendedor → c.costo/división en Ingresar.
