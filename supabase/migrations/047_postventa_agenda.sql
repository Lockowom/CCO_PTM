...
-- Agenda / Calendario de Post-Venta: se agrega fecha (y hora) de visita programada
-- al ticket, para ubicarlo en un calendario. `crear_pv_ticket` y `actualizar_pv_ticket`
-- pasan a manejar estos campos.

-- 1. Columnas nuevas
ALTER TABLE public.tms_postventa_tickets
  ADD COLUMN IF NOT EXISTS fecha_programada date,
  ADD COLUMN IF NOT EXISTS hora_programada  text;
CREATE INDEX IF NOT EXISTS idx_pv_tickets_fprog ON public.tms_postventa_tickets(fecha_programada);

-- 2. crear_pv_ticket: se recrea con 2 parámetros opcionales al final
--    (los callers por nombre — frontend y edge functions — siguen funcionando).
DROP FUNCTION IF EXISTS public.crear_pv_ticket(text,text,text,text,text,text,text,text,text,text,text,text,text,text);
...
REVOKE EXECUTE ON FUNCTION public.crear_pv_ticket(text,text,text,text,text,text,text,text,text,text,text,text,text,text,date,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.crear_pv_ticket(text,text,text,text,text,text,text,text,text,text,text,text,text,text,date,text) TO authenticated, service_role;

-- 3. actualizar_pv_ticket: maneja fecha_programada / hora_programada.
